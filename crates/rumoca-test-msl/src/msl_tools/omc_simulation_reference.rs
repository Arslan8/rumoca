...
use rumoca_solver::timeline::sample_time_match_with_tol;
...
fn ensure_omc_trace_artifacts(paths: &MslPaths, results: &mut BTreeMap<String, SimModelResult>) {
    for (model_name, result) in results {
        ...
        if let Some(trace_path) = resolve_declared_omc_trace_path(paths, model_name, result)
            && trace_path.is_file()
        {
            if let Err(error) = normalize_omc_trace_artifact(&trace_path) {
                result.trace_file = None;
                result.trace_error = Some(format!("failed to normalize cached OMC trace: {error}"));
            }
            continue;
        }
...
fn normalize_omc_trace_artifact(trace_path: &Path) -> Result<()> {
    let mut trace = load_trace_json(trace_path)
        .with_context(|| format!("failed to load '{}'", trace_path.display()))?;
    if normalize_omc_trace_rows(&mut trace)? {
        write_pretty_json(trace_path, &trace)?;
    }
    Ok(())
}
...
fn load_omc_csv_trace(model_name: &str, csv_path: &Path) -> Result<SimTrace> {
    ...
    for row in rows {
        ...
        for (column, source_idx) in data.iter_mut().zip(&value_indices) {
            column.push(
                values
                    .get(*source_idx)
    ...
    normalize_omc_trace_rows(&mut trace)?;
    Ok(trace)
}

/// Apply SPEC_0050 at the OMC producer boundary, including to hydrated cache
/// artifacts created by an older producer. The settled row itself is retained
/// unchanged; only a redundant, roundoff-regressed copy of that evidence is
/// suppressed.
fn normalize_omc_trace_rows(trace: &mut SimTrace) -> Result<bool> {
    if trace
        .data
        .iter()
        .any(|column| column.len() != trace.times.len())
    {
        bail!("OMC trace data columns are not rectangular");
    }
    let mut times = Vec::with_capacity(trace.times.len());
    let mut data = trace
        .data
        .iter()
        .map(|_| Vec::with_capacity(trace.times.len()))
        .collect::<Vec<_>>();
    let mut preceding_row_is_settled = false;
    for (index, time) in trace.times.iter().copied().enumerate() {
        let Some(previous_time) = times.last().copied() else {
            times.push(time);
            for (output, input) in data.iter_mut().zip(&trace.data) {
                output.push(input[index]);
            }
            continue;
        };
        if time < previous_time {
            let repeats_settled_values = data
                .iter()
                .zip(&trace.data)
                .all(|(output, input)| output.last() == Some(&input[index]));
            if preceding_row_is_settled
                && sample_time_match_with_tol(previous_time, time)
                && repeats_settled_values
            {
                continue;
            }
            bail!(
                "OMC trace time regressed from {previous_time} to {time} without a proven settled same-instant row"
            );
        }
        preceding_row_is_settled = previous_time.to_bits() == time.to_bits();
        times.push(time);
        for (output, input) in data.iter_mut().zip(&trace.data) {
            output.push(input[index]);
        }
    }
    let changed = times.len() != trace.times.len();
    trace.times = times;
    trace.data = data;
    Ok(changed)
}
