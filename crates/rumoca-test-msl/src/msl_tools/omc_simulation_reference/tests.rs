...
#[test]
fn omc_csv_suppresses_a_same_instant_row_behind_settled_event_evidence() {
    let temp = tempfile::tempdir().expect("tempdir");
    let csv = temp.path().join("settled.csv");
    std::fs::write(
        &csv,
        "time,y\n0.298,1\n0.3,1\n0.3,0\n0.2999999999999999,0\n0.304,0\n",
    )
    .expect("write CSV");

    let trace = load_omc_csv_trace("Clocked", &csv).expect("load settled OMC trace");

    assert_eq!(trace.times, vec![0.298, 0.3, 0.3, 0.304]);
    assert_eq!(
        trace.data,
        vec![vec![Some(1.0), Some(1.0), Some(0.0), Some(0.0)]]
    );
}

#[test]
fn hydrated_omc_trace_is_normalized_at_the_producer_boundary() {
    let temp = tempfile::tempdir().expect("tempdir");
    let path = temp.path().join("cached.json");
    let trace = SimTrace {
        model_name: Some("Clocked".to_string()),
        times: vec![0.298, 0.3, 0.3, 0.299_999_999_999_999_9, 0.304],
        names: vec!["y".to_string()],
        data: vec![vec![Some(1.0), Some(1.0), Some(0.0), Some(0.0), Some(0.0)]],
        variable_meta: None,
        certification_profile: None,
    };
    write_pretty_json(&path, &trace).expect("write cached trace");

    normalize_omc_trace_artifact(&path).expect("normalize cached trace");
    let normalized = load_trace_json(&path).expect("reload normalized trace");

    assert_eq!(normalized.times, vec![0.298, 0.3, 0.3, 0.304]);
    assert_eq!(
        normalized.data,
        vec![vec![Some(1.0), Some(1.0), Some(0.0), Some(0.0)]]
    );
...
#[test]
fn omc_csv_rejects_a_roundoff_regression_without_settled_evidence() {
    let temp = tempfile::tempdir().expect("tempdir");
    let csv = temp.path().join("nominal.csv");
    std::fs::write(&csv, "time,y\n0.298,1\n0.3,0\n0.2999999999999999,0\n").expect("write CSV");

    let error = load_omc_csv_trace("Clocked", &csv).expect_err("nominal regression must fail");

    assert!(
        error
            .to_string()
            .contains("without a proven settled same-instant row")
    );
}

#[test]
fn omc_csv_rejects_a_regressed_row_that_disagrees_with_settled_values() {
    let temp = tempfile::tempdir().expect("tempdir");
    let csv = temp.path().join("disagrees.csv");
    std::fs::write(
        &csv,
    ...
    let error = load_omc_csv_trace("Clocked", &csv).expect_err("changed evidence must fail");
