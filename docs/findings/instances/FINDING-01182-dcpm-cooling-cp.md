# FINDING-01182: `volumeFlow.medium.cp` in `DCPM_Cooling`
...
| key | value |
|---|---|
| `parameter` | volumeFlow.medium.cp |
| `shape` | direct |
| `path` | volumeFlow.medium.cp |
| `declared_min` | None |
| `divisor_sites` | 2 |
...
`volumeFlow.medium.cp`, and the analysis reached it statically — no value has been observed
