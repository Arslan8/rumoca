...
| key | value |
|---|---|
| `parameter` | filter.normalized |
| `shape` | propagated |
| `path` | filter.normalized -> filter.alpha |
| `declared_min` | None |
| `divisor_sites` | 2 |
...
`filter.normalized`, and the analysis reached it statically — no value has been observed
