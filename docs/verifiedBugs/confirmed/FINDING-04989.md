# FINDING-04989: Zero flux-tube area makes the model undefined
...
| Original report | [BUG-sensors-genericfluxtube-area.md](../../v2/bugs/BUG-sensors-genericfluxtube-area.md) — reviewed as `FINDING-04989-sensors-genericfluxtube-area.md`, which a later run renamed |
| Original SHA-256 | 77283653fee4bcf255450dede552e652b89dc507000390d2bd9d2a4eb81752e6 |
...
| Probe | Outcome |
|---|---|
| `genericFluxTube.area=0` | reported-failure |

Diagnostic: algebraic projection did not converge at event boundary: worst scaled residual row=7 target=genericFluxTube.Phi value=NaN ratio=NaN norm=inf row_scale=1.000000e0 scaled_tolerance=1.000000e-10

[Independent OpenModelica wrappers and complete output](../evidence/omc-7254fa9ac1def6d7.json). Baseline: simulation succeeded.

- `genericFluxTube.area=0` set before translation: LOG_ASSERT        | debug   | division by zero at time 0, (a=1) / (b=0), where divisor b expression is: genericFluxTube.G_m.
- Same value declared final, with final-parameter evaluation: "Error: Internal error IndexReduction.pantelidesIndexReduction failed! System is structurally singular and cannot be handled because the number of unassigned equations is larger than the number of states. Use -d=bltdump to get more information..
