# BUG-007: Zero-storage example has incompatible fixed initial conditions
...
| Original report | [BUG-damper-mass3-m.md](../../v2/bugs/BUG-damper-mass3-m.md) — reviewed as `BUG-007-damper-mass3-m.md`, which a later run renamed |
| Original SHA-256 | 0e084acf4a3ae05df0d14713c4493c18749f87ae1dcd3c8301873a1f9d357c8d |

## Why this is a false positive

The nominal example runs. Recompiling with the reported zero removes a storage state and exposes inconsistent fixed initial equations, not an unavoidable reciprocal in the primitive component. Control Mass3 keeps the zero and relaxes the relevant fixed initial conditions; it simulates successfully. The exact original trigger does fail, but its attribution to a generally invalid library min=0 is false. Fix the example initialization or constrain this particular example if it must retain those starts.
