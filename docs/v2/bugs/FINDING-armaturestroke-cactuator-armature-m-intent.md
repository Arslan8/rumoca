# FINDING-04232: `cActuator.armature.m` in `ArmatureStroke`
...
| **Reached as** | `cActuator.armature.m` |
...
| **Signature** | `physical:physical-intent-question:var:267:a4f811b3` |
...
```
predicate     cActuator.armature.m > 0
premise       unknown
...
> `cActuator.armature.m` is bounded only by min=0, which is unusual for a mass — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
| `required` | cActuator.armature.m &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.ArmatureStroke \
      --target cActuator.armature.m --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "cActuator.armature.m"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param cActuator.armature.m=0
```
