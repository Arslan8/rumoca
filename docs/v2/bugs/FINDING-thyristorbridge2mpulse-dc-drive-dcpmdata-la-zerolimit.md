# FINDING-03483: `dcpmData.La` in `ThyristorBridge2mPulse_DC_Drive`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:972:465c9ca4` |
...
```
contract    Modelica.Electrical.Machines.Utilities.ParameterRecords.DcPermanentMagnetData.La: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
