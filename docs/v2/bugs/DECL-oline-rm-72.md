# DECL-0660: `rm` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Lines/OLine.mo:72` |
| **Parameter** | `rm` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
It should declare `rm` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/Analog/Lines/OLine.mo:72
parameter SI.Resistance rm(min=0) = ...;
```
