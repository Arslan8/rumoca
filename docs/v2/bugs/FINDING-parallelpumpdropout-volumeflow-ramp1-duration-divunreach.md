...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:1388:d550b8b1` |
...
```
denominator            volumeFlow.ramp1.duration
witness                volumeFlow.ramp1.duration = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.2
path condition         not ((time < volumeFlow.ramp1.startTime)) and ((time < (volumeFlow.ramp1.startTime + volumeFlow.ramp1.duration)))
constraints consulted  volumeFlow.ramp1.duration: min=0
```
...
| `path_condition` | not ((time &lt; volumeFlow.ramp1.startTime)) and ((time &lt; (volumeFlow.ramp1.startTime + volumeFlow.ramp1.duration))) |
| `verdict` | UNSAT |
| `proof` | the path conditions cannot hold together: time must be both &gt; 0.2 and &lt; 0.2 |
| `constraints` | volumeFlow.ramp1.duration: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.ParallelPumpDropOut \
      --target volumeFlow.ramp1.duration --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "volumeFlow.ramp1.duration"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param volumeFlow.ramp1.duration=0
```
