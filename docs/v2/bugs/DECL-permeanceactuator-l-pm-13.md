# DECL-0446: `l_PM` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/FluxTubes/Examples/MovingCoilActuator/Components/PermeanceActuator.mo:13` |
| **Parameter** | `l_PM` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '13p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/MovingCoilActuator/Components/PermeanceActuator.mo"
```

It should declare `l_PM` with type `SI.Length` and no `min`.
...
```modelica
// Magnetic/FluxTubes/Examples/MovingCoilActuator/Components/PermeanceActuator.mo:13
parameter SI.Length l_PM(min=0) = ...;
```
