# FINDING-02501: `smeeData.xqSubtransient` in `SMEE_Generator`
...
| **Reached as** | `smeeData.xqSubtransient` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo:80` |
| **Signature** | `divisor:divisor-reachable-zero:exp:1961:a57ca279` |
...
```
denominator            smeeData.xqSubtransient
witness                smeeData.xqSubtransient = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.148387097
path condition         always evaluated
constraints consulted  smeeData.xqSubtransient: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMEE_Generator \
      --target smeeData.xqSubtransient --sanitizer divisor
```
...
```console
$ sed -n '80p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smeeData.xqSubtransient"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param smeeData.xqSubtransient=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo:80` |' docs/v2/bugs/
```
