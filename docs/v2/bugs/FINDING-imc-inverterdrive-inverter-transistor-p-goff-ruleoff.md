...
| **Reached as** | `inverter.transistor_p.Goff` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:547:aa7459f1` |
...
```
predicate     inverter.transistor_p.Goff > 0
premise       refuted
...
| `required` | inverter.transistor_p.Goff &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive \
      --target inverter.transistor_p.Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inverter.transistor_p.Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inverter.transistor_p.Goff=0
```
