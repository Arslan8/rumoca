# FINDING-00329: `H_NMOS.RDS` in `HeatingMOSInverter`
...
| **Signature** | `divisor:divisor-reachable-zero:exp:562:ab4f3085` |
...
```
denominator            H_NMOS.RDS
witness                H_NMOS.RDS = 0
  found by             set to zero
denominator at witness 0.0
at declared values     10000000.0
path condition         not (((H_NMOS.RDS < 1e-20) and (H_NMOS.RDS > -1e-20)))
constraints consulted  H_NMOS.RDS: unbounded
```
...
| `path_condition` | not (((H_NMOS.RDS &lt; 1e-20) and (H_NMOS.RDS &gt; -1e-20))) |
| `verdict` | SAT |
| `proof` | every guard still holds under the witness |
| `constraints` | H_NMOS.RDS: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.HeatingMOSInverter \
      --target H_NMOS.RDS --sanitizer divisor
```
