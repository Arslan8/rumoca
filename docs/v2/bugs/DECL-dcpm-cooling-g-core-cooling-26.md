# DECL-0893: `G_core_cooling` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Machines/Examples/DCMachines/DCPM_Cooling.mo:26` |
...
It should declare `G_core_cooling` with type `SI.ThermalConductance` and no `min`.
...
```modelica
// Electrical/Machines/Examples/DCMachines/DCPM_Cooling.mo:26
parameter SI.ThermalConductance G_core_cooling(min=0) = ...;
```
