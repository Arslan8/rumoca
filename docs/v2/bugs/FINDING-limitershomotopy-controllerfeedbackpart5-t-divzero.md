...
| **Reached as** | `controllerFeedbackPart5.T` |
...
| **Signature** | `divisor:divisor-reachable-zero:exp:718:57e27f23` |
...
```
denominator            controllerFeedbackPart5.T
witness                controllerFeedbackPart5.T = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  controllerFeedbackPart5.T: unbounded
```
...
| `constraints` | controllerFeedbackPart5.T: unbounded |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Blocks.LimitersHomotopy \
      --target controllerFeedbackPart5.T --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "controllerFeedbackPart5.T"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param controllerFeedbackPart5.T=0
```
