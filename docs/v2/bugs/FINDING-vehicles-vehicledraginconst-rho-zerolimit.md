...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:123:f7b9f970` |
...
```
predicate     vehicleDragInConst.rho > 0
premise       refuted
...
| `required` | vehicleDragInConst.rho &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Translational.Vehicles \
      --target vehicleDragInConst.rho --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "vehicleDragInConst.rho"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param vehicleDragInConst.rho=0
```
