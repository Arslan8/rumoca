# DECL-0694: `RonT` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Machines/Examples/ControlledDCDrives/Utilities/SwitchingDcDc.mo:5` |
| **Parameter** | `RonT` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '5p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/ControlledDCDrives/Utilities/SwitchingDcDc.mo"
```

It should declare `RonT` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/Machines/Examples/ControlledDCDrives/Utilities/SwitchingDcDc.mo:5
parameter SI.Resistance RonT(min=0) = ...;
```
