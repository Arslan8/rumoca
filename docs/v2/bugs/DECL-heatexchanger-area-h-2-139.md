# DECL-0015: `area_h_2` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Fluid/Examples/HeatExchanger.mo:139` |
| **Parameter** | `area_h_2` |
| **Type** | `SI.Area` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '139p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Examples/HeatExchanger.mo"
```

It should declare `area_h_2` with type `SI.Area` and no `min`.
...
```modelica
// Fluid/Examples/HeatExchanger.mo:139
parameter SI.Area area_h_2(min=0) = ...;
```
