...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:35:feaa3741` |
...
```
predicate     openTank2.medium.rho > 0
premise       refuted
...
```
contract    Modelica.Thermal.FluidHeatFlow.Media.Medium.rho: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
| `required` | openTank2.medium.rho &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.TwoTanks \
      --target openTank2.medium.rho --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "openTank2.medium.rho"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param openTank2.medium.rho=0
```
