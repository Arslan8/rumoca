# FINDING-03530: `pulse2.twomPulse.filter[3].fa` in `ThyristorBridge2mPulse_DC_Drive`
...
| **Reached as** | `pulse2.twomPulse.filter[3].fa` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/Control/Filter.mo:19` |
| **Signature** | `divisor:divisor-reachable-zero:exp:1804:34bbcca0` |
...
```
denominator            ((2 * (2 * asin(1.0))) * pulse2.twomPulse.filter[3].fa)
witness                pulse2.twomPulse.filter[3].fa = 0
  found by             set to zero
denominator at witness 0.0
at declared values     36.57876381465106
path condition         always evaluated
constraints consulted  pulse2.twomPulse.filter[3].fa: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.ThyristorBridge2mPulse_DC_Drive \
      --target pulse2.twomPulse.filter[3].fa --sanitizer divisor
```
...
```console
$ sed -n '19p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/Control/Filter.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pulse2.twomPulse.filter[3].fa"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param pulse2.twomPulse.filter[3].fa=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/Control/Filter.mo:19` |' docs/v2/bugs/
```
