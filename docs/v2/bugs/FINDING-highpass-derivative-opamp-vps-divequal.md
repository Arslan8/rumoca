...
| **Signature** | `divisor:divisor-zero-when-parameters-equal:exp:258:be3942f8` |
...
```
denominator            (derivative.opAmp.vps - derivative.opAmp.vns)
witness                derivative.opAmp.Vps = -15
  found by             set equal to derivative.opAmp.Vns
denominator at witness 0.0
at declared values     30.0
path condition         always evaluated
constraints consulted  derivative.opAmp.Vps: unbounded
```
...
| `denominator` | (derivative.opAmp.vps - derivative.opAmp.vns) |
| `witness` | derivative.opAmp.Vps = -15 |
| `witness_rationale` | set equal to derivative.opAmp.Vns |
...
| `constraints` | derivative.opAmp.Vps: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.HighPass \
      --target derivative.opAmp.Vps --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "derivative.opAmp.Vps"
```
