...
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:5760:19f8f22e` |
...
```
denominator            (1 + (oLine2.G[3].alpha * (oLine2.G[3].T_heatPort - oLine2.G[3].T_ref)))
witness                none needed
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.CompareLineTrunks \
      --target oLine2.G[3].alpha --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "oLine2.G[3].alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param oLine2.G[3].alpha=0
```
