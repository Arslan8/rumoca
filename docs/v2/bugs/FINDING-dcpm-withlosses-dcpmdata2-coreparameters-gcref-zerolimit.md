...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:652:c3fd59b1` |
...
```
predicate     dcpmData2.coreParameters.GcRef > 0
premise       refuted
...
```
contract    Modelica.Electrical.Machines.Losses.CoreParameters.GcRef: feature_disabled (declared, from bound) — the binding is a conditional with a literal zero branch, so the declaration itself names zero as one of the two values it intends
...
| `required` | dcpmData2.coreParameters.GcRef &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_withLosses \
      --target dcpmData2.coreParameters.GcRef --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcpmData2.coreParameters.GcRef"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param dcpmData2.coreParameters.GcRef=0
```
