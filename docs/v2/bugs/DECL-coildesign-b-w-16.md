# DECL-0463: `b_w` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/FluxTubes/Examples/Utilities/CoilDesign.mo:16` |
| **Parameter** | `b_w` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '16p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/Utilities/CoilDesign.mo"
```

It should declare `b_w` with type `SI.Length` and no `min`.
...
```modelica
// Magnetic/FluxTubes/Examples/Utilities/CoilDesign.mo:16
parameter SI.Length b_w(min=0) = ...;
```
