...
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:290:78944265` |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Utilities.Nand \
      --target TP1.L --sanitizer divisor
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Utilities.Nand --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "TP1.L"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param TP1.L=0
```
