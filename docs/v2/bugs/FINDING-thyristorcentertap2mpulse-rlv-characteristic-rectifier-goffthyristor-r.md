# FINDING-03790: `rectifier.GoffThyristor` in `ThyristorCenterTap2mPulse_RLV_Characteristic`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/ThyristorCenterTap2mPulse.mo:9` |
| **Signature** | `physical:physical-rule-does-not-apply:var:151:ef080656` |
...
```
contract    Modelica.Electrical.Polyphase.Ideal.IdealThyristor.Goff: allowed (declared, from class_catalog) — inherited from rectifier.thyristor_p.Goff, which it binds: an ideal switch family member; zero off-conductance is the documented ideal
behavior    allowed
confidence  declared
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.PowerConverters.ACDC.ThyristorCenterTap2mPulse.GoffThyristor'}
```
...
```console
$ sed -n '9p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/ThyristorCenterTap2mPulse.mo"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/ThyristorCenterTap2mPulse.mo:9` |' docs/v2/bugs/
```
