# FINDING-03291: `transformer.R2` in `TransformerTestbench`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Interfaces/PartialBasicTransformer.mo:21` |
| **Signature** | `physical:physical-rule-does-not-apply:var:542:e655dfac` |
...
```
contract    Modelica.Electrical.Analog.Basic.Resistor.R: allowed (declared, from class_catalog) — inherited from transformer.r2.R, which it binds: the component explicitly supports signed resistance, so neither a positivity rule nor a nonzero rule follows from the quantity alone
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.Transformers.Dy.Dy01.R2'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.Transformers.TransformerTestbench \
      --target transformer.R2 --sanitizer physical
```
...
```console
$ sed -n '21p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Interfaces/PartialBasicTransformer.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "transformer.R2"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param transformer.R2=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Interfaces/PartialBasicTransformer.mo:21` |' docs/v2/bugs/
```
