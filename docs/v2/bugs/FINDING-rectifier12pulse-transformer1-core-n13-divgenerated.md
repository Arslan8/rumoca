# FINDING-03191: `transformer1.core.n13` in `Rectifier12pulse`
...
| **Reached as** | `transformer1.core.n13` |
...
| **Signature** | `divisor:divisor-introduced-by-translation:exp:3462:7f4d8de5` |
...
```
contract    Modelica.Electrical.Machines.BasicMachines.Components.IdealCore.n13: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
behavior    
confidence  proven
source      
```
...
```
denominator            transformer1.core.n13
witness                transformer1.core.n13 = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  transformer1.core.n13: unbounded
```
...
| `constraints` | transformer1.core.n13: unbounded |
| `contract` | Modelica.Electrical.Machines.BasicMachines.Components.IdealCore.n13: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.Transformers.Rectifier12pulse \
      --target transformer1.core.n13 --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "transformer1.core.n13"
```
