...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:710:7e3e9485` |
...
```
predicate     vehicleInclination.rho > 0
premise       refuted
...
| `required` | vehicleInclination.rho &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Translational.Vehicles \
      --target vehicleInclination.rho --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "vehicleInclination.rho"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param vehicleInclination.rho=0
```
