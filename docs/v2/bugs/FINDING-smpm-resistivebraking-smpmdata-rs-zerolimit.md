# FINDING-02759: `smpmData.Rs` in `SMPM_ResistiveBraking`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:22:3186a449` |
...
```
contract    Modelica.Electrical.Machines.Utilities.ParameterRecords.SM_PermanentMagnetData.Rs: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
