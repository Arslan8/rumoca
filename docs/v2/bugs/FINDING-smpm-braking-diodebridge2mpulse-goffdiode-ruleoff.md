...
| **Reached as** | `diodeBridge2mPulse.GoffDiode` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/DiodeBridge2mPulse.mo:8` |
| **Signature** | `physical:physical-rule-does-not-apply:var:624:89c51fbb` |
...
```
predicate     diodeBridge2mPulse.GoffDiode > 0
premise       refuted
...
```
contract    Modelica.Electrical.Polyphase.Ideal.IdealDiode.Goff: allowed (declared, from class_catalog) — inherited from diodeBridge2mPulse.diode_p.Goff, which it binds: an ideal switch family member; zero off-conductance is the documented ideal
behavior    allowed
confidence  declared
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.PowerConverters.ACDC.DiodeBridge2mPulse.GoffDiode'}
```
...
| `required` | diodeBridge2mPulse.GoffDiode &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_Braking \
      --target diodeBridge2mPulse.GoffDiode --sanitizer physical
```
...
```console
$ sed -n '8p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/DiodeBridge2mPulse.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "diodeBridge2mPulse.GoffDiode"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param diodeBridge2mPulse.GoffDiode=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/DiodeBridge2mPulse.mo:8` |' docs/v2/bugs/
```
