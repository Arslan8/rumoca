# FINDING-04142: `hbridge.inverter_p.RonDiode` in `HBridge_TrianglePWM_RL`
...
| **Reached as** | `hbridge.inverter_p.RonDiode` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:84:2fae51d6` |
...
```
predicate     hbridge.inverter_p.RonDiode > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealDiode.Ron: allowed (declared, from class_catalog) — inherited from hbridge.inverter_p.diode_p.Ron, which it binds: an ideal switch family member; zero on-resistance is the documented ideal
behavior    allowed
confidence  declared
...
| `required` | hbridge.inverter_p.RonDiode &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCDC.HBridge.HBridge_TrianglePWM_RL \
      --target hbridge.inverter_p.RonDiode --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "hbridge.inverter_p.RonDiode"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param hbridge.inverter_p.RonDiode=0
```
