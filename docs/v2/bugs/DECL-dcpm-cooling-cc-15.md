# DECL-0093: `Cc` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Machines/Examples/DCMachines/DCPM_Cooling.mo:15` |
| **Parameter** | `Cc` |
...
It should declare `Cc` with type `SI.HeatCapacity` and no `min`.
...
```modelica
// Electrical/Machines/Examples/DCMachines/DCPM_Cooling.mo:15
parameter SI.HeatCapacity Cc(min=0) = ...;
```
