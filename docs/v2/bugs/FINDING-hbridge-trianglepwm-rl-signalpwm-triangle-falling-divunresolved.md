# FINDING-04168: `signalPWM.triangle.falling` in `HBridge_TrianglePWM_RL`
...
| **Reached as** | `signalPWM.triangle.falling` |
...
| **Signature** | `divisor:divisor-zero-unresolved:exp:1266:13f8365f` |
...
```
denominator            signalPWM.triangle.falling
witness                signalPWM.triangle.falling = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.0005
path condition         not ((((time < signalPWM.triangle.startTime) or (signalPWM.triangle.nperiod == 0)) or ((signalPWM.triangle.nperiod > 0) and (signalPWM.triangle.count >= signalPWM.triangle.nperiod)))) and not ((time < (signalPWM.triangle.T_start + signalPWM.triangle.T_rising))) and not ((time < (signalPWM.triangle.T_start + signalPWM.triangle.T_width))) and ((time < (signalPWM.triangle.T_start + signalPWM.triangle.T_falling)))
constraints consulted  signalPWM.triangle.falling: min=0
```
...
| `denominator_at_declared_values` | 0.0005 |
| `denominator_range` | [0, inf] |
| `path_condition` | not ((((time &lt; signalPWM.triangle.startTime) or (signalPWM.triangle.nperiod == 0)) or ((signalPWM.triangle.nperiod &gt; 0) and (signalPWM.triangle.count &gt;= signalPWM.triangle.nperiod)))) and not ((time &lt; (signalPWM.triangle.T_start + signalPWM.triangle.T_rising))) and not ((time &lt; (signalPWM.triangle.T_start + signalPWM.triangle.T_width))) and ((time &lt; (signalPWM.triangle.T_start + signalPWM.triangle.T_falling))) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: (time &lt; (signalPWM.triangle.T_start + signalPWM.triangle.T_falling)); (time &lt; (signalPWM.triangle.T_start + signalPWM.triangle.T_rising)); (time &lt; (signalPWM.triangle.T_start + signalPWM.triangle.T_width)) |
| `constraints` | signalPWM.triangle.falling: min=0 |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCDC.HBridge.HBridge_TrianglePWM_RL \
      --target signalPWM.triangle.falling --sanitizer divisor
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCDC.HBridge.HBridge_TrianglePWM_RL --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "signalPWM.triangle.falling"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param signalPWM.triangle.falling=0
```
