# FINDING-01878: `inverter.diode_p.Goff` in `IMC_InverterDrive`
...
| **Reached as** | `inverter.diode_p.Goff` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:625:d1ecc4c8` |
...
```
predicate     inverter.diode_p.Goff > 0
premise       refuted
...
| `required` | inverter.diode_p.Goff &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive \
      --target inverter.diode_p.Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inverter.diode_p.Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inverter.diode_p.Goff=0
```
