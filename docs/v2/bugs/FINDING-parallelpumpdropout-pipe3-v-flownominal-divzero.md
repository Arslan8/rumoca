...
| **Signature** | `divisor:divisor-reachable-zero:exp:445:a3c3eae3` |
...
```
denominator            ((pipe3.V_flowNominal - pipe3.V_flowLaminar) ^ 2)
witness                pipe3.V_flowNominal = 0.1
  found by             at its declared minimum
denominator at witness 0.0
at declared values     0.81
path condition         always evaluated
constraints consulted  pipe3.V_flowNominal: min=0.1, assert pipe3.V_flowNominal greater 0.1
```
...
| `constraints` | pipe3.V_flowNominal: min=0.1, assert pipe3.V_flowNominal greater 0.1 |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.ParallelPumpDropOut \
      --target pipe3.V_flowNominal --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pipe3.V_flowNominal"
```
