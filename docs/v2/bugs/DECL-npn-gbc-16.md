# DECL-0053: `Gbc` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Semiconductors/NPN.mo:16` |
| **Parameter** | `Gbc` |
| **Type** | `SI.Conductance` |
| **Declared modifiers** | `—` |
...
It should declare `Gbc` with type `SI.Conductance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/NPN.mo" | head
```
...
```modelica
// Electrical/Analog/Semiconductors/NPN.mo:16
parameter SI.Conductance Gbc(min=0) = ...;
```
