...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:307:18378468` |
...
| `denominator_range` |  |
| `path_condition` | ((abs(converter.i) &gt; converter.eps)) |
| `verdict` | UNSAT |
| `proof` | the denominator is zero as declared, and ((abs(converter.i) &gt; converter.eps)) |
| `constraints` |  |
| `guard_that_excludes_it` | ((abs(converter.i) &gt; converter.eps)) |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Magnetic.FluxTubes.BasicComponents \
      --target converter.i --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "converter.i"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param converter.i=0
```
