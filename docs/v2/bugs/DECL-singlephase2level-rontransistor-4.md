...
| **Declaration** | `Electrical/PowerConverters/DCAC/SinglePhase2Level.mo:4` |
| **Parameter** | `RonTransistor` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
It should declare `RonTransistor` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/PowerConverters/DCAC/SinglePhase2Level.mo:4
parameter SI.Resistance RonTransistor(min=0) = ...;
```
