...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:253:4d6560c4` |
...
```
contract    Modelica.Thermal.FluidHeatFlow.Components.Pipe.m: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.IndirectCooling \
      --target outerPipe.m --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "outerPipe.m"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param outerPipe.m=0
```
