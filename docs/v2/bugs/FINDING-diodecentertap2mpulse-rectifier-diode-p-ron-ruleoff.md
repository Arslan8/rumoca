# FINDING-03752: `rectifier.diode_p.Ron` in `DiodeCenterTap2mPulse`
...
| **Reached as** | `rectifier.diode_p.Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:143:c5f94f0d` |
...
```
predicate     rectifier.diode_p.Ron > 0
premise       refuted
...
| `required` | rectifier.diode_p.Ron &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTap2mPulse.DiodeCenterTap2mPulse \
      --target rectifier.diode_p.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "rectifier.diode_p.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param rectifier.diode_p.Ron=0
```
