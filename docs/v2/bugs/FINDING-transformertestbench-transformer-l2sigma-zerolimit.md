# FINDING-03292: `transformer.L2sigma` in `TransformerTestbench`
...
| **Reached as** | `transformer.L2sigma` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Interfaces/PartialBasicTransformer.mo:30` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:545:d2e7bbbc` |
...
```
predicate     transformer.L2sigma > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Basic.Inductor.L: algebraic_limit (declared, from class_catalog) — inherited from transformer.l2sigma.L, which it binds: L*der(i) = v; at L = 0 the element is an ideal short, v = 0, which the component documentation states explicitly
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.Transformers.Dy.Dy01.L2sigma'}
```
...
| `required` | transformer.L2sigma &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.Transformers.TransformerTestbench \
      --target transformer.L2sigma --sanitizer physical
```
...
```console
$ sed -n '30p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Interfaces/PartialBasicTransformer.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "transformer.L2sigma"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param transformer.L2sigma=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Interfaces/PartialBasicTransformer.mo:30` |' docs/v2/bugs/
```
