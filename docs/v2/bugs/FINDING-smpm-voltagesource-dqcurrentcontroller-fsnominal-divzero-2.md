# FINDING-02860: `dqCurrentController.fsNominal` in `SMPM_VoltageSource`
...
| **Reached as** | `dqCurrentController.fsNominal` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/DQCurrentController.mo:16` |
| **Signature** | `divisor:divisor-reachable-zero:exp:1065:bfa2ff76` |
...
```
denominator            ((2 * (2 * asin(1.0))) * dqCurrentController.fsNominal)
witness                dqCurrentController.fsNominal = 0
  found by             set to zero
denominator at witness 0.0
at declared values     314.1592653589793
path condition         always evaluated
constraints consulted  dqCurrentController.fsNominal: unbounded
```
...
| `denominator_at_declared_values` | 314.1592653589793 |
| `denominator_range` | [-inf, inf] |
| `path_condition` | always evaluated |
| `verdict` | SAT |
| `proof` | the division is not inside any branch |
| `constraints` | dqCurrentController.fsNominal: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_VoltageSource \
      --target dqCurrentController.fsNominal --sanitizer divisor
```
...
```console
$ sed -n '16p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/DQCurrentController.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dqCurrentController.fsNominal"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param dqCurrentController.fsNominal=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/DQCurrentController.mo:16` |' docs/v2/bugs/
```
