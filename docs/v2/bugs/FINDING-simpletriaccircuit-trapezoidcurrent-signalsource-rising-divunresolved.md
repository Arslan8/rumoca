# FINDING-01178: `trapezoidCurrent.signalSource.rising` in `SimpleTriacCircuit`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Electrical.Analog.Examples.SimpleTriacCircuit` |
| **Reached as** | `trapezoidCurrent.signalSource.rising` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-zero-unresolved` |
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Blocks/Sources.mo:927` |
| **Signature** | `divisor:divisor-zero-unresolved:exp:1021:fb8e42fc` |
...
```
denominator            trapezoidCurrent.signalSource.rising
witness                trapezoidCurrent.signalSource.rising = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1e-06
path condition         not ((((time < trapezoidCurrent.signalSource.startTime) or (trapezoidCurrent.signalSource.nperiod == 0)) or ((trapezoidCurrent.signalSource.nperiod > 0) and (trapezoidCurrent.signalSource.count >= trapezoidCurrent.signalSource.nperiod)))) and ((time < (trapezoidCurrent.signalSource.T_start + trapezoidCurrent.signalSource.T_rising)))
constraints consulted  trapezoidCurrent.signalSource.rising: min=0
```
...
| `path_condition` | not ((((time &lt; trapezoidCurrent.signalSource.startTime) or (trapezoidCurrent.signalSource.nperiod == 0)) or ((trapezoidCurrent.signalSource.nperiod &gt; 0) and (trapezoidCurrent.signalSource.count &gt;= trapezoidCurrent.signalSource.nperiod)))) and ((time &lt; (trapezoidCurrent.signalSource.T_start + trapezoidCurrent.signalSource.T_rising))) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: (time &lt; (trapezoidCurrent.signalSource.T_start + trapezoidCurrent.signalSource.T_rising)); (time &lt; trapezoidCurrent.signalSource.startTime) |
| `constraints` | trapezoidCurrent.signalSource.rising: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.SimpleTriacCircuit \
      --target trapezoidCurrent.signalSource.rising --sanitizer divisor
```
...
```console
$ sed -n '927p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Blocks/Sources.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "trapezoidCurrent.signalSource.rising"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param trapezoidCurrent.signalSource.rising=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Blocks/Sources.mo:927` |' docs/v2/bugs/
```
