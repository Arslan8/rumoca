...
| **Reached as** | `controllerFeedbackPart4.T` |
...
| **Signature** | `divisor:divisor-reachable-zero:exp:633:5eb7d1f0` |
...
```
denominator            controllerFeedbackPart4.T
witness                controllerFeedbackPart4.T = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  controllerFeedbackPart4.T: unbounded
```
...
| `constraints` | controllerFeedbackPart4.T: unbounded |
...
```console
$ python3 tools/sweep/check_one.py ModelicaTest.Blocks.LimitersHomotopy \
      --target controllerFeedbackPart4.T --sanitizer divisor
```
...
```console
$ python3 tools/sweep/check_one.py ModelicaTest.Blocks.LimitersHomotopy --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "controllerFeedbackPart4.T"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param controllerFeedbackPart4.T=0
```
