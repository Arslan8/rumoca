...
| **Reach** | 5 models set this declaration |
...
`Modelica.Mechanics.Translational.Examples.Damper` runs cleanly at its declared values. Setting `mass2.m = 0` — a value the declaration permits — makes it fail, in Rumoca and in OpenModelica.
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Translational.Examples.Damper \
      --target mass2.m --keep-parameter-chains
```
...
simulate(Modelica.Mechanics.Translational.Examples.Damper, simflags="-override mass2.m=0");
EOF
```
