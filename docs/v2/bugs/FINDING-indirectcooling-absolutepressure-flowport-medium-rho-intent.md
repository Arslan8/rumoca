# FINDING-04773: `absolutePressure.flowPort.medium.rho` in `IndirectCooling`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Thermal.FluidHeatFlow.Examples.IndirectCooling` |
| **Reached as** | `absolutePressure.flowPort.medium.rho` |
...
| **Signature** | `physical:physical-intent-question:var:186:3247b49d` |
...
```
predicate     absolutePressure.flowPort.medium.rho > 0
premise       unknown
...
> `absolutePressure.flowPort.medium.rho` is bounded only by min=0, which is unusual for a density — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
| `required` | absolutePressure.flowPort.medium.rho &gt; 0 |
| `declared_min` | 0.0 |
...
| `question` | `absolutePressure.flowPort.medium.rho` is bounded only by min=0, which is unusual for a density — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.IndirectCooling \
      --target absolutePressure.flowPort.medium.rho --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.IndirectCooling --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "absolutePressure.flowPort.medium.rho"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param absolutePressure.flowPort.medium.rho=0
```
