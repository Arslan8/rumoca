# FINDING-02242: `switchYD.idealOpener.idealOpeningSwitch[3].Goff` in `IMC_YD`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Electrical.Machines.Examples.InductionMachines.IMC_YD` |
| **Reached as** | `switchYD.idealOpener.idealOpeningSwitch[3].Goff` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Interfaces/IdealSwitch.mo:6` |
| **Signature** | `physical:physical-rule-does-not-apply:var:825:3ec64180` |
...
```
predicate     switchYD.idealOpener.idealOpeningSwitch[3].Goff > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealOpeningSwitch.Goff: allowed (declared, from class_catalog) — an ideal switch family member; zero off-conductance is the documented ideal [matched by prefix, lower confidence]
behavior    allowed
confidence  declared
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Ideal.IdealOpeningSwitch.Goff'}
```
...
| `required` | switchYD.idealOpener.idealOpeningSwitch[3].Goff &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_YD \
      --target switchYD.idealOpener.idealOpeningSwitch[3].Goff --sanitizer physical
```
...
```console
$ sed -n '6p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Interfaces/IdealSwitch.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switchYD.idealOpener.idealOpeningSwitch[3].Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switchYD.idealOpener.idealOpeningSwitch[3].Goff=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Interfaces/IdealSwitch.mo:6` |' docs/v2/bugs/
```
