# DECL-0515: `rv` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/Examples/Loops/PlanarLoops_analytic.mo:7` |
| **Parameter** | `rv` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '7p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Examples/Loops/PlanarLoops_analytic.mo"
```

It should declare `rv` with type `SI.Length` and no `min`.
...
```modelica
// Mechanics/MultiBody/Examples/Loops/PlanarLoops_analytic.mo:7
parameter SI.Length rv(min=0) = ...;
```
