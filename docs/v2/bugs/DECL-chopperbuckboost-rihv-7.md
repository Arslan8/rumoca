# DECL-0793: `RiHV` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/PowerConverters/Examples/DCDC/ExampleTemplates/ChopperBuckBoost.mo:7` |
| **Parameter** | `RiHV` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
It should declare `RiHV` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/PowerConverters/Examples/DCDC/ExampleTemplates/ChopperBuckBoost.mo:7
parameter SI.Resistance RiHV(min=0) = ...;
```
