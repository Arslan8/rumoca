# DECL-0574: `gravityLineLength` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/package.mo:276` |
| **Parameter** | `gravityLineLength` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '276p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/package.mo"
```

It should declare `gravityLineLength` with type `SI.Length` and no `min`.
...
```modelica
// Mechanics/MultiBody/package.mo:276
parameter SI.Length gravityLineLength(min=0) = ...;
```
