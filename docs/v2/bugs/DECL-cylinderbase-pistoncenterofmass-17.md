# DECL-0534: `pistonCenterOfMass` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/Examples/Loops/Utilities/CylinderBase.mo:17` |
| **Parameter** | `pistonCenterOfMass` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '17p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Examples/Loops/Utilities/CylinderBase.mo"
```

It should declare `pistonCenterOfMass` with type `SI.Length` and no `min`.
...
```modelica
// Mechanics/MultiBody/Examples/Loops/Utilities/CylinderBase.mo:17
parameter SI.Length pistonCenterOfMass(min=0) = ...;
```
