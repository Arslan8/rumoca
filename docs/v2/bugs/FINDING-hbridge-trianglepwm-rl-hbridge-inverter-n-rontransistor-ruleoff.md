# FINDING-04152: `hbridge.inverter_n.RonTransistor` in `HBridge_TrianglePWM_RL`
...
| **Reached as** | `hbridge.inverter_n.RonTransistor` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCAC/SinglePhase2Level.mo:4` |
| **Signature** | `physical:physical-rule-does-not-apply:var:193:12918ea1` |
...
```
predicate     hbridge.inverter_n.RonTransistor > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealGTOThyristor.Ron: allowed (declared, from class_catalog) — inherited from hbridge.inverter_n.transistor_p.Ron, which it binds: an ideal switch family member; zero on-resistance is the documented ideal
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.PowerConverters.DCAC.SinglePhase2Level.RonTransistor'}
```
...
| `required` | hbridge.inverter_n.RonTransistor &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCDC.HBridge.HBridge_TrianglePWM_RL \
      --target hbridge.inverter_n.RonTransistor --sanitizer physical
```
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCAC/SinglePhase2Level.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "hbridge.inverter_n.RonTransistor"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param hbridge.inverter_n.RonTransistor=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCAC/SinglePhase2Level.mo:4` |' docs/v2/bugs/
```
