# FINDING-03167: `diode4.idealDiode[1].Goff` in `Rectifier12pulse`
...
| **Reached as** | `diode4.idealDiode[1].Goff` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:887:c4200ac4` |
...
```
predicate     diode4.idealDiode[1].Goff > 0
premise       refuted
...
| `required` | diode4.idealDiode[1].Goff &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.Transformers.Rectifier12pulse \
      --target diode4.idealDiode[1].Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "diode4.idealDiode[1].Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param diode4.idealDiode[1].Goff=0
```
