...
| **Signature** | `divisor:divisor-zero-unresolved:exp:558:c0e1a3cb` |
...
```
denominator            trapezoid.rising
witness                trapezoid.rising = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.6
path condition         not ((((time < trapezoid.startTime) or (trapezoid.nperiod == 0)) or ((trapezoid.nperiod > 0) and (trapezoid.count >= trapezoid.nperiod)))) and ((time < (trapezoid.T_start + trapezoid.T_rising)))
constraints consulted  trapezoid.rising: min=0
```
...
| `path_condition` | not ((((time &lt; trapezoid.startTime) or (trapezoid.nperiod == 0)) or ((trapezoid.nperiod &gt; 0) and (trapezoid.count &gt;= trapezoid.nperiod)))) and ((time &lt; (trapezoid.T_start + trapezoid.T_rising))) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: (time &lt; (trapezoid.T_start + trapezoid.T_rising)); (time &lt; trapezoid.startTime) |
| `constraints` | trapezoid.rising: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.WaterPump \
      --target trapezoid.rising --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "trapezoid.rising"
```
