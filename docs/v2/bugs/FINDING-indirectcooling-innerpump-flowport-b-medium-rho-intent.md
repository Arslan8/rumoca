...
| **Signature** | `physical:physical-intent-question:var:225:33079ab4` |
...
```
predicate     innerPump.flowPort_b.medium.rho > 0
premise       unknown
...
> `innerPump.flowPort_b.medium.rho` is bounded only by min=0, which is unusual for a density — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
| `required` | innerPump.flowPort_b.medium.rho &gt; 0 |
| `declared_min` | 0.0 |
...
| `question` | `innerPump.flowPort_b.medium.rho` is bounded only by min=0, which is unusual for a density — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.IndirectCooling \
      --target innerPump.flowPort_b.medium.rho --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "innerPump.flowPort_b.medium.rho"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param innerPump.flowPort_b.medium.rho=0
```
