# FINDING-04685: `m` in `CompareBrakingForce`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Examples/CompareBrakingForce.mo:4` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:0:beea4844` |
...
```
contract    Modelica.Mechanics.Translational.Components.Mass.m: algebraic_limit (proven, from equation) — inherited from mass1.m, which it binds: the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
...
```
{'quantity': 'Mass', 'unit': 'kg', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Mechanics.Translational.Examples.CompareBrakingForce.m'}
```
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Examples/CompareBrakingForce.mo"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Examples/CompareBrakingForce.mo:4` |' docs/v2/bugs/
```
