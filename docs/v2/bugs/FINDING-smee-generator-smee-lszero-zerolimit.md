# FINDING-02437: `smee.Lszero` in `SMEE_Generator`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:14:c1ce972e` |
...
```
contract    Modelica.Electrical.Analog.Basic.Inductor.L: algebraic_limit (proven, from equation) — inherited from smee.lszero.L, which it binds: the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.SynchronousMachines.SM_ElectricalExcited.Lszero'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMEE_Generator \
      --target smee.Lszero --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smee.Lszero"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param smee.Lszero=0
```
