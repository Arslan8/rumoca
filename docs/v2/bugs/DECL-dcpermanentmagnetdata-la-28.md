...
| **Declaration** | `Electrical/Machines/Utilities/ParameterRecords/DcPermanentMagnetData.mo:28` |
| **Parameter** | `La` |
| **Type** | `SI.Inductance` |
| **Declared modifiers** | `—` |
...
It should declare `La` with type `SI.Inductance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/ParameterRecords/DcPermanentMagnetData.mo" | head
```
...
```modelica
// Electrical/Machines/Utilities/ParameterRecords/DcPermanentMagnetData.mo:28
parameter SI.Inductance La(min=0) = ...;
```
