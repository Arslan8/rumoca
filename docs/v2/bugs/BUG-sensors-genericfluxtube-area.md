...
| **Fix site** | [BUG-017: `GenericFluxTube.area` reaches a divisor two classes away, unbounded](../../verified%20bugs/BUG-017-genericfluxtube-area-chain-divisor.md) |
| **Status** | Reported upstream, not fixed |


## The claim

`ModelicaTest.Magnetic.FluxTubes.Sensors` runs cleanly at its declared values. Setting `genericFluxTube.area = 0` — a value the declaration permits — makes it fail, in Rumoca and in OpenModelica.
...
```
denominator            genericFluxTube.A
witness                genericFluxTube.area = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.0001
path condition         always evaluated
constraints consulted  genericFluxTube.area: unbounded
```
...
| `constraints` | genericFluxTube.area: unbounded |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Magnetic.FluxTubes.Sensors \
      --target genericFluxTube.area --keep-parameter-chains
```
...
$ sed -n '37p' "$(find target/msl target/corpus -name 'FixedShape.mo' | head -1)"
```
...
simulate(ModelicaTest.Magnetic.FluxTubes.Sensors, simflags="-override genericFluxTube.area=0");
EOF
```
