# FINDING-01893: `inverter.diode_n.Ron` in `IMC_InverterDrive`
...
| **Reached as** | `inverter.diode_n.Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:786:e5a53450` |
...
```
predicate     inverter.diode_n.Ron > 0
premise       refuted
...
| `required` | inverter.diode_n.Ron &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive \
      --target inverter.diode_n.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inverter.diode_n.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inverter.diode_n.Ron=0
```
