# FINDING-02052: `idealCloser.idealClosingSwitch[1].Goff` in `IMC_Transformer`
...
| **Reached as** | `idealCloser.idealClosingSwitch[1].Goff` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:592:b9700f34` |
...
```
predicate     idealCloser.idealClosingSwitch[1].Goff > 0
premise       refuted
...
| `required` | idealCloser.idealClosingSwitch[1].Goff &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_Transformer \
      --target idealCloser.idealClosingSwitch[1].Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "idealCloser.idealClosingSwitch[1].Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param idealCloser.idealClosingSwitch[1].Goff=0
```
