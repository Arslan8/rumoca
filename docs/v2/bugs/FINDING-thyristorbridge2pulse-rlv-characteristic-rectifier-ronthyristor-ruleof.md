# FINDING-03711: `rectifier.RonThyristor` in `ThyristorBridge2Pulse_RLV_Characteristic`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/ThyristorBridge2Pulse.mo:5` |
| **Signature** | `physical:physical-rule-does-not-apply:var:60:9de763b0` |
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealThyristor.Ron: allowed (declared, from class_catalog) — inherited from rectifier.thyristor_p1.Ron, which it binds: an ideal switch family member; zero on-resistance is the documented ideal
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.PowerConverters.ACDC.ThyristorBridge2Pulse.RonThyristor'}
```
...
```console
$ sed -n '5p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/ThyristorBridge2Pulse.mo"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/ThyristorBridge2Pulse.mo:5` |' docs/v2/bugs/
```
