...
| **Reached as** | `pmActuator.g_mLeak2.l` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-introduced-by-translation` |
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/BaseClasses/Leakage.mo:12` |
| **Signature** | `divisor:divisor-introduced-by-translation:exp:1256:cbda4226` |
...
```
contract    Modelica.Magnetic.FluxTubes.Shapes.Leakage.HalfCylinder.l: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
behavior    
confidence  proven
source      
```
...
```
denominator            pmActuator.g_mLeak2.G_m
witness                pmActuator.g_mLeak2.l = 0
  found by             set to zero
denominator at witness 0.0
at declared values     3.2332824035569943e-08
path condition         always evaluated
constraints consulted  pmActuator.g_mLeak2.l: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.ArmatureStroke \
      --target pmActuator.g_mLeak2.l --sanitizer divisor
```
...
```console
$ sed -n '12p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/BaseClasses/Leakage.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pmActuator.g_mLeak2.l"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/BaseClasses/Leakage.mo:12` |' docs/v2/bugs/
```
