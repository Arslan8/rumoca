...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:44:22cc1106` |
...
```
predicate     inverseCapacity.mass.C > 0
premise       refuted
...
| `required` | inverseCapacity.mass.C &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.HeatTransfer.Examples.GenerationOfFMUs \
      --target inverseCapacity.mass.C --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inverseCapacity.mass.C"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inverseCapacity.mass.C=0
```
