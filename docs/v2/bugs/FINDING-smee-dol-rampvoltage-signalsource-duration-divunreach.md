# FINDING-02384: `rampVoltage.signalSource.duration` in `SMEE_DOL`
...
| **Reached as** | `rampVoltage.signalSource.duration` |
...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:3892:1c0a443a` |
...
```
denominator            rampVoltage.signalSource.duration
witness                rampVoltage.signalSource.duration = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.1
path condition         not ((time < rampVoltage.signalSource.startTime)) and ((time < (rampVoltage.signalSource.startTime + rampVoltage.signalSource.duration)))
constraints consulted  rampVoltage.signalSource.duration: min=0
```
...
| `path_condition` | not ((time &lt; rampVoltage.signalSource.startTime)) and ((time &lt; (rampVoltage.signalSource.startTime + rampVoltage.signalSource.duration))) |
| `verdict` | UNSAT |
| `proof` | the path conditions cannot hold together: time must be both &gt; 0.5 and &lt; 0.5 |
| `constraints` | rampVoltage.signalSource.duration: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMEE_DOL \
      --target rampVoltage.signalSource.duration --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "rampVoltage.signalSource.duration"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param rampVoltage.signalSource.duration=0
```
