# DECL-0455: `l_arm` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/FluxTubes/Examples/SolenoidActuator/Components/AdvancedSolenoid.mo:36` |
| **Parameter** | `l_arm` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '36p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/SolenoidActuator/Components/AdvancedSolenoid.mo"
```

It should declare `l_arm` with type `SI.Length` and no `min`.
...
```modelica
// Magnetic/FluxTubes/Examples/SolenoidActuator/Components/AdvancedSolenoid.mo:36
parameter SI.Length l_arm(min=0) = ...;
```
