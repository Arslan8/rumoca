# FINDING-04660: `Jmotor` in `FirstGrounded`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Rotational/Examples/FirstGrounded.mo:8` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:2:d383cf95` |
...
```
{'quantity': 'MomentOfInertia', 'unit': 'kg.m2', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Mechanics.Rotational.Examples.FirstGrounded.Jmotor'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Rotational.Examples.FirstGrounded \
      --target Jmotor --sanitizer physical
```
...
```console
$ sed -n '8p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Rotational/Examples/FirstGrounded.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "Jmotor"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param Jmotor=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Rotational/Examples/FirstGrounded.mo:8` |' docs/v2/bugs/
```
