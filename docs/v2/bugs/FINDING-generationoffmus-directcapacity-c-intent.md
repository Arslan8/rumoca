...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/HeatTransfer/Examples/Utilities/DirectCapacity.mo:5` |
| **Signature** | `physical:physical-intent-question:var:7:e7d32aa3` |
...
```
predicate     directCapacity.C > 0
premise       unknown
authority     quantity_or_unit
role          —
declaration   Modelica.Thermal.HeatTransfer.Examples.Utilities.DirectCapacity.C
```
...
> `directCapacity.C` is bounded only by min=0, which is unusual for a heat capacity — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
```
{'quantity': 'HeatCapacity', 'unit': 'J/K', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Thermal.HeatTransfer.Examples.Utilities.DirectCapacity.C'}
```
...
| `required` | directCapacity.C &gt; 0 |
| `declared_min` | 0.0 |
| `variable_role` | parameter |
| `observation` | permissive-declaration |
| `premise_state` | unknown |
| `authority` | quantity_or_unit |
| `canonical_declaration` | Modelica.Thermal.HeatTransfer.Examples.Utilities.DirectCapacity.C |
| `question` | `directCapacity.C` is bounded only by min=0, which is unusual for a heat capacity — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.HeatTransfer.Examples.GenerationOfFMUs \
      --target directCapacity.C --sanitizer physical
```
...
```console
$ sed -n '5p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/HeatTransfer/Examples/Utilities/DirectCapacity.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "directCapacity.C"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param directCapacity.C=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/HeatTransfer/Examples/Utilities/DirectCapacity.mo:5` |' docs/v2/bugs/
```
