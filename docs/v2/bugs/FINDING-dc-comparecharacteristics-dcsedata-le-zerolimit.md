# FINDING-01335: `dcseData.Le` in `DC_CompareCharacteristics`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/ParameterRecords/DcSeriesExcitedData.mo:14` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:746:37315a5b` |
...
```
contract    Modelica.Electrical.Machines.BasicMachines.QuasiStaticDCMachines.DC_SeriesExcited.Le: feature_disabled (proven, from equation) — inherited from dcse.Le, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.Utilities.ParameterRecords.DcSeriesExcitedData.Le'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DC_CompareCharacteristics \
      --target dcseData.Le --sanitizer physical
```
...
```console
$ sed -n '14p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/ParameterRecords/DcSeriesExcitedData.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcseData.Le"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param dcseData.Le=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/ParameterRecords/DcSeriesExcitedData.mo:14` |' docs/v2/bugs/
```
