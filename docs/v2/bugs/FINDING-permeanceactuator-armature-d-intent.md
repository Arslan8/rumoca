# FINDING-04269: `armature.d` in `PermeanceActuator`
...
| **Signature** | `physical:physical-intent-question:var:31:3652c0ff` |
...
| `question` | `armature.d` carries no bound that would exclude it, which is unusual for a translational damping constant — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.Components.PermeanceActuator \
      --target armature.d --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.Components.PermeanceActuator --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "armature.d"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param armature.d=0
```
