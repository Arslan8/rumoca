# DECL-0695: `RonD` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Machines/Examples/ControlledDCDrives/Utilities/SwitchingDcDc.mo:11` |
| **Parameter** | `RonD` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '11p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/ControlledDCDrives/Utilities/SwitchingDcDc.mo"
```

It should declare `RonD` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/Machines/Examples/ControlledDCDrives/Utilities/SwitchingDcDc.mo:11
parameter SI.Resistance RonD(min=0) = ...;
```
