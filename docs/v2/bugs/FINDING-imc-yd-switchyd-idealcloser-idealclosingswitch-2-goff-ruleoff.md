# FINDING-02232: `switchYD.idealCloser.idealClosingSwitch[2].Goff` in `IMC_YD`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Electrical.Machines.Examples.InductionMachines.IMC_YD` |
| **Reached as** | `switchYD.idealCloser.idealClosingSwitch[2].Goff` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:733:753f84ba` |
...
```
predicate     switchYD.idealCloser.idealClosingSwitch[2].Goff > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealClosingSwitch.Goff: allowed (declared, from class_catalog) — an ideal switch family member; zero off-conductance is the documented ideal [matched by prefix, lower confidence]
behavior    allowed
confidence  declared
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Ideal.IdealClosingSwitch.Goff'}
```
...
| `required` | switchYD.idealCloser.idealClosingSwitch[2].Goff &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_YD \
      --target switchYD.idealCloser.idealClosingSwitch[2].Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switchYD.idealCloser.idealClosingSwitch[2].Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switchYD.idealCloser.idealClosingSwitch[2].Goff=0
```
