# FINDING-01242: `TN1.Tnom` in `Nand`
...
| **Signature** | `divisor:divisor-zero-unresolved:exp:778:9bc8d891` |
...
```
denominator            TN1.Tnom
witness                TN1.Tnom = 0
  found by             set to zero
denominator at witness 0.0
at declared values     300.15
path condition         (TN1.useTemperatureDependency)
constraints consulted  TN1.Tnom: min=0
```
...
| `path_condition` | (TN1.useTemperatureDependency) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: TN1.useTemperatureDependency |
| `constraints` | TN1.Tnom: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Utilities.Nand \
      --target TN1.Tnom --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "TN1.Tnom"
```
