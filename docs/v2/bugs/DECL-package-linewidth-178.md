# DECL-0569: `lineWidth` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/package.mo:178` |
| **Parameter** | `lineWidth` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '178p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/package.mo"
```

It should declare `lineWidth` with type `SI.Length` and no `min`.
...
```modelica
// Mechanics/MultiBody/package.mo:178
parameter SI.Length lineWidth(min=0) = ...;
```
