# FINDING-02480: `smeeData.VsNominal` in `SMEE_Generator`
...
| **Reached as** | `smeeData.VsNominal` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo:10` |
| **Signature** | `divisor:divisor-reachable-zero:exp:1776:0a03cb30` |
...
```
denominator            (3 * smeeData.VsNominal)
witness                smeeData.VsNominal = 0
  found by             set to zero
denominator at witness 0.0
at declared values     300.0
path condition         always evaluated
constraints consulted  smeeData.VsNominal: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMEE_Generator \
      --target smeeData.VsNominal --sanitizer divisor
```
...
```console
$ sed -n '10p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smeeData.VsNominal"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo:10` |' docs/v2/bugs/
```
