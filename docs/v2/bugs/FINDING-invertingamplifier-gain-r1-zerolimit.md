# FINDING-00894: `gain.R1` in `InvertingAmplifier`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/OpAmps/OpAmpCircuits/Gain.mo:5` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:74:8ace0a51` |
...
```
contract    Modelica.Electrical.Analog.Basic.Resistor.R: feature_disabled (proven, from equation) — inherited from gain.r1.R, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Examples.OpAmps.OpAmpCircuits.Gain.R1'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.InvertingAmplifier \
      --target gain.R1 --sanitizer physical
```
...
```console
$ sed -n '5p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/OpAmps/OpAmpCircuits/Gain.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "gain.R1"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param gain.R1=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/OpAmps/OpAmpCircuits/Gain.mo:5` |' docs/v2/bugs/
```
