...
| **Declaration** | `Electrical/PowerConverters/DCAC/SinglePhase2Level.mo:6` |
...
It should declare `GoffTransistor` with type `SI.Conductance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCAC/SinglePhase2Level.mo" | head
```
...
```modelica
// Electrical/PowerConverters/DCAC/SinglePhase2Level.mo:6
parameter SI.Conductance GoffTransistor(min=0) = ...;
```
