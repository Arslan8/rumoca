...
| **Signature** | `physical:physical-intent-question:var:77:db7a17b1` |
...
> `elastoGap.c` is bounded only by min=0, which is unusual for a translational spring constant — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
| `question` | `elastoGap.c` is bounded only by min=0, which is unusual for a translational spring constant — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
```console
$ python3 tools/sweep/check_one.py ModelicaTest.Translational.AllComponents \
      --target elastoGap.c --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py ModelicaTest.Translational.AllComponents --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "elastoGap.c"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param elastoGap.c=0
```
