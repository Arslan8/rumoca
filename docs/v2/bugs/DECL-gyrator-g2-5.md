# DECL-0045: `G2` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Basic/Gyrator.mo:5` |
| **Parameter** | `G2` |
| **Type** | `SI.Conductance` |
...
```console
$ sed -n '5p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/Gyrator.mo"
```

It should declare `G2` with type `SI.Conductance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/Gyrator.mo" | head
```
...
```modelica
// Electrical/Analog/Basic/Gyrator.mo:5
parameter SI.Conductance G2(min=0) = ...;
```
