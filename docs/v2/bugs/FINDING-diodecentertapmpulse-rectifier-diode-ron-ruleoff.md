# FINDING-03896: `rectifier.diode.Ron` in `DiodeCenterTapmPulse`
...
| **Reached as** | `rectifier.diode.Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:136:d0e6bc29` |
...
```
predicate     rectifier.diode.Ron > 0
premise       refuted
...
| `required` | rectifier.diode.Ron &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTapmPulse.DiodeCenterTapmPulse \
      --target rectifier.diode.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "rectifier.diode.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param rectifier.diode.Ron=0
```
