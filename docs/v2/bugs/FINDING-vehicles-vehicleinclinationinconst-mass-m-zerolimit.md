# FINDING-05445: `vehicleInclinationInConst.mass.m` in `Vehicles`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `ModelicaTest.Translational.Vehicles` |
| **Reached as** | `vehicleInclinationInConst.mass.m` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:914:e61fb934` |
...
```
predicate     vehicleInclinationInConst.mass.m > 0
premise       refuted
...
| `required` | vehicleInclinationInConst.mass.m &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Translational.Vehicles \
      --target vehicleInclinationInConst.mass.m --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "vehicleInclinationInConst.mass.m"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param vehicleInclinationInConst.mass.m=0
```
