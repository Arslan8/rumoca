# DECL-0360: `I_axis` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/Parts/RollingWheel.mo:10` |
| **Parameter** | `I_axis` |
| **Type** | `SI.Inertia` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '10p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/RollingWheel.mo"
```

It should declare `I_axis` with type `SI.Inertia` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/RollingWheel.mo" | head
```
...
```modelica
// Mechanics/MultiBody/Parts/RollingWheel.mo:10
parameter SI.Inertia I_axis(min=0) = ...;
```
