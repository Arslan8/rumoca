...
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:6601:1d86360a` |
...
```
denominator            (1 + (oLine3.G[3].alpha * (oLine3.G[3].T_heatPort - oLine3.G[3].T_ref)))
witness                none needed
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.CompareLineTrunks \
      --target oLine3.G[3].alpha --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "oLine3.G[3].alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param oLine3.G[3].alpha=0
```
