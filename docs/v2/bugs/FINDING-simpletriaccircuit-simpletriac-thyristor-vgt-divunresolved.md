...
| **Signature** | `divisor:divisor-zero-unresolved:exp:494:bc7e09c3` |
...
```
denominator            simpleTriac.thyristor.VGT
witness                simpleTriac.thyristor.VGT = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.7
path condition         not ((simpleTriac.thyristor.vGK < simpleTriac.thyristor.vRef))
constraints consulted  simpleTriac.thyristor.VGT: unbounded
```
...
| `path_condition` | not ((simpleTriac.thyristor.vGK &lt; simpleTriac.thyristor.vRef)) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: (simpleTriac.thyristor.vGK &lt; simpleTriac.thyristor.vRef) |
| `constraints` | simpleTriac.thyristor.VGT: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.SimpleTriacCircuit \
      --target simpleTriac.thyristor.VGT --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "simpleTriac.thyristor.VGT"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param simpleTriac.thyristor.VGT=0
```
