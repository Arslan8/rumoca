# FINDING-01729: `imc.airGap.Lm` in `IMC_DCBraking`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:425:9669f24e` |
...
```
contract    Modelica.Electrical.Machines.BasicMachines.Components.AirGapS.L: feature_disabled (proven, from equation) — inherited from imc.airGap.L, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_DCBraking \
      --target imc.airGap.Lm --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "imc.airGap.Lm"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param imc.airGap.Lm=0
```
