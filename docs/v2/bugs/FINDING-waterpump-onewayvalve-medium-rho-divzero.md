# FINDING-05123: `oneWayValve.medium.rho` in `WaterPump`
...
| **Signature** | `divisor:divisor-reachable-zero:exp:886:af589755` |
...
```
denominator            oneWayValve.medium.rho
witness                oneWayValve.medium.rho = 0
  found by             set to zero
denominator at witness 0.0
at declared values     995.6
path condition         always evaluated
constraints consulted  oneWayValve.medium.rho: min=0
```
...
| `constraints` | oneWayValve.medium.rho: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.WaterPump \
      --target oneWayValve.medium.rho --sanitizer divisor
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param oneWayValve.medium.rho=0
```
