...
`ModelicaTest.Magnetic.FluxTubes.Sources` runs cleanly at its declared values. Setting `genericFluxTube2.l = 0` — a value the declaration permits — makes it fail, in Rumoca and in OpenModelica.
...
$ python3 tools/sweep/check_one.py ModelicaTest.Magnetic.FluxTubes.Sources \
      --target genericFluxTube2.l --keep-parameter-chains
```
...
simulate(ModelicaTest.Magnetic.FluxTubes.Sources, simflags="-override genericFluxTube2.l=0");
EOF
```
