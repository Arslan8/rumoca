# DECL-0229: `L1sigma` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/QuasiStatic/Machines/Interfaces/PartialBasicTransformer.mo:20` |
| **Parameter** | `L1sigma` |
| **Type** | `SI.Inductance` |
| **Declared modifiers** | `(start=78E-6/(if C1 == "D" then 1 else 3)` |
...
```console
$ sed -n '20p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/QuasiStatic/Machines/Interfaces/PartialBasicTransformer.mo"
```

It should declare `L1sigma` with type `SI.Inductance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/QuasiStatic/Machines/Interfaces/PartialBasicTransformer.mo" | head
```
...
```modelica
// Electrical/QuasiStatic/Machines/Interfaces/PartialBasicTransformer.mo:20
parameter SI.Inductance L1sigma(min=0) = ...;
```
