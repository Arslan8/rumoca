...
| **Signature** | `divisor:divisor-reachable-zero:exp:155:9783776d` |
...
```
denominator            simpleTriac.thyristor.VTM
witness                simpleTriac.thyristor.VTM = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.7
path condition         always evaluated
constraints consulted  simpleTriac.thyristor.VTM: unbounded
```
...
| `constraints` | simpleTriac.thyristor.VTM: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.SimpleTriacCircuit \
      --target simpleTriac.thyristor.VTM --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "simpleTriac.thyristor.VTM"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param simpleTriac.thyristor.VTM=0
```
