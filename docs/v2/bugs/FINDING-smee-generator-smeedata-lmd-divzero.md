# FINDING-02484: `smeeData.Lmd` in `SMEE_Generator`
...
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo:21` |
| **Signature** | `divisor:divisor-reachable-zero:exp:2444:2a400fca` |
...
```
denominator            ((smeeData.omega * smeeData.Lmd) * smeeData.IeOpenCircuit)
witness                smeeData.Lmd = 0
  found by             set to zero
denominator at witness 0.0
at declared values     15.0
path condition         always evaluated
constraints consulted  smeeData.Lmd: unbounded
```
...
| `constraints` | smeeData.Lmd: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMEE_Generator \
      --target smeeData.Lmd --sanitizer divisor
```
...
```console
$ sed -n '21p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo:21` |' docs/v2/bugs/
```
