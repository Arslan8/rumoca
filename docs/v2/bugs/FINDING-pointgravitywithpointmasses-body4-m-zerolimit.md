# FINDING-04508: `body4.m` in `PointGravityWithPointMasses`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:310:42f087d8` |
...
```
contract    Modelica.Mechanics.MultiBody.Parts.PointMass.m: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Mass', 'unit': 'kg', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Mechanics.MultiBody.Parts.PointMass.m'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Elementary.PointGravityWithPointMasses \
      --target body4.m --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "body4.m"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param body4.m=0
```
