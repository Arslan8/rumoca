# DECL-0018: `pistonCrossArea` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Fluid/Machines.mo:9` |
| **Parameter** | `pistonCrossArea` |
| **Type** | `SI.Area` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '9p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Machines.mo"
```

It should declare `pistonCrossArea` with type `SI.Area` and no `min`.
...
```modelica
// Fluid/Machines.mo:9
parameter SI.Area pistonCrossArea(min=0) = ...;
```
