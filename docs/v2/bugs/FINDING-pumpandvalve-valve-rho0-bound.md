...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/Components/Valve.mo:19` |
| **Signature** | `physical:physical-bound-permits-zero:var:120:2bacd2fa` |
...
```
predicate     valve.rho0 > 0
premise       established
authority     source_arithmetic
role          —
declaration   Modelica.Thermal.FluidHeatFlow.Components.Valve.rho0
```
...
```
{'quantity': 'Density', 'unit': 'kg/m3', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Thermal.FluidHeatFlow.Components.Valve.rho0'}
```
...
| `required` | valve.rho0 &gt; 0 |
| `declared_min` | 0.0 |
| `variable_role` | parameter |
| `premise_state` | established |
| `authority` | source_arithmetic |
| `canonical_declaration` | Modelica.Thermal.FluidHeatFlow.Components.Valve.rho0 |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.PumpAndValve \
      --target valve.rho0 --sanitizer physical
```
...
```console
$ sed -n '19p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/Components/Valve.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "valve.rho0"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param valve.rho0=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/Components/Valve.mo:19` |' docs/v2/bugs/
```
