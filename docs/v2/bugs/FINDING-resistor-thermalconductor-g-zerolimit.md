# FINDING-01117: `thermalConductor.G` in `Resistor`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Electrical.Analog.Examples.Resistor` |
| **Reached as** | `thermalConductor.G` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:42:9cd097ee` |
...
```
predicate     thermalConductor.G > 0
premise       refuted
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Resistor \
      --target thermalConductor.G --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Resistor --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "thermalConductor.G"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param thermalConductor.G=0
```
