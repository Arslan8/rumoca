...
| **Signature** | `divisor:divisor-reachable-zero:exp:221:80ad08ab` |
...
```
denominator            simpleTriac.thyristor1.VTM
witness                simpleTriac.thyristor1.VTM = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.7
path condition         always evaluated
constraints consulted  simpleTriac.thyristor1.VTM: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.SimpleTriacCircuit \
      --target simpleTriac.thyristor1.VTM --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "simpleTriac.thyristor1.VTM"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param simpleTriac.thyristor1.VTM=0
```
