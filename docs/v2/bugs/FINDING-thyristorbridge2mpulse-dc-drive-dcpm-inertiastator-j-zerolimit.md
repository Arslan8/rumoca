# FINDING-03477: `dcpm.inertiaStator.J` in `ThyristorBridge2mPulse_DC_Drive`
...
| **Reached as** | `dcpm.inertiaStator.J` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Rotational/Components/Inertia.mo:4` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:902:7ba5cba3` |
...
```
predicate     dcpm.inertiaStator.J > 0
premise       refuted
...
```
contract    Modelica.Mechanics.Rotational.Components.Inertia.J: algebraic_limit (proven, from equation) — the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
...
```
{'quantity': 'MomentOfInertia', 'unit': 'kg.m2', 'semantic_role': 'component.rotational.inertia', 'binding_source': 'component_type', 'confidence': 'QUANTITY_AND_UNIT', 'declaring_class': 'Modelica.Mechanics.Rotational.Components.Inertia', 'member': 'J', 'premise_state': 'established', 'authority': 'component_contract', 'canonical_declaration': 'Modelica.Mechanics.Rotational.Components.Inertia.J'}
```
...
| `required` | dcpm.inertiaStator.J &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.ThyristorBridge2mPulse_DC_Drive \
      --target dcpm.inertiaStator.J --sanitizer physical
```
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Rotational/Components/Inertia.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcpm.inertiaStator.J"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param dcpm.inertiaStator.J=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Rotational/Components/Inertia.mo:4` |' docs/v2/bugs/
```
