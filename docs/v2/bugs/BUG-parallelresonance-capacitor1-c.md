...
| **Reach** | 2 models set this declaration |
...
| **Fix site** | [BUG-022: `ChuaCircuit.C2.C` fails at `0`, a value its declaration permits](../../verified%20bugs/BUG-022-chuacircuit-c2-c.md) |
| **Status** | Reported upstream, not fixed |


## The claim

`Modelica.Electrical.Analog.Examples.ParallelResonance` runs cleanly at its declared values. Setting `capacitor1.C = 0` — a value the declaration permits — makes it fail, in Rumoca and in OpenModelica.
...
```
{'quantity': 'Capacitance', 'unit': 'F', 'semantic_role': 'component.passive.capacitance', 'binding_source': 'component_type', 'confidence': 'QUANTITY_AND_UNIT', 'declaring_class': 'Modelica.Electrical.Analog.Basic.Capacitor', 'member': 'C'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.ParallelResonance \
      --target capacitor1.C --keep-parameter-chains
```
...
```console
$ find target/msl target/corpus -name 'Capacitor.mo'
$ sed -n '4p' "$(find target/msl target/corpus -name 'Capacitor.mo' | head -1)"
```
...
```console
$ ./target/debug/rumoca compile "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/ParallelResonance.mo" --model Modelica.Electrical.Analog.Examples.ParallelResonance \
...
simulate(Modelica.Electrical.Analog.Examples.ParallelResonance, simflags="-override capacitor1.C=0");
EOF
```
