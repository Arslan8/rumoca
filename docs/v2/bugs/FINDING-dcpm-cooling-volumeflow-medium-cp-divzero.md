...
| **Signature** | `divisor:divisor-reachable-zero:exp:1630:a4b49b0f` |
...
```
denominator            volumeFlow.medium.cp
witness                volumeFlow.medium.cp = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  volumeFlow.medium.cp: unbounded
```
...
| `constraints` | volumeFlow.medium.cp: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Cooling \
      --target volumeFlow.medium.cp --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "volumeFlow.medium.cp"
```
