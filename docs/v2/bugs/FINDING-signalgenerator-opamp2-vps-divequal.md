...
| **Signature** | `divisor:divisor-zero-when-parameters-equal:exp:247:0eff5053` |
...
```
denominator            (opAmp2.vps - opAmp2.vns)
witness                opAmp2.Vps = -15
  found by             set equal to opAmp2.Vns
denominator at witness 0.0
at declared values     30.0
path condition         always evaluated
constraints consulted  opAmp2.Vps: unbounded
```
...
| `denominator` | (opAmp2.vps - opAmp2.vns) |
| `witness` | opAmp2.Vps = -15 |
| `witness_rationale` | set equal to opAmp2.Vns |
...
| `constraints` | opAmp2.Vps: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.SignalGenerator \
      --target opAmp2.Vps --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "opAmp2.Vps"
```
