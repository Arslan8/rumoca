...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Examples/Utilities/SpringDamper.mo:6` |
| **Signature** | `physical:physical-intent-question:var:87:edd2ccb5` |
...
```
predicate     springDamper.d >= 0
premise       unknown
authority     quantity_or_unit
role          —
declaration   Modelica.Mechanics.Translational.Examples.Utilities.SpringDamper.d
```
...
```
{'quantity': 'TranslationalDampingConstant', 'unit': 'N.s/m', 'confidence': 'QUANTITY', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Mechanics.Translational.Examples.Utilities.SpringDamper.d'}
```
...
| `required` | springDamper.d &gt;= 0 |
| `declared_min` | None |
| `variable_role` | parameter |
| `observation` | permissive-declaration |
| `premise_state` | unknown |
| `authority` | quantity_or_unit |
| `canonical_declaration` | Modelica.Mechanics.Translational.Examples.Utilities.SpringDamper.d |
| `question` | `springDamper.d` carries no bound that would exclude it, which is unusual for a translational damping constant — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Translational.Examples.GenerationOfFMUs \
      --target springDamper.d --sanitizer physical
```
...
```console
$ sed -n '6p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Examples/Utilities/SpringDamper.mo"
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Translational.Examples.GenerationOfFMUs --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "springDamper.d"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param springDamper.d=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Examples/Utilities/SpringDamper.mo:6` |' docs/v2/bugs/
```
