...
| **Reached as** | `diodeBridge2mPulse.diode_p.idealDiode[2].Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:666:edb648f5` |
...
```
predicate     diodeBridge2mPulse.diode_p.idealDiode[2].Ron > 0
premise       refuted
...
| `required` | diodeBridge2mPulse.diode_p.idealDiode[2].Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_Braking \
      --target diodeBridge2mPulse.diode_p.idealDiode[2].Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "diodeBridge2mPulse.diode_p.idealDiode[2].Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param diodeBridge2mPulse.diode_p.idealDiode[2].Ron=0
```
