# FINDING-00834: `data.R1` in `DifferentialAmplifier`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/OpAmps/OpAmpCircuits/DifferentialAmplifierData.mo:18` |
| **Signature** | `physical:physical-intent-question:var:7:4097c199` |
...
role          —
declaration   Modelica.Electrical.Analog.Examples.OpAmps.OpAmpCircuits.DifferentialAmplifierData.R1
```
...
> `data.R1` carries no bound that would exclude it, which is unusual for a resistance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Examples.OpAmps.OpAmpCircuits.DifferentialAmplifierData.R1'}
```
...
| `authority` | quantity_or_unit |
| `canonical_declaration` | Modelica.Electrical.Analog.Examples.OpAmps.OpAmpCircuits.DifferentialAmplifierData.R1 |
| `question` | `data.R1` carries no bound that would exclude it, which is unusual for a resistance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.DifferentialAmplifier \
      --target data.R1 --sanitizer physical
```
...
```console
$ sed -n '18p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/OpAmps/OpAmpCircuits/DifferentialAmplifierData.mo"
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.DifferentialAmplifier --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "data.R1"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param data.R1=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/OpAmps/OpAmpCircuits/DifferentialAmplifierData.mo:18` |' docs/v2/bugs/
```
