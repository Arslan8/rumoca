...
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Batteries/BatteryStacks/SuperCap.mo:22` |
| **Signature** | `divisor:divisor-reachable-zero:exp:262:25e6daf1` |
...
```
denominator            superCap.Vnom
witness                superCap.Vnom = 0
  found by             set to zero
denominator at witness 0.0
at declared values     48.0
path condition         always evaluated
constraints consulted  superCap.Vnom: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Batteries.Examples.SuperCapDischargeCharge \
      --target superCap.Vnom --sanitizer divisor
```
...
```console
$ sed -n '22p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Batteries/BatteryStacks/SuperCap.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "superCap.Vnom"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Batteries/BatteryStacks/SuperCap.mo:22` |' docs/v2/bugs/
```
