# DECL-0878: `Ron` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/QuasiStatic/FundamentalWave/Utilities/SwitchYD.mo:4` |
| **Parameter** | `Ron` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/QuasiStatic/FundamentalWave/Utilities/SwitchYD.mo"
```

It should declare `Ron` with type `SI.Resistance` and no `min`.
...
```modelica
// Magnetic/QuasiStatic/FundamentalWave/Utilities/SwitchYD.mo:4
parameter SI.Resistance Ron(min=0) = ...;
```
