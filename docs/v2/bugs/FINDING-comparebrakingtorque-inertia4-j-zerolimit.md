...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:63:d938a999` |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Rotational.Examples.CompareBrakingTorque \
      --target inertia4.J --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Rotational.Examples.CompareBrakingTorque --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inertia4.J"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inertia4.J=0
```
