# FINDING-01865: `inverter.RonTransistor` in `IMC_InverterDrive`
...
| **Reached as** | `inverter.RonTransistor` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCAC/Polyphase2Level.mo:5` |
| **Signature** | `physical:physical-rule-does-not-apply:var:520:4b1bbf3c` |
...
```
predicate     inverter.RonTransistor > 0
premise       refuted
...
```
contract    Modelica.Electrical.Polyphase.Ideal.IdealGTOThyristor.Ron: allowed (declared, from class_catalog) — inherited from inverter.transistor_p.Ron, which it binds: an ideal switch family member; zero on-resistance is the documented ideal
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.PowerConverters.DCAC.Polyphase2Level.RonTransistor'}
```
...
| `required` | inverter.RonTransistor &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive \
      --target inverter.RonTransistor --sanitizer physical
```
...
```console
$ sed -n '5p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCAC/Polyphase2Level.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inverter.RonTransistor"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inverter.RonTransistor=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCAC/Polyphase2Level.mo:5` |' docs/v2/bugs/
```
