# DECL-0600: `Rdm` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Basic/OpAmpDetailed.mo:5` |
| **Parameter** | `Rdm` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '5p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/OpAmpDetailed.mo"
```

It should declare `Rdm` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/Analog/Basic/OpAmpDetailed.mo:5
parameter SI.Resistance Rdm(min=0) = ...;
```
