# DECL-0898: `V0` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Fluid/Examples/AST_BatchPlant.mo:1446` |
| **Parameter** | `V0` |
| **Type** | `SI.Volume` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '1446p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Examples/AST_BatchPlant.mo"
```

It should declare `V0` with type `SI.Volume` and no `min`.
...
```modelica
// Fluid/Examples/AST_BatchPlant.mo:1446
parameter SI.Volume V0(min=0) = ...;
```
