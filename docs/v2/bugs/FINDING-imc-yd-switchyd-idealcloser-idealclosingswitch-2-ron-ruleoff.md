# FINDING-02231: `switchYD.idealCloser.idealClosingSwitch[2].Ron` in `IMC_YD`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Electrical.Machines.Examples.InductionMachines.IMC_YD` |
| **Reached as** | `switchYD.idealCloser.idealClosingSwitch[2].Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:732:344bbbee` |
...
```
predicate     switchYD.idealCloser.idealClosingSwitch[2].Ron > 0
premise       refuted
...
| `required` | switchYD.idealCloser.idealClosingSwitch[2].Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_YD \
      --target switchYD.idealCloser.idealClosingSwitch[2].Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switchYD.idealCloser.idealClosingSwitch[2].Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switchYD.idealCloser.idealClosingSwitch[2].Ron=0
```
