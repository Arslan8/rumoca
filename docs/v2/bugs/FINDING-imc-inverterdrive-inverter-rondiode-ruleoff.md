# FINDING-01867: `inverter.RonDiode` in `IMC_InverterDrive`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCAC/Polyphase2Level.mo:11` |
| **Signature** | `physical:physical-rule-does-not-apply:var:523:4cbafcfd` |
...
```
contract    Modelica.Electrical.Polyphase.Ideal.IdealDiode.Ron: allowed (declared, from class_catalog) — inherited from inverter.diode_p.Ron, which it binds: an ideal switch family member; zero on-resistance is the documented ideal
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.PowerConverters.DCAC.Polyphase2Level.RonDiode'}
```
...
```console
$ sed -n '11p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCAC/Polyphase2Level.mo"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCAC/Polyphase2Level.mo:11` |' docs/v2/bugs/
```
