...
| **Reached as** | `inverseCapacitor1.C` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/Utilities/InverseCapacitor.mo:4` |
| **Signature** | `physical:physical-intent-question:var:52:37346153` |
...
```
predicate     inverseCapacitor1.C > 0
premise       unknown
authority     quantity_or_unit
role          —
declaration   Modelica.Electrical.Analog.Examples.Utilities.InverseCapacitor.C
```
...
> `inverseCapacitor1.C` is bounded only by min=0, which is unusual for a capacitance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
```
{'quantity': 'Capacitance', 'unit': 'F', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Examples.Utilities.InverseCapacitor.C'}
```
...
| `required` | inverseCapacitor1.C &gt; 0 |
| `declared_min` | 0.0 |
| `variable_role` | parameter |
| `observation` | permissive-declaration |
| `premise_state` | unknown |
| `authority` | quantity_or_unit |
| `canonical_declaration` | Modelica.Electrical.Analog.Examples.Utilities.InverseCapacitor.C |
| `question` | `inverseCapacitor1.C` is bounded only by min=0, which is unusual for a capacitance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.GenerationOfFMUs \
      --target inverseCapacitor1.C --sanitizer physical
```
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/Utilities/InverseCapacitor.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inverseCapacitor1.C"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inverseCapacitor1.C=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/Utilities/InverseCapacitor.mo:4` |' docs/v2/bugs/
```
