# FINDING-02336: `aimsData.VrLockedRotor` in `IMS_Start`
...
| **Reached as** | `aimsData.VrLockedRotor` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/ParameterRecords/IM_SlipRingData.mo:31` |
| **Signature** | `divisor:divisor-reachable-zero:exp:2726:3b9f478f` |
...
```
denominator            aimsData.VrLockedRotor
witness                aimsData.VrLockedRotor = 0
  found by             set to zero
denominator at witness 0.0
at declared values     96.60262311821904
path condition         always evaluated
constraints consulted  aimsData.VrLockedRotor: unbounded
```
...
| `constraints` | aimsData.VrLockedRotor: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMS_Start \
      --target aimsData.VrLockedRotor --sanitizer divisor
```
...
```console
$ sed -n '31p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/ParameterRecords/IM_SlipRingData.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "aimsData.VrLockedRotor"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param aimsData.VrLockedRotor=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/ParameterRecords/IM_SlipRingData.mo:31` |' docs/v2/bugs/
```
