# DECL-0635: `R3` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Examples/OpAmps/OpAmpCircuits/DifferentialAmplifierData.mo:22` |
| **Parameter** | `R3` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
It should declare `R3` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/Analog/Examples/OpAmps/OpAmpCircuits/DifferentialAmplifierData.mo:22
parameter SI.Resistance R3(min=0) = ...;
```
