...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:669:3919be7b` |
...
```
denominator            V2.signalSource.duration
witness                V2.signalSource.duration = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1e-09
path condition         not ((time < V2.signalSource.startTime)) and ((time < (V2.signalSource.startTime + V2.signalSource.duration)))
constraints consulted  V2.signalSource.duration: min=0
```
...
| `path_condition` | not ((time &lt; V2.signalSource.startTime)) and ((time &lt; (V2.signalSource.startTime + V2.signalSource.duration))) |
| `verdict` | UNSAT |
| `proof` | the path conditions cannot hold together: time must be both &gt; 0 and &lt; 0 |
| `constraints` | V2.signalSource.duration: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.DifferenceAmplifier \
      --target V2.signalSource.duration --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "V2.signalSource.duration"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param V2.signalSource.duration=0
```
