# FINDING-05080: `openTank2.medium.cp` in `TwoTanks`
...
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/BaseClasses/SinglePortBottom.mo:20` |
| **Signature** | `divisor:divisor-reachable-zero:exp:281:bb42dbe1` |
...
```
denominator            openTank2.medium.cp
witness                openTank2.medium.cp = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  openTank2.medium.cp: unbounded
```
...
| `constraints` | openTank2.medium.cp: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.TwoTanks \
      --target openTank2.medium.cp --sanitizer divisor
```
...
```console
$ sed -n '20p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/BaseClasses/SinglePortBottom.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "openTank2.medium.cp"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/BaseClasses/SinglePortBottom.mo:20` |' docs/v2/bugs/
```
