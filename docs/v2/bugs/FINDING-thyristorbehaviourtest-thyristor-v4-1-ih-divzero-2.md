...
| **Signature** | `divisor:divisor-reachable-zero:exp:153:f7a55f9e` |
...
```
denominator            thyristor_v4_1.IH
witness                thyristor_v4_1.IH = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.006
path condition         always evaluated
constraints consulted  thyristor_v4_1.IH: unbounded
```
...
| `constraints` | thyristor_v4_1.IH: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.ThyristorBehaviourTest \
      --target thyristor_v4_1.IH --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "thyristor_v4_1.IH"
```
