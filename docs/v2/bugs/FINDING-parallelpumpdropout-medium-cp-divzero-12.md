# FINDING-04918: `medium.cp` in `ParallelPumpDropOut`
...
| **Signature** | `divisor:divisor-reachable-zero:exp:1258:f4b33a56` |
...
| `constraints` | medium.cp: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.ParallelPumpDropOut \
      --target medium.cp --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "medium.cp"
```
