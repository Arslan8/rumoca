# FINDING-04338: `g_mFeYokeSide.alpha` in `SimpleSolenoid`
...
| **Reached as** | `g_mFeYokeSide.alpha` |
...
| **Signature** | `divisor:divisor-introduced-by-translation:exp:803:3ce2b922` |
...
| `constraints` | g_mFeYokeSide.alpha: unbounded |
| `contract` | Modelica.Magnetic.FluxTubes.Shapes.FixedShape.HollowCylinderAxialFlux.alpha: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected |
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.SolenoidActuator.Components.SimpleSolenoid \
      --target g_mFeYokeSide.alpha --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "g_mFeYokeSide.alpha"
```
