# FINDING-00302: `Transistor2.Tr.Phic` in `DifferenceAmplifier`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Electrical.Analog.Examples.DifferenceAmplifier` |
| **Reached as** | `Transistor2.Tr.Phic` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/NPN.mo:81` |
| **Signature** | `divisor:divisor-reachable-zero:exp:1591:718a4216` |
...
```
denominator            Transistor2.Tr.Phic
witness                Transistor2.Tr.Phic = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.8
path condition         always evaluated
constraints consulted  Transistor2.Tr.Phic: unbounded
```
...
| `constraints` | Transistor2.Tr.Phic: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.DifferenceAmplifier \
      --target Transistor2.Tr.Phic --sanitizer divisor
```
...
```console
$ sed -n '81p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/NPN.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "Transistor2.Tr.Phic"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/NPN.mo:81` |' docs/v2/bugs/
```
