...
```
predicate     simpleTriac.idealDiode1.Ron > 0
premise       refuted
...
| `required` | simpleTriac.idealDiode1.Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.SimpleTriacCircuit \
      --target simpleTriac.idealDiode1.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "simpleTriac.idealDiode1.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param simpleTriac.idealDiode1.Ron=0
```
