# DECL-0242: `M` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/QuasiStatic/SinglePhase/Examples/MultipleResonance.mo:13` |
| **Parameter** | `M` |
| **Type** | `SI.Inductance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '13p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/QuasiStatic/SinglePhase/Examples/MultipleResonance.mo"
```

It should declare `M` with type `SI.Inductance` and no `min`.
...
```modelica
// Electrical/QuasiStatic/SinglePhase/Examples/MultipleResonance.mo:13
parameter SI.Inductance M(min=0) = ...;
```
