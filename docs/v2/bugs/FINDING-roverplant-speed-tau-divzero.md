...
| **Reached as** | `speed_tau` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/cmm/CMM-a642c381/RigidBody/Examples/RoverPlant.mo:56` |
| **Signature** | `divisor:divisor-reachable-zero:exp:635:926c11ae` |
...
```
denominator            speed_tau
witness                speed_tau = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.35
path condition         always evaluated
constraints consulted  speed_tau: unbounded
```
...
$ python3 tools/sweep/check_one.py RigidBody.Examples.RoverPlant \
      --target speed_tau --sanitizer divisor
```
...
```console
$ sed -n '56p' "target/cmm/CMM-a642c381/RigidBody/Examples/RoverPlant.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "speed_tau"
```
...
```console
$ grep -rl '| `target/cmm/CMM-a642c381/RigidBody/Examples/RoverPlant.mo:56` |' docs/v2/bugs/
```
