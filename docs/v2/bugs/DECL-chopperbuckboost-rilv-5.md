# DECL-0792: `RiLV` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/PowerConverters/Examples/DCDC/ExampleTemplates/ChopperBuckBoost.mo:5` |
| **Parameter** | `RiLV` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
It should declare `RiLV` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/PowerConverters/Examples/DCDC/ExampleTemplates/ChopperBuckBoost.mo:5
parameter SI.Resistance RiLV(min=0) = ...;
```
