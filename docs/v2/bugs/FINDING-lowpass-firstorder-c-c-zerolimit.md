...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.LowPass \
      --target firstOrder.c.C --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "firstOrder.c.C"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param firstOrder.c.C=0
```
