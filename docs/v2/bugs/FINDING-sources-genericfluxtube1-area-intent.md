...
| **Signature** | `physical:physical-intent-question:var:71:dd66cf30` |
...
```
predicate     genericFluxTube1.area > 0
premise       unknown
...
| `required` | genericFluxTube1.area &gt; 0 |
| `declared_min` | None |
...
| `question` | `genericFluxTube1.area` carries no bound that would exclude it, which is unusual for an area — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Magnetic.FluxTubes.Sources \
      --target genericFluxTube1.area --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "genericFluxTube1.area"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param genericFluxTube1.area=0
```
