# FINDING-01313: `dceeData.Js` in `DC_CompareCharacteristics`
...
| **Signature** | `physical:physical-intent-question:var:340:a699fefc` |
...
```
predicate     dceeData.Js > 0
premise       unknown
authority     quantity_or_unit
role          —
declaration   Modelica.Electrical.Machines.Utilities.ParameterRecords.DcElectricalExcitedData.Js
```
...
> `dceeData.Js` carries no bound that would exclude it, which is unusual for a moment of inertia — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
```
{'quantity': 'MomentOfInertia', 'unit': 'kg.m2', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.Utilities.ParameterRecords.DcElectricalExcitedData.Js'}
```
...
| `required` | dceeData.Js &gt; 0 |
| `declared_min` | None |
| `variable_role` | parameter |
| `observation` | permissive-declaration |
| `premise_state` | unknown |
| `authority` | quantity_or_unit |
| `canonical_declaration` | Modelica.Electrical.Machines.Utilities.ParameterRecords.DcElectricalExcitedData.Js |
| `question` | `dceeData.Js` carries no bound that would exclude it, which is unusual for a moment of inertia — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DC_CompareCharacteristics \
      --target dceeData.Js --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DC_CompareCharacteristics --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dceeData.Js"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param dceeData.Js=0
```
