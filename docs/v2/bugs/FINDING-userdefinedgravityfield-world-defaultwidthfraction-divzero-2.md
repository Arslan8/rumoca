# FINDING-04551: `world.defaultWidthFraction` in `UserDefinedGravityField`
...
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/package.mo:69` |
| **Signature** | `divisor:divisor-reachable-zero:exp:1544:54e4ba1f` |
...
```
denominator            world.defaultWidthFraction
witness                world.defaultWidthFraction = 0
  found by             set to zero
denominator at witness 0.0
at declared values     20.0
path condition         always evaluated
constraints consulted  world.defaultWidthFraction: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Elementary.UserDefinedGravityField \
      --target world.defaultWidthFraction --sanitizer divisor
```
...
```console
$ sed -n '69p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/package.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "world.defaultWidthFraction"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param world.defaultWidthFraction=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/package.mo:69` |' docs/v2/bugs/
```
