# FINDING-01162: `simpleTriac.thyristor.Ron` in `SimpleTriacCircuit`
...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:631:0e45ae1e` |
...
```
denominator            (sqrt((simpleTriac.thyristor.Ron * simpleTriac.thyristor.Roff)) * ((simpleTriac.thyristor.Ron / simpleTriac.thyristor.Roff) ^ (((3 * ((((2 * simpleTriac.thyristor.vControl) - simpleTriac.thyristor.Von) - simpleTriac.thyristor.Voff) / (2 * (simpleTriac.thyristor.Von - simpleTriac.thyristor.Voff)))) -
witness                simpleTriac.thyristor.Ron = 0
  found by             set to zero
denominator at witness 0.0
at declared values     12.806836436146927
path condition         not ((simpleTriac.thyristor.vAK < -simpleTriac.thyristor.VRRM)) and not ((simpleTriac.thyristor.vControl < simpleTriac.thyristor.Voff)) and ((simpleTriac.thyristor.vControl < simpleTriac.thyristor.Von))
constraints consulted  simpleTriac.thyristor.Ron: unbounded
```
...
| `denominator_at_declared_values` | 12.806836436146927 |
| `denominator_range` | [-inf, inf] |
| `path_condition` | not ((simpleTriac.thyristor.vAK &lt; -simpleTriac.thyristor.VRRM)) and not ((simpleTriac.thyristor.vControl &lt; simpleTriac.thyristor.Voff)) and ((simpleTriac.thyristor.vControl &lt; simpleTriac.thyristor.Von)) |
| `verdict` | UNSAT |
| `proof` | the guard not ((simpleTriac.thyristor.vControl &lt; simpleTriac.thyristor.Voff)) is false under the witness |
| `constraints` | simpleTriac.thyristor.Ron: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.SimpleTriacCircuit \
      --target simpleTriac.thyristor.Ron --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "simpleTriac.thyristor.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param simpleTriac.thyristor.Ron=0
```
