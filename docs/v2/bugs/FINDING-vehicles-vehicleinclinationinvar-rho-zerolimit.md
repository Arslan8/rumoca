...
| **Reached as** | `vehicleInclinationInVar.rho` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:944:275faf3d` |
...
```
predicate     vehicleInclinationInVar.rho > 0
premise       refuted
...
| `required` | vehicleInclinationInVar.rho &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Translational.Vehicles \
      --target vehicleInclinationInVar.rho --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "vehicleInclinationInVar.rho"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param vehicleInclinationInVar.rho=0
```
