# DECL-0863: `RRef` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/QuasiStatic/FundamentalWave/BasicMachines/Components/SymmetricPolyphaseCageWinding.mo:9` |
| **Parameter** | `RRef` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '9p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/QuasiStatic/FundamentalWave/BasicMachines/Components/SymmetricPolyphaseCageWinding.mo"
```

It should declare `RRef` with type `SI.Resistance` and no `min`.
...
```modelica
// Magnetic/QuasiStatic/FundamentalWave/BasicMachines/Components/SymmetricPolyphaseCageWinding.mo:9
parameter SI.Resistance RRef(min=0) = ...;
```
