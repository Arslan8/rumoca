...
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:8130:ca5b499b` |
...
```
denominator            (1 + (oLine5.G[4].alpha * (oLine5.G[4].T_heatPort - oLine5.G[4].T_ref)))
witness                none needed
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.SmoothStep \
      --target oLine5.G[4].alpha --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "oLine5.G[4].alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param oLine5.G[4].alpha=0
```
