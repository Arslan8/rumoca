# FINDING-02566: `diodeBridge2mPulse.diode_p.idealDiode[3].Ron` in `SMPM_Braking`
...
| **Reached as** | `diodeBridge2mPulse.diode_p.idealDiode[3].Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:683:fc39cd60` |
...
```
predicate     diodeBridge2mPulse.diode_p.idealDiode[3].Ron > 0
premise       refuted
...
| `required` | diodeBridge2mPulse.diode_p.idealDiode[3].Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_Braking \
      --target diodeBridge2mPulse.diode_p.idealDiode[3].Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "diodeBridge2mPulse.diode_p.idealDiode[3].Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param diodeBridge2mPulse.diode_p.idealDiode[3].Ron=0
```
