# BUG-011: `genericFluxTube.l` in `Sensors`
...
| **Fix site** | [BUG-014: `GenericFluxTube.l` is an unbounded divisor](../../verified%20bugs/BUG-014-genericfluxtube-l-unbounded-divisor.md) |
| **Status** | Reported upstream, not fixed |


## The claim

`ModelicaTest.Magnetic.FluxTubes.Sensors` runs cleanly at its declared values. Setting `genericFluxTube.l = 0` — a value the declaration permits — makes it fail, in Rumoca and in OpenModelica.
...
```
denominator            genericFluxTube.l
witness                genericFluxTube.l = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.01
path condition         always evaluated
constraints consulted  genericFluxTube.l: unbounded
```
...
| `constraints` | genericFluxTube.l: unbounded |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Magnetic.FluxTubes.Sensors \
      --target genericFluxTube.l --keep-parameter-chains
```
...
```console
$ find target/msl target/corpus -name 'GenericFluxTube.mo'
$ sed -n '15p' "$(find target/msl target/corpus -name 'GenericFluxTube.mo' | head -1)"
```
...
```console
$ ./target/debug/rumoca compile "target/corpus/ModelicaStandardLibrary-4.1.0/ModelicaTest/Magnetic/FluxTubes.mo" --model ModelicaTest.Magnetic.FluxTubes.Sensors \
...
simulate(ModelicaTest.Magnetic.FluxTubes.Sensors, simflags="-override genericFluxTube.l=0");
EOF
```
