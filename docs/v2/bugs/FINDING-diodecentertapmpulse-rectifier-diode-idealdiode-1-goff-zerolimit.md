# FINDING-03899: `rectifier.diode.idealDiode[1].Goff` in `DiodeCenterTapmPulse`
...
| **Reached as** | `rectifier.diode.idealDiode[1].Goff` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:140:7ecd5f9c` |
...
```
predicate     rectifier.diode.idealDiode[1].Goff > 0
premise       refuted
...
| `required` | rectifier.diode.idealDiode[1].Goff &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTapmPulse.DiodeCenterTapmPulse \
      --target rectifier.diode.idealDiode[1].Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "rectifier.diode.idealDiode[1].Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param rectifier.diode.idealDiode[1].Goff=0
```
