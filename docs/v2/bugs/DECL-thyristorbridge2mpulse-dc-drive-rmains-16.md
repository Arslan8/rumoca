# DECL-0768: `RMains` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/PowerConverters/Examples/ACDC/RectifierBridge2mPulse/ThyristorBridge2mPulse_DC_Drive.mo:16` |
| **Parameter** | `RMains` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
It should declare `RMains` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/PowerConverters/Examples/ACDC/RectifierBridge2mPulse/ThyristorBridge2mPulse_DC_Drive.mo:16
parameter SI.Resistance RMains(min=0) = ...;
```
