# FINDING-04316: `g_mLeak1.l` in `AdvancedSolenoid`
...
| **Signature** | `divisor:divisor-introduced-by-translation:exp:1793:56ac46b8` |
...
```
contract    Modelica.Magnetic.FluxTubes.Shapes.Leakage.QuarterCylinder.l: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
behavior    
confidence  proven
source      
```
...
```
denominator            g_mLeak1.G_m
witness                g_mLeak1.l = 0
  found by             set to zero
denominator at witness 0.0
at declared values     2.1863147681194913e-08
path condition         always evaluated
constraints consulted  g_mLeak1.l: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.SolenoidActuator.Components.AdvancedSolenoid \
      --target g_mLeak1.l --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "g_mLeak1.l"
```
