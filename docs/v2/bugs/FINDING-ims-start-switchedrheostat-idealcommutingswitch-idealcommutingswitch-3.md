# FINDING-02304: `switchedRheostat.idealCommutingSwitch.idealCommutingSwitch[3].Ron` in `IMS_Start`
...
| **Reached as** | `switchedRheostat.idealCommutingSwitch.idealCommutingSwitch[3].Ron` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Ideal/IdealTwoWaySwitch.mo:3` |
| **Signature** | `physical:physical-rule-does-not-apply:var:974:a1e42c39` |
...
```
predicate     switchedRheostat.idealCommutingSwitch.idealCommutingSwitch[3].Ron > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealTwoWaySwitch.Ron: allowed (declared, from class_catalog) — an ideal switch family member; zero on-resistance is the documented ideal [matched by prefix, lower confidence]
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Ideal.IdealTwoWaySwitch.Ron'}
```
...
| `required` | switchedRheostat.idealCommutingSwitch.idealCommutingSwitch[3].Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMS_Start \
      --target switchedRheostat.idealCommutingSwitch.idealCommutingSwitch[3].Ron --sanitizer physical
```
...
```console
$ sed -n '3p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Ideal/IdealTwoWaySwitch.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switchedRheostat.idealCommutingSwitch.idealCommutingSwitch[3].Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switchedRheostat.idealCommutingSwitch.idealCommutingSwitch[3].Ron=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Ideal/IdealTwoWaySwitch.mo:3` |' docs/v2/bugs/
```
