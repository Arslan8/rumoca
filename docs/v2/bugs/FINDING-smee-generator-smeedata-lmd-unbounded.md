# FINDING-02465: `smeeData.Lmd` in `SMEE_Generator`
...
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo:101` |
| **Signature** | `physical:physical-domain-unenforced:var:895:088b09e8` |
...
```
predicate     smeeData.Lmd > 0
premise       established
authority     source_arithmetic
role          —
declaration   Modelica.Electrical.Machines.Utilities.SynchronousMachineData.Lmd
```
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.Utilities.SynchronousMachineData.Lmd'}
```
...
| `required` | smeeData.Lmd &gt; 0 |
| `declared_min` | None |
| `variable_role` | parameter |
| `premise_state` | established |
| `authority` | source_arithmetic |
| `canonical_declaration` | Modelica.Electrical.Machines.Utilities.SynchronousMachineData.Lmd |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMEE_Generator \
      --target smeeData.Lmd --sanitizer physical
```
...
```console
$ sed -n '101p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smeeData.Lmd"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param smeeData.Lmd=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo:101` |' docs/v2/bugs/
```
