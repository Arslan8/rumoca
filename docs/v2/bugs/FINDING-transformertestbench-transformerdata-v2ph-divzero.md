# FINDING-03305: `transformerData.V2ph` in `TransformerTestbench`
...
| **Reached as** | `transformerData.V2ph` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/TransformerData.mo:33` |
| **Signature** | `divisor:divisor-reachable-zero:exp:1191:b4d7eed4` |
...
```
denominator            (3 * transformerData.V2ph)
witness                transformerData.V2ph = 0
  found by             set to zero
denominator at witness 0.0
at declared values     None
path condition         always evaluated
constraints consulted  transformerData.V2ph: unbounded
```
...
| `constraints` | transformerData.V2ph: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.Transformers.TransformerTestbench \
      --target transformerData.V2ph --sanitizer divisor
```
...
```console
$ sed -n '33p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/TransformerData.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "transformerData.V2ph"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/TransformerData.mo:33` |' docs/v2/bugs/
```
