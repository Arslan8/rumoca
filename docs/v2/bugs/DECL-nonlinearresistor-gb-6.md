# DECL-0050: `Gb` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Examples/Utilities/NonlinearResistor.mo:6` |
| **Parameter** | `Gb` |
| **Type** | `SI.Conductance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '6p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/Utilities/NonlinearResistor.mo"
```

It should declare `Gb` with type `SI.Conductance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/Utilities/NonlinearResistor.mo" | head
```
...
```modelica
// Electrical/Analog/Examples/Utilities/NonlinearResistor.mo:6
parameter SI.Conductance Gb(min=0) = ...;
```
