# FINDING-03173: `transformer2.L1sigma` in `Rectifier12pulse`
...
| **Reached as** | `transformer2.L1sigma` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:953:bf3bfaf2` |
...
```
predicate     transformer2.L1sigma > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Basic.Inductor.L: algebraic_limit (declared, from class_catalog) — inherited from transformer2.l1sigma.L, which it binds: L*der(i) = v; at L = 0 the element is an ideal short, v = 0, which the component documentation states explicitly
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.Transformers.Dd.Dd00.L1sigma'}
```
...
| `required` | transformer2.L1sigma &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.Transformers.Rectifier12pulse \
      --target transformer2.L1sigma --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "transformer2.L1sigma"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param transformer2.L1sigma=0
```
