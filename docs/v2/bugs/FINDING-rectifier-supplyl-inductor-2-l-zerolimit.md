...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:129:e1267c4a` |
...
```
predicate     supplyL.inductor[2].L > 0
premise       refuted
...
| `required` | supplyL.inductor[2].L &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Polyphase.Examples.Rectifier \
      --target supplyL.inductor[2].L --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "supplyL.inductor[2].L"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param supplyL.inductor[2].L=0
```
