# FINDING-02483: `smeeData.IsNominal` in `SMEE_Generator`
...
| **Signature** | `divisor:divisor-reachable-zero:exp:1787:caf9ccc5` |
...
```
denominator            smeeData.IsNominal
witness                smeeData.IsNominal = 0
  found by             set to zero
denominator at witness 0.0
at declared values     100.0
path condition         always evaluated
constraints consulted  smeeData.IsNominal: unbounded
```
...
| `constraints` | smeeData.IsNominal: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMEE_Generator \
      --target smeeData.IsNominal --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smeeData.IsNominal"
```
