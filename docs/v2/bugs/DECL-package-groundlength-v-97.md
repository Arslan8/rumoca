# DECL-0557: `groundLength_v` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/package.mo:97` |
| **Parameter** | `groundLength_v` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '97p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/package.mo"
```

It should declare `groundLength_v` with type `SI.Length` and no `min`.
...
```modelica
// Mechanics/MultiBody/package.mo:97
parameter SI.Length groundLength_v(min=0) = ...;
```
