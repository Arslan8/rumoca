# FINDING-01268: `GoffT` in `DcdcInverter`
...
| **Signature** | `physical:physical-intent-question:var:7:037584fb` |
...
| `question` | `GoffT` carries no bound that would exclude it, which is unusual for a conductance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.ControlledDCDrives.Utilities.DcdcInverter \
      --target GoffT --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "GoffT"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param GoffT=0
```
