# FINDING-01337: `dcse.Le` in `DC_CompareCharacteristics`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/DCMachines/DC_SeriesExcited.mo:35` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:752:ea137307` |
...
```
contract    Modelica.Electrical.Machines.BasicMachines.QuasiStaticDCMachines.DC_SeriesExcited.Le: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.QuasiStaticDCMachines.DC_SeriesExcited.Le'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DC_CompareCharacteristics \
      --target dcse.Le --sanitizer physical
```
...
```console
$ sed -n '35p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/DCMachines/DC_SeriesExcited.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcse.Le"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param dcse.Le=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/DCMachines/DC_SeriesExcited.mo:35` |' docs/v2/bugs/
```
