# FINDING-04176: `leftLeg.a` in `QuadraticCoreAirgap`
...
| **Signature** | `divisor:divisor-introduced-by-translation:exp:371:bad3a200` |
...
| `constraints` | leftLeg.a: unbounded |
| `contract` | Modelica.Magnetic.FluxTubes.Shapes.FixedShape.Cuboid.a: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected |
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.BasicExamples.QuadraticCoreAirgap \
      --target leftLeg.a --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "leftLeg.a"
```
