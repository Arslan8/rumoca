...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/HeatTransfer/Components/HeatCapacitor.mo:3` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:8:02712168` |
...
```
predicate     directCapacity.heatCapacitor.C > 0
premise       refuted
...
```
contract    Modelica.Thermal.HeatTransfer.Components.HeatCapacitor.C: algebraic_limit (proven, from equation) — the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
...
```
{'quantity': 'HeatCapacity', 'unit': 'J/K', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Thermal.HeatTransfer.Components.HeatCapacitor.C'}
```
...
| `required` | directCapacity.heatCapacitor.C &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.HeatTransfer.Examples.GenerationOfFMUs \
      --target directCapacity.heatCapacitor.C --sanitizer physical
```
...
```console
$ sed -n '3p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/HeatTransfer/Components/HeatCapacitor.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "directCapacity.heatCapacitor.C"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param directCapacity.heatCapacitor.C=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/HeatTransfer/Components/HeatCapacitor.mo:3` |' docs/v2/bugs/
```
