# DECL-0185: `Linf` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Polyphase/Basic/SaturatingInductor.mo:10` |
| **Parameter** | `Linf` |
| **Type** | `SI.Inductance` |
| **Declared modifiers** | `(start={Lnom[j]/2 for j in 1 :m})` |
...
```console
$ sed -n '10p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Basic/SaturatingInductor.mo"
```

It should declare `Linf` with type `SI.Inductance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Basic/SaturatingInductor.mo" | head
```
...
```modelica
// Electrical/Polyphase/Basic/SaturatingInductor.mo:10
parameter SI.Inductance Linf(min=0) = ...;
```
