# DECL-0005: `A_cross` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Fluid/Dissipation.mo:12629` |
| **Parameter** | `A_cross` |
| **Type** | `SI.Area` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '12629p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Dissipation.mo"
```

It should declare `A_cross` with type `SI.Area` and no `min`.
...
```modelica
// Fluid/Dissipation.mo:12629
parameter SI.Area A_cross(min=0) = ...;
```
