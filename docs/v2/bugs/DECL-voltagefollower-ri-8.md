# DECL-0652: `Ri` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Examples/OpAmps/VoltageFollower.mo:8` |
| **Parameter** | `Ri` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
It should declare `Ri` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/Analog/Examples/OpAmps/VoltageFollower.mo:8
parameter SI.Resistance Ri(min=0) = ...;
```
