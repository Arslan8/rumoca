# FINDING-03123: `diode1.idealDiode[1].Goff` in `Rectifier12pulse`
...
| **Reached as** | `diode1.idealDiode[1].Goff` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:149:f3db215e` |
...
```
predicate     diode1.idealDiode[1].Goff > 0
premise       refuted
...
| `required` | diode1.idealDiode[1].Goff &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.Transformers.Rectifier12pulse \
      --target diode1.idealDiode[1].Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "diode1.idealDiode[1].Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param diode1.idealDiode[1].Goff=0
```
