# FINDING-03196: `transformerData1.V2ph` in `Rectifier12pulse`
...
| **Reached as** | `transformerData1.V2ph` |
...
| **Signature** | `divisor:divisor-reachable-zero:exp:710:2175df11` |
...
```
denominator            (3 * transformerData1.V2ph)
witness                transformerData1.V2ph = 0
  found by             set to zero
denominator at witness 0.0
at declared values     None
path condition         always evaluated
constraints consulted  transformerData1.V2ph: unbounded
```
...
| `constraints` | transformerData1.V2ph: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.Transformers.Rectifier12pulse \
      --target transformerData1.V2ph --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "transformerData1.V2ph"
```
