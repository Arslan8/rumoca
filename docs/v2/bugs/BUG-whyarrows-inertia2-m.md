# BUG-017: `inertia2.m` in `WhyArrows`
...
`Modelica.Mechanics.Translational.Examples.WhyArrows` runs cleanly at its declared values. Setting `inertia2.m = 0` — a value the declaration permits — makes it fail, in Rumoca and in OpenModelica.
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Translational.Examples.WhyArrows \
      --target inertia2.m --keep-parameter-chains
```
...
```console
$ ./target/debug/rumoca compile "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Examples/WhyArrows.mo" --model Modelica.Mechanics.Translational.Examples.WhyArrows \
...
simulate(Modelica.Mechanics.Translational.Examples.WhyArrows, simflags="-override inertia2.m=0");
EOF
```
