...
| **Signature** | `divisor:divisor-reachable-zero:exp:1708:450dc432` |
...
```
denominator            cooling.medium.cp
witness                cooling.medium.cp = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  cooling.medium.cp: unbounded
```
...
| `constraints` | cooling.medium.cp: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Cooling \
      --target cooling.medium.cp --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "cooling.medium.cp"
```
