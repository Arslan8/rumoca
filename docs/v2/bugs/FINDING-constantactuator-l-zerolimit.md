# FINDING-04252: `L` in `ConstantActuator`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:2:c00aa058` |
...
```
contract    Modelica.Electrical.Analog.Basic.Inductor.L: algebraic_limit (proven, from equation) — inherited from l.L, which it binds: the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
