# FINDING-05455: `R` in `SwitchedRLC_MSL`
...
| **Severity** | low |
| **Declaration** | `examples/models/SwitchedRLC_MSL.mo:7` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:2:28268fb6` |
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'SwitchedRLC_MSL.R'}
```
...
```console
$ sed -n '7p' "examples/models/SwitchedRLC_MSL.mo"
```
...
```console
$ grep -rl '| `examples/models/SwitchedRLC_MSL.mo:7` |' docs/v2/bugs/
```
