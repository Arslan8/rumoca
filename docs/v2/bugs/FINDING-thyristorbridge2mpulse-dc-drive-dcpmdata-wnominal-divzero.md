# FINDING-03509: `dcpmData.wNominal` in `ThyristorBridge2mPulse_DC_Drive`
...
| **Reached as** | `dcpmData.wNominal` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/Examples/ACDC/RectifierBridge2mPulse/ThyristorBridge2mPulse_DC_Drive.mo:23` |
| **Signature** | `divisor:divisor-reachable-zero:exp:2991:cdbf8d40` |
...
```
denominator            dcpmData.wNominal
witness                dcpmData.wNominal = 0
  found by             set to zero
denominator at witness 0.0
at declared values     149.22565104551515
path condition         always evaluated
constraints consulted  dcpmData.wNominal: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.ThyristorBridge2mPulse_DC_Drive \
      --target dcpmData.wNominal --sanitizer divisor
```
...
```console
$ sed -n '23p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/Examples/ACDC/RectifierBridge2mPulse/ThyristorBridge2mPulse_DC_Drive.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcpmData.wNominal"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/Examples/ACDC/RectifierBridge2mPulse/ThyristorBridge2mPulse_DC_Drive.mo:23` |' docs/v2/bugs/
```
