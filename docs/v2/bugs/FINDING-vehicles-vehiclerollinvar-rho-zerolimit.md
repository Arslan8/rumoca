...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:591:ea6bacf2` |
...
```
predicate     vehicleRollInVar.rho > 0
premise       refuted
...
| `required` | vehicleRollInVar.rho &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Translational.Vehicles \
      --target vehicleRollInVar.rho --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "vehicleRollInVar.rho"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param vehicleRollInVar.rho=0
```
