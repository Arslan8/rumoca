# DECL-0567: `headWidth` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/package.mo:175` |
| **Parameter** | `headWidth` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '175p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/package.mo"
```

It should declare `headWidth` with type `SI.Length` and no `min`.
...
```modelica
// Mechanics/MultiBody/package.mo:175
parameter SI.Length headWidth(min=0) = ...;
```
