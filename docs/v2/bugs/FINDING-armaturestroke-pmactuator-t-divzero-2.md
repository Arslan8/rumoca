# FINDING-04246: `pmActuator.t` in `ArmatureStroke`
...
| **Reached as** | `pmActuator.t` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/Force/HollowCylinderRadialFlux.mo:26` |
| **Signature** | `divisor:divisor-reachable-zero:exp:1181:f9e5afa9` |
...
```
denominator            pmActuator.g_mb.A_avg
witness                pmActuator.t = -0.008
  found by             the value that solves the denominator to zero
denominator at witness 0.0
at declared values     0.001385442360233099
path condition         always evaluated
constraints consulted  pmActuator.t: unbounded
```
...
| `denominator` | pmActuator.g_mb.A_avg |
| `witness` | pmActuator.t = -0.008 |
| `witness_rationale` | the value that solves the denominator to zero |
| `denominator_at_witness` | 0.0 |
| `denominator_at_declared_values` | 0.001385442360233099 |
| `denominator_range` | [-inf, inf] |
| `path_condition` | always evaluated |
| `verdict` | SAT |
| `proof` | the division is not inside any branch |
| `constraints` | pmActuator.t: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.ArmatureStroke \
      --target pmActuator.t --sanitizer divisor
```
...
```console
$ sed -n '26p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/Force/HollowCylinderRadialFlux.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pmActuator.t"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/Force/HollowCylinderRadialFlux.mo:26` |' docs/v2/bugs/
```
