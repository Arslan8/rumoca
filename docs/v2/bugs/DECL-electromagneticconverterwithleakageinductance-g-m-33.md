# DECL-0589: `G_m` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/FluxTubes/Basic/ElectroMagneticConverterWithLeakageInductance.mo:33` |
| **Parameter** | `G_m` |
| **Type** | `SI.Permeance` |
| **Declared modifiers** | `—` |
...
`SI.Permeance` declares no `min` in `Units.mo`, and this declaration adds none of its own. It therefore accepts a negative value, and the magnetic analogue of conductance.
...
It should declare `G_m` with type `SI.Permeance` and no `min`.

**2. Confirm the type supplies no bound either** — this is the whole claim, and
a bound inherited from the type would refute it:

```console
$ grep -n "type Permeance " "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Units.mo"
```
...
- `SI.Permeance` turns out to carry a `min` after all, or this declaration carries one that the catalog missed.
...
```modelica
// Magnetic/FluxTubes/Basic/ElectroMagneticConverterWithLeakageInductance.mo:33
