...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:1411:7b8b84cc` |
...
```
denominator            volumeFlow.ramp2.duration
witness                volumeFlow.ramp2.duration = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.2
path condition         not ((time < volumeFlow.ramp2.startTime)) and ((time < (volumeFlow.ramp2.startTime + volumeFlow.ramp2.duration)))
constraints consulted  volumeFlow.ramp2.duration: min=0
```
...
| `denominator_at_declared_values` | 0.2 |
| `denominator_range` | [0, inf] |
| `path_condition` | not ((time &lt; volumeFlow.ramp2.startTime)) and ((time &lt; (volumeFlow.ramp2.startTime + volumeFlow.ramp2.duration))) |
| `verdict` | UNSAT |
| `proof` | the path conditions cannot hold together: time must be both &gt; 0.6 and &lt; 0.6 |
| `constraints` | volumeFlow.ramp2.duration: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.ParallelPumpDropOut \
      --target volumeFlow.ramp2.duration --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "volumeFlow.ramp2.duration"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param volumeFlow.ramp2.duration=0
```
