# FINDING-02158: `switchYDwithArc.idealCloser.closerWithArc[1].Goff` in `IMC_YDarc`
...
| **Reached as** | `switchYDwithArc.idealCloser.closerWithArc[1].Goff` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Interfaces/IdealSwitchWithArc.mo:5` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:724:3826f5f2` |
...
```
predicate     switchYDwithArc.idealCloser.closerWithArc[1].Goff > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Ideal.CloserWithArc.Goff: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Ideal.CloserWithArc.Goff'}
```
...
| `required` | switchYDwithArc.idealCloser.closerWithArc[1].Goff &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_YDarc \
      --target switchYDwithArc.idealCloser.closerWithArc[1].Goff --sanitizer physical
```
...
```console
$ sed -n '5p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Interfaces/IdealSwitchWithArc.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switchYDwithArc.idealCloser.closerWithArc[1].Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switchYDwithArc.idealCloser.closerWithArc[1].Goff=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Interfaces/IdealSwitchWithArc.mo:5` |' docs/v2/bugs/
```
