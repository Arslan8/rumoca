# FINDING-04957: `valve.kv0` in `PumpAndValve`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Thermal.FluidHeatFlow.Examples.PumpAndValve` |
| **Reached as** | `valve.kv0` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-guarded-by-assertion` |
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/Components/Valve.mo:43` |
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:759:7d4956c2` |
...
```
denominator            valve.kv0
witness                none needed
  found by             the denominator is zero as declared
denominator at witness 0.0
at declared values     0.01
path condition         not (valve.LinearCharacteristic)
...
| `denominator_at_declared_values` | 0.01 |
| `denominator_range` |  |
| `path_condition` | not (valve.LinearCharacteristic) |
| `verdict` | UNSAT |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.PumpAndValve \
      --target valve.kv0 --sanitizer divisor
```
...
```console
$ sed -n '43p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/Components/Valve.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "valve.kv0"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param valve.kv0=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/Components/Valve.mo:43` |' docs/v2/bugs/
```
