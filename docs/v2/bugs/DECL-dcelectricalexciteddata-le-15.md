# DECL-0158: `Le` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Machines/Utilities/ParameterRecords/DcElectricalExcitedData.mo:15` |
| **Parameter** | `Le` |
| **Type** | `SI.Inductance` |
| **Declared modifiers** | `—` |
...
It should declare `Le` with type `SI.Inductance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/ParameterRecords/DcElectricalExcitedData.mo" | head
```
...
```modelica
// Electrical/Machines/Utilities/ParameterRecords/DcElectricalExcitedData.mo:15
parameter SI.Inductance Le(min=0) = ...;
```
