# FINDING-00236: `L2` in `CompareTransformers`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/CompareTransformers.mo:24` |
| **Signature** | `physical:physical-intent-question:var:12:6987c339` |
...
role          —
declaration   Modelica.Electrical.Analog.Examples.CompareTransformers.L2
```
...
> `L2` carries no bound that would exclude it, which is unusual for an inductance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Examples.CompareTransformers.L2'}
```
...
| `authority` | quantity_or_unit |
| `canonical_declaration` | Modelica.Electrical.Analog.Examples.CompareTransformers.L2 |
| `question` | `L2` carries no bound that would exclude it, which is unusual for an inductance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
```console
$ sed -n '24p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/CompareTransformers.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "L2"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param L2=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/CompareTransformers.mo:24` |' docs/v2/bugs/
```
