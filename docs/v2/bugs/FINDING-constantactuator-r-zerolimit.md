# FINDING-04251: `R` in `ConstantActuator`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:1:dd6a0aa7` |
...
```
contract    Modelica.Electrical.Analog.Basic.Resistor.R: feature_disabled (proven, from equation) — inherited from r.R, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
