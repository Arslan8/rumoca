# FINDING-03474: `dcpm.Jr` in `ThyristorBridge2mPulse_DC_Drive`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:873:b58c5959` |
...
```
contract    Modelica.Mechanics.Rotational.Components.Inertia.J: algebraic_limit (proven, from equation) — inherited from dcpm.inertiaRotor.J, which it binds: the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
...
```
{'quantity': 'MomentOfInertia', 'unit': 'kg.m2', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.DCMachines.DC_PermanentMagnet.Jr'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.ThyristorBridge2mPulse_DC_Drive \
      --target dcpm.Jr --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcpm.Jr"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param dcpm.Jr=0
```
