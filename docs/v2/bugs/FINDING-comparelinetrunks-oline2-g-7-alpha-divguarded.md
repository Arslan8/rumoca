...
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:5908:68efe226` |
...
```
denominator            (1 + (oLine2.G[7].alpha * (oLine2.G[7].T_heatPort - oLine2.G[7].T_ref)))
witness                none needed
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.CompareLineTrunks \
      --target oLine2.G[7].alpha --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "oLine2.G[7].alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param oLine2.G[7].alpha=0
```
