...
| **Signature** | `divisor:divisor-reachable-zero:exp:636:961aa951` |
...
```
denominator            ambient1.medium.cp
witness                ambient1.medium.cp = 0
  found by             set to zero
denominator at witness 0.0
at declared values     4177.0
path condition         always evaluated
constraints consulted  ambient1.medium.cp: unbounded
```
...
| `constraints` | ambient1.medium.cp: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.WaterPump \
      --target ambient1.medium.cp --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "ambient1.medium.cp"
```
