# DECL-0148: `LGrid` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Machines/Examples/InductionMachines/IMC_InverterDrive.mo:12` |
| **Parameter** | `LGrid` |
| **Type** | `SI.Inductance` |
| **Declared modifiers** | `—` |
...
It should declare `LGrid` with type `SI.Inductance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/InductionMachines/IMC_InverterDrive.mo" | head
```
...
```modelica
// Electrical/Machines/Examples/InductionMachines/IMC_InverterDrive.mo:12
parameter SI.Inductance LGrid(min=0) = ...;
```
