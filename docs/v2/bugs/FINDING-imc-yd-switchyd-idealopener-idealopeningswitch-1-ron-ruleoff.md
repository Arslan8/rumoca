# FINDING-02237: `switchYD.idealOpener.idealOpeningSwitch[1].Ron` in `IMC_YD`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Electrical.Machines.Examples.InductionMachines.IMC_YD` |
| **Reached as** | `switchYD.idealOpener.idealOpeningSwitch[1].Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:790:104ec555` |
...
```
predicate     switchYD.idealOpener.idealOpeningSwitch[1].Ron > 0
premise       refuted
...
| `required` | switchYD.idealOpener.idealOpeningSwitch[1].Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_YD \
      --target switchYD.idealOpener.idealOpeningSwitch[1].Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switchYD.idealOpener.idealOpeningSwitch[1].Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switchYD.idealOpener.idealOpeningSwitch[1].Ron=0
```
