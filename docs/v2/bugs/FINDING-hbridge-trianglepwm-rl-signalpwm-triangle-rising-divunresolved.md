# FINDING-04167: `signalPWM.triangle.rising` in `HBridge_TrianglePWM_RL`
...
| **Reached as** | `signalPWM.triangle.rising` |
...
| **Signature** | `divisor:divisor-zero-unresolved:exp:1240:c6dcd478` |
...
```
denominator            signalPWM.triangle.rising
witness                signalPWM.triangle.rising = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.0005
path condition         not ((((time < signalPWM.triangle.startTime) or (signalPWM.triangle.nperiod == 0)) or ((signalPWM.triangle.nperiod > 0) and (signalPWM.triangle.count >= signalPWM.triangle.nperiod)))) and ((time < (signalPWM.triangle.T_start + signalPWM.triangle.T_rising)))
constraints consulted  signalPWM.triangle.rising: min=0
```
...
| `path_condition` | not ((((time &lt; signalPWM.triangle.startTime) or (signalPWM.triangle.nperiod == 0)) or ((signalPWM.triangle.nperiod &gt; 0) and (signalPWM.triangle.count &gt;= signalPWM.triangle.nperiod)))) and ((time &lt; (signalPWM.triangle.T_start + signalPWM.triangle.T_rising))) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: (time &lt; (signalPWM.triangle.T_start + signalPWM.triangle.T_rising)); (time &lt; signalPWM.triangle.startTime) |
| `constraints` | signalPWM.triangle.rising: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCDC.HBridge.HBridge_TrianglePWM_RL \
      --target signalPWM.triangle.rising --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "signalPWM.triangle.rising"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param signalPWM.triangle.rising=0
```
