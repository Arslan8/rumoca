# FINDING-02935: `smrData.statorCoreParameters.GcRef` in `SMR_Inverter`
...
| **Reached as** | `smrData.statorCoreParameters.GcRef` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:718:19238c53` |
...
| `observed` | 0.0 |
| `required` | smrData.statorCoreParameters.GcRef &gt; 0 |
| `where` | declaration |
| `contract` | Modelica.Electrical.Machines.Losses.CoreParameters.GcRef: feature_disabled (declared, from bound) — the declaration&#x27;s own binding evaluates to zero at the declared configuration, so zero is the value the library chose rather than one it failed to exclude |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMR_Inverter \
      --target smrData.statorCoreParameters.GcRef --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smrData.statorCoreParameters.GcRef"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param smrData.statorCoreParameters.GcRef=0
```
