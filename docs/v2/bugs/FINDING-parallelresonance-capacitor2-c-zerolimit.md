...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:103:6cd536ec` |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.ParallelResonance \
      --target capacitor2.C --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.ParallelResonance --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "capacitor2.C"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param capacitor2.C=0
```
