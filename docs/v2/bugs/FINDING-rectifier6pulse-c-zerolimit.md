# FINDING-03227: `C` in `Rectifier6pulse`
...
```
contract    Modelica.Electrical.Machines.Examples.Transformers.Rectifier6pulse.C: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Capacitance', 'unit': 'F', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.Examples.Transformers.Rectifier6pulse.C'}
```
