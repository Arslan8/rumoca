# DECL-0572: `gravityHeadLength` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/package.mo:273` |
| **Parameter** | `gravityHeadLength` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '273p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/package.mo"
```

It should declare `gravityHeadLength` with type `SI.Length` and no `min`.
...
```modelica
// Mechanics/MultiBody/package.mo:273
parameter SI.Length gravityHeadLength(min=0) = ...;
```
