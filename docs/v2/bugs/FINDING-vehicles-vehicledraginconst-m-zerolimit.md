...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:117:24c0df8e` |
...
```
predicate     vehicleDragInConst.m > 0
premise       refuted
...
| `required` | vehicleDragInConst.m &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Translational.Vehicles \
      --target vehicleDragInConst.m --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "vehicleDragInConst.m"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param vehicleDragInConst.m=0
```
