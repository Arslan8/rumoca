# DECL-0533: `pistonLength` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/Examples/Loops/Utilities/CylinderBase.mo:15` |
| **Parameter** | `pistonLength` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '15p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Examples/Loops/Utilities/CylinderBase.mo"
```

It should declare `pistonLength` with type `SI.Length` and no `min`.
...
```modelica
// Mechanics/MultiBody/Examples/Loops/Utilities/CylinderBase.mo:15
parameter SI.Length pistonLength(min=0) = ...;
```
