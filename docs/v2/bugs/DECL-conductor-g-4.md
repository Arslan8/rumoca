# DECL-0048: `G` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Examples/Utilities/Conductor.mo:4` |
| **Parameter** | `G` |
| **Type** | `SI.Conductance` |
| **Declared modifiers** | `—` |
...
It should declare `G` with type `SI.Conductance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/Utilities/Conductor.mo" | head
```
...
```modelica
// Electrical/Analog/Examples/Utilities/Conductor.mo:4
