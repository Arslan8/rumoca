# DECL-0602: `Rout` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Basic/OpAmpDetailed.mo:23` |
| **Parameter** | `Rout` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '23p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/OpAmpDetailed.mo"
```

It should declare `Rout` with type `SI.Resistance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/OpAmpDetailed.mo" | head
```
...
```modelica
// Electrical/Analog/Basic/OpAmpDetailed.mo:23
parameter SI.Resistance Rout(min=0) = ...;
```
