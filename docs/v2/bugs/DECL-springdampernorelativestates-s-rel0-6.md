# DECL-0578: `s_rel0` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/Translational/Examples/Utilities/SpringDamperNoRelativeStates.mo:6` |
| **Parameter** | `s_rel0` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '6p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Examples/Utilities/SpringDamperNoRelativeStates.mo"
```

It should declare `s_rel0` with type `SI.Length` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Examples/Utilities/SpringDamperNoRelativeStates.mo" | head
```
...
```modelica
// Mechanics/Translational/Examples/Utilities/SpringDamperNoRelativeStates.mo:6
parameter SI.Length s_rel0(min=0) = ...;
```
