...
| **Signature** | `divisor:divisor-zero-when-parameters-equal:exp:786:deb897a5` |
...
```
denominator            (feedbackA.opAmp.vps - feedbackA.opAmp.vns)
witness                feedbackA.opAmp.Vps = -15
  found by             set equal to feedbackA.opAmp.Vns
denominator at witness 0.0
at declared values     30.0
path condition         always evaluated
constraints consulted  feedbackA.opAmp.Vps: unbounded
```
...
| `denominator` | (feedbackA.opAmp.vps - feedbackA.opAmp.vns) |
| `witness` | feedbackA.opAmp.Vps = -15 |
| `witness_rationale` | set equal to feedbackA.opAmp.Vns |
...
| `constraints` | feedbackA.opAmp.Vps: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.ControlCircuit \
      --target feedbackA.opAmp.Vps --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "feedbackA.opAmp.Vps"
```
