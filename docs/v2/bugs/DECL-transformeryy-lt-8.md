# DECL-0196: `LT` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Polyphase/Examples/TransformerYY.mo:8` |
| **Parameter** | `LT` |
| **Type** | `SI.Inductance` |
| **Declared modifiers** | `—` |
...
It should declare `LT` with type `SI.Inductance` and no `min`.
...
```modelica
// Electrical/Polyphase/Examples/TransformerYY.mo:8
parameter SI.Inductance LT(min=0) = ...;
```
