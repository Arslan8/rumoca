...
| **Reached as** | `diodeBridge2mPulse.diode_n.Goff` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:721:f7304d02` |
...
```
predicate     diodeBridge2mPulse.diode_n.Goff > 0
premise       refuted
...
| `required` | diodeBridge2mPulse.diode_n.Goff &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_Braking \
      --target diodeBridge2mPulse.diode_n.Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "diodeBridge2mPulse.diode_n.Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param diodeBridge2mPulse.diode_n.Goff=0
```
