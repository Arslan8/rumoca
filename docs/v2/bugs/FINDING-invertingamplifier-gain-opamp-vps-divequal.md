...
```
denominator            (gain.opAmp.vps - gain.opAmp.vns)
witness                gain.opAmp.Vps = -15
  found by             set equal to gain.opAmp.Vns
denominator at witness 0.0
at declared values     30.0
path condition         always evaluated
constraints consulted  gain.opAmp.Vps: unbounded
```
...
| `denominator` | (gain.opAmp.vps - gain.opAmp.vns) |
| `witness` | gain.opAmp.Vps = -15 |
| `witness_rationale` | set equal to gain.opAmp.Vns |
...
| `constraints` | gain.opAmp.Vps: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.InvertingAmplifier \
      --target gain.opAmp.Vps --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "gain.opAmp.Vps"
```
