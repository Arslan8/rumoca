# FINDING-00003: `load.J` in `ClockedWithDiscreteTextbookController`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Clocked.Examples.SimpleControlledDrive.ClockedWithDiscreteTextbookController` |
| **Reached as** | `load.J` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:4:252f966f` |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Clocked.Examples.SimpleControlledDrive.ClockedWithDiscreteTextbookController \
      --target load.J --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Clocked.Examples.SimpleControlledDrive.ClockedWithDiscreteTextbookController --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "load.J"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param load.J=0
```
