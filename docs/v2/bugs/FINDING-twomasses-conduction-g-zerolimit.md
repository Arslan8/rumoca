...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:17:14419fcc` |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Thermal.HeatTransfer.Examples.TwoMasses \
      --target conduction.G --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Thermal.HeatTransfer.Examples.TwoMasses --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "conduction.G"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param conduction.G=0
```
