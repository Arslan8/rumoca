# FINDING-02155: `switchYDwithArc.idealCloser.Ron` in `IMC_YDarc`
...
| **Reached as** | `switchYDwithArc.idealCloser.Ron` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Ideal/CloserWithArc.mo:4` |
| **Signature** | `physical:physical-rule-does-not-apply:var:717:29c6ebd9` |
...
```
predicate     switchYDwithArc.idealCloser.Ron > 0
premise       refuted
...
```
contract    Modelica.Electrical.Polyphase.Ideal.CloserWithArc.Ron: allowed (declared, from class_catalog) — an ideal switch family member; zero on-resistance is the documented ideal [matched by prefix, lower confidence]
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Polyphase.Ideal.CloserWithArc.Ron'}
```
...
| `required` | switchYDwithArc.idealCloser.Ron &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_YDarc \
      --target switchYDwithArc.idealCloser.Ron --sanitizer physical
```
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Ideal/CloserWithArc.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switchYDwithArc.idealCloser.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switchYDwithArc.idealCloser.Ron=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Ideal/CloserWithArc.mo:4` |' docs/v2/bugs/
```
