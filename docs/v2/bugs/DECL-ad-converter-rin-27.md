...
| **Declaration** | `Electrical/Analog/Ideal/AD_Converter.mo:27` |
| **Parameter** | `Rin` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `(start=10^6)` |
...
```console
$ sed -n '27p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Ideal/AD_Converter.mo"
```

It should declare `Rin` with type `SI.Resistance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Ideal/AD_Converter.mo" | head
```
...
```modelica
// Electrical/Analog/Ideal/AD_Converter.mo:27
parameter SI.Resistance Rin(min=0) = ...;
```
