...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Ideal/IdealOpeningSwitch.mo:6` |
| **Signature** | `physical:physical-rule-does-not-apply:var:788:75981854` |
...
```
predicate     switchYD.idealOpener.Goff > 0
premise       refuted
...
```
contract    Modelica.Electrical.Polyphase.Ideal.IdealOpeningSwitch.Goff: allowed (declared, from class_catalog) — an ideal switch family member; zero off-conductance is the documented ideal [matched by prefix, lower confidence]
behavior    allowed
confidence  declared
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Polyphase.Ideal.IdealOpeningSwitch.Goff'}
```
...
| `required` | switchYD.idealOpener.Goff &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_YD \
      --target switchYD.idealOpener.Goff --sanitizer physical
```
...
```console
$ sed -n '6p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Ideal/IdealOpeningSwitch.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switchYD.idealOpener.Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switchYD.idealOpener.Goff=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Ideal/IdealOpeningSwitch.mo:6` |' docs/v2/bugs/
```
