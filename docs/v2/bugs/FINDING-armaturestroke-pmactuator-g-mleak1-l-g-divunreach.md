...
| **Reached as** | `pmActuator.g_mLeak1.l_g` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-unreachable-under-witness` |
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/Leakage/CoaxCylindersEndFaces.mo:21` |
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:1242:026faa18` |
...
```
denominator            pmActuator.g_mLeak1.l_g
witness                pmActuator.g_mLeak1.l_g = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.006499999999999999
path condition         not ((pmActuator.g_mLeak1.t <= pmActuator.g_mLeak1.r_0))
constraints consulted  pmActuator.g_mLeak1.l_g: min=0
```
...
| `denominator_at_declared_values` | 0.006499999999999999 |
| `denominator_range` | [0, inf] |
| `path_condition` | not ((pmActuator.g_mLeak1.t &lt;= pmActuator.g_mLeak1.r_0)) |
| `verdict` | UNSAT |
| `proof` | the guard not ((pmActuator.g_mLeak1.t &lt;= pmActuator.g_mLeak1.r_0)) is false under the witness |
| `constraints` | pmActuator.g_mLeak1.l_g: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.ArmatureStroke \
      --target pmActuator.g_mLeak1.l_g --sanitizer divisor
```
...
```console
$ sed -n '21p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/Leakage/CoaxCylindersEndFaces.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pmActuator.g_mLeak1.l_g"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param pmActuator.g_mLeak1.l_g=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/Leakage/CoaxCylindersEndFaces.mo:21` |' docs/v2/bugs/
```
