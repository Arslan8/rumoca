...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:5:418aa254` |
...
```
predicate     openTank1.medium.rho > 0
premise       refuted
...
| `required` | openTank1.medium.rho &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.TwoTanks \
      --target openTank1.medium.rho --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "openTank1.medium.rho"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param openTank1.medium.rho=0
```
