# FINDING-01005: `c.C` in `FirstOrder`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:76:74dc5a3e` |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.OpAmpCircuits.FirstOrder \
      --target c.C --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.OpAmpCircuits.FirstOrder --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "c.C"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param c.C=0
```
