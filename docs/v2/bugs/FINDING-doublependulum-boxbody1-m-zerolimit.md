# FINDING-04388: `boxBody1.m` in `DoublePendulum`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:338:9abe9055` |
...
```
contract    Modelica.Mechanics.MultiBody.Parts.Body.m: feature_disabled (proven, from equation) — inherited from boxBody1.body.m, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Elementary.DoublePendulum \
      --target boxBody1.m --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "boxBody1.m"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param boxBody1.m=0
```
