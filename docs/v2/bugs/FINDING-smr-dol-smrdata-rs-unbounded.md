# FINDING-02903: `smrData.Rs` in `SMR_DOL`
...
| **Signature** | `physical:physical-domain-unenforced:var:635:e272bf1d` |
...
```
predicate     smrData.Rs >= 0
premise       established
authority     component_contract
role          component.machine.winding_resistance
declaration   Modelica.Electrical.Machines.Utilities.ParameterRecords.SM_ReluctanceRotorData.Rs
```
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'semantic_role': 'component.machine.winding_resistance', 'binding_source': 'component_type', 'confidence': 'QUANTITY_AND_UNIT', 'declaring_class': 'Modelica.Electrical.Machines.Utilities.ParameterRecords.SM_ReluctanceRotorData', 'member': 'Rs', 'premise_state': 'established', 'authority': 'component_contract', 'canonical_declaration': 'Modelica.Electrical.Machines.Utilities.ParameterRecords.SM_ReluctanceRotorData.Rs'}
```
...
| `required` | smrData.Rs &gt;= 0 |
| `declared_min` | None |
| `variable_role` | parameter |
| `premise_state` | established |
| `authority` | component_contract |
| `semantic_role` | component.machine.winding_resistance |
| `canonical_declaration` | Modelica.Electrical.Machines.Utilities.ParameterRecords.SM_ReluctanceRotorData.Rs |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMR_DOL \
      --target smrData.Rs --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smrData.Rs"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param smrData.Rs=0
```
