# FINDING-05126: `oneWayValve.V_flowNominal` in `WaterPump`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Thermal.FluidHeatFlow.Examples.WaterPump` |
| **Reached as** | `oneWayValve.V_flowNominal` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-zero-unresolved` |
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/Components/OneWayValve.mo:21` |
| **Signature** | `divisor:divisor-zero-unresolved:exp:971:45c46731` |
...
```
denominator            oneWayValve.V_flowNominal
witness                oneWayValve.V_flowNominal = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.18
path condition         not (oneWayValve.backward)
constraints consulted  oneWayValve.V_flowNominal: unbounded
```
...
| `denominator_at_declared_values` | 0.18 |
| `denominator_range` | [-inf, inf] |
| `path_condition` | not (oneWayValve.backward) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: oneWayValve.backward |
| `constraints` | oneWayValve.V_flowNominal: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.WaterPump \
      --target oneWayValve.V_flowNominal --sanitizer divisor
```
...
```console
$ sed -n '21p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/Components/OneWayValve.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "oneWayValve.V_flowNominal"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param oneWayValve.V_flowNominal=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/Components/OneWayValve.mo:21` |' docs/v2/bugs/
```
