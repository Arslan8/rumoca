# FINDING-01161: `simpleTriac.thyristor.Roff` in `SimpleTriacCircuit`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Electrical.Analog.Examples.SimpleTriacCircuit` |
| **Reached as** | `simpleTriac.thyristor.Roff` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-zero-unresolved` |
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/Thyristor.mo:58` |
| **Signature** | `divisor:divisor-zero-unresolved:exp:582:3bdf3c93` |
...
```
denominator            simpleTriac.thyristor.Roff
witness                simpleTriac.thyristor.Roff = 0
  found by             set to zero
denominator at witness 0.0
at declared values     15686274.509803923
path condition         not ((simpleTriac.thyristor.vAK < -simpleTriac.thyristor.VRRM)) and ((simpleTriac.thyristor.vControl < simpleTriac.thyristor.Voff))
constraints consulted  simpleTriac.thyristor.Roff: unbounded
```
...
| `denominator_at_declared_values` | 15686274.509803923 |
| `denominator_range` | [-inf, inf] |
| `path_condition` | not ((simpleTriac.thyristor.vAK &lt; -simpleTriac.thyristor.VRRM)) and ((simpleTriac.thyristor.vControl &lt; simpleTriac.thyristor.Voff)) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: (simpleTriac.thyristor.vAK &lt; -simpleTriac.thyristor.VRRM) |
| `constraints` | simpleTriac.thyristor.Roff: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.SimpleTriacCircuit \
      --target simpleTriac.thyristor.Roff --sanitizer divisor
```
...
```console
$ sed -n '58p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/Thyristor.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "simpleTriac.thyristor.Roff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param simpleTriac.thyristor.Roff=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/Thyristor.mo:58` |' docs/v2/bugs/
```
