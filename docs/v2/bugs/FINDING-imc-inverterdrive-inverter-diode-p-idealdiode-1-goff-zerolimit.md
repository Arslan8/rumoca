# FINDING-01880: `inverter.diode_p.idealDiode[1].Goff` in `IMC_InverterDrive`
...
| **Reached as** | `inverter.diode_p.idealDiode[1].Goff` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:628:45f24e8e` |
...
```
predicate     inverter.diode_p.idealDiode[1].Goff > 0
premise       refuted
...
| `required` | inverter.diode_p.idealDiode[1].Goff &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive \
      --target inverter.diode_p.idealDiode[1].Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inverter.diode_p.idealDiode[1].Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inverter.diode_p.idealDiode[1].Goff=0
```
