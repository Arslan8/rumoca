...
| **Signature** | `physical:physical-intent-question:var:400:7d3ad946` |
...
```
predicate     cooling.flowPort_b.medium.rho > 0
premise       unknown
...
> `cooling.flowPort_b.medium.rho` is bounded only by min=0, which is unusual for a density — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
| `required` | cooling.flowPort_b.medium.rho &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Cooling \
      --target cooling.flowPort_b.medium.rho --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "cooling.flowPort_b.medium.rho"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param cooling.flowPort_b.medium.rho=0
```
