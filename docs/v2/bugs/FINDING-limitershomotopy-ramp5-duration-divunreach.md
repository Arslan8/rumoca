...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:776:4aa1ce98` |
...
```
denominator            ramp5.duration
witness                ramp5.duration = 0
  found by             set to zero
denominator at witness 0.0
at declared values     100.0
path condition         not ((time < ramp5.startTime)) and ((time < (ramp5.startTime + ramp5.duration)))
constraints consulted  ramp5.duration: min=0
```
...
| `path_condition` | not ((time &lt; ramp5.startTime)) and ((time &lt; (ramp5.startTime + ramp5.duration))) |
| `verdict` | UNSAT |
| `proof` | the path conditions cannot hold together: time must be both &gt; 1 and &lt; 1 |
| `constraints` | ramp5.duration: min=0 |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Blocks.LimitersHomotopy \
      --target ramp5.duration --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "ramp5.duration"
```
