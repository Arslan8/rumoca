...
| **Reached as** | `controllerFeedbackPart1.T` |
...
| **Signature** | `divisor:divisor-reachable-zero:exp:365:572cedea` |
...
```
denominator            controllerFeedbackPart1.T
witness                controllerFeedbackPart1.T = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  controllerFeedbackPart1.T: unbounded
```
...
| `constraints` | controllerFeedbackPart1.T: unbounded |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Blocks.LimitersHomotopy \
      --target controllerFeedbackPart1.T --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "controllerFeedbackPart1.T"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param controllerFeedbackPart1.T=0
```
