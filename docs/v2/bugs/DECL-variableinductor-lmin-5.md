# DECL-0189: `Lmin` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Polyphase/Basic/VariableInductor.mo:5` |
| **Parameter** | `Lmin` |
| **Type** | `SI.Inductance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '5p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Basic/VariableInductor.mo"
```

It should declare `Lmin` with type `SI.Inductance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Basic/VariableInductor.mo" | head
```
...
```modelica
// Electrical/Polyphase/Basic/VariableInductor.mo:5
parameter SI.Inductance Lmin(min=0) = ...;
```
