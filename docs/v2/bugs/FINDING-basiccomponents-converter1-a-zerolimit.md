...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Basic/ElectroMagneticConverterWithLeakageInductance.mo:28` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:34:60d5648b` |
...
```
contract    Modelica.Magnetic.FluxTubes.Basic.ElectroMagneticConverterWithLeakageInductance.A: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Area', 'unit': 'm2', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Magnetic.FluxTubes.Basic.ElectroMagneticConverterWithLeakageInductance.A'}
```
...
$ python3 tools/sweep/check_one.py ModelicaTest.Magnetic.FluxTubes.BasicComponents \
      --target converter1.A --sanitizer physical
```
...
```console
$ sed -n '28p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Basic/ElectroMagneticConverterWithLeakageInductance.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "converter1.A"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param converter1.A=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Basic/ElectroMagneticConverterWithLeakageInductance.mo:28` |' docs/v2/bugs/
```
