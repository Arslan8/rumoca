# BUG-018: `genericFluxTube1.material.B_myMax` in `Sources`

|  |  |
|---|---|
| **Tier** | **Confirmed** — reproduced in two independent tools |
| **Model** | `ModelicaTest.Magnetic.FluxTubes.Sources` |
| **Trigger** | `genericFluxTube1.material.B_myMax = 0` |
...
`ModelicaTest.Magnetic.FluxTubes.Sources` runs cleanly at its declared values. Setting `genericFluxTube1.material.B_myMax = 0` — a value the declaration permits — makes it fail, in Rumoca and in OpenModelica.
...
$ python3 tools/sweep/check_one.py ModelicaTest.Magnetic.FluxTubes.Sources \
      --target genericFluxTube1.material.B_myMax --keep-parameter-chains
```
...
simulate(ModelicaTest.Magnetic.FluxTubes.Sources, simflags="-override genericFluxTube1.material.B_myMax=0");
EOF
```
