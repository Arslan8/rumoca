# FINDING-01245: `TN2.Tnom` in `Nand`
...
| **Signature** | `divisor:divisor-zero-unresolved:exp:972:4a3e84ea` |
...
```
denominator            TN2.Tnom
witness                TN2.Tnom = 0
  found by             set to zero
denominator at witness 0.0
at declared values     300.15
path condition         (TN2.useTemperatureDependency)
constraints consulted  TN2.Tnom: min=0
```
...
| `path_condition` | (TN2.useTemperatureDependency) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: TN2.useTemperatureDependency |
| `constraints` | TN2.Tnom: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Utilities.Nand \
      --target TN2.Tnom --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "TN2.Tnom"
```
