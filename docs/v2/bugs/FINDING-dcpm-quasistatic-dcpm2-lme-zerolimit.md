# FINDING-01549: `dcpm2.Lme` in `DCPM_QuasiStatic`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:339:d7fac30c` |
...
```
contract    Modelica.Electrical.Machines.BasicMachines.QuasiStaticDCMachines.DC_PermanentMagnet.Lme: forbidden (proven, from bound) — the declaration excludes zero (declared constant)
behavior    forbidden
confidence  proven
source      bound
```
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.QuasiStaticDCMachines.DC_PermanentMagnet.Lme'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_QuasiStatic \
      --target dcpm2.Lme --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcpm2.Lme"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param dcpm2.Lme=0
```
