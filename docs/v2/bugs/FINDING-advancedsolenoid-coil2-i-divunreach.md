...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:1904:47a5b93e` |
...
path condition         ((abs(coil2.i) > coil2.eps))
...
| `denominator_range` |  |
| `path_condition` | ((abs(coil2.i) &gt; coil2.eps)) |
| `verdict` | UNSAT |
| `proof` | the denominator is zero as declared, and ((abs(coil2.i) &gt; coil2.eps)) |
| `constraints` |  |
| `guard_that_excludes_it` | ((abs(coil2.i) &gt; coil2.eps)) |
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.SolenoidActuator.Components.AdvancedSolenoid \
      --target coil2.i --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "coil2.i"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param coil2.i=0
```
