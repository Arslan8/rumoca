# FINDING-00401: `HeatingDiode1.R` in `HeatingRectifier`
...
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/Diode.mo:8` |
| **Signature** | `physical:physical-domain-unenforced:var:16:e9bb47a4` |
...
```
predicate     HeatingDiode1.R > 0
premise       established
authority     source_arithmetic
role          —
declaration   Modelica.Electrical.Analog.Semiconductors.Diode.R
```
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Semiconductors.Diode.R'}
```
...
| `required` | HeatingDiode1.R &gt; 0 |
| `declared_min` | None |
| `variable_role` | parameter |
| `premise_state` | established |
| `authority` | source_arithmetic |
| `canonical_declaration` | Modelica.Electrical.Analog.Semiconductors.Diode.R |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.HeatingRectifier \
      --target HeatingDiode1.R --sanitizer physical
```
...
```console
$ sed -n '8p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/Diode.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "HeatingDiode1.R"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param HeatingDiode1.R=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/Diode.mo:8` |' docs/v2/bugs/
```
