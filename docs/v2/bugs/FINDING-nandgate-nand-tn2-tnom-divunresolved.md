...
| **Signature** | `divisor:divisor-zero-unresolved:exp:1336:b0bdbc84` |
...
```
denominator            Nand.TN2.Tnom
witness                Nand.TN2.Tnom = 0
  found by             set to zero
denominator at witness 0.0
at declared values     300.15
path condition         (Nand.TN2.useTemperatureDependency)
constraints consulted  Nand.TN2.Tnom: min=0
```
...
| `path_condition` | (Nand.TN2.useTemperatureDependency) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: Nand.TN2.useTemperatureDependency |
| `constraints` | Nand.TN2.Tnom: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.NandGate \
      --target Nand.TN2.Tnom --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "Nand.TN2.Tnom"
```
