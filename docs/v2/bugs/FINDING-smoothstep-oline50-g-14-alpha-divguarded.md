...
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:12443:0f25066f` |
...
```
denominator            (1 + (oLine50.G[14].alpha * (oLine50.G[14].T_heatPort - oLine50.G[14].T_ref)))
witness                none needed
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.SmoothStep \
      --target oLine50.G[14].alpha --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "oLine50.G[14].alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param oLine50.G[14].alpha=0
```
