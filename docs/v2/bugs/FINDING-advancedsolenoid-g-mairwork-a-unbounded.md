# FINDING-04287: `g_mAirWork.A` in `AdvancedSolenoid`
...
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/Force/HollowCylinderAxialFlux.mo:16` |
| **Signature** | `physical:physical-domain-unenforced:var:130:ea5eb12d` |
...
```
predicate     g_mAirWork.A > 0
premise       established
authority     source_arithmetic
role          —
declaration   Modelica.Magnetic.FluxTubes.Shapes.Force.HollowCylinderAxialFlux.A
```
...
```
{'quantity': 'Area', 'unit': 'm2', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Magnetic.FluxTubes.Shapes.Force.HollowCylinderAxialFlux.A'}
```
...
| `authority` | source_arithmetic |
| `canonical_declaration` | Modelica.Magnetic.FluxTubes.Shapes.Force.HollowCylinderAxialFlux.A |
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.SolenoidActuator.Components.AdvancedSolenoid \
      --target g_mAirWork.A --sanitizer physical
```
...
```console
$ sed -n '16p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/Force/HollowCylinderAxialFlux.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "g_mAirWork.A"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param g_mAirWork.A=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/Force/HollowCylinderAxialFlux.mo:16` |' docs/v2/bugs/
```
