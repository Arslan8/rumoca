...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:75:38d788b0` |
...
```
denominator            phi.duration
witness                phi.duration = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         not ((time < phi.startTime)) and ((time < (phi.startTime + phi.duration)))
constraints consulted  phi.duration: min=0
```
...
| `path_condition` | not ((time &lt; phi.startTime)) and ((time &lt; (phi.startTime + phi.duration))) |
| `verdict` | UNSAT |
| `proof` | the path conditions cannot hold together: time must be both &gt; 0 and &lt; 0 |
| `constraints` | phi.duration: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.ComplexBlocks.Examples.TestConversionBlock \
      --target phi.duration --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "phi.duration"
```
