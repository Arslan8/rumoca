# FINDING-04442: `spring.c` in `InitSpringConstant`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:480:12bd11e2` |
...
```
contract    Modelica.Mechanics.Translational.Components.Spring.c: feature_disabled (proven, from equation) — inherited from spring.spring.c, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
