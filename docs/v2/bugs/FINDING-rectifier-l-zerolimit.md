...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Examples/Rectifier.mo:8` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:3:f7bfed6e` |
...
```
contract    Modelica.Electrical.Analog.Basic.Inductor.L: algebraic_limit (declared, from class_catalog) — inherited from supplyL.L, which it binds: L*der(i) = v; at L = 0 the element is an ideal short, v = 0, which the component documentation states explicitly
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Polyphase.Examples.Rectifier.L'}
```
...
```console
$ sed -n '8p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Examples/Rectifier.mo"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Examples/Rectifier.mo:8` |' docs/v2/bugs/
```
