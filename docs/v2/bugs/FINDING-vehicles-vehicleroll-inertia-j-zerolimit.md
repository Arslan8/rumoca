...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:410:5a394037` |
...
```
predicate     vehicleRoll.inertia.J > 0
premise       refuted
...
| `required` | vehicleRoll.inertia.J &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Translational.Vehicles \
      --target vehicleRoll.inertia.J --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "vehicleRoll.inertia.J"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param vehicleRoll.inertia.J=0
```
