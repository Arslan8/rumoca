# FINDING-05467: `side_tau` in `RoverPlant`
...
| **Severity** | high |
| **Declaration** | `target/cmm/CMM-a642c381/RigidBody/Examples/RoverPlant.mo:57` |
| **Signature** | `divisor:divisor-reachable-zero:exp:649:89ed20ec` |
...
```
denominator            side_tau
witness                side_tau = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.12
path condition         always evaluated
constraints consulted  side_tau: unbounded
```
...
$ python3 tools/sweep/check_one.py RigidBody.Examples.RoverPlant \
      --target side_tau --sanitizer divisor
```
...
```console
$ sed -n '57p' "target/cmm/CMM-a642c381/RigidBody/Examples/RoverPlant.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "side_tau"
```
...
```console
$ grep -rl '| `target/cmm/CMM-a642c381/RigidBody/Examples/RoverPlant.mo:57` |' docs/v2/bugs/
```
