# FINDING-03131: `diode2.idealDiode[1].Goff` in `Rectifier12pulse`
...
| **Reached as** | `diode2.idealDiode[1].Goff` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:233:be30542d` |
...
```
predicate     diode2.idealDiode[1].Goff > 0
premise       refuted
...
| `required` | diode2.idealDiode[1].Goff &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.Transformers.Rectifier12pulse \
      --target diode2.idealDiode[1].Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "diode2.idealDiode[1].Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param diode2.idealDiode[1].Goff=0
```
