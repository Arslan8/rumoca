# DECL-0675: `R0` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Batteries/ParameterRecords/CellData.mo:31` |
| **Parameter** | `R0` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '31p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Batteries/ParameterRecords/CellData.mo"
```

It should declare `R0` with type `SI.Resistance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Batteries/ParameterRecords/CellData.mo" | head
```
...
```modelica
// Electrical/Batteries/ParameterRecords/CellData.mo:31
parameter SI.Resistance R0(min=0) = ...;
```
