# DECL-0049: `Ga` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Examples/Utilities/NonlinearResistor.mo:5` |
| **Parameter** | `Ga` |
| **Type** | `SI.Conductance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '5p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/Utilities/NonlinearResistor.mo"
```

It should declare `Ga` with type `SI.Conductance` and no `min`.
...
```modelica
// Electrical/Analog/Examples/Utilities/NonlinearResistor.mo:5
parameter SI.Conductance Ga(min=0) = ...;
```
