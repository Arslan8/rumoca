# FINDING-01739: `imcData.fsNominal` in `IMC_DCBraking`
...
| **Signature** | `divisor:divisor-reachable-zero:exp:251:e43a6383` |
...
```
denominator            ((2 * (2 * asin(1.0))) * imcData.fsNominal)
witness                imcData.fsNominal = 0
  found by             set to zero
denominator at witness 0.0
at declared values     314.1592653589793
path condition         always evaluated
constraints consulted  imcData.fsNominal: unbounded
```
...
| `constraints` | imcData.fsNominal: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_DCBraking \
      --target imcData.fsNominal --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "imcData.fsNominal"
```
