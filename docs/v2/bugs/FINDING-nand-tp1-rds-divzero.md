...
| **Signature** | `divisor:divisor-reachable-zero:exp:278:9748c4d7` |
...
path condition         not (((TP1.RDS < 1e-20) and (TP1.RDS > -1e-20)))
constraints consulted  TP1.RDS: unbounded
```
...
| `path_condition` | not (((TP1.RDS &lt; 1e-20) and (TP1.RDS &gt; -1e-20))) |
| `verdict` | SAT |
| `proof` | every guard still holds under the witness |
| `constraints` | TP1.RDS: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Utilities.Nand \
      --target TP1.RDS --sanitizer divisor
```
