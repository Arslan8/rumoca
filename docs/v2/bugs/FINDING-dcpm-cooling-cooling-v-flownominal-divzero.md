...
| **Reached as** | `cooling.V_flowNominal` |
...
| **Signature** | `divisor:divisor-reachable-zero:exp:617:64fc3f10` |
...
```
denominator            ((cooling.V_flowNominal - cooling.V_flowLaminar) ^ 2)
witness                cooling.V_flowNominal = 0.1
  found by             at its declared minimum
denominator at witness 0.0
at declared values     0.81
path condition         always evaluated
constraints consulted  cooling.V_flowNominal: min=0.1, assert cooling.V_flowNominal greater 0.1
```
...
| `constraints` | cooling.V_flowNominal: min=0.1, assert cooling.V_flowNominal greater 0.1 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Cooling \
      --target cooling.V_flowNominal --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "cooling.V_flowNominal"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param cooling.V_flowNominal=0.1
```
