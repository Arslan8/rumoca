# FINDING-02334: `aimsData.turnsRatio` in `IMS_Start`
...
| **Reached as** | `aimsData.turnsRatio` |
...
| **Signature** | `divisor:divisor-reachable-zero:exp:3010:7bb8c27b` |
...
```
denominator            (aimsData.turnsRatio ^ 2)
witness                aimsData.turnsRatio = 2.22507e-308
  found by             at its declared minimum
denominator at witness 0.0
at declared values     0.9999999999999998
path condition         always evaluated
constraints consulted  aimsData.turnsRatio: min=2.22507e-308
```
...
| `denominator_at_declared_values` | 0.9999999999999998 |
| `denominator_range` | [0, inf] |
| `path_condition` | always evaluated |
| `verdict` | SAT |
| `proof` | the division is not inside any branch |
| `constraints` | aimsData.turnsRatio: min=2.22507e-308 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMS_Start \
      --target aimsData.turnsRatio --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "aimsData.turnsRatio"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param aimsData.turnsRatio=2.22507e-308
```
