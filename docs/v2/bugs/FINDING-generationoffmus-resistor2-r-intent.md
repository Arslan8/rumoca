...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/Utilities/Resistor.mo:4` |
| **Signature** | `physical:physical-intent-question:var:90:8f07cd10` |
...
```
predicate     resistor2.R > 0
premise       unknown
authority     quantity_or_unit
role          —
declaration   Modelica.Electrical.Analog.Examples.Utilities.Resistor.R
```
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Examples.Utilities.Resistor.R'}
```
...
| `required` | resistor2.R &gt; 0 |
| `declared_min` | None |
| `variable_role` | parameter |
| `observation` | permissive-declaration |
| `premise_state` | unknown |
| `authority` | quantity_or_unit |
| `canonical_declaration` | Modelica.Electrical.Analog.Examples.Utilities.Resistor.R |
| `question` | `resistor2.R` carries no bound that would exclude it, which is unusual for a resistance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.GenerationOfFMUs \
      --target resistor2.R --sanitizer physical
```
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/Utilities/Resistor.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "resistor2.R"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param resistor2.R=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/Utilities/Resistor.mo:4` |' docs/v2/bugs/
```
