# DECL-0083: `GcRef` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/QuasiStatic/FundamentalWave/BasicMachines/Components/SymmetricPolyphaseWinding.mo:44` |
| **Parameter** | `GcRef` |
| **Type** | `SI.Conductance` |
| **Declared modifiers** | `—` |
...
`SI.Conductance` declares no `min` in `Units.mo`, and this declaration adds none of its own. It therefore accepts a negative value, and the reciprocal of a resistance; negative means generation.
...
```console
$ sed -n '44p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/QuasiStatic/FundamentalWave/BasicMachines/Components/SymmetricPolyphaseWinding.mo"
```

It should declare `GcRef` with type `SI.Conductance` and no `min`.

**2. Confirm the type supplies no bound either** — this is the whole claim, and
a bound inherited from the type would refute it:

```console
$ grep -n "type Conductance " "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Units.mo"
```
...
- `SI.Conductance` turns out to carry a `min` after all, or this declaration carries one that the catalog missed.
...
```modelica
// Magnetic/QuasiStatic/FundamentalWave/BasicMachines/Components/SymmetricPolyphaseWinding.mo:44
parameter SI.Conductance GcRef(min=0) = ...;
```
