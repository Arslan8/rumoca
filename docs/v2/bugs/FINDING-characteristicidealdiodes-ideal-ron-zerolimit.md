...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:0:2520dc0b` |
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealDiode.Ron: feature_disabled (declared, from bound) — the declaration's own binding evaluates to zero at the declared configuration, so zero is the value the library chose rather than one it failed to exclude
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.CharacteristicIdealDiodes \
      --target Ideal.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "Ideal.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param Ideal.Ron=0
```
