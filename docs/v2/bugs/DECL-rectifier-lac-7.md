# DECL-0115: `LAC` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Examples/Rectifier.mo:7` |
| **Parameter** | `LAC` |
| **Type** | `SI.Inductance` |
| **Declared modifiers** | `—` |
...
It should declare `LAC` with type `SI.Inductance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/Rectifier.mo" | head
```
...
```modelica
// Electrical/Analog/Examples/Rectifier.mo:7
parameter SI.Inductance LAC(min=0) = ...;
```
