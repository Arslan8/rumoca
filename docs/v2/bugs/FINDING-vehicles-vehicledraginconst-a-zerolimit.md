...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:121:53b034dd` |
...
```
predicate     vehicleDragInConst.A > 0
premise       refuted
...
| `required` | vehicleDragInConst.A &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Translational.Vehicles \
      --target vehicleDragInConst.A --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "vehicleDragInConst.A"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param vehicleDragInConst.A=0
```
