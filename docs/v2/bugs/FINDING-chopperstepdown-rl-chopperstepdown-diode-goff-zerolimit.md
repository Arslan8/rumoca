# FINDING-04022: `chopperStepDown.diode.Goff` in `ChopperStepDown_RL`
...
| **Reached as** | `chopperStepDown.diode.Goff` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:74:e280de6d` |
...
```
predicate     chopperStepDown.diode.Goff > 0
premise       refuted
...
| `required` | chopperStepDown.diode.Goff &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCDC.ChopperStepDown.ChopperStepDown_RL \
      --target chopperStepDown.diode.Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "chopperStepDown.diode.Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param chopperStepDown.diode.Goff=0
```
