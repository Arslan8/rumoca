# FINDING-05211: `negativeToPositiveRamp.duration` in `Exponentiation`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `ModelicaTest.Blocks.Exponentiation` |
| **Reached as** | `negativeToPositiveRamp.duration` |
...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:66:87d04cc1` |
...
```
denominator            negativeToPositiveRamp.duration
witness                negativeToPositiveRamp.duration = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         not ((time < negativeToPositiveRamp.startTime)) and ((time < (negativeToPositiveRamp.startTime + negativeToPositiveRamp.duration)))
constraints consulted  negativeToPositiveRamp.duration: min=0
```
...
| `path_condition` | not ((time &lt; negativeToPositiveRamp.startTime)) and ((time &lt; (negativeToPositiveRamp.startTime + negativeToPositiveRamp.duration))) |
| `verdict` | UNSAT |
| `proof` | the path conditions cannot hold together: time must be both &gt; 0 and &lt; 0 |
| `constraints` | negativeToPositiveRamp.duration: min=0 |
...
```console
$ python3 tools/sweep/check_one.py ModelicaTest.Blocks.Exponentiation \
      --target negativeToPositiveRamp.duration --sanitizer divisor
```
...
```console
$ python3 tools/sweep/check_one.py ModelicaTest.Blocks.Exponentiation --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "negativeToPositiveRamp.duration"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param negativeToPositiveRamp.duration=0
```
