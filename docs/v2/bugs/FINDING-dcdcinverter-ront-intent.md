# FINDING-01267: `RonT` in `DcdcInverter`
...
| **Signature** | `physical:physical-intent-question:var:6:73384f9b` |
...
> `RonT` carries no bound that would exclude it, which is unusual for a resistance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
| `question` | `RonT` carries no bound that would exclude it, which is unusual for a resistance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.ControlledDCDrives.Utilities.DcdcInverter \
      --target RonT --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.ControlledDCDrives.Utilities.DcdcInverter --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "RonT"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param RonT=0
```
