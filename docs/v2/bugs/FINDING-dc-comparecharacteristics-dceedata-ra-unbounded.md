# FINDING-01314: `dceeData.Ra` in `DC_CompareCharacteristics`
...
| **Signature** | `physical:physical-domain-unenforced:var:345:e4a735a6` |
...
```
predicate     dceeData.Ra >= 0
premise       established
authority     component_contract
role          component.machine.winding_resistance
declaration   Modelica.Electrical.Machines.Utilities.ParameterRecords.DcElectricalExcitedData.Ra
```
...
| `required` | dceeData.Ra &gt;= 0 |
| `declared_min` | None |
| `variable_role` | parameter |
| `premise_state` | established |
| `authority` | component_contract |
| `semantic_role` | component.machine.winding_resistance |
| `canonical_declaration` | Modelica.Electrical.Machines.Utilities.ParameterRecords.DcElectricalExcitedData.Ra |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DC_CompareCharacteristics \
      --target dceeData.Ra --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dceeData.Ra"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param dceeData.Ra=0
```
