# FINDING-00539: `tLine4.Z0` in `CompareLineTrunks`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Electrical.Analog.Examples.Lines.CompareLineTrunks` |
| **Reached as** | `tLine4.Z0` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-guarded-by-assertion` |
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Lines/TLine.mo:19` |
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:7888:f03e0233` |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.CompareLineTrunks \
      --target tLine4.Z0 --sanitizer divisor
```
...
```console
$ sed -n '19p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Lines/TLine.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "tLine4.Z0"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param tLine4.Z0=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Lines/TLine.mo:19` |' docs/v2/bugs/
```
