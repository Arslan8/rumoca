...
| **Signature** | `physical:physical-intent-question:var:317:9c78084c` |
...
```
predicate     inlet.flowPort.medium.rho > 0
premise       unknown
...
| `required` | inlet.flowPort.medium.rho &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Cooling \
      --target inlet.flowPort.medium.rho --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inlet.flowPort.medium.rho"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inlet.flowPort.medium.rho=0
```
