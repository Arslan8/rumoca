# FINDING-01711: `imcData.Jr` in `IMC_DCBraking`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:13:e4d2c769` |
...
```
contract    Modelica.Electrical.Machines.Utilities.ParameterRecords.IM_SquirrelCageData.Jr: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_DCBraking \
      --target imcData.Jr --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "imcData.Jr"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param imcData.Jr=0
```
