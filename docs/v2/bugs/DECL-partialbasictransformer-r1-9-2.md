# DECL-0801: `R1` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/QuasiStatic/Machines/Interfaces/PartialBasicTransformer.mo:9` |
| **Parameter** | `R1` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `(start=5E-3/(if C1 == "D" then 1 else 3)` |
...
```console
$ sed -n '9p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/QuasiStatic/Machines/Interfaces/PartialBasicTransformer.mo"
```

It should declare `R1` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/QuasiStatic/Machines/Interfaces/PartialBasicTransformer.mo:9
parameter SI.Resistance R1(min=0) = ...;
```
