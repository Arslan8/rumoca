...
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:6638:b03e7a32` |
...
```
denominator            (1 + (oLine3.G[4].alpha * (oLine3.G[4].T_heatPort - oLine3.G[4].T_ref)))
witness                none needed
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.CompareLineTrunks \
      --target oLine3.G[4].alpha --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "oLine3.G[4].alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param oLine3.G[4].alpha=0
```
