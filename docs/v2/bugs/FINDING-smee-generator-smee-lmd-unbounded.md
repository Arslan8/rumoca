# FINDING-02449: `smee.Lmd` in `SMEE_Generator`
...
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/SynchronousMachines/SM_ElectricalExcited.mo:48` |
| **Signature** | `physical:physical-domain-unenforced:var:414:1b2e01af` |
...
```
predicate     smee.Lmd > 0
premise       established
authority     source_arithmetic
role          —
declaration   Modelica.Electrical.Machines.BasicMachines.SynchronousMachines.SM_ElectricalExcited.Lmd
```
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.SynchronousMachines.SM_ElectricalExcited.Lmd'}
```
...
| `authority` | source_arithmetic |
| `canonical_declaration` | Modelica.Electrical.Machines.BasicMachines.SynchronousMachines.SM_ElectricalExcited.Lmd |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMEE_Generator \
      --target smee.Lmd --sanitizer physical
```
...
```console
$ sed -n '48p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/SynchronousMachines/SM_ElectricalExcited.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smee.Lmd"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param smee.Lmd=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/SynchronousMachines/SM_ElectricalExcited.mo:48` |' docs/v2/bugs/
```
