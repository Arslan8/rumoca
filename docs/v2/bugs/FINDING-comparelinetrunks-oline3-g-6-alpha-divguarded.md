...
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:6712:11333a0d` |
...
```
denominator            (1 + (oLine3.G[6].alpha * (oLine3.G[6].T_heatPort - oLine3.G[6].T_ref)))
witness                none needed
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.CompareLineTrunks \
      --target oLine3.G[6].alpha --sanitizer divisor
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.CompareLineTrunks --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "oLine3.G[6].alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param oLine3.G[6].alpha=0
```
