# FINDING-03443: `rectifier.diode_n.idealDiode[3].Goff` in `HalfControlledBridge2mPulse`
...
| **Reached as** | `rectifier.diode_n.idealDiode[3].Goff` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:289:c5ce73f1` |
...
```
predicate     rectifier.diode_n.idealDiode[3].Goff > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealDiode.Goff: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Ideal.IdealDiode.Goff'}
```
...
| `required` | rectifier.diode_n.idealDiode[3].Goff &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.HalfControlledBridge2mPulse \
      --target rectifier.diode_n.idealDiode[3].Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "rectifier.diode_n.idealDiode[3].Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param rectifier.diode_n.idealDiode[3].Goff=0
```
