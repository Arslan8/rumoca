# DECL-0394: `XJ` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Spice3.mo:4850` |
| **Parameter** | `XJ` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '4850p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Spice3.mo"
```

It should declare `XJ` with type `SI.Length` and no `min`.
...
```modelica
// Electrical/Spice3.mo:4850
parameter SI.Length XJ(min=0) = ...;
```
