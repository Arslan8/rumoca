...
| **Reached as** | `g_mFeYokeBot.r_i` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/FixedShape/HollowCylinderRadialFlux.mo:21` |
| **Signature** | `divisor:divisor-reachable-zero:exp:1328:a4f7ca76` |
...
```
denominator            g_mFeYokeBot.r_i
witness                g_mFeYokeBot.r_i = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.0056500000000000005
path condition         always evaluated
constraints consulted  g_mFeYokeBot.r_i: min=0
```
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.SolenoidActuator.Components.AdvancedSolenoid \
      --target g_mFeYokeBot.r_i --sanitizer divisor
```
...
```console
$ sed -n '21p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/FixedShape/HollowCylinderRadialFlux.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "g_mFeYokeBot.r_i"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/FixedShape/HollowCylinderRadialFlux.mo:21` |' docs/v2/bugs/
```
