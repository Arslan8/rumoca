# DECL-0632: `RGround` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Examples/OpAmps/OpAmpCircuits/DifferentialAmplifierData.mo:10` |
| **Parameter** | `RGround` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
It should declare `RGround` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/Analog/Examples/OpAmps/OpAmpCircuits/DifferentialAmplifierData.mo:10
parameter SI.Resistance RGround(min=0) = ...;
```
