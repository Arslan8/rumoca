...
| **Signature** | `divisor:divisor-introduced-by-translation:exp:950:08f0622f` |
...
| `constraints` | g_mLeak2.l: unbounded |
| `contract` | Modelica.Magnetic.FluxTubes.Shapes.Leakage.HalfCylinder.l: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected |
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.Components.PermeanceActuator \
      --target g_mLeak2.l --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "g_mLeak2.l"
```
