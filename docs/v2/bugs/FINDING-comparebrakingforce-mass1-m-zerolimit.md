# FINDING-04686: `mass1.m` in `CompareBrakingForce`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:11:a594e0ed` |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Translational.Examples.CompareBrakingForce \
      --target mass1.m --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Translational.Examples.CompareBrakingForce --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "mass1.m"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param mass1.m=0
```
