# DECL-0094: `k` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/Examples/Loops/Utilities/GasForce2.mo:12` |
| **Parameter** | `k` |
| **Type** | `SI.HeatCapacity` |
| **Declared modifiers** | `—` |
...
`SI.HeatCapacity` declares no `min` in `Units.mo`, and this declaration adds none of its own. It therefore accepts a negative value, and a negative heat capacity makes heating cool the body.
...
It should declare `k` with type `SI.HeatCapacity` and no `min`.

**2. Confirm the type supplies no bound either** — this is the whole claim, and
a bound inherited from the type would refute it:

```console
$ grep -n "type HeatCapacity " "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Units.mo"
```
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Examples/Loops/Utilities/GasForce2.mo" | head
```
...
- `SI.HeatCapacity` turns out to carry a `min` after all, or this declaration carries one that the catalog missed.
...
```modelica
// Mechanics/MultiBody/Examples/Loops/Utilities/GasForce2.mo:12
parameter SI.HeatCapacity k(min=0) = ...;
```
