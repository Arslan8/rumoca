# FINDING-02757: `smpmData.Jr` in `SMPM_ResistiveBraking`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:18:5da4f879` |
...
```
contract    Modelica.Electrical.Machines.Utilities.ParameterRecords.SM_PermanentMagnetData.Jr: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
