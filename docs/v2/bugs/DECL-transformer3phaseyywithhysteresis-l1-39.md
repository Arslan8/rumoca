# DECL-0439: `l1` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/FluxTubes/Examples/Hysteresis/Components/Transformer3PhaseYyWithHysteresis.mo:39` |
| **Parameter** | `l1` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `(displayUnit="mm")` |
...
`SI.Length` declares no `min` in `Units.mo`, and this declaration adds none of its own. It therefore accepts a negative value, and a negative length is not a geometry.
...
```console
$ sed -n '39p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/Hysteresis/Components/Transformer3PhaseYyWithHysteresis.mo"
```

It should declare `l1` with type `SI.Length` and no `min`.

**2. Confirm the type supplies no bound either** — this is the whole claim, and
a bound inherited from the type would refute it:

```console
$ grep -n "type Length " "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Units.mo"
```
...
- `SI.Length` turns out to carry a `min` after all, or this declaration carries one that the catalog missed.
...
```modelica
// Magnetic/FluxTubes/Examples/Hysteresis/Components/Transformer3PhaseYyWithHysteresis.mo:39
parameter SI.Length l1(min=0) = ...;
```
