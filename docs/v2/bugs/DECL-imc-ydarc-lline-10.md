# DECL-0149: `LLine` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Machines/Examples/InductionMachines/IMC_YDarc.mo:10` |
| **Parameter** | `LLine` |
| **Type** | `SI.Inductance` |
| **Declared modifiers** | `—` |
...
It should declare `LLine` with type `SI.Inductance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/InductionMachines/IMC_YDarc.mo" | head
```
...
```modelica
// Electrical/Machines/Examples/InductionMachines/IMC_YDarc.mo:10
parameter SI.Inductance LLine(min=0) = ...;
```
