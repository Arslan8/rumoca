# FINDING-03677: `pulse2.twoPulse.filter[1].firstOrder.T` in `ThyristorBridge2Pulse_DC_Drive`
...
| **Reached as** | `pulse2.twoPulse.filter[1].firstOrder.T` |
...
| **Signature** | `divisor:divisor-reachable-zero:exp:2172:cb1ec471` |
...
```
denominator            pulse2.twoPulse.filter[1].firstOrder.T
witness                pulse2.twoPulse.filter[1].firstOrder.T = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  pulse2.twoPulse.filter[1].firstOrder.T: unbounded
```
...
| `constraints` | pulse2.twoPulse.filter[1].firstOrder.T: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2Pulse.ThyristorBridge2Pulse_DC_Drive \
      --target pulse2.twoPulse.filter[1].firstOrder.T --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pulse2.twoPulse.filter[1].firstOrder.T"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param pulse2.twoPulse.filter[1].firstOrder.T=0
```
