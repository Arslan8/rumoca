# FINDING-02301: `switchedRheostat.idealCommutingSwitch.idealCommutingSwitch[1].Goff` in `IMS_Start`
...
| **Reached as** | `switchedRheostat.idealCommutingSwitch.idealCommutingSwitch[1].Goff` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Ideal/IdealTwoWaySwitch.mo:4` |
| **Signature** | `physical:physical-rule-does-not-apply:var:941:36819bf7` |
...
```
predicate     switchedRheostat.idealCommutingSwitch.idealCommutingSwitch[1].Goff > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealTwoWaySwitch.Goff: allowed (declared, from class_catalog) — an ideal switch family member; zero off-conductance is the documented ideal [matched by prefix, lower confidence]
behavior    allowed
confidence  declared
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Ideal.IdealTwoWaySwitch.Goff'}
```
...
| `required` | switchedRheostat.idealCommutingSwitch.idealCommutingSwitch[1].Goff &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMS_Start \
      --target switchedRheostat.idealCommutingSwitch.idealCommutingSwitch[1].Goff --sanitizer physical
```
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Ideal/IdealTwoWaySwitch.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switchedRheostat.idealCommutingSwitch.idealCommutingSwitch[1].Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switchedRheostat.idealCommutingSwitch.idealCommutingSwitch[1].Goff=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Ideal/IdealTwoWaySwitch.mo:4` |' docs/v2/bugs/
```
