# FINDING-02170: `switchYDwithArc.idealOpener.openerWithArc[3].Goff` in `IMC_YDarc`
...
| **Reached as** | `switchYDwithArc.idealOpener.openerWithArc[3].Goff` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:846:f733bed5` |
...
```
predicate     switchYDwithArc.idealOpener.openerWithArc[3].Goff > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Ideal.OpenerWithArc.Goff: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Ideal.OpenerWithArc.Goff'}
```
...
| `required` | switchYDwithArc.idealOpener.openerWithArc[3].Goff &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_YDarc \
      --target switchYDwithArc.idealOpener.openerWithArc[3].Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switchYDwithArc.idealOpener.openerWithArc[3].Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switchYDwithArc.idealOpener.openerWithArc[3].Goff=0
```
