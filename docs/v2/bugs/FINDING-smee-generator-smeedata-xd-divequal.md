# FINDING-02486: `smeeData.xd` in `SMEE_Generator`
...
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo:74` |
| **Signature** | `divisor:divisor-zero-when-parameters-equal:exp:1878:6608e188` |
...
```
denominator            (smeeData.xd - smeeData.xdTransient)
witness                smeeData.xd = 0.1375
  found by             set equal to smeeData.xdTransient
denominator at witness 0.0
at declared values     1.4625000000000001
path condition         always evaluated
constraints consulted  smeeData.xd: unbounded
```
...
| `denominator` | (smeeData.xd - smeeData.xdTransient) |
| `witness` | smeeData.xd = 0.1375 |
| `witness_rationale` | set equal to smeeData.xdTransient |
| `denominator_at_witness` | 0.0 |
| `denominator_at_declared_values` | 1.4625000000000001 |
| `denominator_range` | [-inf, inf] |
| `path_condition` | always evaluated |
| `verdict` | SAT |
| `proof` | the division is not inside any branch |
| `constraints` | smeeData.xd: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMEE_Generator \
      --target smeeData.xd --sanitizer divisor
```
...
```console
$ sed -n '74p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smeeData.xd"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo:74` |' docs/v2/bugs/
```
