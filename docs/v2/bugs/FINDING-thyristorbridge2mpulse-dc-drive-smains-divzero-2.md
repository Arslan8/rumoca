# FINDING-03506: `SMains` in `ThyristorBridge2mPulse_DC_Drive`
...
| **Signature** | `divisor:divisor-reachable-zero:exp:20:db7d0428` |
...
at declared values     250000.0
path condition         always evaluated
constraints consulted  SMains: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.ThyristorBridge2mPulse_DC_Drive \
      --target SMains --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "SMains"
```
