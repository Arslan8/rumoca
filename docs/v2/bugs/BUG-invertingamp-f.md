...
| **Declaration** | `InvertingAmp.mo:18` |
...
| **Fix site** | [BUG-019: `InvertingAmp.f` is an unbounded frequency reaching four divisors](../../verified%20bugs/BUG-019-invertingamp-frequency-unbounded-divisor.md) |
| **Status** | Reported upstream, not fixed |


## The claim

`Modelica.Electrical.Analog.Examples.InvertingAmp` runs cleanly at its declared values. Setting `f = 0` — a value the declaration permits — makes it fail, in Rumoca and in OpenModelica.
...
| `denominator_at_declared_values` | 10.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.InvertingAmp \
      --target f --keep-parameter-chains
```
...
```console
$ find target/msl target/corpus -name 'InvertingAmp.mo'
$ sed -n '18p' "$(find target/msl target/corpus -name 'InvertingAmp.mo' | head -1)"
```
...
```console
$ ./target/debug/rumoca compile "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/InvertingAmp.mo" --model Modelica.Electrical.Analog.Examples.InvertingAmp \
...
simulate(Modelica.Electrical.Analog.Examples.InvertingAmp, simflags="-override f=0");
EOF
```
