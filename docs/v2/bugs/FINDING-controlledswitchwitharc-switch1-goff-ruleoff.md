...
| **Signature** | `physical:physical-rule-does-not-apply:var:31:f362de4f` |
...
```
contract    Modelica.Electrical.Analog.Ideal.ControlledIdealClosingSwitch.Goff: allowed (declared, from class_catalog) — an ideal switch family member; zero off-conductance is the documented ideal [matched by prefix, lower confidence]
behavior    allowed
confidence  declared
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Ideal.ControlledIdealClosingSwitch.Goff'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.ControlledSwitchWithArc \
      --target switch1.Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switch1.Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switch1.Goff=0
```
