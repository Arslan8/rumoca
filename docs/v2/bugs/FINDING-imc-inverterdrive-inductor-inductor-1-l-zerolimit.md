# FINDING-01843: `inductor.inductor[1].L` in `IMC_InverterDrive`
...
| **Reached as** | `inductor.inductor[1].L` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:250:92da1cbc` |
...
```
predicate     inductor.inductor[1].L > 0
premise       refuted
...
| `required` | inductor.inductor[1].L &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive \
      --target inductor.inductor[1].L --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inductor.inductor[1].L"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inductor.inductor[1].L=0
```
