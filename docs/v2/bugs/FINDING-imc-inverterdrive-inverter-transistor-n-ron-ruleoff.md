# FINDING-01885: `inverter.transistor_n.Ron` in `IMC_InverterDrive`
...
| **Reached as** | `inverter.transistor_n.Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:708:fd9cc35e` |
...
```
predicate     inverter.transistor_n.Ron > 0
premise       refuted
...
| `required` | inverter.transistor_n.Ron &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive \
      --target inverter.transistor_n.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inverter.transistor_n.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inverter.transistor_n.Ron=0
```
