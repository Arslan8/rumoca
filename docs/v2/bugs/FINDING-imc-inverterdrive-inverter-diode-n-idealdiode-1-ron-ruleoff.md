# FINDING-01895: `inverter.diode_n.idealDiode[1].Ron` in `IMC_InverterDrive`
...
| **Reached as** | `inverter.diode_n.idealDiode[1].Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:789:52af69ad` |
...
```
predicate     inverter.diode_n.idealDiode[1].Ron > 0
premise       refuted
...
| `required` | inverter.diode_n.idealDiode[1].Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive \
      --target inverter.diode_n.idealDiode[1].Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inverter.diode_n.idealDiode[1].Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inverter.diode_n.idealDiode[1].Ron=0
```
