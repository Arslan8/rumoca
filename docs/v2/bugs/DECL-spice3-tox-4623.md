# DECL-0388: `TOX` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Spice3.mo:4623` |
| **Parameter** | `TOX` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '4623p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Spice3.mo"
```

It should declare `TOX` with type `SI.Length` and no `min`.
...
```modelica
// Electrical/Spice3.mo:4623
parameter SI.Length TOX(min=0) = ...;
```
