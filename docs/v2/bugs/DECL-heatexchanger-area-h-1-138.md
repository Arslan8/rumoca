# DECL-0014: `area_h_1` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Fluid/Examples/HeatExchanger.mo:138` |
| **Parameter** | `area_h_1` |
| **Type** | `SI.Area` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '138p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Examples/HeatExchanger.mo"
```

It should declare `area_h_1` with type `SI.Area` and no `min`.
...
```modelica
// Fluid/Examples/HeatExchanger.mo:138
parameter SI.Area area_h_1(min=0) = ...;
```
