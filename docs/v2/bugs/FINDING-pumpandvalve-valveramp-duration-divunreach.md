...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:1081:612ca09a` |
...
```
denominator            valveRamp.duration
witness                valveRamp.duration = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.1
path condition         not ((time < valveRamp.startTime)) and ((time < (valveRamp.startTime + valveRamp.duration)))
constraints consulted  valveRamp.duration: min=0
```
...
| `path_condition` | not ((time &lt; valveRamp.startTime)) and ((time &lt; (valveRamp.startTime + valveRamp.duration))) |
| `verdict` | UNSAT |
| `proof` | the path conditions cannot hold together: time must be both &gt; 0.9 and &lt; 0.9 |
| `constraints` | valveRamp.duration: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.PumpAndValve \
      --target valveRamp.duration --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "valveRamp.duration"
```
