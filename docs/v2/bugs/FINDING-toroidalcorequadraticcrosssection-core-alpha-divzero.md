# FINDING-04209: `core.alpha` in `ToroidalCoreQuadraticCrossSection`
...
| **Reached as** | `core.alpha` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/FixedShape/HollowCylinderCircumferentialFlux.mo:20` |
| **Signature** | `divisor:divisor-reachable-zero:exp:355:67edb9c1` |
...
```
denominator            (((core.r_o + core.r_i) / 2) * core.alpha)
witness                core.alpha = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.31315926535897937
path condition         always evaluated
constraints consulted  core.alpha: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.BasicExamples.ToroidalCoreQuadraticCrossSection \
      --target core.alpha --sanitizer divisor
```
...
```console
$ sed -n '20p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/FixedShape/HollowCylinderCircumferentialFlux.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "core.alpha"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/FixedShape/HollowCylinderCircumferentialFlux.mo:20` |' docs/v2/bugs/
```
