# FINDING-01564: `dcpm2.wNominal` in `DCPM_QuasiStatic`
...
| **Signature** | `divisor:divisor-reachable-zero:exp:650:31f9bdb7` |
...
```
denominator            (dcpm2.wNominal * dcpm2.psi_eNominal)
witness                dcpm2.wNominal = 0
  found by             set to zero
denominator at witness 0.0
at declared values     149.22565104551515
path condition         always evaluated
constraints consulted  dcpm2.wNominal: unbounded
```
...
| `constraints` | dcpm2.wNominal: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_QuasiStatic \
      --target dcpm2.wNominal --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcpm2.wNominal"
```
