# FINDING-00061: `filter.n` in `MixingUnitWithContinuousControl`
...
| **Signature** | `divisor:divisor-zero-unresolved:exp:231:688a72c0` |
...
```
denominator            filter.n
witness                filter.n = 0
  found by             set to zero
denominator at witness 0.0
at declared values     3.0
path condition         (filter.normalized)
constraints consulted  filter.n: unbounded
```
...
| `path_condition` | (filter.normalized) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: filter.normalized |
| `constraints` | filter.n: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Clocked.Examples.Systems.Utilities.ComponentsMixingUnit.MixingUnitWithContinuousControl \
      --target filter.n --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "filter.n"
```
