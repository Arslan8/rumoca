# DECL-0584: `samplePeriod` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Blocks/Interfaces.mo:1303` |
...
`SI.Period` declares no `min` in `Units.mo`, and this declaration adds none of its own. It therefore accepts a negative value, and a negative period is not a physical value.
...
```console
$ sed -n '1303p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Blocks/Interfaces.mo"
```

It should declare `samplePeriod` with type `SI.Period` and no `min`.

**2. Confirm the type supplies no bound either** — this is the whole claim, and
a bound inherited from the type would refute it:

```console
$ grep -n "type Period " "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Units.mo"
```
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Blocks/Interfaces.mo" | head
```
...
- `SI.Period` turns out to carry a `min` after all, or this declaration carries one that the catalog missed.
...
```modelica
// Blocks/Interfaces.mo:1303
parameter SI.Period samplePeriod(min=0) = ...;
```
