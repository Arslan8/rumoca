# DECL-0442: `b` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/FluxTubes/Examples/Hysteresis/Components/Transformer3PhaseYyWithHysteresis.mo:44` |
| **Parameter** | `b` |
| **Type** | `SI.Length` |
...
```console
$ sed -n '44p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/Hysteresis/Components/Transformer3PhaseYyWithHysteresis.mo"
```

It should declare `b` with type `SI.Length` and no `min`.
...
```modelica
// Magnetic/FluxTubes/Examples/Hysteresis/Components/Transformer3PhaseYyWithHysteresis.mo:44
parameter SI.Length b(min=0) = ...;
```
