...
| **Signature** | `divisor:divisor-reachable-zero:exp:1060:d941411a` |
...
```
denominator            transferFunction.a_end
witness                transferFunction.a_end = 0
  found by             set to zero
denominator at witness 0.0
at declared values     None
path condition         always evaluated
constraints consulted  transferFunction.a_end: unbounded
```
...
| `constraints` | transferFunction.a_end: unbounded |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Blocks.Continuous_InitialState \
      --target transferFunction.a_end --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "transferFunction.a_end"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param transferFunction.a_end=0
```
