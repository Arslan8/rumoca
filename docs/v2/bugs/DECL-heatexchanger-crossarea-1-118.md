# DECL-0012: `crossArea_1` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Fluid/Examples/HeatExchanger.mo:118` |
| **Parameter** | `crossArea_1` |
| **Type** | `SI.Area` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '118p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Examples/HeatExchanger.mo"
```

It should declare `crossArea_1` with type `SI.Area` and no `min`.
...
```modelica
// Fluid/Examples/HeatExchanger.mo:118
parameter SI.Area crossArea_1(min=0) = ...;
```
