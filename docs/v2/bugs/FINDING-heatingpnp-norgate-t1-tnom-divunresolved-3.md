# FINDING-00392: `T1.Tnom` in `HeatingPNP_NORGate`
...
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/PNP.mo:76` |
| **Signature** | `divisor:divisor-zero-unresolved:exp:1036:cbb771cc` |
...
path condition         (T1.useTemperatureDependency)
constraints consulted  T1.Tnom: min=0
```
...
| `path_condition` | (T1.useTemperatureDependency) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: T1.useTemperatureDependency |
| `constraints` | T1.Tnom: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.HeatingPNP_NORGate \
      --target T1.Tnom --sanitizer divisor
```
...
```console
$ sed -n '76p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/PNP.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "T1.Tnom"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/PNP.mo:76` |' docs/v2/bugs/
```
