# FINDING-02794: `switch3.idealClosingSwitch[2].Ron` in `SMPM_ResistiveBraking`
...
| **Reached as** | `switch3.idealClosingSwitch[2].Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:960:b9851ed6` |
...
```
predicate     switch3.idealClosingSwitch[2].Ron > 0
premise       refuted
...
| `required` | switch3.idealClosingSwitch[2].Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_ResistiveBraking \
      --target switch3.idealClosingSwitch[2].Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switch3.idealClosingSwitch[2].Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switch3.idealClosingSwitch[2].Ron=0
```
