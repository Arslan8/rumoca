# FINDING-03513: `pulse2.twomPulse.filter[1].fCut` in `ThyristorBridge2mPulse_DC_Drive`
...
| **Reached as** | `pulse2.twomPulse.filter[1].fCut` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/Control/Filter.mo:14` |
| **Signature** | `divisor:divisor-reachable-zero:exp:1015:65b1ecc2` |
...
```
denominator            ((2 * (2 * asin(1.0))) * pulse2.twomPulse.filter[1].fCut)
witness                pulse2.twomPulse.filter[1].fCut = 0
  found by             set to zero
denominator at witness 0.0
at declared values     628.3185307179587
path condition         always evaluated
constraints consulted  pulse2.twomPulse.filter[1].fCut: unbounded
```
...
| `denominator_at_declared_values` | 628.3185307179587 |
| `denominator_range` | [-inf, inf] |
| `path_condition` | always evaluated |
| `verdict` | SAT |
| `proof` | the division is not inside any branch |
| `constraints` | pulse2.twomPulse.filter[1].fCut: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.ThyristorBridge2mPulse_DC_Drive \
      --target pulse2.twomPulse.filter[1].fCut --sanitizer divisor
```
...
```console
$ sed -n '14p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/Control/Filter.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pulse2.twomPulse.filter[1].fCut"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param pulse2.twomPulse.filter[1].fCut=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/Control/Filter.mo:14` |' docs/v2/bugs/
```
