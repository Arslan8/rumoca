# FINDING-03792: `rectifier.thyristor_p.Goff` in `ThyristorCenterTap2mPulse_RLV_Characteristic`
...
| **Reached as** | `rectifier.thyristor_p.Goff` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Ideal/IdealThyristor.mo:6` |
| **Signature** | `physical:physical-rule-does-not-apply:var:176:5b4d79f7` |
...
```
predicate     rectifier.thyristor_p.Goff > 0
premise       refuted
...
```
contract    Modelica.Electrical.Polyphase.Ideal.IdealThyristor.Goff: allowed (declared, from class_catalog) — an ideal switch family member; zero off-conductance is the documented ideal [matched by prefix, lower confidence]
behavior    allowed
confidence  declared
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Polyphase.Ideal.IdealThyristor.Goff'}
```
...
| `required` | rectifier.thyristor_p.Goff &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTap2mPulse.ThyristorCenterTap2mPulse_RLV_Characteristic \
      --target rectifier.thyristor_p.Goff --sanitizer physical
```
...
```console
$ sed -n '6p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Ideal/IdealThyristor.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "rectifier.thyristor_p.Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param rectifier.thyristor_p.Goff=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Ideal/IdealThyristor.mo:6` |' docs/v2/bugs/
```
