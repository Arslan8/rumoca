# FINDING-00049: `invMixingUnit.tau0` in `MixingUnitWithContinuousControl`
...
| **Signature** | `divisor:divisor-reachable-zero:exp:149:68c5b53a` |
...
```
denominator            invMixingUnit.tau0
witness                invMixingUnit.tau0 = 0
  found by             set to zero
denominator at witness 0.0
at declared values     60.0
path condition         always evaluated
constraints consulted  invMixingUnit.tau0: unbounded
```
...
| `constraints` | invMixingUnit.tau0: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Clocked.Examples.Systems.Utilities.ComponentsMixingUnit.MixingUnitWithContinuousControl \
      --target invMixingUnit.tau0 --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "invMixingUnit.tau0"
```
