# DECL-0899: `V_t` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Fluid/Examples/DrumBoiler.mo:187` |
| **Parameter** | `V_t` |
| **Type** | `SI.Volume` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '187p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Examples/DrumBoiler.mo"
```

It should declare `V_t` with type `SI.Volume` and no `min`.
...
```modelica
// Fluid/Examples/DrumBoiler.mo:187
parameter SI.Volume V_t(min=0) = ...;
```
