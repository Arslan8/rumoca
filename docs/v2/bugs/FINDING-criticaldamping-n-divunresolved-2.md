# FINDING-00010: `n` in `CriticalDamping`
...
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Clocked/Examples/Systems/Utilities/ComponentsMixingUnit/CriticalDamping.mo:14` |
| **Signature** | `divisor:divisor-zero-unresolved:exp:23:239c61fb` |
...
path condition         (normalized)
constraints consulted  n: unbounded
```
...
| `path_condition` | (normalized) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: normalized |
| `constraints` | n: unbounded |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Clocked.Examples.Systems.Utilities.ComponentsMixingUnit.CriticalDamping \
      --target n --sanitizer divisor
```
...
```console
$ sed -n '14p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Clocked/Examples/Systems/Utilities/ComponentsMixingUnit/CriticalDamping.mo"
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Clocked.Examples.Systems.Utilities.ComponentsMixingUnit.CriticalDamping --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "n"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Clocked/Examples/Systems/Utilities/ComponentsMixingUnit/CriticalDamping.mo:14` |' docs/v2/bugs/
```
