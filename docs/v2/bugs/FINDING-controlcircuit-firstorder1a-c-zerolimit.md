...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/OpAmps/OpAmpCircuits/FirstOrder.mo:9` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:234:8c180594` |
...
```
contract    Modelica.Electrical.Analog.Basic.Capacitor.C: algebraic_limit (proven, from equation) — inherited from firstOrder1A.c.C, which it binds: the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
...
```
{'quantity': 'Capacitance', 'unit': 'F', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Examples.OpAmps.OpAmpCircuits.FirstOrder.C'}
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.ControlCircuit \
      --target firstOrder1A.C --sanitizer physical
```
...
```console
$ sed -n '9p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/OpAmps/OpAmpCircuits/FirstOrder.mo"
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.ControlCircuit --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "firstOrder1A.C"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param firstOrder1A.C=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/OpAmps/OpAmpCircuits/FirstOrder.mo:9` |' docs/v2/bugs/
```
