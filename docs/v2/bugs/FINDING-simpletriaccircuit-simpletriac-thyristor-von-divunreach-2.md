...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:622:e7702a32` |
...
```
denominator            (2 * (simpleTriac.thyristor.Von - simpleTriac.thyristor.Voff))
witness                simpleTriac.thyristor.Von = 1.5
  found by             the value that solves the denominator to zero
denominator at witness 0.0
at declared values     7.0
...
constraints consulted  simpleTriac.thyristor.Von: unbounded
```
...
| `denominator_at_declared_values` | 7.0 |
...
| `constraints` | simpleTriac.thyristor.Von: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.SimpleTriacCircuit \
      --target simpleTriac.thyristor.Von --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "simpleTriac.thyristor.Von"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param simpleTriac.thyristor.Von=1.5
```
