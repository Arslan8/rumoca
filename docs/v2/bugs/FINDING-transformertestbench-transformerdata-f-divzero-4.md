# FINDING-03318: `transformerData.f` in `TransformerTestbench`
...
| **Signature** | `divisor:divisor-reachable-zero:exp:1328:d7a49df4` |
...
```
denominator            ((2 * (2 * asin(1.0))) * transformerData.f)
witness                transformerData.f = 0
  found by             set to zero
denominator at witness 0.0
at declared values     314.1592653589793
path condition         always evaluated
constraints consulted  transformerData.f: unbounded
```
...
| `constraints` | transformerData.f: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.Transformers.TransformerTestbench \
      --target transformerData.f --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "transformerData.f"
```
