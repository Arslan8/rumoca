# FINDING-01389: `G_core_cooling` in `DCPM_Cooling`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/DCMachines/DCPM_Cooling.mo:26` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:13:86f99c2c` |
...
```
contract    Modelica.Thermal.HeatTransfer.Components.ThermalConductor.G: feature_disabled (proven, from equation) — inherited from coreCooling.G, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'ThermalConductance', 'unit': 'W/K', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Cooling.G_core_cooling'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Cooling \
      --target G_core_cooling --sanitizer physical
```
...
```console
$ sed -n '26p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/DCMachines/DCPM_Cooling.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "G_core_cooling"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param G_core_cooling=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/DCMachines/DCPM_Cooling.mo:26` |' docs/v2/bugs/
```
