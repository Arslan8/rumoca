# DECL-0258: `Lesigma` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/FundamentalWave/BasicMachines/SynchronousMachines/SM_ElectricalExcited.mo:169` |
| **Parameter** | `Lesigma` |
| **Type** | `SI.Inductance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '169p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FundamentalWave/BasicMachines/SynchronousMachines/SM_ElectricalExcited.mo"
```

It should declare `Lesigma` with type `SI.Inductance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FundamentalWave/BasicMachines/SynchronousMachines/SM_ElectricalExcited.mo" | head
```
...
```modelica
// Magnetic/FundamentalWave/BasicMachines/SynchronousMachines/SM_ElectricalExcited.mo:169
parameter SI.Inductance Lesigma(min=0) = ...;
```
