# DECL-0820: `RD` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Spice3.mo:5192` |
| **Parameter** | `RD` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '5192p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Spice3.mo"
```

It should declare `RD` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/Spice3.mo:5192
parameter SI.Resistance RD(min=0) = ...;
```
