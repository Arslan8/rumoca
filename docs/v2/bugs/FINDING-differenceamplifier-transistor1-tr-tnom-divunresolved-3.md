# FINDING-00295: `Transistor1.Tr.Tnom` in `DifferenceAmplifier`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Electrical.Analog.Examples.DifferenceAmplifier` |
| **Reached as** | `Transistor1.Tr.Tnom` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-zero-unresolved` |
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/NPN.mo:76` |
| **Signature** | `divisor:divisor-zero-unresolved:exp:1162:8d1090fe` |
...
```
denominator            Transistor1.Tr.Tnom
witness                Transistor1.Tr.Tnom = 0
  found by             set to zero
denominator at witness 0.0
at declared values     300.15
path condition         (Transistor1.Tr.useTemperatureDependency)
constraints consulted  Transistor1.Tr.Tnom: min=0
```
...
| `path_condition` | (Transistor1.Tr.useTemperatureDependency) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: Transistor1.Tr.useTemperatureDependency |
| `constraints` | Transistor1.Tr.Tnom: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.DifferenceAmplifier \
      --target Transistor1.Tr.Tnom --sanitizer divisor
```
...
```console
$ sed -n '76p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/NPN.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "Transistor1.Tr.Tnom"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/NPN.mo:76` |' docs/v2/bugs/
```
