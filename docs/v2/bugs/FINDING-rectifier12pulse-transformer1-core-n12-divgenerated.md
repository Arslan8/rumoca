# FINDING-03190: `transformer1.core.n12` in `Rectifier12pulse`
...
| **Reached as** | `transformer1.core.n12` |
...
| **Signature** | `divisor:divisor-introduced-by-translation:exp:3458:d733722d` |
...
```
denominator            transformer1.core.n12
witness                transformer1.core.n12 = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  transformer1.core.n12: unbounded
```
...
| `constraints` | transformer1.core.n12: unbounded |
| `contract` | Modelica.Electrical.Machines.BasicMachines.Components.IdealCore.n12: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.Transformers.Rectifier12pulse \
      --target transformer1.core.n12 --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "transformer1.core.n12"
```
