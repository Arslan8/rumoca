# FINDING-02153: `switchYDwithArc.Ron` in `IMC_YDarc`
...
| **Reached as** | `switchYDwithArc.Ron` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SwitchYDwithArc.mo:4` |
| **Signature** | `physical:physical-rule-does-not-apply:var:645:be71b164` |
...
```
predicate     switchYDwithArc.Ron > 0
premise       refuted
...
```
contract    Modelica.Electrical.Polyphase.Ideal.CloserWithArc.Ron: allowed (declared, from class_catalog) — inherited from switchYDwithArc.idealCloser.Ron, which it binds: an ideal switch family member; zero on-resistance is the documented ideal
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.Utilities.SwitchYDwithArc.Ron'}
```
...
| `required` | switchYDwithArc.Ron &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_YDarc \
      --target switchYDwithArc.Ron --sanitizer physical
```
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SwitchYDwithArc.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switchYDwithArc.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switchYDwithArc.Ron=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SwitchYDwithArc.mo:4` |' docs/v2/bugs/
```
