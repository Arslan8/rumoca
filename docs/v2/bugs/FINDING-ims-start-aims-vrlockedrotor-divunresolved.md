# FINDING-02322: `aims.VrLockedRotor` in `IMS_Start`
...
| **Reached as** | `aims.VrLockedRotor` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-zero-unresolved` |
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/InductionMachines/IM_SlipRing.mo:76` |
| **Signature** | `divisor:divisor-zero-unresolved:exp:878:3759b388` |
...
```
denominator            aims.VrLockedRotor
witness                aims.VrLockedRotor = 0
  found by             set to zero
denominator at witness 0.0
at declared values     96.60262311821904
path condition         not (aims.useTurnsRatio)
constraints consulted  aims.VrLockedRotor: unbounded
```
...
| `denominator_at_declared_values` | 96.60262311821904 |
| `denominator_range` | [-inf, inf] |
| `path_condition` | not (aims.useTurnsRatio) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: aims.useTurnsRatio |
| `constraints` | aims.VrLockedRotor: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMS_Start \
      --target aims.VrLockedRotor --sanitizer divisor
```
...
```console
$ sed -n '76p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/InductionMachines/IM_SlipRing.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "aims.VrLockedRotor"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/InductionMachines/IM_SlipRing.mo:76` |' docs/v2/bugs/
```
