...
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:5871:39f1ef81` |
...
```
denominator            (1 + (oLine2.G[6].alpha * (oLine2.G[6].T_heatPort - oLine2.G[6].T_ref)))
witness                none needed
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.CompareLineTrunks \
      --target oLine2.G[6].alpha --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "oLine2.G[6].alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param oLine2.G[6].alpha=0
```
