...
| **Signature** | `divisor:divisor-reachable-zero:exp:474:c41edbc0` |
...
path condition         not (((TP2.RDS < 1e-20) and (TP2.RDS > -1e-20)))
constraints consulted  TP2.RDS: unbounded
```
...
| `path_condition` | not (((TP2.RDS &lt; 1e-20) and (TP2.RDS &gt; -1e-20))) |
| `verdict` | SAT |
| `proof` | every guard still holds under the witness |
| `constraints` | TP2.RDS: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Utilities.Nand \
      --target TP2.RDS --sanitizer divisor
```
