# DECL-0514: `rh` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/Examples/Loops/PlanarLoops_analytic.mo:5` |
| **Parameter** | `rh` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '5p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Examples/Loops/PlanarLoops_analytic.mo"
```

It should declare `rh` with type `SI.Length` and no `min`.
...
```modelica
// Mechanics/MultiBody/Examples/Loops/PlanarLoops_analytic.mo:5
parameter SI.Length rh(min=0) = ...;
```
