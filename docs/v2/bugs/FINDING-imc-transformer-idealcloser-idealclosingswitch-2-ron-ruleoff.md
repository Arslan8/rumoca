# FINDING-02053: `idealCloser.idealClosingSwitch[2].Ron` in `IMC_Transformer`
...
| **Reached as** | `idealCloser.idealClosingSwitch[2].Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:608:68c63346` |
...
```
predicate     idealCloser.idealClosingSwitch[2].Ron > 0
premise       refuted
...
| `required` | idealCloser.idealClosingSwitch[2].Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_Transformer \
      --target idealCloser.idealClosingSwitch[2].Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "idealCloser.idealClosingSwitch[2].Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param idealCloser.idealClosingSwitch[2].Ron=0
```
