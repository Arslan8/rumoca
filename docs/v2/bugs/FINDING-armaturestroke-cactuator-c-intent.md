# FINDING-04230: `cActuator.c` in `ArmatureStroke`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/MovingCoilActuator/Components/ConstantActuator.mo:10` |
| **Signature** | `physical:physical-intent-question:var:247:173c2a27` |
...
```
predicate     cActuator.c > 0
premise       unknown
authority     quantity_or_unit
role          —
declaration   Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.Components.ConstantActuator.c
```
...
> `cActuator.c` carries no bound that would exclude it, which is unusual for a translational spring constant — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
```
{'quantity': 'TranslationalSpringConstant', 'unit': 'N/m', 'confidence': 'QUANTITY', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.Components.ConstantActuator.c'}
```
...
| `required` | cActuator.c &gt; 0 |
| `declared_min` | None |
| `variable_role` | parameter |
| `observation` | permissive-declaration |
| `premise_state` | unknown |
| `authority` | quantity_or_unit |
| `canonical_declaration` | Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.Components.ConstantActuator.c |
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.ArmatureStroke \
      --target cActuator.c --sanitizer physical
```
...
```console
$ sed -n '10p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/MovingCoilActuator/Components/ConstantActuator.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "cActuator.c"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param cActuator.c=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/MovingCoilActuator/Components/ConstantActuator.mo:10` |' docs/v2/bugs/
```
