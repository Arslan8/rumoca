...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:0:09fd202f` |
...
```
contract    Modelica.Thermal.HeatTransfer.Components.ThermalConductor.G: feature_disabled (proven, from equation) — inherited from thermalConductor.G, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
