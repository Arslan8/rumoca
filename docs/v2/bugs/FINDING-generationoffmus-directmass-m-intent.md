...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Examples/Utilities/DirectMass.mo:4` |
| **Signature** | `physical:physical-intent-question:var:7:41b855f6` |
...
```
predicate     directMass.m > 0
premise       unknown
authority     quantity_or_unit
role          —
declaration   Modelica.Mechanics.Translational.Examples.Utilities.DirectMass.m
```
...
> `directMass.m` is bounded only by min=0, which is unusual for a mass — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
```
{'quantity': 'Mass', 'unit': 'kg', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Mechanics.Translational.Examples.Utilities.DirectMass.m'}
```
...
| `required` | directMass.m &gt; 0 |
| `declared_min` | 0.0 |
| `variable_role` | parameter |
| `observation` | permissive-declaration |
| `premise_state` | unknown |
| `authority` | quantity_or_unit |
| `canonical_declaration` | Modelica.Mechanics.Translational.Examples.Utilities.DirectMass.m |
| `question` | `directMass.m` is bounded only by min=0, which is unusual for a mass — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Translational.Examples.GenerationOfFMUs \
      --target directMass.m --sanitizer physical
```
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Examples/Utilities/DirectMass.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "directMass.m"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param directMass.m=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Examples/Utilities/DirectMass.mo:4` |' docs/v2/bugs/
```
