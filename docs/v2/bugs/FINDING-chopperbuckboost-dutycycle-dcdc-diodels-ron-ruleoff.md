# FINDING-03999: `dcdc.diodeLS.Ron` in `ChopperBuckBoost_DutyCycle`
...
| **Signature** | `physical:physical-rule-does-not-apply:var:72:6ec5d8f3` |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCDC.ChopperBuckBoost.ChopperBuckBoost_DutyCycle \
      --target dcdc.diodeLS.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcdc.diodeLS.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param dcdc.diodeLS.Ron=0
```
