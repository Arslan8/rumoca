# FINDING-04476: `referenceSystem.pointMass6.m` in `PointGravityWithPointMasses2`
...
| **Reached as** | `referenceSystem.pointMass6.m` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/Body.mo:16` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:997:f89b0e5c` |
...
```
predicate     referenceSystem.pointMass6.m > 0
premise       refuted
...
```
contract    Modelica.Mechanics.MultiBody.Examples.Elementary.PointGravityWithPointMasses2.SystemWithStandardBodies.PointMass.m: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Mass', 'unit': 'kg', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Mechanics.MultiBody.Examples.Elementary.PointGravityWithPointMasses2.SystemWithStandardBodies.PointMass.m'}
```
...
| `required` | referenceSystem.pointMass6.m &gt; 0 |
| `declared_min` | 0.0 |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Elementary.PointGravityWithPointMasses2 \
      --target referenceSystem.pointMass6.m --sanitizer physical
```
...
```console
$ sed -n '16p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/Body.mo"
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Elementary.PointGravityWithPointMasses2 --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "referenceSystem.pointMass6.m"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param referenceSystem.pointMass6.m=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/Body.mo:16` |' docs/v2/bugs/
```
