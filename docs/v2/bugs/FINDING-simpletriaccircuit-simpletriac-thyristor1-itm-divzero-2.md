...
| **Signature** | `divisor:divisor-reachable-zero:exp:209:8ff24e03` |
...
```
denominator            simpleTriac.thyristor1.ITM
witness                simpleTriac.thyristor1.ITM = 0
  found by             set to zero
denominator at witness 0.0
at declared values     25.0
path condition         always evaluated
constraints consulted  simpleTriac.thyristor1.ITM: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.SimpleTriacCircuit \
      --target simpleTriac.thyristor1.ITM --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "simpleTriac.thyristor1.ITM"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param simpleTriac.thyristor1.ITM=0
```
