# DECL-0003: `AD` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Spice3.mo:4663` |
| **Parameter** | `AD` |
| **Type** | `SI.Area` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '4663p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Spice3.mo"
```

It should declare `AD` with type `SI.Area` and no `min`.
...
```modelica
// Electrical/Spice3.mo:4663
parameter SI.Area AD(min=0) = ...;
```
