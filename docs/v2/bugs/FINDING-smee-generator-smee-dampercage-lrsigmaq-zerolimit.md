# FINDING-02457: `smee.damperCage.Lrsigmaq` in `SMEE_Generator`
...
| **Reached as** | `smee.damperCage.Lrsigmaq` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/Components/DamperCage.mo:5` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:441:572c5210` |
...
```
predicate     smee.damperCage.Lrsigmaq > 0
premise       refuted
...
```
contract    Modelica.Electrical.Machines.BasicMachines.Components.DamperCage.Lrsigmaq: algebraic_limit (proven, from equation) — the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.Components.DamperCage.Lrsigmaq'}
```
...
| `required` | smee.damperCage.Lrsigmaq &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMEE_Generator \
      --target smee.damperCage.Lrsigmaq --sanitizer physical
```
...
```console
$ sed -n '5p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/Components/DamperCage.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smee.damperCage.Lrsigmaq"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param smee.damperCage.Lrsigmaq=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/Components/DamperCage.mo:5` |' docs/v2/bugs/
```
