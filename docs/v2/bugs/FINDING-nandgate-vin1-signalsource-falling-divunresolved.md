...
| **Reached as** | `VIN1.signalSource.falling` |
...
| **Signature** | `divisor:divisor-zero-unresolved:exp:473:99c0b7bf` |
...
```
denominator            VIN1.signalSource.falling
witness                VIN1.signalSource.falling = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1e-09
path condition         not ((((time < VIN1.signalSource.startTime) or (VIN1.signalSource.nperiod == 0)) or ((VIN1.signalSource.nperiod > 0) and (VIN1.signalSource.count >= VIN1.signalSource.nperiod)))) and not ((time < (VIN1.signalSource.T_start + VIN1.signalSource.T_rising))) and not ((time < (VIN1.signalSource.T_start + VIN1.signalSource.T_width))) and ((time < (VIN1.signalSource.T_start + VIN1.signalSource.T_falling)))
constraints consulted  VIN1.signalSource.falling: min=0
```
...
| `denominator_at_declared_values` | 1e-09 |
| `denominator_range` | [0, inf] |
| `path_condition` | not ((((time &lt; VIN1.signalSource.startTime) or (VIN1.signalSource.nperiod == 0)) or ((VIN1.signalSource.nperiod &gt; 0) and (VIN1.signalSource.count &gt;= VIN1.signalSource.nperiod)))) and not ((time &lt; (VIN1.signalSource.T_start + VIN1.signalSource.T_rising))) and not ((time &lt; (VIN1.signalSource.T_start + VIN1.signalSource.T_width))) and ((time &lt; (VIN1.signalSource.T_start + VIN1.signalSource.T_falling))) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: (time &lt; (VIN1.signalSource.T_start + VIN1.signalSource.T_falling)); (time &lt; (VIN1.signalSource.T_start + VIN1.signalSource.T_rising)); (time &lt; (VIN1.signalSource.T_start + VIN1.signalSource.T_width)) |
| `constraints` | VIN1.signalSource.falling: min=0 |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.NandGate \
      --target VIN1.signalSource.falling --sanitizer divisor
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.NandGate --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "VIN1.signalSource.falling"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param VIN1.signalSource.falling=0
```
