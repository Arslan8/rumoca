# FINDING-04241: `pmActuator.g_ma.r_i` in `ArmatureStroke`
...
| **Reached as** | `pmActuator.g_ma.r_i` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/Force/HollowCylinderRadialFlux.mo:21` |
| **Signature** | `divisor:divisor-reachable-zero:exp:1049:06a83fcc` |
...
```
denominator            pmActuator.g_ma.r_i
witness                pmActuator.g_ma.r_i = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.0125
path condition         always evaluated
constraints consulted  pmActuator.g_ma.r_i: min=0
```
...
| `constraints` | pmActuator.g_ma.r_i: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.ArmatureStroke \
      --target pmActuator.g_ma.r_i --sanitizer divisor
```
...
```console
$ sed -n '21p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/Force/HollowCylinderRadialFlux.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pmActuator.g_ma.r_i"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/Force/HollowCylinderRadialFlux.mo:21` |' docs/v2/bugs/
```
