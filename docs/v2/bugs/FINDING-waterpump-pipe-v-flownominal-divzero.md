...
| **Signature** | `divisor:divisor-reachable-zero:exp:449:f0987bd8` |
...
```
denominator            ((pipe.V_flowNominal - pipe.V_flowLaminar) ^ 2)
witness                pipe.V_flowNominal = 0.09
  found by             at its declared minimum
denominator at witness 0.0
at declared values     0.0081
path condition         always evaluated
constraints consulted  pipe.V_flowNominal: min=0.09, assert pipe.V_flowNominal greater 0.09
```
...
| `denominator_at_declared_values` | 0.0081 |
| `denominator_range` | [0, inf] |
| `path_condition` | always evaluated |
| `verdict` | SAT |
| `proof` | the division is not inside any branch |
| `constraints` | pipe.V_flowNominal: min=0.09, assert pipe.V_flowNominal greater 0.09 |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.WaterPump \
      --target pipe.V_flowNominal --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pipe.V_flowNominal"
```
