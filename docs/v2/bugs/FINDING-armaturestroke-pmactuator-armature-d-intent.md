# FINDING-04222: `pmActuator.armature.d` in `ArmatureStroke`
...
| **Reached as** | `pmActuator.armature.d` |
| **Found by** | **PhysicalSan** — `packages/modelsan/modelsan/sanitizers/physical.py`, rule `mech.damping.non_negative` |
| **Reported as** | `physical-intent-question` |
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/Utilities/TranslatoryArmatureAndStopper.mo:11` |
| **Signature** | `physical:physical-intent-question:var:47:f67e33a0` |
...
```
predicate     pmActuator.armature.d >= 0
premise       unknown
authority     quantity_or_unit
role          —
declaration   Modelica.Magnetic.FluxTubes.Examples.Utilities.TranslatoryArmatureAndStopper.d
```
...
> `pmActuator.armature.d` carries no bound that would exclude it, which is unusual for a translational damping constant — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.

The rule it applied is `mech.damping.non_negative`:

> a damper dissipates; d < 0 injects energy. Zero is a legitimate frictionless idealisation, so the bound is >=
...
```
{'quantity': 'TranslationalDampingConstant', 'unit': 'N.s/m', 'confidence': 'QUANTITY', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Magnetic.FluxTubes.Examples.Utilities.TranslatoryArmatureAndStopper.d'}
```
...
| `required` | pmActuator.armature.d &gt;= 0 |
| `declared_min` | None |
| `variable_role` | parameter |
| `observation` | permissive-declaration |
| `premise_state` | unknown |
| `authority` | quantity_or_unit |
| `canonical_declaration` | Modelica.Magnetic.FluxTubes.Examples.Utilities.TranslatoryArmatureAndStopper.d |
| `question` | `pmActuator.armature.d` carries no bound that would exclude it, which is unusual for a translational damping constant — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.ArmatureStroke \
      --target pmActuator.armature.d --sanitizer physical
```
...
```console
$ sed -n '11p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/Utilities/TranslatoryArmatureAndStopper.mo"
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.ArmatureStroke --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pmActuator.armature.d"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param pmActuator.armature.d=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/Utilities/TranslatoryArmatureAndStopper.mo:11` |' docs/v2/bugs/
```
