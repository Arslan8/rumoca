# FINDING-02495: `smeeData.xq` in `SMEE_Generator`
...
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo:78` |
| **Signature** | `divisor:divisor-zero-when-parameters-equal:exp:1941:fb1b9a8b` |
...
```
denominator            (smeeData.xq - smeeData.xqSubtransient)
witness                smeeData.xq = 0.148387
  found by             set equal to smeeData.xqSubtransient
denominator at witness 0.0
at declared values     1.451612903
path condition         always evaluated
constraints consulted  smeeData.xq: unbounded
```
...
| `denominator` | (smeeData.xq - smeeData.xqSubtransient) |
| `witness` | smeeData.xq = 0.148387 |
| `witness_rationale` | set equal to smeeData.xqSubtransient |
| `denominator_at_witness` | 0.0 |
| `denominator_at_declared_values` | 1.451612903 |
| `denominator_range` | [-inf, inf] |
| `path_condition` | always evaluated |
| `verdict` | SAT |
| `proof` | the division is not inside any branch |
| `constraints` | smeeData.xq: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMEE_Generator \
      --target smeeData.xq --sanitizer divisor
```
...
```console
$ sed -n '78p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smeeData.xq"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo:78` |' docs/v2/bugs/
```
