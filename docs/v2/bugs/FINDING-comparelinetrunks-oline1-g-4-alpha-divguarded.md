# FINDING-00508: `oLine1.G[4].alpha` in `CompareLineTrunks`
...
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:4956:67a89575` |
...
```
denominator            (1 + (oLine1.G[4].alpha * (oLine1.G[4].T_heatPort - oLine1.G[4].T_ref)))
witness                none needed
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.CompareLineTrunks \
      --target oLine1.G[4].alpha --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "oLine1.G[4].alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param oLine1.G[4].alpha=0
```
