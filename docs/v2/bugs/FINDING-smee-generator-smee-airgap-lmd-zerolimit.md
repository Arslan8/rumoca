# FINDING-02446: `smee.airGap.Lmd` in `SMEE_Generator`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:409:a7be7707` |
...
```
contract    Modelica.Electrical.Machines.BasicMachines.Components.AirGapR.L: feature_disabled (proven, from equation) — inherited from smee.airGap.L, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMEE_Generator \
      --target smee.airGap.Lmd --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smee.airGap.Lmd"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param smee.airGap.Lmd=0
```
