...
| **Signature** | `physical:physical-bound-permits-zero:var:111:37cf0885` |
...
```
predicate     volumeFlowSensor.medium.rho > 0
premise       established
...
| `required` | volumeFlowSensor.medium.rho &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.WaterPump \
      --target volumeFlowSensor.medium.rho --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "volumeFlowSensor.medium.rho"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param volumeFlowSensor.medium.rho=0
```
