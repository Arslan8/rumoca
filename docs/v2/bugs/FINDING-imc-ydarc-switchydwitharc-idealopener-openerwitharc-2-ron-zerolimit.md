# FINDING-02167: `switchYDwithArc.idealOpener.openerWithArc[2].Ron` in `IMC_YDarc`
...
| **Reached as** | `switchYDwithArc.idealOpener.openerWithArc[2].Ron` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:826:5209f13d` |
...
```
predicate     switchYDwithArc.idealOpener.openerWithArc[2].Ron > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Ideal.OpenerWithArc.Ron: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Ideal.OpenerWithArc.Ron'}
```
...
| `required` | switchYDwithArc.idealOpener.openerWithArc[2].Ron &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_YDarc \
      --target switchYDwithArc.idealOpener.openerWithArc[2].Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switchYDwithArc.idealOpener.openerWithArc[2].Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switchYDwithArc.idealOpener.openerWithArc[2].Ron=0
```
