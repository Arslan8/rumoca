...
| **Signature** | `divisor:divisor-zero-when-parameters-equal:exp:902:e7911165` |
...
```
denominator            (addA.opAmp.vps - addA.opAmp.vns)
witness                addA.opAmp.Vps = -15
  found by             set equal to addA.opAmp.Vns
denominator at witness 0.0
at declared values     30.0
path condition         always evaluated
constraints consulted  addA.opAmp.Vps: unbounded
```
...
| `denominator` | (addA.opAmp.vps - addA.opAmp.vns) |
| `witness` | addA.opAmp.Vps = -15 |
| `witness_rationale` | set equal to addA.opAmp.Vns |
...
| `constraints` | addA.opAmp.Vps: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.ControlCircuit \
      --target addA.opAmp.Vps --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "addA.opAmp.Vps"
```
