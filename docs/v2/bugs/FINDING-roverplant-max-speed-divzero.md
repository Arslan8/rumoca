...
| **Severity** | high |
| **Declaration** | `target/cmm/CMM-a642c381/RigidBody/Examples/RoverPlant.mo:52` |
| **Signature** | `divisor:divisor-reachable-zero:exp:507:e4de6182` |
...
```
denominator            max_speed
witness                max_speed = 0
  found by             set to zero
denominator at witness 0.0
at declared values     8.0
path condition         always evaluated
constraints consulted  max_speed: unbounded
```
...
$ python3 tools/sweep/check_one.py RigidBody.Examples.RoverPlant \
      --target max_speed --sanitizer divisor
```
...
```console
$ sed -n '52p' "target/cmm/CMM-a642c381/RigidBody/Examples/RoverPlant.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "max_speed"
```
...
```console
$ grep -rl '| `target/cmm/CMM-a642c381/RigidBody/Examples/RoverPlant.mo:52` |' docs/v2/bugs/
```
