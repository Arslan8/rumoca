# FINDING-02591: `wNominal` in `SMPM_Braking`
...
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/SynchronousMachines/SMPM_Braking.mo:76` |
| **Signature** | `divisor:divisor-reachable-zero:exp:1638:3f7de2ad` |
...
```
denominator            wNominal
witness                wNominal = 0
  found by             set to zero
denominator at witness 0.0
at declared values     157.07963267948966
path condition         always evaluated
constraints consulted  wNominal: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_Braking \
      --target wNominal --sanitizer divisor
```
...
```console
$ sed -n '76p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/SynchronousMachines/SMPM_Braking.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "wNominal"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/SynchronousMachines/SMPM_Braking.mo:76` |' docs/v2/bugs/
```
