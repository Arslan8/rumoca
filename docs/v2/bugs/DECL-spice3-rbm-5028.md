# DECL-0819: `RBM` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Spice3.mo:5028` |
| **Parameter** | `RBM` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '5028p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Spice3.mo"
```

It should declare `RBM` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/Spice3.mo:5028
parameter SI.Resistance RBM(min=0) = ...;
```
