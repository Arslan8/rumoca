# DECL-0364: `nJ` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/Parts/Rotor1D.mo:147` |
| **Parameter** | `nJ` |
| **Type** | `SI.Inertia` |
| **Declared modifiers** | `—` |
...
It should declare `nJ` with type `SI.Inertia` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/Rotor1D.mo" | head
```
...
```modelica
// Mechanics/MultiBody/Parts/Rotor1D.mo:147
parameter SI.Inertia nJ(min=0) = ...;
```
