# FINDING-04179: `upperYoke.a` in `QuadraticCoreAirgap`
...
| **Signature** | `divisor:divisor-introduced-by-translation:exp:450:08ab8ef6` |
...
```
denominator            upperYoke.A
witness                upperYoke.a = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.0001
path condition         always evaluated
constraints consulted  upperYoke.a: unbounded
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.BasicExamples.QuadraticCoreAirgap \
      --target upperYoke.a --sanitizer divisor
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.BasicExamples.QuadraticCoreAirgap --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "upperYoke.a"
```
