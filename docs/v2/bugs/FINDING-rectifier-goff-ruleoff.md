# FINDING-03324: `Goff` in `Rectifier`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Examples/Rectifier.mo:13` |
| **Signature** | `physical:physical-rule-does-not-apply:var:8:357d3fe4` |
...
```
contract    Modelica.Electrical.Polyphase.Ideal.IdealDiode.Goff: allowed (declared, from class_catalog) — inherited from idealDiode1.Goff, which it binds: an ideal switch family member; zero off-conductance is the documented ideal
behavior    allowed
confidence  declared
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Polyphase.Examples.Rectifier.Goff'}
```
...
```console
$ sed -n '13p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Examples/Rectifier.mo"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Examples/Rectifier.mo:13` |' docs/v2/bugs/
```
