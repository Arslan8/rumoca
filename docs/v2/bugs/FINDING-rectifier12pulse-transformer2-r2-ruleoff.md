# FINDING-03174: `transformer2.R2` in `Rectifier12pulse`
...
| **Signature** | `physical:physical-rule-does-not-apply:var:954:dcbd59b0` |
...
```
contract    Modelica.Electrical.Analog.Basic.Resistor.R: allowed (declared, from class_catalog) — inherited from transformer2.r2.R, which it binds: the component explicitly supports signed resistance, so neither a positivity rule nor a nonzero rule follows from the quantity alone
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.Transformers.Dd.Dd00.R2'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.Transformers.Rectifier12pulse \
      --target transformer2.R2 --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "transformer2.R2"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param transformer2.R2=0
```
