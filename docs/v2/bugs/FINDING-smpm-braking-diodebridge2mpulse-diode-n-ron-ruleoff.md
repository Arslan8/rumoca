# FINDING-02568: `diodeBridge2mPulse.diode_n.Ron` in `SMPM_Braking`
...
| **Reached as** | `diodeBridge2mPulse.diode_n.Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:720:a0cefaf7` |
...
```
predicate     diodeBridge2mPulse.diode_n.Ron > 0
premise       refuted
...
| `required` | diodeBridge2mPulse.diode_n.Ron &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_Braking \
      --target diodeBridge2mPulse.diode_n.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "diodeBridge2mPulse.diode_n.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param diodeBridge2mPulse.diode_n.Ron=0
```
