# FINDING-04680: `springDamper.c` in `SpringDamper`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Rotational/Examples/Utilities/SpringDamperNoRelativeStates.mo:4` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:37:a51bc662` |
...
```
contract    Modelica.Mechanics.Rotational.Examples.Utilities.SpringDamperNoRelativeStates.c: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'RotationalSpringConstant', 'unit': 'N.m/rad', 'confidence': 'QUANTITY', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Mechanics.Rotational.Examples.Utilities.SpringDamperNoRelativeStates.c'}
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Rotational.Examples.Utilities.SpringDamper \
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Rotational/Examples/Utilities/SpringDamperNoRelativeStates.mo"
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Rotational.Examples.Utilities.SpringDamper --keep-artifact /tmp/m.rbc \
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Rotational/Examples/Utilities/SpringDamperNoRelativeStates.mo:4` |' docs/v2/bugs/
```
