...
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Basic/EddyCurrent.mo:23` |
| **Signature** | `physical:physical-domain-unenforced:var:72:854efbeb` |
...
```
predicate     eddyCurrent.R > 0
premise       established
authority     source_arithmetic
role          —
declaration   Modelica.Magnetic.FluxTubes.Basic.EddyCurrent.R
```
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Magnetic.FluxTubes.Basic.EddyCurrent.R'}
```
...
| `required` | eddyCurrent.R &gt; 0 |
| `declared_min` | None |
| `variable_role` | parameter |
| `premise_state` | established |
| `authority` | source_arithmetic |
| `canonical_declaration` | Modelica.Magnetic.FluxTubes.Basic.EddyCurrent.R |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Magnetic.FluxTubes.BasicComponents \
      --target eddyCurrent.R --sanitizer physical
```
...
```console
$ sed -n '23p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Basic/EddyCurrent.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "eddyCurrent.R"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param eddyCurrent.R=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Basic/EddyCurrent.mo:23` |' docs/v2/bugs/
```
