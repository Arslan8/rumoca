# FINDING-00132: `c5` in `CauerLowPassSC`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/CauerLowPassSC.mo:13` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:6:2fdf4404` |
...
```
contract    Modelica.Electrical.Analog.Basic.Capacitor.C: algebraic_limit (proven, from equation) — inherited from C9.C, which it binds: the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
...
```
{'quantity': 'Capacitance', 'unit': 'F', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Examples.CauerLowPassSC.c5'}
```
...
```console
$ sed -n '13p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/CauerLowPassSC.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "c5"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param c5=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/CauerLowPassSC.mo:13` |' docs/v2/bugs/
```
