# DECL-0869: `Re` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/QuasiStatic/FundamentalWave/BasicMachines/SynchronousMachines/SM_ElectricalExcited.mo:92` |
| **Parameter** | `Re` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `(start=2.5)` |
...
```console
$ sed -n '92p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/QuasiStatic/FundamentalWave/BasicMachines/SynchronousMachines/SM_ElectricalExcited.mo"
```

It should declare `Re` with type `SI.Resistance` and no `min`.
...
```modelica
// Magnetic/QuasiStatic/FundamentalWave/BasicMachines/SynchronousMachines/SM_ElectricalExcited.mo:92
parameter SI.Resistance Re(min=0) = ...;
```
