# DECL-0401: `DEFW` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Spice3.mo:5492` |
| **Parameter** | `DEFW` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '5492p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Spice3.mo"
```

It should declare `DEFW` with type `SI.Length` and no `min`.
...
```modelica
// Electrical/Spice3.mo:5492
parameter SI.Length DEFW(min=0) = ...;
```
