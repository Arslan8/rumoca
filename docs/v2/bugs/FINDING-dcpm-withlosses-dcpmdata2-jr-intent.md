...
| **Signature** | `physical:physical-intent-question:var:629:7f5fbfe0` |
...
> `dcpmData2.Jr` carries no bound that would exclude it, which is unusual for a moment of inertia — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
| `question` | `dcpmData2.Jr` carries no bound that would exclude it, which is unusual for a moment of inertia — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_withLosses \
      --target dcpmData2.Jr --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcpmData2.Jr"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param dcpmData2.Jr=0
```
