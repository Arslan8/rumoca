...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:596:dbf2f5ff` |
...
```
denominator            VDD.signalSource.duration
witness                VDD.signalSource.duration = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1e-09
path condition         not ((time < VDD.signalSource.startTime)) and ((time < (VDD.signalSource.startTime + VDD.signalSource.duration)))
constraints consulted  VDD.signalSource.duration: min=0
```
...
| `path_condition` | not ((time &lt; VDD.signalSource.startTime)) and ((time &lt; (VDD.signalSource.startTime + VDD.signalSource.duration))) |
| `verdict` | UNSAT |
| `proof` | the path conditions cannot hold together: time must be both &gt; 0 and &lt; 0 |
| `constraints` | VDD.signalSource.duration: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.NandGate \
      --target VDD.signalSource.duration --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "VDD.signalSource.duration"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param VDD.signalSource.duration=0
```
