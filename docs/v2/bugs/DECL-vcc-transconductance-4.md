# DECL-0046: `transConductance` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Basic/VCC.mo:4` |
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/VCC.mo"
```

It should declare `transConductance` with type `SI.Conductance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/VCC.mo" | head
```
...
```modelica
// Electrical/Analog/Basic/VCC.mo:4
parameter SI.Conductance transConductance(min=0) = ...;
```
