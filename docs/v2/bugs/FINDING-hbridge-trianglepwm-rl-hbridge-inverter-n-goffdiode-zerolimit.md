# FINDING-04155: `hbridge.inverter_n.GoffDiode` in `HBridge_TrianglePWM_RL`
...
| **Reached as** | `hbridge.inverter_n.GoffDiode` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCAC/SinglePhase2Level.mo:12` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:197:6b3019eb` |
...
```
predicate     hbridge.inverter_n.GoffDiode > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealDiode.Goff: feature_disabled (proven, from equation) — inherited from hbridge.inverter_n.diode_p.Goff, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.PowerConverters.DCAC.SinglePhase2Level.GoffDiode'}
```
...
| `required` | hbridge.inverter_n.GoffDiode &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCDC.HBridge.HBridge_TrianglePWM_RL \
      --target hbridge.inverter_n.GoffDiode --sanitizer physical
```
...
```console
$ sed -n '12p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCAC/SinglePhase2Level.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "hbridge.inverter_n.GoffDiode"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param hbridge.inverter_n.GoffDiode=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCAC/SinglePhase2Level.mo:12` |' docs/v2/bugs/
```
