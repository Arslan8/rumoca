...
| **Signature** | `divisor:divisor-zero-unresolved:exp:352:f163a12e` |
...
```
denominator            (zDiode.Nbv * zDiode.Vt)
witness                zDiode.Nbv = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.0296
path condition         not ((zDiode.v > (zDiode.Maxexp * zDiode.Vt))) and (((zDiode.v + zDiode.Bv) < -(zDiode.Maxexp * (zDiode.Nbv * zDiode.Vt))))
constraints consulted  zDiode.Nbv: unbounded
```
...
| `path_condition` | not ((zDiode.v &gt; (zDiode.Maxexp * zDiode.Vt))) and (((zDiode.v + zDiode.Bv) &lt; -(zDiode.Maxexp * (zDiode.Nbv * zDiode.Vt)))) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: ((zDiode.v + zDiode.Bv) &lt; -(zDiode.Maxexp * (zDiode.Nbv * zDiode.Vt))); (zDiode.v &gt; (zDiode.Maxexp * zDiode.Vt)) |
| `constraints` | zDiode.Nbv: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OvervoltageProtection \
      --target zDiode.Nbv --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "zDiode.Nbv"
```
