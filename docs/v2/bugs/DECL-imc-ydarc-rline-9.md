# DECL-0699: `RLine` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Machines/Examples/InductionMachines/IMC_YDarc.mo:9` |
| **Parameter** | `RLine` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
It should declare `RLine` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/Machines/Examples/InductionMachines/IMC_YDarc.mo:9
parameter SI.Resistance RLine(min=0) = ...;
```
