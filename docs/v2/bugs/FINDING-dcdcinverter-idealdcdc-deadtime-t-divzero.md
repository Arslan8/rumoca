# FINDING-01273: `idealDcDc.deadTime.T` in `DcdcInverter`
...
| **Reached as** | `idealDcDc.deadTime.T` |
...
| **Signature** | `divisor:divisor-reachable-zero:exp:383:b9482f98` |
...
```
denominator            idealDcDc.deadTime.T
witness                idealDcDc.deadTime.T = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  idealDcDc.deadTime.T: unbounded
```
...
| `constraints` | idealDcDc.deadTime.T: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.ControlledDCDrives.Utilities.DcdcInverter \
      --target idealDcDc.deadTime.T --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "idealDcDc.deadTime.T"
```
