...
| **Reached as** | `idealTransformer.idealTransformer[1].n` |
...
| **Signature** | `divisor:divisor-reachable-zero:exp:895:5ecdb918` |
...
```
denominator            idealTransformer.idealTransformer[1].n
witness                idealTransformer.idealTransformer[1].n = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  idealTransformer.idealTransformer[1].n: unbounded
```
...
| `constraints` | idealTransformer.idealTransformer[1].n: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Polyphase.Examples.TransformerYY \
      --target idealTransformer.idealTransformer[1].n --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "idealTransformer.idealTransformer[1].n"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param idealTransformer.idealTransformer[1].n=0
```
