...
| **Signature** | `divisor:divisor-reachable-zero:exp:2705:ac55a278` |
...
```
denominator            dcdcInverter1.vDCFilter.T
witness                dcdcInverter1.vDCFilter.T = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.002
path condition         always evaluated
constraints consulted  dcdcInverter1.vDCFilter.T: unbounded
```
...
| `constraints` | dcdcInverter1.vDCFilter.T: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Drive \
      --target dcdcInverter1.vDCFilter.T --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcdcInverter1.vDCFilter.T"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param dcdcInverter1.vDCFilter.T=0
```
