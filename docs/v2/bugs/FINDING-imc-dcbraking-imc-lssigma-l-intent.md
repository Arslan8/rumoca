# FINDING-01722: `imc.lssigma.L` in `IMC_DCBraking`
...
| **Signature** | `physical:physical-intent-question:var:178:af5c6d3e` |
...
> `imc.lssigma.L` carries no bound that would exclude it, which is unusual for an inductance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
| `question` | `imc.lssigma.L` carries no bound that would exclude it, which is unusual for an inductance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_DCBraking \
      --target imc.lssigma.L --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "imc.lssigma.L"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param imc.lssigma.L=0
```
