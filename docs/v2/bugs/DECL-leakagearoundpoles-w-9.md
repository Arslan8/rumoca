# DECL-0477: `w` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/FluxTubes/Shapes/Force/LeakageAroundPoles.mo:9` |
| **Parameter** | `w` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '9p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/Force/LeakageAroundPoles.mo"
```

It should declare `w` with type `SI.Length` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/Force/LeakageAroundPoles.mo" | head
```
...
```modelica
// Magnetic/FluxTubes/Shapes/Force/LeakageAroundPoles.mo:9
