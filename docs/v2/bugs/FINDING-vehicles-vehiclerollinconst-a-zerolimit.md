...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:470:515dac13` |
...
| `observed` | 0.0 |
| `required` | vehicleRollInConst.A &gt; 0 |
| `where` | declaration |
| `contract` | Modelica.Mechanics.Translational.Components.Vehicle.A: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Translational.Vehicles \
      --target vehicleRollInConst.A --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "vehicleRollInConst.A"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param vehicleRollInConst.A=0
```
