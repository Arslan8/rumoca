# DECL-0892: `G_armature_core` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Machines/Examples/DCMachines/DCPM_Cooling.mo:24` |
| **Parameter** | `G_armature_core` |
| **Type** | `SI.ThermalConductance` |
| **Declared modifiers** | `—` |
...
`SI.ThermalConductance` declares no `min` in `Units.mo`, and this declaration adds none of its own. It therefore accepts a negative value, and a negative thermalconductance is not a physical value.
...
It should declare `G_armature_core` with type `SI.ThermalConductance` and no `min`.

**2. Confirm the type supplies no bound either** — this is the whole claim, and
a bound inherited from the type would refute it:

```console
$ grep -n "type ThermalConductance " "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Units.mo"
```
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/DCMachines/DCPM_Cooling.mo" | head
```
...
- `SI.ThermalConductance` turns out to carry a `min` after all, or this declaration carries one that the catalog missed.
...
```modelica
// Electrical/Machines/Examples/DCMachines/DCPM_Cooling.mo:24
parameter SI.ThermalConductance G_armature_core(min=0) = ...;
```
