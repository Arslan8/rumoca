...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:472:641aa0de` |
...
```
predicate     vehicleRollInConst.rho > 0
premise       refuted
...
| `required` | vehicleRollInConst.rho &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Translational.Vehicles \
      --target vehicleRollInConst.rho --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "vehicleRollInConst.rho"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param vehicleRollInConst.rho=0
```
