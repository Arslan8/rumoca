# DECL-0804: `R_ref` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/QuasiStatic/Polyphase/Basic/Resistor.mo:4` |
| **Parameter** | `R_ref` |
| **Type** | `SI.Resistance` |
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/QuasiStatic/Polyphase/Basic/Resistor.mo"
```

It should declare `R_ref` with type `SI.Resistance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/QuasiStatic/Polyphase/Basic/Resistor.mo" | head
```
...
```modelica
// Electrical/QuasiStatic/Polyphase/Basic/Resistor.mo:4
parameter SI.Resistance R_ref(min=0) = ...;
```
