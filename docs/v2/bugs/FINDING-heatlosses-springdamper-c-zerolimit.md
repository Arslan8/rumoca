# FINDING-04422: `springDamper.c` in `HeatLosses`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Forces/SpringDamperParallel.mo:5` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:559:293c59c7` |
...
```
contract    Modelica.Mechanics.MultiBody.Forces.SpringDamperParallel.c: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'TranslationalSpringConstant', 'unit': 'N/m', 'confidence': 'QUANTITY', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Mechanics.MultiBody.Forces.SpringDamperParallel.c'}
```
...
```console
$ sed -n '5p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Forces/SpringDamperParallel.mo"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Forces/SpringDamperParallel.mo:5` |' docs/v2/bugs/
```
