# FINDING-05179: `lowpassButterworth.w` in `Continuous_InitialState`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `ModelicaTest.Blocks.Continuous_InitialState` |
| **Reached as** | `lowpassButterworth.w` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-introduced-by-translation` |
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Blocks/Continuous.mo:1424` |
| **Signature** | `divisor:divisor-introduced-by-translation:exp:1100:432e9a45` |
...
```
contract    Modelica.Blocks.Continuous.LowpassButterworth.w: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
behavior    
confidence  proven
source      
```
...
```
denominator            lowpassButterworth.realpol
witness                lowpassButterworth.w = 0
  found by             set to zero
denominator at witness 0.0
at declared values     6.283185307179586
path condition         always evaluated
constraints consulted  lowpassButterworth.w: unbounded
```
...
```console
$ python3 tools/sweep/check_one.py ModelicaTest.Blocks.Continuous_InitialState \
      --target lowpassButterworth.w --sanitizer divisor
```
...
```console
$ sed -n '1424p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Blocks/Continuous.mo"
```
...
```console
$ python3 tools/sweep/check_one.py ModelicaTest.Blocks.Continuous_InitialState --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "lowpassButterworth.w"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Blocks/Continuous.mo:1424` |' docs/v2/bugs/
```
