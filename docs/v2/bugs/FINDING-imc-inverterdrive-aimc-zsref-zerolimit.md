# FINDING-01906: `aimc.ZsRef` in `IMC_InverterDrive`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:1233:f0656a5c` |
...
```
contract    Modelica.Electrical.Machines.BasicMachines.InductionMachines.IM_SquirrelCage.ZsRef: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.InductionMachines.IM_SquirrelCage.ZsRef'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive \
      --target aimc.ZsRef --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "aimc.ZsRef"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param aimc.ZsRef=0
```
