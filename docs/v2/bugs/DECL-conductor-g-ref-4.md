# DECL-0076: `G_ref` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/QuasiStatic/Polyphase/Basic/Conductor.mo:4` |
| **Parameter** | `G_ref` |
| **Type** | `SI.Conductance` |
| **Declared modifiers** | `(start=fill(1, m)` |
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/QuasiStatic/Polyphase/Basic/Conductor.mo"
```

It should declare `G_ref` with type `SI.Conductance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/QuasiStatic/Polyphase/Basic/Conductor.mo" | head
```
...
```modelica
// Electrical/QuasiStatic/Polyphase/Basic/Conductor.mo:4
parameter SI.Conductance G_ref(min=0) = ...;
```
