# FINDING-01894: `inverter.diode_n.Goff` in `IMC_InverterDrive`
...
| **Reached as** | `inverter.diode_n.Goff` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:787:aadab4bb` |
...
```
predicate     inverter.diode_n.Goff > 0
premise       refuted
...
| `required` | inverter.diode_n.Goff &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive \
      --target inverter.diode_n.Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inverter.diode_n.Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inverter.diode_n.Goff=0
```
