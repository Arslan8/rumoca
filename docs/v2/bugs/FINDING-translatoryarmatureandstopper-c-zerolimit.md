# FINDING-04358: `c` in `TranslatoryArmatureAndStopper`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:2:feec379d` |
...
```
contract    Modelica.Mechanics.Translational.Components.ElastoGap.c: feature_disabled (proven, from equation) — inherited from stopper_xMax.c, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```console
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.Utilities.TranslatoryArmatureAndStopper \
...
```console
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.Utilities.TranslatoryArmatureAndStopper --keep-artifact /tmp/m.rbc \
