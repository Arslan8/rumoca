...
| **Signature** | `divisor:divisor-reachable-zero:exp:2801:25c723d5` |
...
```
denominator            dcdcInverter1.idealDcDc.deadTime.T
witness                dcdcInverter1.idealDcDc.deadTime.T = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.0005
path condition         always evaluated
constraints consulted  dcdcInverter1.idealDcDc.deadTime.T: unbounded
```
...
| `constraints` | dcdcInverter1.idealDcDc.deadTime.T: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Drive \
      --target dcdcInverter1.idealDcDc.deadTime.T --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcdcInverter1.idealDcDc.deadTime.T"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param dcdcInverter1.idealDcDc.deadTime.T=0
```
