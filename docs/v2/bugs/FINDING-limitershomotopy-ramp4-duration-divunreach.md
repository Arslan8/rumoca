...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:691:caa84558` |
...
```
denominator            ramp4.duration
witness                ramp4.duration = 0
  found by             set to zero
denominator at witness 0.0
at declared values     100.0
path condition         not ((time < ramp4.startTime)) and ((time < (ramp4.startTime + ramp4.duration)))
constraints consulted  ramp4.duration: min=0
```
...
| `path_condition` | not ((time &lt; ramp4.startTime)) and ((time &lt; (ramp4.startTime + ramp4.duration))) |
| `verdict` | UNSAT |
| `proof` | the path conditions cannot hold together: time must be both &gt; 1 and &lt; 1 |
| `constraints` | ramp4.duration: min=0 |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Blocks.LimitersHomotopy \
      --target ramp4.duration --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "ramp4.duration"
```
