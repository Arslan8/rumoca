...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:109:11b0298e` |
...
```
denominator            rampPermeance.duration
witness                rampPermeance.duration = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.3
path condition         not ((time < rampPermeance.startTime)) and ((time < (rampPermeance.startTime + rampPermeance.duration)))
constraints consulted  rampPermeance.duration: min=0
```
...
| `path_condition` | not ((time &lt; rampPermeance.startTime)) and ((time &lt; (rampPermeance.startTime + rampPermeance.duration))) |
| `verdict` | UNSAT |
| `proof` | the path conditions cannot hold together: time must be both &gt; 0.6 and &lt; 0.6 |
| `constraints` | rampPermeance.duration: min=0 |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Magnetic.FluxTubes.VariableComponents \
      --target rampPermeance.duration --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "rampPermeance.duration"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param rampPermeance.duration=0
```
