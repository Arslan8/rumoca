# FINDING-03852: `rectifier.RonDiode` in `DiodeCenterTap2Pulse`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/DiodeCenterTap2Pulse.mo:5` |
| **Signature** | `physical:physical-rule-does-not-apply:var:57:cb956535` |
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealDiode.Ron: allowed (declared, from class_catalog) — inherited from rectifier.diode_p.Ron, which it binds: an ideal switch family member; zero on-resistance is the documented ideal
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.PowerConverters.ACDC.DiodeCenterTap2Pulse.RonDiode'}
```
...
```console
$ sed -n '5p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/DiodeCenterTap2Pulse.mo"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/DiodeCenterTap2Pulse.mo:5` |' docs/v2/bugs/
```
