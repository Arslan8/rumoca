...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:0:e25520cb` |
...
```
contract    Modelica.Mechanics.Translational.Examples.Utilities.SpringDamperNoRelativeStates.c: feature_disabled (proven, from equation) — inherited from springDamper.c, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
