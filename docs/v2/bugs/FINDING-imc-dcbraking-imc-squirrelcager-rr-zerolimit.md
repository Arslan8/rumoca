# FINDING-01735: `imc.squirrelCageR.Rr` in `IMC_DCBraking`
...
| **Reached as** | `imc.squirrelCageR.Rr` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/Components/SquirrelCage.mo:5` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:441:abc24d87` |
...
```
predicate     imc.squirrelCageR.Rr >= 0
premise       refuted
...
```
contract    Modelica.Electrical.Machines.BasicMachines.Components.SquirrelCage.Rr: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'semantic_role': 'component.machine.winding_resistance', 'binding_source': 'component_type', 'confidence': 'QUANTITY_AND_UNIT', 'declaring_class': 'Modelica.Electrical.Machines.BasicMachines.Components.SquirrelCage', 'member': 'Rr', 'premise_state': 'established', 'authority': 'component_contract', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.Components.SquirrelCage.Rr'}
```
...
| `required` | imc.squirrelCageR.Rr &gt;= 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_DCBraking \
      --target imc.squirrelCageR.Rr --sanitizer physical
```
...
```console
$ sed -n '5p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/Components/SquirrelCage.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "imc.squirrelCageR.Rr"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param imc.squirrelCageR.Rr=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/Components/SquirrelCage.mo:5` |' docs/v2/bugs/
```
