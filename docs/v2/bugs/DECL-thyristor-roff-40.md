# DECL-0671: `Roff` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Semiconductors/Thyristor.mo:40` |
| **Parameter** | `Roff` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
It should declare `Roff` with type `SI.Resistance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/Thyristor.mo" | head
```
...
```modelica
// Electrical/Analog/Semiconductors/Thyristor.mo:40
parameter SI.Resistance Roff(min=0) = ...;
```
