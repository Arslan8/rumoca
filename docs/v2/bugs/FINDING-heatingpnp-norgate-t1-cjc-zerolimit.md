# FINDING-00376: `T1.Cjc` in `HeatingPNP_NORGate`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/PNP.mo:11` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:183:722bae02` |
...
```
contract    Modelica.Electrical.Analog.Semiconductors.PNP.Cjc: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Capacitance', 'unit': 'F', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Semiconductors.PNP.Cjc'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.HeatingPNP_NORGate \
      --target T1.Cjc --sanitizer physical
```
...
```console
$ sed -n '11p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/PNP.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "T1.Cjc"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param T1.Cjc=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/PNP.mo:11` |' docs/v2/bugs/
```
