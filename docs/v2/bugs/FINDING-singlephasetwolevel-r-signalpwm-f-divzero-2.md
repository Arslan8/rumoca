# FINDING-03986: `signalPWM.f` in `SinglePhaseTwoLevel_R`
...
| **Reached as** | `signalPWM.f` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCDC/Control/SignalPWM.mo:41` |
| **Signature** | `divisor:divisor-reachable-zero:exp:246:dfafed9a` |
...
| `constraints` | signalPWM.f: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCAC.SinglePhaseTwoLevel.SinglePhaseTwoLevel_R \
      --target signalPWM.f --sanitizer divisor
```
...
```console
$ sed -n '41p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCDC/Control/SignalPWM.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "signalPWM.f"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCDC/Control/SignalPWM.mo:41` |' docs/v2/bugs/
```
