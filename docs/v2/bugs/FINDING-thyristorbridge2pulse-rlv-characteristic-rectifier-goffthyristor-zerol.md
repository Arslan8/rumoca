# FINDING-03712: `rectifier.GoffThyristor` in `ThyristorBridge2Pulse_RLV_Characteristic`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/ThyristorBridge2Pulse.mo:7` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:61:506bb344` |
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealThyristor.Goff: feature_disabled (proven, from equation) — inherited from rectifier.thyristor_p1.Goff, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.PowerConverters.ACDC.ThyristorBridge2Pulse.GoffThyristor'}
```
...
```console
$ sed -n '7p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/ThyristorBridge2Pulse.mo"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/ThyristorBridge2Pulse.mo:7` |' docs/v2/bugs/
```
