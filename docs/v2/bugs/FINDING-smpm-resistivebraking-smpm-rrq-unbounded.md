# FINDING-02780: `smpm.Rrq` in `SMPM_ResistiveBraking`
...
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/SynchronousMachines/SM_PermanentMagnet.mo:75` |
| **Signature** | `physical:physical-domain-unenforced:var:456:cb6decc2` |
...
```
predicate     smpm.Rrq >= 0
premise       established
authority     component_contract
role          component.machine.winding_resistance
declaration   Modelica.Electrical.Machines.BasicMachines.SynchronousMachines.SM_PermanentMagnet.Rrq
```
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'semantic_role': 'component.machine.winding_resistance', 'binding_source': 'component_type', 'confidence': 'QUANTITY_AND_UNIT', 'declaring_class': 'Modelica.Electrical.Machines.BasicMachines.SynchronousMachines.SM_PermanentMagnet', 'member': 'Rrq', 'premise_state': 'established', 'authority': 'component_contract', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.SynchronousMachines.SM_PermanentMagnet.Rrq'}
```
...
| `canonical_declaration` | Modelica.Electrical.Machines.BasicMachines.SynchronousMachines.SM_PermanentMagnet.Rrq |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_ResistiveBraking \
      --target smpm.Rrq --sanitizer physical
```
...
```console
$ sed -n '75p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/SynchronousMachines/SM_PermanentMagnet.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smpm.Rrq"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param smpm.Rrq=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/SynchronousMachines/SM_PermanentMagnet.mo:75` |' docs/v2/bugs/
```
