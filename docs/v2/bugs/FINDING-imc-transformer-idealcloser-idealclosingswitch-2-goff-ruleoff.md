# FINDING-02054: `idealCloser.idealClosingSwitch[2].Goff` in `IMC_Transformer`
...
| **Reached as** | `idealCloser.idealClosingSwitch[2].Goff` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:609:f273896b` |
...
```
predicate     idealCloser.idealClosingSwitch[2].Goff > 0
premise       refuted
...
| `required` | idealCloser.idealClosingSwitch[2].Goff &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_Transformer \
      --target idealCloser.idealClosingSwitch[2].Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "idealCloser.idealClosingSwitch[2].Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param idealCloser.idealClosingSwitch[2].Goff=0
```
