# FINDING-00549: `tLine4.F` in `CompareLineTrunks`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Lines/TLine.mo:14` |
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:3835:a2b66b1f` |
...
| `denominator_range` |  |
| `path_condition` | ((tLine4.F &gt; 0)) |
| `verdict` | UNSAT |
| `proof` | the denominator is zero as declared, and ((tLine4.F &gt; 0)) |
| `constraints` |  |
| `guard_that_excludes_it` | ((tLine4.F &gt; 0)) |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.CompareLineTrunks \
      --target tLine4.F --sanitizer divisor
```
...
```console
$ sed -n '14p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Lines/TLine.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "tLine4.F"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param tLine4.F=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Lines/TLine.mo:14` |' docs/v2/bugs/
```
