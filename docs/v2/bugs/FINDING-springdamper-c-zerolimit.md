# FINDING-04678: `c` in `SpringDamper`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Rotational/Examples/Utilities/SpringDamper.mo:4` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:0:3c00790b` |
...
```
contract    Modelica.Mechanics.Rotational.Examples.Utilities.SpringDamperNoRelativeStates.c: feature_disabled (proven, from equation) — inherited from springDamper.c, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'RotationalSpringConstant', 'unit': 'N.m/rad', 'confidence': 'QUANTITY', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Mechanics.Rotational.Examples.Utilities.SpringDamper.c'}
```
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Rotational/Examples/Utilities/SpringDamper.mo"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Rotational/Examples/Utilities/SpringDamper.mo:4` |' docs/v2/bugs/
```
