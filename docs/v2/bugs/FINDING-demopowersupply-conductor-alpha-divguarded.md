# FINDING-00267: `conductor.alpha` in `DemoPowerSupply`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Electrical.Analog.Examples.DemoPowerSupply` |
| **Reached as** | `conductor.alpha` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-guarded-by-assertion` |
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/VariableConductor.mo:19` |
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:312:9f3c46cd` |
...
```
denominator            (1 + (conductor.alpha * (conductor.T_heatPort - conductor.T_ref)))
witness                none needed
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.DemoPowerSupply \
      --target conductor.alpha --sanitizer divisor
```
...
```console
$ sed -n '19p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/VariableConductor.mo"
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.DemoPowerSupply --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "conductor.alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param conductor.alpha=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/VariableConductor.mo:19` |' docs/v2/bugs/
```
