...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:704:ffd58804` |
...
```
predicate     vehicleInclination.m > 0
premise       refuted
...
| `required` | vehicleInclination.m &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Translational.Vehicles \
      --target vehicleInclination.m --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "vehicleInclination.m"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param vehicleInclination.m=0
```
