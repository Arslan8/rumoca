...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:678:3801e539` |
...
```
predicate     vehicleRollInVar.mass.m > 0
premise       refuted
...
| `required` | vehicleRollInVar.mass.m &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Translational.Vehicles \
      --target vehicleRollInVar.mass.m --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "vehicleRollInVar.mass.m"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param vehicleRollInVar.mass.m=0
```
