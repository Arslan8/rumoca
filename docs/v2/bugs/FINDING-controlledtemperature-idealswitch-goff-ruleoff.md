...
| **Signature** | `physical:physical-rule-does-not-apply:var:47:0e0f8dfe` |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.HeatTransfer.Examples.ControlledTemperature \
      --target idealSwitch.Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "idealSwitch.Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param idealSwitch.Goff=0
```
