...
| **Signature** | `physical:physical-inertia-tensor-undecided:par:513:2e12ab5c` |
...
This one is an **aggregate** rule: its subject is the `symmetric_inertia_tensor` that `body3.body` assembles from several declarations, not any one of them. The fields it covers are excluded from the scalar rule, so this finding is the only thing said about them.

```
matrix       [[I_11, I_21, I_31], [I_21, I_22, I_32], [I_31, I_32, I_33]] (not numerically known)
eigenvalues  not computed
verdict      unknown
reason       `body3.body.I_11` is bound to `body3.I[1, 1]`, which this analysis cannot evaluate, so the tensor cannot be assembled
```
...
| `component` | body3.body |
| `declaring_class` | Modelica.Mechanics.MultiBody.Parts.Body |
| `fields` | body3.body.I_11, body3.body.I_22, body3.body.I_33, body3.body.I_21, body3.body.I_31, body3.body.I_32 |
| `matrix` | [[I_11, I_21, I_31], [I_21, I_22, I_32], [I_31, I_32, I_33]] (not numerically known) |
| `required` | body3.body.I is positive semidefinite |
| `verdict` | unknown |
| `reason` | `body3.body.I_11` is bound to `body3.I[1, 1]`, which this analysis cannot evaluate, so the tensor cannot be assembled |
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Loops.PlanarFourbar \
      --target body3.body.I --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "body3.body.I"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param body3.body.I=0
```
