# FINDING-04357: `m` in `TranslatoryArmatureAndStopper`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:1:ed3a2a5f` |
...
```
contract    Modelica.Mechanics.Translational.Components.Mass.m: algebraic_limit (proven, from equation) — inherited from mass.m, which it binds: the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "m"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param m=0
```
