# FINDING-01840: `CDC` in `IMC_InverterDrive`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/InductionMachines/IMC_InverterDrive.mo:14` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:6:72117689` |
...
```
{'quantity': 'Capacitance', 'unit': 'F', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive.CDC'}
```
...
```console
$ sed -n '14p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/InductionMachines/IMC_InverterDrive.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "CDC"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param CDC=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/InductionMachines/IMC_InverterDrive.mo:14` |' docs/v2/bugs/
```
