...
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:7590:c13f50c8` |
...
```
denominator            (1 + (oLine4.G[7].alpha * (oLine4.G[7].T_heatPort - oLine4.G[7].T_ref)))
witness                none needed
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.CompareLineTrunks \
      --target oLine4.G[7].alpha --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "oLine4.G[7].alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param oLine4.G[7].alpha=0
```
