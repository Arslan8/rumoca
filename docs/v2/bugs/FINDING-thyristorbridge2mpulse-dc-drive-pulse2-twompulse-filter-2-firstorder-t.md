# FINDING-03492: `pulse2.twomPulse.filter[2].firstOrder.T` in `ThyristorBridge2mPulse_DC_Drive`
...
| **Reached as** | `pulse2.twomPulse.filter[2].firstOrder.T` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Blocks/Continuous.mo:370` |
| **Signature** | `divisor:divisor-reachable-zero:exp:4161:de0db5b3` |
...
```
denominator            pulse2.twomPulse.filter[2].firstOrder.T
witness                pulse2.twomPulse.filter[2].firstOrder.T = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  pulse2.twomPulse.filter[2].firstOrder.T: unbounded
```
...
| `constraints` | pulse2.twomPulse.filter[2].firstOrder.T: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.ThyristorBridge2mPulse_DC_Drive \
      --target pulse2.twomPulse.filter[2].firstOrder.T --sanitizer divisor
```
...
```console
$ sed -n '370p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Blocks/Continuous.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pulse2.twomPulse.filter[2].firstOrder.T"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param pulse2.twomPulse.filter[2].firstOrder.T=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Blocks/Continuous.mo:370` |' docs/v2/bugs/
```
