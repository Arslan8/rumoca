...
| **Reached as** | `switchYD.idealCloser.idealClosingSwitch[1].Goff` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:716:55306184` |
...
```
predicate     switchYD.idealCloser.idealClosingSwitch[1].Goff > 0
premise       refuted
...
| `required` | switchYD.idealCloser.idealClosingSwitch[1].Goff &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_YD \
      --target switchYD.idealCloser.idealClosingSwitch[1].Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switchYD.idealCloser.idealClosingSwitch[1].Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switchYD.idealCloser.idealClosingSwitch[1].Goff=0
```
