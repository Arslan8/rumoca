# FINDING-01873: `inverter.transistor_p.idealGTOThyristor[2].Ron` in `IMC_InverterDrive`
...
| **Reached as** | `inverter.transistor_p.idealGTOThyristor[2].Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:568:174655f8` |
...
```
predicate     inverter.transistor_p.idealGTOThyristor[2].Ron > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealGTOThyristor.Ron: allowed (declared, from class_catalog) — an ideal switch family member; zero on-resistance is the documented ideal [matched by prefix, lower confidence]
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Ideal.IdealGTOThyristor.Ron'}
```
...
| `required` | inverter.transistor_p.idealGTOThyristor[2].Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive \
      --target inverter.transistor_p.idealGTOThyristor[2].Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inverter.transistor_p.idealGTOThyristor[2].Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inverter.transistor_p.idealGTOThyristor[2].Ron=0
```
