...
| **Signature** | `divisor:divisor-reachable-zero:exp:2922:b1f98b0f` |
...
```
denominator            dcdcInverter2.vDCFilter.T
witness                dcdcInverter2.vDCFilter.T = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.002
path condition         always evaluated
constraints consulted  dcdcInverter2.vDCFilter.T: unbounded
```
...
| `constraints` | dcdcInverter2.vDCFilter.T: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Drive \
      --target dcdcInverter2.vDCFilter.T --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcdcInverter2.vDCFilter.T"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param dcdcInverter2.vDCFilter.T=0
```
