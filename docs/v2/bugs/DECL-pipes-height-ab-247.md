# DECL-0409: `height_ab` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Fluid/Pipes.mo:247` |
| **Parameter** | `height_ab` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '247p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Pipes.mo"
```

It should declare `height_ab` with type `SI.Length` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Pipes.mo" | head
```
...
```modelica
// Fluid/Pipes.mo:247
parameter SI.Length height_ab(min=0) = ...;
```
