# FINDING-02762: `smpm.Rs` in `SMPM_ResistiveBraking`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Interfaces/PartialBasicInductionMachine.mo:11` |
| **Signature** | `physical:physical-rule-does-not-apply:var:60:d2377631` |
...
```
contract    Modelica.Electrical.Analog.Basic.Resistor.R: allowed (declared, from class_catalog) — inherited from smpm.rs.R, which it binds: the component explicitly supports signed resistance, so neither a positivity rule nor a nonzero rule follows from the quantity alone
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'semantic_role': 'component.machine.winding_resistance', 'binding_source': 'component_type', 'confidence': 'QUANTITY_AND_UNIT', 'declaring_class': 'Modelica.Electrical.Machines.BasicMachines.SynchronousMachines.SM_PermanentMagnet', 'member': 'Rs', 'premise_state': 'established', 'authority': 'component_contract', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.SynchronousMachines.SM_PermanentMagnet.Rs'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_ResistiveBraking \
      --target smpm.Rs --sanitizer physical
```
...
```console
$ sed -n '11p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Interfaces/PartialBasicInductionMachine.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smpm.Rs"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param smpm.Rs=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Interfaces/PartialBasicInductionMachine.mo:11` |' docs/v2/bugs/
```
