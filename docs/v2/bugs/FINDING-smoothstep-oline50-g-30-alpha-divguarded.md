...
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:13035:30a813ff` |
...
```
denominator            (1 + (oLine50.G[30].alpha * (oLine50.G[30].T_heatPort - oLine50.G[30].T_ref)))
witness                none needed
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.SmoothStep \
      --target oLine50.G[30].alpha --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "oLine50.G[30].alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param oLine50.G[30].alpha=0
```
