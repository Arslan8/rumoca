# FINDING-04139: `hbridge.GoffDiode` in `HBridge_TrianglePWM_RL`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCDC/HBridge.mo:16` |
| **Signature** | `physical:physical-intent-question:var:43:c37df1a3` |
...
```
predicate     hbridge.GoffDiode > 0
premise       unknown
authority     quantity_or_unit
role          —
declaration   Modelica.Electrical.PowerConverters.DCDC.HBridge.GoffDiode
```
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.PowerConverters.DCDC.HBridge.GoffDiode'}
```
...
| `required` | hbridge.GoffDiode &gt; 0 |
| `declared_min` | None |
| `variable_role` | parameter |
| `observation` | permissive-declaration |
| `premise_state` | unknown |
| `authority` | quantity_or_unit |
| `canonical_declaration` | Modelica.Electrical.PowerConverters.DCDC.HBridge.GoffDiode |
| `question` | `hbridge.GoffDiode` carries no bound that would exclude it, which is unusual for a conductance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCDC.HBridge.HBridge_TrianglePWM_RL \
      --target hbridge.GoffDiode --sanitizer physical
```
...
```console
$ sed -n '16p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCDC/HBridge.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "hbridge.GoffDiode"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param hbridge.GoffDiode=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCDC/HBridge.mo:16` |' docs/v2/bugs/
```
