# FINDING-03877: `rectifier.RonThyristor` in `ThyristorCenterTap2Pulse_RLV`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/ThyristorCenterTap2Pulse.mo:6` |
| **Signature** | `physical:physical-rule-does-not-apply:var:76:e5442baf` |
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealThyristor.Ron: allowed (declared, from class_catalog) — inherited from rectifier.thyristor_p.Ron, which it binds: an ideal switch family member; zero on-resistance is the documented ideal
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.PowerConverters.ACDC.ThyristorCenterTap2Pulse.RonThyristor'}
```
...
```console
$ sed -n '6p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/ThyristorCenterTap2Pulse.mo"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/ThyristorCenterTap2Pulse.mo:6` |' docs/v2/bugs/
```
