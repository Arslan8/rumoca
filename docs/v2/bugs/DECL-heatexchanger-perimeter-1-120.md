# DECL-0404: `perimeter_1` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Fluid/Examples/HeatExchanger.mo:120` |
| **Parameter** | `perimeter_1` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '120p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Examples/HeatExchanger.mo"
```

It should declare `perimeter_1` with type `SI.Length` and no `min`.
...
```modelica
// Fluid/Examples/HeatExchanger.mo:120
parameter SI.Length perimeter_1(min=0) = ...;
```
