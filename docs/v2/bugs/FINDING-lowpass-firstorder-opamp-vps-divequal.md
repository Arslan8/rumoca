...
```
denominator            (firstOrder.opAmp.vps - firstOrder.opAmp.vns)
witness                firstOrder.opAmp.Vps = -15
  found by             set equal to firstOrder.opAmp.Vns
denominator at witness 0.0
at declared values     30.0
path condition         always evaluated
constraints consulted  firstOrder.opAmp.Vps: unbounded
```
...
| `denominator` | (firstOrder.opAmp.vps - firstOrder.opAmp.vns) |
| `witness` | firstOrder.opAmp.Vps = -15 |
| `witness_rationale` | set equal to firstOrder.opAmp.Vns |
...
| `constraints` | firstOrder.opAmp.Vps: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.LowPass \
      --target firstOrder.opAmp.Vps --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "firstOrder.opAmp.Vps"
```
