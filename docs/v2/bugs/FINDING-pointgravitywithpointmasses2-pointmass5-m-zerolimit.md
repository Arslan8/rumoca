# FINDING-04469: `pointMass5.m` in `PointGravityWithPointMasses2`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/PointMass.mo:11` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:110:d7aeac71` |
...
```
contract    Modelica.Mechanics.MultiBody.Examples.Elementary.PointGravityWithPointMasses2.PointMass.m: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Mass', 'unit': 'kg', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Mechanics.MultiBody.Examples.Elementary.PointGravityWithPointMasses2.PointMass.m'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Elementary.PointGravityWithPointMasses2 \
      --target pointMass5.m --sanitizer physical
```
...
```console
$ sed -n '11p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/PointMass.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pointMass5.m"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param pointMass5.m=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/PointMass.mo:11` |' docs/v2/bugs/
```
