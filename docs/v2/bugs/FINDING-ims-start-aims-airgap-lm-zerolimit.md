# FINDING-02280: `aims.airGap.Lm` in `IMS_Start`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:415:181a6dc9` |
...
```
contract    Modelica.Electrical.Machines.BasicMachines.Components.AirGapS.L: feature_disabled (proven, from equation) — inherited from aims.airGap.L, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMS_Start \
      --target aims.airGap.Lm --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "aims.airGap.Lm"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param aims.airGap.Lm=0
```
