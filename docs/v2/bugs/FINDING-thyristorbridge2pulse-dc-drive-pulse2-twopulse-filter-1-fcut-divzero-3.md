# FINDING-03691: `pulse2.twoPulse.filter[1].fCut` in `ThyristorBridge2Pulse_DC_Drive`
...
| **Reached as** | `pulse2.twoPulse.filter[1].fCut` |
...
| **Signature** | `divisor:divisor-reachable-zero:exp:515:9c01374e` |
...
```
denominator            ((2 * (2 * asin(1.0))) * pulse2.twoPulse.filter[1].fCut)
witness                pulse2.twoPulse.filter[1].fCut = 0
  found by             set to zero
denominator at witness 0.0
at declared values     628.3185307179587
path condition         always evaluated
constraints consulted  pulse2.twoPulse.filter[1].fCut: unbounded
```
...
| `constraints` | pulse2.twoPulse.filter[1].fCut: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2Pulse.ThyristorBridge2Pulse_DC_Drive \
      --target pulse2.twoPulse.filter[1].fCut --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pulse2.twoPulse.filter[1].fCut"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param pulse2.twoPulse.filter[1].fCut=0
```
