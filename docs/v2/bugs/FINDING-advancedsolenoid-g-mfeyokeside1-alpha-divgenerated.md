# FINDING-04298: `g_mFeYokeSide1.alpha` in `AdvancedSolenoid`
...
| **Reached as** | `g_mFeYokeSide1.alpha` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-introduced-by-translation` |
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/BaseClasses/FixedShape.mo:37` |
| **Signature** | `divisor:divisor-introduced-by-translation:exp:1042:1fb4d316` |
...
Zero is a supported limit of this component; the quotient the dae shows is the compiler's solved form, not a division in the source.

It does **not** claim that the source contract is wrong.
...
```
contract    Modelica.Magnetic.FluxTubes.Shapes.FixedShape.HollowCylinderAxialFlux.alpha: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
behavior    
confidence  proven
source      
```
...
```
denominator            g_mFeYokeSide1.A
witness                g_mFeYokeSide1.alpha = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.0001343030859409637
path condition         always evaluated
constraints consulted  g_mFeYokeSide1.alpha: unbounded
```
...
| `denominator_at_declared_values` | 0.0001343030859409637 |
| `denominator_range` | [-inf, inf] |
| `path_condition` | always evaluated |
| `verdict` | SAT |
| `proof` | the division is not inside any branch |
| `constraints` | g_mFeYokeSide1.alpha: unbounded |
| `contract` | Modelica.Magnetic.FluxTubes.Shapes.FixedShape.HollowCylinderAxialFlux.alpha: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected |
| `contract_confidence` | proven |
| `note` | zero is a supported limit of this component rather than a division by zero; the quotient the DAE shows is the compiler&#x27;s solved form, not the source |
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.SolenoidActuator.Components.AdvancedSolenoid \
      --target g_mFeYokeSide1.alpha --sanitizer divisor
```
...
```console
$ sed -n '37p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/BaseClasses/FixedShape.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "g_mFeYokeSide1.alpha"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/BaseClasses/FixedShape.mo:37` |' docs/v2/bugs/
```
