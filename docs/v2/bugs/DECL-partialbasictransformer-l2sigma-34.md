# DECL-0230: `L2sigma` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/QuasiStatic/Machines/Interfaces/PartialBasicTransformer.mo:34` |
| **Parameter** | `L2sigma` |
| **Type** | `SI.Inductance` |
| **Declared modifiers** | `(start=78E-6/(if C2 == "d" then 1 else 3)` |
...
```console
$ sed -n '34p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/QuasiStatic/Machines/Interfaces/PartialBasicTransformer.mo"
```

It should declare `L2sigma` with type `SI.Inductance` and no `min`.
...
```modelica
// Electrical/QuasiStatic/Machines/Interfaces/PartialBasicTransformer.mo:34
parameter SI.Inductance L2sigma(min=0) = ...;
```
