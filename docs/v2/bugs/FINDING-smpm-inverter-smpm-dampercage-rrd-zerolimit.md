...
| **Reached as** | `smpm.damperCage.Rrd` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:456:4e0485ce` |
...
```
predicate     smpm.damperCage.Rrd >= 0
premise       refuted
...
| `required` | smpm.damperCage.Rrd &gt;= 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_Inverter \
      --target smpm.damperCage.Rrd --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smpm.damperCage.Rrd"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param smpm.damperCage.Rrd=0
```
