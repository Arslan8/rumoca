...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/HalfControlledBridge2mPulse.mo:7` |
| **Signature** | `physical:physical-rule-does-not-apply:var:125:8a0dc771` |
...
```
contract    Modelica.Electrical.Polyphase.Ideal.IdealDiode.Ron: allowed (declared, from class_catalog) — inherited from rectifier.diode_n.Ron, which it binds: an ideal switch family member; zero on-resistance is the documented ideal
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.PowerConverters.ACDC.HalfControlledBridge2mPulse.RonDiode'}
```
...
```console
$ sed -n '7p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/HalfControlledBridge2mPulse.mo"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/HalfControlledBridge2mPulse.mo:7` |' docs/v2/bugs/
```
