# FINDING-02934: `smr.statorCore.coreParameters.GcRef` in `SMR_Inverter`
...
| **Reached as** | `smr.statorCore.coreParameters.GcRef` |
...
| `observed` | 0.0 |
| `required` | smr.statorCore.coreParameters.GcRef &gt; 0 |
| `where` | declaration |
| `contract` | Modelica.Electrical.Machines.Losses.CoreParameters.GcRef: feature_disabled (declared, from bound) — the declaration&#x27;s own binding evaluates to zero at the declared configuration, so zero is the value the library chose rather than one it failed to exclude |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMR_Inverter \
      --target smr.statorCore.coreParameters.GcRef --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smr.statorCore.coreParameters.GcRef"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param smr.statorCore.coreParameters.GcRef=0
```
