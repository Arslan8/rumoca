...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:815:2b6d8a8f` |
...
```
denominator            (2 * (simpleTriac.thyristor1.Von - simpleTriac.thyristor1.Voff))
witness                simpleTriac.thyristor1.Von = 1.5
  found by             the value that solves the denominator to zero
denominator at witness 0.0
at declared values     7.0
path condition         not ((simpleTriac.thyristor1.vAK < -simpleTriac.thyristor1.VRRM)) and not ((simpleTriac.thyristor1.vControl < simpleTriac.thyristor1.Voff)) and ((simpleTriac.thyristor1.vControl < simpleTriac.thyristor1.Von))
constraints consulted  simpleTriac.thyristor1.Von: unbounded
```
...
| `path_condition` | not ((simpleTriac.thyristor1.vAK &lt; -simpleTriac.thyristor1.VRRM)) and not ((simpleTriac.thyristor1.vControl &lt; simpleTriac.thyristor1.Voff)) and ((simpleTriac.thyristor1.vControl &lt; simpleTriac.thyristor1.Von)) |
| `verdict` | UNSAT |
| `proof` | the guard not ((simpleTriac.thyristor1.vControl &lt; simpleTriac.thyristor1.Voff)) is false under the witness |
| `constraints` | simpleTriac.thyristor1.Von: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.SimpleTriacCircuit \
      --target simpleTriac.thyristor1.Von --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "simpleTriac.thyristor1.Von"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param simpleTriac.thyristor1.Von=1.5
```
