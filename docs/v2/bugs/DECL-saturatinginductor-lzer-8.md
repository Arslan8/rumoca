# DECL-0184: `Lzer` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Polyphase/Basic/SaturatingInductor.mo:8` |
| **Parameter** | `Lzer` |
| **Type** | `SI.Inductance` |
| **Declared modifiers** | `(start={2*Lnom[j] for j in 1 :m})` |
...
```console
$ sed -n '8p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Basic/SaturatingInductor.mo"
```

It should declare `Lzer` with type `SI.Inductance` and no `min`.
...
```modelica
// Electrical/Polyphase/Basic/SaturatingInductor.mo:8
parameter SI.Inductance Lzer(min=0) = ...;
```
