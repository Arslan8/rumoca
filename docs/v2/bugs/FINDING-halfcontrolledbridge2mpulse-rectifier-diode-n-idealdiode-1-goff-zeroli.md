# FINDING-03439: `rectifier.diode_n.idealDiode[1].Goff` in `HalfControlledBridge2mPulse`
...
| **Reached as** | `rectifier.diode_n.idealDiode[1].Goff` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:255:6b4eae47` |
...
```
predicate     rectifier.diode_n.idealDiode[1].Goff > 0
premise       refuted
...
| `required` | rectifier.diode_n.idealDiode[1].Goff &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.HalfControlledBridge2mPulse \
      --target rectifier.diode_n.idealDiode[1].Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "rectifier.diode_n.idealDiode[1].Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param rectifier.diode_n.idealDiode[1].Goff=0
```
