# FINDING-04005: `capacitorLV.C` in `ChopperBuckBoost_DutyCycle`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:153:100f90bf` |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCDC.ChopperBuckBoost.ChopperBuckBoost_DutyCycle \
      --target capacitorLV.C --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCDC.ChopperBuckBoost.ChopperBuckBoost_DutyCycle --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "capacitorLV.C"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param capacitorLV.C=0
```
