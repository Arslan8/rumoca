# DECL-0561: `defaultJointWidth` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/package.mo:116` |
| **Parameter** | `defaultJointWidth` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '116p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/package.mo"
```

It should declare `defaultJointWidth` with type `SI.Length` and no `min`.
...
```modelica
// Mechanics/MultiBody/package.mo:116
parameter SI.Length defaultJointWidth(min=0) = ...;
```
