# FINDING-04381: `boxBody1.body.I` in `DoublePendulum`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Mechanics.MultiBody.Examples.Elementary.DoublePendulum` |
| **Reached as** | `boxBody1.body.I` |
| **Found by** | **PhysicalSan** — `packages/modelsan/modelsan/sanitizers/physical.py` |
| **Reported as** | `physical-inertia-tensor-undecided` |
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/Body.mo:114` |
| **Signature** | `physical:physical-inertia-tensor-undecided:par:375:f4742246` |
...
This one is an **aggregate** rule: its subject is the `symmetric_inertia_tensor` that `boxBody1.body` assembles from several declarations, not any one of them. The fields it covers are excluded from the scalar rule, so this finding is the only thing said about them.

```
matrix       [[I_11, I_21, I_31], [I_21, I_22, I_32], [I_31, I_32, I_33]] (not numerically known)
eigenvalues  not computed
verdict      unknown
reason       `boxBody1.body.I_11` is bound to `boxBody1.I[1, 1]`, which this analysis cannot evaluate, so the tensor cannot be assembled
```
...
| `component` | boxBody1.body |
| `declaring_class` | Modelica.Mechanics.MultiBody.Parts.Body |
| `fields` | boxBody1.body.I_11, boxBody1.body.I_22, boxBody1.body.I_33, boxBody1.body.I_21, boxBody1.body.I_31, boxBody1.body.I_32 |
| `matrix` | [[I_11, I_21, I_31], [I_21, I_22, I_32], [I_31, I_32, I_33]] (not numerically known) |
| `required` | boxBody1.body.I is positive semidefinite |
| `verdict` | unknown |
| `reason` | `boxBody1.body.I_11` is bound to `boxBody1.I[1, 1]`, which this analysis cannot evaluate, so the tensor cannot be assembled |
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Elementary.DoublePendulum \
      --target boxBody1.body.I --sanitizer physical
```
...
```console
$ sed -n '114p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/Body.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "boxBody1.body.I"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param boxBody1.body.I=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/Body.mo:114` |' docs/v2/bugs/
```
