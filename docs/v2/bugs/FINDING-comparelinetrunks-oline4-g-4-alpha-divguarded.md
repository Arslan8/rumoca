...
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:7479:0dfb7245` |
...
```
denominator            (1 + (oLine4.G[4].alpha * (oLine4.G[4].T_heatPort - oLine4.G[4].T_ref)))
witness                none needed
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.CompareLineTrunks \
      --target oLine4.G[4].alpha --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "oLine4.G[4].alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param oLine4.G[4].alpha=0
```
