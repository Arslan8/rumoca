# FINDING-00045: `invMixingUnit.c0` in `MixingUnitWithContinuousControl`
...
| **Reached as** | `invMixingUnit.c0` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Clocked/Examples/Systems/Utilities/ComponentsMixingUnit/MixingUnit.mo:24` |
| **Signature** | `divisor:divisor-reachable-zero:exp:131:bd8c1911` |
...
```
denominator            invMixingUnit.c0
witness                invMixingUnit.c0 = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.848
path condition         always evaluated
constraints consulted  invMixingUnit.c0: unbounded
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Clocked.Examples.Systems.Utilities.ComponentsMixingUnit.MixingUnitWithContinuousControl \
      --target invMixingUnit.c0 --sanitizer divisor
```
...
```console
$ sed -n '24p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Clocked/Examples/Systems/Utilities/ComponentsMixingUnit/MixingUnit.mo"
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Clocked.Examples.Systems.Utilities.ComponentsMixingUnit.MixingUnitWithContinuousControl --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "invMixingUnit.c0"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Clocked/Examples/Systems/Utilities/ComponentsMixingUnit/MixingUnit.mo:24` |' docs/v2/bugs/
```
