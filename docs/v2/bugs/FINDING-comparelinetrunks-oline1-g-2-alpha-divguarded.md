...
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:4882:3ab03e1c` |
...
```
denominator            (1 + (oLine1.G[2].alpha * (oLine1.G[2].T_heatPort - oLine1.G[2].T_ref)))
witness                none needed
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.CompareLineTrunks \
      --target oLine1.G[2].alpha --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "oLine1.G[2].alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param oLine1.G[2].alpha=0
```
