# FINDING-05099: `pressureSensor.flowPort.medium.rho` in `WaterPump`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Thermal.FluidHeatFlow.Examples.WaterPump` |
| **Reached as** | `pressureSensor.flowPort.medium.rho` |
...
| **Signature** | `physical:physical-intent-question:var:152:c5e44ab3` |
...
```
predicate     pressureSensor.flowPort.medium.rho > 0
premise       unknown
...
> `pressureSensor.flowPort.medium.rho` is bounded only by min=0, which is unusual for a density — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
| `required` | pressureSensor.flowPort.medium.rho &gt; 0 |
| `declared_min` | 0.0 |
...
| `question` | `pressureSensor.flowPort.medium.rho` is bounded only by min=0, which is unusual for a density — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.WaterPump \
      --target pressureSensor.flowPort.medium.rho --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pressureSensor.flowPort.medium.rho"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param pressureSensor.flowPort.medium.rho=0
```
