...
| **Reached as** | `vertical_tau` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/cmm/CMM-a642c381/RigidBody/Examples/RoverPlant.mo:58` |
| **Signature** | `divisor:divisor-reachable-zero:exp:663:f508c2a6` |
...
| `constraints` | vertical_tau: unbounded |
...
$ python3 tools/sweep/check_one.py RigidBody.Examples.RoverPlant \
      --target vertical_tau --sanitizer divisor
```
...
```console
$ sed -n '58p' "target/cmm/CMM-a642c381/RigidBody/Examples/RoverPlant.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "vertical_tau"
```
...
```console
$ grep -rl '| `target/cmm/CMM-a642c381/RigidBody/Examples/RoverPlant.mo:58` |' docs/v2/bugs/
```
