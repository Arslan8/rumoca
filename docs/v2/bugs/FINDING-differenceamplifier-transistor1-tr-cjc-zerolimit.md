...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/NPN.mo:11` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:205:8c6ab885` |
...
```
predicate     Transistor1.Tr.Cjc > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Semiconductors.NPN.Cjc: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Capacitance', 'unit': 'F', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Semiconductors.NPN.Cjc'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.DifferenceAmplifier \
      --target Transistor1.Tr.Cjc --sanitizer physical
```
...
```console
$ sed -n '11p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/NPN.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "Transistor1.Tr.Cjc"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param Transistor1.Tr.Cjc=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/NPN.mo:11` |' docs/v2/bugs/
```
