# DECL-0240: `L1` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/QuasiStatic/SinglePhase/Examples/MultipleResonance.mo:10` |
| **Parameter** | `L1` |
| **Type** | `SI.Inductance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '10p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/QuasiStatic/SinglePhase/Examples/MultipleResonance.mo"
```

It should declare `L1` with type `SI.Inductance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/QuasiStatic/SinglePhase/Examples/MultipleResonance.mo" | head
```
...
```modelica
// Electrical/QuasiStatic/SinglePhase/Examples/MultipleResonance.mo:10
parameter SI.Inductance L1(min=0) = ...;
```
