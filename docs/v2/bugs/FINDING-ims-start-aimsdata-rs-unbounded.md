...
| **Signature** | `physical:physical-domain-unenforced:var:1076:2a3feb14` |
...
```
predicate     aimsData.Rs >= 0
premise       established
authority     source_arithmetic
role          —
declaration   Modelica.Electrical.Machines.Utilities.ParameterRecords.IM_SlipRingData.Rs
```
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'semantic_role': 'component.machine.winding_resistance', 'binding_source': 'component_type', 'confidence': 'QUANTITY_AND_UNIT', 'declaring_class': 'Modelica.Electrical.Machines.Utilities.ParameterRecords.IM_SlipRingData', 'member': 'Rs', 'premise_state': 'established', 'authority': 'component_contract', 'canonical_declaration': 'Modelica.Electrical.Machines.Utilities.ParameterRecords.IM_SlipRingData.Rs'}
```
...
| `required` | aimsData.Rs &gt;= 0 |
| `declared_min` | None |
| `variable_role` | parameter |
| `premise_state` | established |
| `authority` | source_arithmetic |
| `canonical_declaration` | Modelica.Electrical.Machines.Utilities.ParameterRecords.IM_SlipRingData.Rs |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMS_Start \
      --target aimsData.Rs --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "aimsData.Rs"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param aimsData.Rs=0
```
