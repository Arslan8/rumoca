# FINDING-02154: `switchYDwithArc.Goff` in `IMC_YDarc`
...
| **Reached as** | `switchYDwithArc.Goff` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SwitchYDwithArc.mo:5` |
| **Signature** | `physical:physical-rule-does-not-apply:var:646:145edb6f` |
...
```
predicate     switchYDwithArc.Goff > 0
premise       refuted
...
```
contract    Modelica.Electrical.Polyphase.Ideal.CloserWithArc.Goff: allowed (declared, from class_catalog) — inherited from switchYDwithArc.idealCloser.Goff, which it binds: an ideal switch family member; zero off-conductance is the documented ideal
behavior    allowed
confidence  declared
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.Utilities.SwitchYDwithArc.Goff'}
```
...
| `required` | switchYDwithArc.Goff &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_YDarc \
      --target switchYDwithArc.Goff --sanitizer physical
```
...
```console
$ sed -n '5p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SwitchYDwithArc.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switchYDwithArc.Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switchYDwithArc.Goff=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SwitchYDwithArc.mo:5` |' docs/v2/bugs/
```
