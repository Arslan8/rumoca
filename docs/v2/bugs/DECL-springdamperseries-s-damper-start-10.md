# DECL-0545: `s_damper_start` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/Forces/SpringDamperSeries.mo:10` |
| **Parameter** | `s_damper_start` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '10p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Forces/SpringDamperSeries.mo"
```

It should declare `s_damper_start` with type `SI.Length` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Forces/SpringDamperSeries.mo" | head
```
...
```modelica
// Mechanics/MultiBody/Forces/SpringDamperSeries.mo:10
parameter SI.Length s_damper_start(min=0) = ...;
```
