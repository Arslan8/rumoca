...
| **Declaration** | `Electrical/QuasiStatic/Machines/Interfaces/PartialBasicTransformer.mo:23` |
| **Parameter** | `R2` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `(start=5E-3/(if C2 == "d" then 1 else 3)` |
...
```console
$ sed -n '23p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/QuasiStatic/Machines/Interfaces/PartialBasicTransformer.mo"
```

It should declare `R2` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/QuasiStatic/Machines/Interfaces/PartialBasicTransformer.mo:23
parameter SI.Resistance R2(min=0) = ...;
```
