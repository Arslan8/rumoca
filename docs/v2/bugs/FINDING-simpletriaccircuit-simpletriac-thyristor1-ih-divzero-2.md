...
| **Signature** | `divisor:divisor-reachable-zero:exp:223:e85e5b5f` |
...
```
denominator            simpleTriac.thyristor1.IH
witness                simpleTriac.thyristor1.IH = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.006
path condition         always evaluated
constraints consulted  simpleTriac.thyristor1.IH: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.SimpleTriacCircuit \
      --target simpleTriac.thyristor1.IH --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "simpleTriac.thyristor1.IH"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param simpleTriac.thyristor1.IH=0
```
