...
| **Signature** | `physical:physical-intent-question:var:767:2e8da6b1` |
...
```
predicate     dcdcInverter2.GoffD > 0
premise       unknown
...
| `required` | dcdcInverter2.GoffD &gt; 0 |
| `declared_min` | None |
...
| `question` | `dcdcInverter2.GoffD` carries no bound that would exclude it, which is unusual for a conductance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Drive \
      --target dcdcInverter2.GoffD --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcdcInverter2.GoffD"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param dcdcInverter2.GoffD=0
```
