# DECL-0571: `labelStart` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/package.mo:183` |
| **Parameter** | `labelStart` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '183p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/package.mo"
```

It should declare `labelStart` with type `SI.Length` and no `min`.
...
```modelica
// Mechanics/MultiBody/package.mo:183
parameter SI.Length labelStart(min=0) = ...;
```
