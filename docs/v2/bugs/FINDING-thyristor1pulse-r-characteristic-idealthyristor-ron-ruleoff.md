# FINDING-03396: `idealthyristor.Ron` in `Thyristor1Pulse_R_Characteristic`
...
| **Signature** | `physical:physical-rule-does-not-apply:var:141:83ca15b8` |
...
```
predicate     idealthyristor.Ron > 0
premise       refuted
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.Rectifier1Pulse.Thyristor1Pulse_R_Characteristic \
      --target idealthyristor.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "idealthyristor.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param idealthyristor.Ron=0
```
