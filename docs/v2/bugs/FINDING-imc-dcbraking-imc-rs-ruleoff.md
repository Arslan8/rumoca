# FINDING-01719: `imc.Rs` in `IMC_DCBraking`
...
| **Signature** | `physical:physical-rule-does-not-apply:var:52:31ef946c` |
...
```
contract    Modelica.Electrical.Analog.Basic.Resistor.R: allowed (declared, from class_catalog) — inherited from imc.rs.R, which it binds: the component explicitly supports signed resistance, so neither a positivity rule nor a nonzero rule follows from the quantity alone
behavior    allowed
confidence  declared
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_DCBraking \
      --target imc.Rs --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "imc.Rs"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param imc.Rs=0
```
