...
| **Signature** | `physical:physical-rule-does-not-apply:var:34:6c5bedc1` |
...
```
predicate     With_Ron_Goff_Vknee.Ron > 0
premise       refuted
...
| `required` | With_Ron_Goff_Vknee.Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.CharacteristicIdealDiodes \
      --target With_Ron_Goff_Vknee.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "With_Ron_Goff_Vknee.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param With_Ron_Goff_Vknee.Ron=0
```
