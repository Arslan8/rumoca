# FINDING-03304: `transformerData.V1ph` in `TransformerTestbench`
...
| **Reached as** | `transformerData.V1ph` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/TransformerData.mo:29` |
| **Signature** | `divisor:divisor-reachable-zero:exp:1168:adeba104` |
...
```
denominator            (3 * transformerData.V1ph)
witness                transformerData.V1ph = 0
  found by             set to zero
denominator at witness 0.0
at declared values     None
path condition         always evaluated
constraints consulted  transformerData.V1ph: unbounded
```
...
| `constraints` | transformerData.V1ph: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.Transformers.TransformerTestbench \
      --target transformerData.V1ph --sanitizer divisor
```
...
```console
$ sed -n '29p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/TransformerData.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "transformerData.V1ph"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/TransformerData.mo:29` |' docs/v2/bugs/
```
