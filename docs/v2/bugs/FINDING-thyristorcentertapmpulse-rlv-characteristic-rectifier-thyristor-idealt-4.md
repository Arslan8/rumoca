# FINDING-03924: `rectifier.thyristor.idealThyristor[2].Goff` in `ThyristorCenterTapmPulse_RLV_Characteristic`
...
```
predicate     rectifier.thyristor.idealThyristor[2].Goff > 0
premise       refuted
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTapmPulse.ThyristorCenterTapmPulse_RLV_Characteristic \
      --target rectifier.thyristor.idealThyristor[2].Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "rectifier.thyristor.idealThyristor[2].Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param rectifier.thyristor.idealThyristor[2].Goff=0
```
