# DECL-0056: `Gbe` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Semiconductors/PNP.mo:17` |
| **Parameter** | `Gbe` |
| **Type** | `SI.Conductance` |
| **Declared modifiers** | `—` |
...
It should declare `Gbe` with type `SI.Conductance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/PNP.mo" | head
```
...
```modelica
// Electrical/Analog/Semiconductors/PNP.mo:17
parameter SI.Conductance Gbe(min=0) = ...;
```
