# FINDING-05367: `slidingMass.m` in `TestMove`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `ModelicaTest.Rotational.TestMove` |
| **Reached as** | `slidingMass.m` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:383:7dadf4cc` |
...
```console
$ python3 tools/sweep/check_one.py ModelicaTest.Rotational.TestMove \
      --target slidingMass.m --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py ModelicaTest.Rotational.TestMove --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "slidingMass.m"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param slidingMass.m=0
```
