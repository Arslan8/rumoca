...
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Clocked/Examples/Systems/Utilities/ComponentsThrottleControl/ThrottleBody.mo:30` |
| **Signature** | `divisor:divisor-reachable-zero:exp:37:d9fd970d` |
...
constraints consulted  P_0: unbounded
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Clocked.Examples.Systems.Utilities.ComponentsThrottleControl.ThrottleBody \
      --target P_0 --sanitizer divisor
```
...
```console
$ sed -n '30p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Clocked/Examples/Systems/Utilities/ComponentsThrottleControl/ThrottleBody.mo"
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Clocked.Examples.Systems.Utilities.ComponentsThrottleControl.ThrottleBody --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "P_0"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Clocked/Examples/Systems/Utilities/ComponentsThrottleControl/ThrottleBody.mo:30` |' docs/v2/bugs/
```
