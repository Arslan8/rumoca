# FINDING-02263: `aims.statorCore.coreParameters.GcRef` in `IMS_Start`
...
| **Reached as** | `aims.statorCore.coreParameters.GcRef` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:164:c4de49cd` |
...
| `observed` | 0.0 |
| `required` | aims.statorCore.coreParameters.GcRef &gt; 0 |
| `where` | declaration |
| `contract` | Modelica.Electrical.Machines.Losses.CoreParameters.GcRef: feature_disabled (declared, from bound) — the declaration&#x27;s own binding evaluates to zero at the declared configuration, so zero is the value the library chose rather than one it failed to exclude |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMS_Start \
      --target aims.statorCore.coreParameters.GcRef --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "aims.statorCore.coreParameters.GcRef"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param aims.statorCore.coreParameters.GcRef=0
```
