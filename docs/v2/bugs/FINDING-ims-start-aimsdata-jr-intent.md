# FINDING-02306: `aimsData.Jr` in `IMS_Start`
...
| **Signature** | `physical:physical-intent-question:var:1072:9d098c09` |
...
```
predicate     aimsData.Jr > 0
premise       unknown
authority     quantity_or_unit
role          —
declaration   Modelica.Electrical.Machines.Utilities.ParameterRecords.IM_SlipRingData.Jr
```
...
> `aimsData.Jr` carries no bound that would exclude it, which is unusual for a moment of inertia — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
```
{'quantity': 'MomentOfInertia', 'unit': 'kg.m2', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.Utilities.ParameterRecords.IM_SlipRingData.Jr'}
```
...
| `required` | aimsData.Jr &gt; 0 |
| `declared_min` | None |
| `variable_role` | parameter |
| `observation` | permissive-declaration |
| `premise_state` | unknown |
| `authority` | quantity_or_unit |
| `canonical_declaration` | Modelica.Electrical.Machines.Utilities.ParameterRecords.IM_SlipRingData.Jr |
| `question` | `aimsData.Jr` carries no bound that would exclude it, which is unusual for a moment of inertia — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMS_Start \
      --target aimsData.Jr --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "aimsData.Jr"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param aimsData.Jr=0
```
