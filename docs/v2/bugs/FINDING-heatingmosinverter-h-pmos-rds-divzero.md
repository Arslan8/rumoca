...
| **Signature** | `divisor:divisor-reachable-zero:exp:369:cf37c80c` |
...
```
denominator            H_PMOS.RDS
witness                H_PMOS.RDS = 0
  found by             set to zero
denominator at witness 0.0
at declared values     10000000.0
path condition         not (((H_PMOS.RDS < 1e-20) and (H_PMOS.RDS > -1e-20)))
constraints consulted  H_PMOS.RDS: unbounded
```
...
| `path_condition` | not (((H_PMOS.RDS &lt; 1e-20) and (H_PMOS.RDS &gt; -1e-20))) |
| `verdict` | SAT |
| `proof` | every guard still holds under the witness |
| `constraints` | H_PMOS.RDS: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.HeatingMOSInverter \
      --target H_PMOS.RDS --sanitizer divisor
```
