# DECL-0668: `RDS` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Semiconductors/NMOS.mo:20` |
| **Parameter** | `RDS` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
It should declare `RDS` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/Analog/Semiconductors/NMOS.mo:20
parameter SI.Resistance RDS(min=0) = ...;
```
