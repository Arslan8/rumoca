# DECL-0706: `RL` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Machines/Examples/Transformers/TransformerTestbench.mo:4` |
| **Parameter** | `RL` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
It should declare `RL` with type `SI.Resistance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/Transformers/TransformerTestbench.mo" | head
```
...
```modelica
// Electrical/Machines/Examples/Transformers/TransformerTestbench.mo:4
parameter SI.Resistance RL(min=0) = ...;
```
