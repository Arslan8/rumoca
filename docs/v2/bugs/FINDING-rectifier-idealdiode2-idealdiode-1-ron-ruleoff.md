...
| **Signature** | `physical:physical-rule-does-not-apply:var:244:dead4bb4` |
...
```
predicate     idealDiode2.idealDiode[1].Ron > 0
premise       refuted
...
| `required` | idealDiode2.idealDiode[1].Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Polyphase.Examples.Rectifier \
      --target idealDiode2.idealDiode[1].Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "idealDiode2.idealDiode[1].Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param idealDiode2.idealDiode[1].Ron=0
```
