# DECL-0891: `cv` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Thermal/FluidHeatFlow/Media/Medium.mo:7` |
| **Parameter** | `cv` |
...
```console
$ sed -n '7p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/Media/Medium.mo"
```

It should declare `cv` with type `SI.SpecificHeatCapacity` and no `min`.
...
```modelica
// Thermal/FluidHeatFlow/Media/Medium.mo:7
parameter SI.SpecificHeatCapacity cv(min=0) = ...;
```
