...
| **Reached as** | `idealTriac.capacitor.C` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:101:4a2bfddf` |
...
```
predicate     idealTriac.capacitor.C > 0
premise       refuted
...
| `required` | idealTriac.capacitor.C &gt; 0 |
| `declared_min` | 0.0 |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.IdealTriacCircuit \
      --target idealTriac.capacitor.C --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.IdealTriacCircuit --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "idealTriac.capacitor.C"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param idealTriac.capacitor.C=0
```
