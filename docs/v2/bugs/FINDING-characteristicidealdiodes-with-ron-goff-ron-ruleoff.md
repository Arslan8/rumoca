...
| **Signature** | `physical:physical-rule-does-not-apply:var:17:6435f90a` |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.CharacteristicIdealDiodes \
      --target With_Ron_Goff.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "With_Ron_Goff.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param With_Ron_Goff.Ron=0
```
