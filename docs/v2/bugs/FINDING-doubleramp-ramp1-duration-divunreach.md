# FINDING-05086: `ramp1.duration` in `DoubleRamp`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Thermal.FluidHeatFlow.Examples.Utilities.DoubleRamp` |
| **Reached as** | `ramp1.duration` |
...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:72:10d61f6e` |
...
```
denominator            ramp1.duration
witness                ramp1.duration = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.2
path condition         not ((time < ramp1.startTime)) and ((time < (ramp1.startTime + ramp1.duration)))
constraints consulted  ramp1.duration: min=0
```
...
| `path_condition` | not ((time &lt; ramp1.startTime)) and ((time &lt; (ramp1.startTime + ramp1.duration))) |
| `verdict` | UNSAT |
| `proof` | the path conditions cannot hold together: time must be both &gt; 0.2 and &lt; 0.2 |
| `constraints` | ramp1.duration: min=0 |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.Utilities.DoubleRamp \
      --target ramp1.duration --sanitizer divisor
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.Utilities.DoubleRamp --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "ramp1.duration"
```
