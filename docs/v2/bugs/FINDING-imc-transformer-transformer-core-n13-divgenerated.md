# FINDING-02093: `transformer.core.n13` in `IMC_Transformer`
...
| **Reached as** | `transformer.core.n13` |
...
| **Signature** | `divisor:divisor-introduced-by-translation:exp:3532:93e5ff5a` |
...
```
denominator            transformer.core.n13
witness                transformer.core.n13 = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  transformer.core.n13: unbounded
```
...
| `constraints` | transformer.core.n13: unbounded |
| `contract` | Modelica.Electrical.Machines.BasicMachines.Components.IdealCore.n13: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_Transformer \
      --target transformer.core.n13 --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "transformer.core.n13"
```
