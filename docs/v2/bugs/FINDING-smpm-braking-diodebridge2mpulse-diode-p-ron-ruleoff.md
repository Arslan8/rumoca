# FINDING-02560: `diodeBridge2mPulse.diode_p.Ron` in `SMPM_Braking`
...
| **Reached as** | `diodeBridge2mPulse.diode_p.Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:646:1826fa2c` |
...
```
predicate     diodeBridge2mPulse.diode_p.Ron > 0
premise       refuted
...
| `required` | diodeBridge2mPulse.diode_p.Ron &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_Braking \
      --target diodeBridge2mPulse.diode_p.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "diodeBridge2mPulse.diode_p.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param diodeBridge2mPulse.diode_p.Ron=0
```
