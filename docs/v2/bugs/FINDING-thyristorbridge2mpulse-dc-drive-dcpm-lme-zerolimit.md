# FINDING-03479: `dcpm.Lme` in `ThyristorBridge2mPulse_DC_Drive`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/DCMachines/DC_PermanentMagnet.mo:38` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:961:776547a9` |
...
```
contract    Modelica.Electrical.Machines.BasicMachines.DCMachines.DC_PermanentMagnet.Lme: forbidden (proven, from bound) — the declaration excludes zero (declared constant)
behavior    forbidden
confidence  proven
source      bound
```
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.DCMachines.DC_PermanentMagnet.Lme'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.ThyristorBridge2mPulse_DC_Drive \
      --target dcpm.Lme --sanitizer physical
```
...
```console
$ sed -n '38p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/DCMachines/DC_PermanentMagnet.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcpm.Lme"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param dcpm.Lme=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/DCMachines/DC_PermanentMagnet.mo:38` |' docs/v2/bugs/
```
