# FINDING-01269: `RonD` in `DcdcInverter`
...
| **Signature** | `physical:physical-intent-question:var:9:6bd07f32` |
...
> `RonD` carries no bound that would exclude it, which is unusual for a resistance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
| `question` | `RonD` carries no bound that would exclude it, which is unusual for a resistance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.ControlledDCDrives.Utilities.DcdcInverter \
      --target RonD --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "RonD"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param RonD=0
```
