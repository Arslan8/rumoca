# FINDING-04062: `chopperStepUp.diode.Goff` in `ChopperStepUp_R`
...
| **Reached as** | `chopperStepUp.diode.Goff` |
...
```
predicate     chopperStepUp.diode.Goff > 0
premise       refuted
...
| `required` | chopperStepUp.diode.Goff &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCDC.ChopperStepUp.ChopperStepUp_R \
      --target chopperStepUp.diode.Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "chopperStepUp.diode.Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param chopperStepUp.diode.Goff=0
```
