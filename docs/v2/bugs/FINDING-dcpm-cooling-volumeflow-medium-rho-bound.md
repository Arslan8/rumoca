...
| **Signature** | `physical:physical-bound-permits-zero:var:334:9db4a558` |
...
```
predicate     volumeFlow.medium.rho > 0
premise       established
...
| `required` | volumeFlow.medium.rho &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Cooling \
      --target volumeFlow.medium.rho --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "volumeFlow.medium.rho"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param volumeFlow.medium.rho=0
```
