...
| **Signature** | `divisor:divisor-reachable-zero:exp:671:7df6b505` |
...
```
denominator            height_tau
witness                height_tau = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.08
path condition         always evaluated
constraints consulted  height_tau: unbounded
```
...
$ python3 tools/sweep/check_one.py RigidBody.Examples.RoverPlant \
      --target height_tau --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "height_tau"
```
