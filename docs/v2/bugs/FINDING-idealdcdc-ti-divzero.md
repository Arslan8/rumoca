...
| **Signature** | `divisor:divisor-reachable-zero:exp:95:c1140867` |
...
constraints consulted  Ti: unbounded
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.ControlledDCDrives.Utilities.IdealDcDc \
      --target Ti --sanitizer divisor
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.ControlledDCDrives.Utilities.IdealDcDc --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "Ti"
```
