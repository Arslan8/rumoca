...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:239:fe468c12` |
...
```
predicate     vehicleDragInVar.rho > 0
premise       refuted
...
| `required` | vehicleDragInVar.rho &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Translational.Vehicles \
      --target vehicleDragInVar.rho --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "vehicleDragInVar.rho"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param vehicleDragInVar.rho=0
```
