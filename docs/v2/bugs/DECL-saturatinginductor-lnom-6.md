# DECL-0183: `Lnom` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Polyphase/Basic/SaturatingInductor.mo:6` |
| **Parameter** | `Lnom` |
| **Type** | `SI.Inductance` |
...
```console
$ sed -n '6p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Basic/SaturatingInductor.mo"
```

It should declare `Lnom` with type `SI.Inductance` and no `min`.
...
```modelica
// Electrical/Polyphase/Basic/SaturatingInductor.mo:6
parameter SI.Inductance Lnom(min=0) = ...;
```
