# FINDING-05132: `ambient2.medium.cp` in `WaterPump`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Thermal.FluidHeatFlow.Examples.WaterPump` |
| **Reached as** | `ambient2.medium.cp` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/BaseClasses/SinglePortLeft.mo:20` |
| **Signature** | `divisor:divisor-reachable-zero:exp:1155:62a4256c` |
...
```
denominator            ambient2.medium.cp
witness                ambient2.medium.cp = 0
  found by             set to zero
denominator at witness 0.0
at declared values     4177.0
path condition         always evaluated
constraints consulted  ambient2.medium.cp: unbounded
```
...
| `constraints` | ambient2.medium.cp: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.WaterPump \
      --target ambient2.medium.cp --sanitizer divisor
```
...
```console
$ sed -n '20p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/BaseClasses/SinglePortLeft.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "ambient2.medium.cp"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/BaseClasses/SinglePortLeft.mo:20` |' docs/v2/bugs/
```
