...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:1:a94386c5` |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Thermal.HeatTransfer.Examples.Utilities.DirectCapacity \
      --target heatCapacitor.C --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Thermal.HeatTransfer.Examples.Utilities.DirectCapacity --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "heatCapacitor.C"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param heatCapacitor.C=0
```
