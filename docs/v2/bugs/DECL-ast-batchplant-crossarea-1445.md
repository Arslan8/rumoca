# DECL-0011: `crossArea` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Fluid/Examples/AST_BatchPlant.mo:1445` |
| **Parameter** | `crossArea` |
| **Type** | `SI.Area` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '1445p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Examples/AST_BatchPlant.mo"
```

It should declare `crossArea` with type `SI.Area` and no `min`.
...
```modelica
// Fluid/Examples/AST_BatchPlant.mo:1445
