# DECL-0017: `area_h` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Fluid/Examples/HeatExchanger.mo:407` |
| **Parameter** | `area_h` |
| **Type** | `SI.Area` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '407p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Examples/HeatExchanger.mo"
```

It should declare `area_h` with type `SI.Area` and no `min`.
...
```modelica
// Fluid/Examples/HeatExchanger.mo:407
parameter SI.Area area_h(min=0) = ...;
```
