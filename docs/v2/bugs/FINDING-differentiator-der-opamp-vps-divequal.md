...
| **Signature** | `divisor:divisor-zero-when-parameters-equal:exp:242:8bbeeb1f` |
...
```
denominator            (der_.opAmp.vps - der_.opAmp.vns)
witness                der_.opAmp.Vps = -15
  found by             set equal to der_.opAmp.Vns
denominator at witness 0.0
at declared values     30.0
path condition         always evaluated
constraints consulted  der_.opAmp.Vps: unbounded
```
...
| `denominator` | (der_.opAmp.vps - der_.opAmp.vns) |
| `witness` | der_.opAmp.Vps = -15 |
| `witness_rationale` | set equal to der_.opAmp.Vns |
...
| `constraints` | der_.opAmp.Vps: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.Differentiator \
      --target der_.opAmp.Vps --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "der_.opAmp.Vps"
```
