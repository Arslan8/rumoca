# DECL-0060: `GoffD` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Machines/Examples/ControlledDCDrives/Utilities/SwitchingDcDc.mo:13` |
| **Parameter** | `GoffD` |
| **Type** | `SI.Conductance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '13p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/ControlledDCDrives/Utilities/SwitchingDcDc.mo"
```

It should declare `GoffD` with type `SI.Conductance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/ControlledDCDrives/Utilities/SwitchingDcDc.mo" | head
```
...
```modelica
// Electrical/Machines/Examples/ControlledDCDrives/Utilities/SwitchingDcDc.mo:13
parameter SI.Conductance GoffD(min=0) = ...;
```
