...
| **Reached as** | `rightLeg.material.B_myMax` |
...
| **Signature** | `divisor:divisor-reachable-zero:exp:489:89e6cc1e` |
...
```
denominator            rightLeg.material.B_myMax
witness                rightLeg.material.B_myMax = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  rightLeg.material.B_myMax: unbounded
```
...
| `constraints` | rightLeg.material.B_myMax: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.BasicExamples.QuadraticCoreAirgap \
      --target rightLeg.material.B_myMax --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "rightLeg.material.B_myMax"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param rightLeg.material.B_myMax=0
```
