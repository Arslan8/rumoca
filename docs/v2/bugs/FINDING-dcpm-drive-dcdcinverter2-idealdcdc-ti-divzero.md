# FINDING-01531: `dcdcInverter2.idealDcDc.Ti` in `DCPM_Drive`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Drive` |
| **Reached as** | `dcdcInverter2.idealDcDc.Ti` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/ControlledDCDrives/Utilities/IdealDcDc.mo:31` |
| **Signature** | `divisor:divisor-reachable-zero:exp:1673:baf4702f` |
...
```
denominator            dcdcInverter2.idealDcDc.Ti
witness                dcdcInverter2.idealDcDc.Ti = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1e-06
path condition         always evaluated
constraints consulted  dcdcInverter2.idealDcDc.Ti: unbounded
```
...
| `constraints` | dcdcInverter2.idealDcDc.Ti: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Drive \
      --target dcdcInverter2.idealDcDc.Ti --sanitizer divisor
```
...
```console
$ sed -n '31p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/ControlledDCDrives/Utilities/IdealDcDc.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcdcInverter2.idealDcDc.Ti"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param dcdcInverter2.idealDcDc.Ti=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/ControlledDCDrives/Utilities/IdealDcDc.mo:31` |' docs/v2/bugs/
```
