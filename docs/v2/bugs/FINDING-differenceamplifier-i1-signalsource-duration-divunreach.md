...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:708:0d2f0fed` |
...
```
denominator            I1.signalSource.duration
witness                I1.signalSource.duration = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1e-09
path condition         not ((time < I1.signalSource.startTime)) and ((time < (I1.signalSource.startTime + I1.signalSource.duration)))
constraints consulted  I1.signalSource.duration: min=0
```
...
| `path_condition` | not ((time &lt; I1.signalSource.startTime)) and ((time &lt; (I1.signalSource.startTime + I1.signalSource.duration))) |
| `verdict` | UNSAT |
| `proof` | the path conditions cannot hold together: time must be both &gt; 0 and &lt; 0 |
| `constraints` | I1.signalSource.duration: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.DifferenceAmplifier \
      --target I1.signalSource.duration --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "I1.signalSource.duration"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param I1.signalSource.duration=0
```
