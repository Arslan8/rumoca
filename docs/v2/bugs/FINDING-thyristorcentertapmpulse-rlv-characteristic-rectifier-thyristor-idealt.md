# FINDING-03921: `rectifier.thyristor.idealThyristor[1].Ron` in `ThyristorCenterTapmPulse_RLV_Characteristic`
...
| **Reached as** | `rectifier.thyristor.idealThyristor[1].Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:162:5453d780` |
...
```
predicate     rectifier.thyristor.idealThyristor[1].Ron > 0
premise       refuted
...
| `required` | rectifier.thyristor.idealThyristor[1].Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTapmPulse.ThyristorCenterTapmPulse_RLV_Characteristic \
      --target rectifier.thyristor.idealThyristor[1].Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "rectifier.thyristor.idealThyristor[1].Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param rectifier.thyristor.idealThyristor[1].Ron=0
```
