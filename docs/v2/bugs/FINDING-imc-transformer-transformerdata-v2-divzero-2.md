# FINDING-02099: `transformerData.V2` in `IMC_Transformer`
...
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/TransformerData.mo:24` |
| **Signature** | `divisor:divisor-reachable-zero:exp:2129:e51771c0` |
...
```
denominator            transformerData.V2
witness                transformerData.V2 = 0
  found by             set to zero
denominator at witness 0.0
at declared values     100.0
path condition         always evaluated
constraints consulted  transformerData.V2: unbounded
```
...
| `constraints` | transformerData.V2: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_Transformer \
      --target transformerData.V2 --sanitizer divisor
```
...
```console
$ sed -n '24p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/TransformerData.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "transformerData.V2"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/TransformerData.mo:24` |' docs/v2/bugs/
```
