...
| **Reach** | 12 models set this declaration |
...
`Modelica.Electrical.Analog.Examples.ChuaCircuit` runs cleanly at its declared values. Setting `C2.C = 0` — a value the declaration permits — makes it fail, in Rumoca and in OpenModelica.
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.ChuaCircuit \
      --target C2.C --keep-parameter-chains
```
...
simulate(Modelica.Electrical.Analog.Examples.ChuaCircuit, simflags="-override C2.C=0");
EOF
```
