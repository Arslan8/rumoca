...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:47:71ae4912` |
...
```
contract    Modelica.Electrical.Analog.Basic.Resistor.R: feature_disabled (proven, from equation) — inherited from firstOrder.r1.R, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.LowPass \
      --target firstOrder.R1 --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "firstOrder.R1"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param firstOrder.R1=0
```
