...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:714:2b697dfa` |
...
path condition         ((abs(coil.i) > coil.eps))
...
| `denominator_range` |  |
| `path_condition` | ((abs(coil.i) &gt; coil.eps)) |
| `verdict` | UNSAT |
| `proof` | the denominator is zero as declared, and ((abs(coil.i) &gt; coil.eps)) |
| `constraints` |  |
| `guard_that_excludes_it` | ((abs(coil.i) &gt; coil.eps)) |
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.SolenoidActuator.Components.SimpleSolenoid \
      --target coil.i --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "coil.i"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param coil.i=0
```
