# FINDING-03973: `inverter.GoffTransistor` in `SinglePhaseTwoLevel_R`
...
| **Reached as** | `inverter.GoffTransistor` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:45:ac848d63` |
...
```
predicate     inverter.GoffTransistor > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealGTOThyristor.Goff: feature_disabled (proven, from equation) — inherited from inverter.transistor_p.Goff, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
| `required` | inverter.GoffTransistor &gt; 0 |
| `declared_min` | None |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCAC.SinglePhaseTwoLevel.SinglePhaseTwoLevel_R \
      --target inverter.GoffTransistor --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCAC.SinglePhaseTwoLevel.SinglePhaseTwoLevel_R --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inverter.GoffTransistor"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inverter.GoffTransistor=0
```
