...
| **Reached as** | `dcpmData2.brushParameters.ILinear` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-zero-unresolved` |
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Losses/DCMachines/Brush.mo:15` |
| **Signature** | `divisor:divisor-zero-unresolved:exp:2187:07b31ccf` |
...
```
denominator            dcpmData2.brushParameters.ILinear
witness                dcpmData2.brushParameters.ILinear = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         not ((dcpm2.brush.i > plus dcpmData2.brushParameters.ILinear)) and not ((dcpm2.brush.i < -dcpmData2.brushParameters.ILinear))
constraints consulted  dcpmData2.brushParameters.ILinear: unbounded
```
...
| `path_condition` | not ((dcpm2.brush.i &gt; plus dcpmData2.brushParameters.ILinear)) and not ((dcpm2.brush.i &lt; -dcpmData2.brushParameters.ILinear)) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: (dcpm2.brush.i &lt; -dcpmData2.brushParameters.ILinear); (dcpm2.brush.i &gt; plus dcpmData2.brushParameters.ILinear) |
| `constraints` | dcpmData2.brushParameters.ILinear: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_withLosses \
      --target dcpmData2.brushParameters.ILinear --sanitizer divisor
```
...
```console
$ sed -n '15p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Losses/DCMachines/Brush.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcpmData2.brushParameters.ILinear"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param dcpmData2.brushParameters.ILinear=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Losses/DCMachines/Brush.mo:15` |' docs/v2/bugs/
```
