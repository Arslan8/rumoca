# DECL-0266: `Lrsigmaq` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/FundamentalWave/Examples/BasicMachines/SynchronousMachines/ComparisonPolyphase/SMEE_Generator_Polyphase.mo:30` |
| **Parameter** | `Lrsigmaq` |
| **Type** | `SI.Inductance` |
| **Declared modifiers** | `—` |
...
`SI.Inductance` declares no `min` in `Units.mo`, and this declaration adds none of its own. It therefore accepts a negative value, and stored energy is `L*i^2/2`, which a negative `L` makes negative.
...
```console
$ sed -n '30p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FundamentalWave/Examples/BasicMachines/SynchronousMachines/ComparisonPolyphase/SMEE_Generator_Polyphase.mo"
```

It should declare `Lrsigmaq` with type `SI.Inductance` and no `min`.

**2. Confirm the type supplies no bound either** — this is the whole claim, and
a bound inherited from the type would refute it:

```console
$ grep -n "type Inductance " "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Units.mo"
```
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FundamentalWave/Examples/BasicMachines/SynchronousMachines/ComparisonPolyphase/SMEE_Generator_Polyphase.mo" | head
```
...
- `SI.Inductance` turns out to carry a `min` after all, or this declaration carries one that the catalog missed.
...
```modelica
// Magnetic/FundamentalWave/Examples/BasicMachines/SynchronousMachines/ComparisonPolyphase/SMEE_Generator_Polyphase.mo:30
parameter SI.Inductance Lrsigmaq(min=0) = ...;
```
