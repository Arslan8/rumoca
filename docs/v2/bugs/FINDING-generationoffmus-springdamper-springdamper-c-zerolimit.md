...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Examples/Utilities/SpringDamperNoRelativeStates.mo:4` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:121:fd306f9d` |
...
```
predicate     springDamper.springDamper.c > 0
premise       refuted
...
```
contract    Modelica.Mechanics.Translational.Examples.Utilities.SpringDamperNoRelativeStates.c: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'TranslationalSpringConstant', 'unit': 'N/m', 'confidence': 'QUANTITY', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Mechanics.Translational.Examples.Utilities.SpringDamperNoRelativeStates.c'}
```
...
| `required` | springDamper.springDamper.c &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Translational.Examples.GenerationOfFMUs \
      --target springDamper.springDamper.c --sanitizer physical
```
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Examples/Utilities/SpringDamperNoRelativeStates.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "springDamper.springDamper.c"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param springDamper.springDamper.c=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Examples/Utilities/SpringDamperNoRelativeStates.mo:4` |' docs/v2/bugs/
```
