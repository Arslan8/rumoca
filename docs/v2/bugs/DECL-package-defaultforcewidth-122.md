# DECL-0563: `defaultForceWidth` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/package.mo:122` |
| **Parameter** | `defaultForceWidth` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '122p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/package.mo"
```

It should declare `defaultForceWidth` with type `SI.Length` and no `min`.
...
```modelica
// Mechanics/MultiBody/package.mo:122
parameter SI.Length defaultForceWidth(min=0) = ...;
```
