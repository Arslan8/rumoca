...
| **Signature** | `divisor:divisor-reachable-zero:exp:1012:86d3b02f` |
...
| `constraints` | pipe.medium.cp: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.WaterPump \
      --target pipe.medium.cp --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pipe.medium.cp"
```
