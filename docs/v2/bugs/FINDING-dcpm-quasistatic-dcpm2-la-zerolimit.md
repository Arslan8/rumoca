# FINDING-01551: `dcpm2.La` in `DCPM_QuasiStatic`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:349:9b343649` |
...
```
contract    Modelica.Electrical.Machines.BasicMachines.Components.InductorDC.L: algebraic_limit (proven, from equation) — inherited from dcpm2.la.L, which it binds: the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_QuasiStatic \
      --target dcpm2.La --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcpm2.La"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param dcpm2.La=0
```
