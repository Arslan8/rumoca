# FINDING-01934: `aimcData.fsNominal` in `IMC_InverterDrive`
...
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/InductionMachines/IMC_InverterDrive.mo:106` |
| **Signature** | `divisor:divisor-reachable-zero:exp:2771:ba3b25cb` |
...
```
denominator            ((2 * (2 * asin(1.0))) * aimcData.fsNominal)
witness                aimcData.fsNominal = 0
  found by             set to zero
denominator at witness 0.0
at declared values     314.1592653589793
path condition         always evaluated
constraints consulted  aimcData.fsNominal: unbounded
```
...
| `constraints` | aimcData.fsNominal: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive \
      --target aimcData.fsNominal --sanitizer divisor
```
...
```console
$ sed -n '106p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/InductionMachines/IMC_InverterDrive.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "aimcData.fsNominal"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/InductionMachines/IMC_InverterDrive.mo:106` |' docs/v2/bugs/
```
