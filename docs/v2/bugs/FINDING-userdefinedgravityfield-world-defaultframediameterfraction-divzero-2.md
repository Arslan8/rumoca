# FINDING-04549: `world.defaultFrameDiameterFraction` in `UserDefinedGravityField`
...
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/package.mo:47` |
| **Signature** | `divisor:divisor-reachable-zero:exp:1537:515ec69b` |
...
```
denominator            world.defaultFrameDiameterFraction
witness                world.defaultFrameDiameterFraction = 0
  found by             set to zero
denominator at witness 0.0
at declared values     40.0
path condition         always evaluated
constraints consulted  world.defaultFrameDiameterFraction: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Elementary.UserDefinedGravityField \
      --target world.defaultFrameDiameterFraction --sanitizer divisor
```
...
```console
$ sed -n '47p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/package.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "world.defaultFrameDiameterFraction"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param world.defaultFrameDiameterFraction=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/package.mo:47` |' docs/v2/bugs/
```
