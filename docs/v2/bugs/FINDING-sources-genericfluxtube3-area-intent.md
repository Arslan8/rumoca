...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/FixedShape/GenericFluxTube.mo:11` |
| **Signature** | `physical:physical-intent-question:var:129:4ebe2630` |
...
```
predicate     genericFluxTube3.area > 0
premise       unknown
authority     quantity_or_unit
role          —
declaration   Modelica.Magnetic.FluxTubes.Shapes.FixedShape.GenericFluxTube.area
```
...
> `genericFluxTube3.area` carries no bound that would exclude it, which is unusual for an area — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
```
{'quantity': 'Area', 'unit': 'm2', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Magnetic.FluxTubes.Shapes.FixedShape.GenericFluxTube.area'}
```
...
| `required` | genericFluxTube3.area &gt; 0 |
| `declared_min` | None |
| `variable_role` | parameter |
| `observation` | permissive-declaration |
| `premise_state` | unknown |
| `authority` | quantity_or_unit |
| `canonical_declaration` | Modelica.Magnetic.FluxTubes.Shapes.FixedShape.GenericFluxTube.area |
| `question` | `genericFluxTube3.area` carries no bound that would exclude it, which is unusual for an area — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Magnetic.FluxTubes.Sources \
      --target genericFluxTube3.area --sanitizer physical
```
...
```console
$ sed -n '11p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/FixedShape/GenericFluxTube.mo"
```
...
```console
$ python3 tools/sweep/check_one.py ModelicaTest.Magnetic.FluxTubes.Sources --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "genericFluxTube3.area"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param genericFluxTube3.area=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/FixedShape/GenericFluxTube.mo:11` |' docs/v2/bugs/
```
