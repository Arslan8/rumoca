# FINDING-04558: `body3.I` in `PlanarFourbar`
...
| **Signature** | `physical:physical-inertia-tensor-undecided:par:480:c1a5f764` |
...
This one is an **aggregate** rule: its subject is the `symmetric_inertia_tensor` that `body3` assembles from several declarations, not any one of them. The fields it covers are excluded from the scalar rule, so this finding is the only thing said about them.

```
matrix       [[I_11, I_21, I_31], [I_21, I_22, I_32], [I_31, I_32, I_33]] (not numerically known)
eigenvalues  not computed
verdict      unknown
reason       `body3.I` is declared as a 3x3 tensor whose entries this analysis cannot evaluate, so neither its validity nor its invalidity is claimed
```
...
| `fields` | body3.I |
| `matrix` | [[I_11, I_21, I_31], [I_21, I_22, I_32], [I_31, I_32, I_33]] (not numerically known) |
| `required` | body3.I is positive semidefinite |
| `verdict` | unknown |
| `reason` | `body3.I` is declared as a 3x3 tensor whose entries this analysis cannot evaluate, so neither its validity nor its invalidity is claimed |
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Loops.PlanarFourbar \
      --target body3.I --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "body3.I"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param body3.I=0
```
