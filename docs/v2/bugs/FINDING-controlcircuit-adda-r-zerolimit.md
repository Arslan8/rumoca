# FINDING-00808: `addA.R` in `ControlCircuit`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/OpAmps/OpAmpCircuits/Add.mo:8` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:311:3c1e9745` |
...
```
contract    Modelica.Electrical.Analog.Basic.Resistor.R: feature_disabled (proven, from equation) — inherited from addA.r.R, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Examples.OpAmps.OpAmpCircuits.Add.R'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.ControlCircuit \
      --target addA.R --sanitizer physical
```
...
```console
$ sed -n '8p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/OpAmps/OpAmpCircuits/Add.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "addA.R"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param addA.R=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/OpAmps/OpAmpCircuits/Add.mo:8` |' docs/v2/bugs/
```
