# DECL-0562: `defaultForceLength` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/package.mo:119` |
| **Parameter** | `defaultForceLength` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '119p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/package.mo"
```

It should declare `defaultForceLength` with type `SI.Length` and no `min`.
...
```modelica
// Mechanics/MultiBody/package.mo:119
parameter SI.Length defaultForceLength(min=0) = ...;
```
