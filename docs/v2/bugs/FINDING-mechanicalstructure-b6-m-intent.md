# FINDING-04616: `b6.m` in `MechanicalStructure`
...
| **Signature** | `physical:physical-intent-question:var:1132:79f552ce` |
...
> `b6.m` is bounded only by min=0, which is unusual for a mass — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
| `question` | `b6.m` is bounded only by min=0, which is unusual for a mass — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Systems.RobotR3.Utilities.MechanicalStructure \
      --target b6.m --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "b6.m"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param b6.m=0
```
