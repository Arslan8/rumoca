# FINDING-02479: `smee.fsNominal` in `SMEE_Generator`
...
| **Reached as** | `smee.fsNominal` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/SynchronousMachines/SM_ElectricalExcited.mo:159` |
| **Signature** | `divisor:divisor-reachable-zero:exp:1047:f038776b` |
...
```
denominator            ((((2 * smee.pi) * smee.fsNominal) * smee.Lmd) * smee.IeOpenCircuit)
witness                smee.fsNominal = 0
  found by             set to zero
denominator at witness 0.0
at declared values     15.0
path condition         always evaluated
constraints consulted  smee.fsNominal: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMEE_Generator \
      --target smee.fsNominal --sanitizer divisor
```
...
```console
$ sed -n '159p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/SynchronousMachines/SM_ElectricalExcited.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smee.fsNominal"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/SynchronousMachines/SM_ElectricalExcited.mo:159` |' docs/v2/bugs/
```
