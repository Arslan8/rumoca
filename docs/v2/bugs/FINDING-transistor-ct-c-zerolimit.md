# FINDING-01249: `ct.C` in `Transistor`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:34:9f0fb762` |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Utilities.Transistor \
      --target ct.C --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Utilities.Transistor --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "ct.C"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param ct.C=0
```
