...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:495:73681e1d` |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Elementary.DoublePendulum \
      --target boxBody2.density --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Elementary.DoublePendulum --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "boxBody2.density"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param boxBody2.density=0
```
