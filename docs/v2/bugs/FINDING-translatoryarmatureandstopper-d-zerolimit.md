...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:3:9df8309b` |
...
```
contract    Modelica.Mechanics.Translational.Components.ElastoGap.d: algebraic_limit (proven, from equation) — inherited from stopper_xMax.d, which it binds: the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
