# FINDING-04303: `g_mAirWork.A` in `AdvancedSolenoid`
...
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/Force/HollowCylinderAxialFlux.mo:24` |
| **Signature** | `divisor:divisor-reachable-zero:exp:1240:ea330b5f` |
...
```
denominator            g_mAirWork.A
witness                g_mAirWork.A = 0
  found by             set to zero
denominator at witness 0.0
at declared values     7.853981633974483e-05
path condition         always evaluated
constraints consulted  g_mAirWork.A: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.SolenoidActuator.Components.AdvancedSolenoid \
      --target g_mAirWork.A --sanitizer divisor
```
...
```console
$ sed -n '24p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/Force/HollowCylinderAxialFlux.mo"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/Force/HollowCylinderAxialFlux.mo:24` |' docs/v2/bugs/
```
