# DECL-0558: `nominalLength` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/package.mo:108` |
| **Parameter** | `nominalLength` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '108p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/package.mo"
```

It should declare `nominalLength` with type `SI.Length` and no `min`.
...
```modelica
// Mechanics/MultiBody/package.mo:108
parameter SI.Length nominalLength(min=0) = ...;
```
