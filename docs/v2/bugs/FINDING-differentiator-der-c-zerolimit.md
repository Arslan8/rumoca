# FINDING-00843: `der_.C` in `Differentiator`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/OpAmps/OpAmpCircuits/Der.mo:8` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:76:d6b619c3` |
...
```
contract    Modelica.Electrical.Analog.Basic.Capacitor.C: algebraic_limit (proven, from equation) — inherited from der_.c.C, which it binds: the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
...
```
{'quantity': 'Capacitance', 'unit': 'F', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Examples.OpAmps.OpAmpCircuits.Der.C'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.Differentiator \
      --target der_.C --sanitizer physical
```
...
```console
$ sed -n '8p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/OpAmps/OpAmpCircuits/Der.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "der_.C"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param der_.C=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/OpAmps/OpAmpCircuits/Der.mo:8` |' docs/v2/bugs/
```
