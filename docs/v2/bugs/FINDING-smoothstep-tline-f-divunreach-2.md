...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:6673:a908f437` |
...
| `denominator_range` |  |
| `path_condition` | ((tLine.F &gt; 0)) |
| `verdict` | UNSAT |
| `proof` | the denominator is zero as declared, and ((tLine.F &gt; 0)) |
| `constraints` |  |
| `guard_that_excludes_it` | ((tLine.F &gt; 0)) |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.SmoothStep \
      --target tLine.F --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "tLine.F"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param tLine.F=0
```
