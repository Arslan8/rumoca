# FINDING-01844: `inductor.inductor[2].L` in `IMC_InverterDrive`
...
| **Reached as** | `inductor.inductor[2].L` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:257:5495e3a2` |
...
```
predicate     inductor.inductor[2].L > 0
premise       refuted
...
| `required` | inductor.inductor[2].L &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive \
      --target inductor.inductor[2].L --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inductor.inductor[2].L"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inductor.inductor[2].L=0
```
