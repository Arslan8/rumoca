# FINDING-03897: `rectifier.diode.Goff` in `DiodeCenterTapmPulse`
...
| **Reached as** | `rectifier.diode.Goff` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:137:d5a50346` |
...
```
predicate     rectifier.diode.Goff > 0
premise       refuted
...
| `required` | rectifier.diode.Goff &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTapmPulse.DiodeCenterTapmPulse \
      --target rectifier.diode.Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "rectifier.diode.Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param rectifier.diode.Goff=0
```
