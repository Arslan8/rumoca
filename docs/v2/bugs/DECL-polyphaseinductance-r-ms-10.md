# DECL-0593: `R_ms` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/FundamentalWave/Examples/Components/PolyphaseInductance.mo:10` |
| **Parameter** | `R_ms` |
| **Type** | `SI.Reluctance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '10p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FundamentalWave/Examples/Components/PolyphaseInductance.mo"
```

It should declare `R_ms` with type `SI.Reluctance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FundamentalWave/Examples/Components/PolyphaseInductance.mo" | head
```
...
```modelica
// Magnetic/FundamentalWave/Examples/Components/PolyphaseInductance.mo:10
parameter SI.Reluctance R_ms(min=0) = ...;
```
