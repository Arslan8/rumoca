...
| **Signature** | `physical:physical-rule-does-not-apply:var:38:38a38ddb` |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.DemoPowerSupplyWithBuffer \
      --target diode.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "diode.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param diode.Ron=0
```
