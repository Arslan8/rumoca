# FINDING-04061: `chopperStepUp.diode.Ron` in `ChopperStepUp_R`
...
| **Reached as** | `chopperStepUp.diode.Ron` |
...
```
predicate     chopperStepUp.diode.Ron > 0
premise       refuted
...
| `required` | chopperStepUp.diode.Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCDC.ChopperStepUp.ChopperStepUp_R \
      --target chopperStepUp.diode.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "chopperStepUp.diode.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param chopperStepUp.diode.Ron=0
```
