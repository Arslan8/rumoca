# FINDING-01136: `SaturatingInductance1.Lzer` in `ShowSaturatingInductor`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/SaturatingInductor.mo:12` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:35:ff68f4dc` |
...
```
predicate     SaturatingInductance1.Lzer > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Basic.SaturatingInductor.Lzer: forbidden (proven, from bound) — the declaration excludes zero (assert SaturatingInductance1.Lzer greater 1)
behavior    forbidden
confidence  proven
source      bound
```
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Basic.SaturatingInductor.Lzer'}
```
...
| `required` | SaturatingInductance1.Lzer &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.ShowSaturatingInductor \
      --target SaturatingInductance1.Lzer --sanitizer physical
```
...
```console
$ sed -n '12p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/SaturatingInductor.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "SaturatingInductance1.Lzer"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param SaturatingInductance1.Lzer=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/SaturatingInductor.mo:12` |' docs/v2/bugs/
```
