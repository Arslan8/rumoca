# FINDING-05322: `eddyCurrent.A` in `BasicComponents`
...
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Basic/EddyCurrent.mo:20` |
| **Signature** | `physical:physical-domain-unenforced:var:71:1479f3b1` |
...
```
predicate     eddyCurrent.A > 0
premise       established
authority     source_arithmetic
role          —
declaration   Modelica.Magnetic.FluxTubes.Basic.EddyCurrent.A
```
...
```
{'quantity': 'Area', 'unit': 'm2', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Magnetic.FluxTubes.Basic.EddyCurrent.A'}
```
...
| `authority` | source_arithmetic |
| `canonical_declaration` | Modelica.Magnetic.FluxTubes.Basic.EddyCurrent.A |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Magnetic.FluxTubes.BasicComponents \
      --target eddyCurrent.A --sanitizer physical
```
...
```console
$ sed -n '20p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Basic/EddyCurrent.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "eddyCurrent.A"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param eddyCurrent.A=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Basic/EddyCurrent.mo:20` |' docs/v2/bugs/
```
