...
| **Signature** | `divisor:divisor-reachable-zero:exp:528:96af3542` |
...
```
denominator            ((outerPipe.V_flowNominal - outerPipe.V_flowLaminar) ^ 2)
witness                outerPipe.V_flowNominal = 0.1
  found by             at its declared minimum
denominator at witness 0.0
at declared values     0.81
path condition         always evaluated
constraints consulted  outerPipe.V_flowNominal: min=0.1, assert outerPipe.V_flowNominal greater 0.1
```
...
| `constraints` | outerPipe.V_flowNominal: min=0.1, assert outerPipe.V_flowNominal greater 0.1 |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.IndirectCooling \
      --target outerPipe.V_flowNominal --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "outerPipe.V_flowNominal"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param outerPipe.V_flowNominal=0.1
```
