...
| **Signature** | `divisor:divisor-zero-when-parameters-equal:exp:234:18dcb2d9` |
...
```
denominator            (feedback.opAmp.vps - feedback.opAmp.vns)
witness                feedback.opAmp.Vps = -15
  found by             set equal to feedback.opAmp.Vns
denominator at witness 0.0
at declared values     30.0
path condition         always evaluated
constraints consulted  feedback.opAmp.Vps: unbounded
```
...
| `denominator` | (feedback.opAmp.vps - feedback.opAmp.vns) |
| `witness` | feedback.opAmp.Vps = -15 |
| `witness_rationale` | set equal to feedback.opAmp.Vns |
...
| `constraints` | feedback.opAmp.Vps: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.Subtracter \
      --target feedback.opAmp.Vps --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "feedback.opAmp.Vps"
```
