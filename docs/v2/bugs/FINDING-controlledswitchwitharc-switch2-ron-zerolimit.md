...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:79:95a69974` |
...
```
contract    Modelica.Electrical.Analog.Ideal.ControlledCloserWithArc.Ron: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Ideal.ControlledCloserWithArc.Ron'}
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.ControlledSwitchWithArc \
      --target switch2.Ron --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.ControlledSwitchWithArc --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switch2.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switch2.Ron=0
```
