...
`Modelica.Mechanics.Rotational.Examples.ElasticBearing` runs cleanly at its declared values. Setting `housing.J = 0` — a value the declaration permits — makes it fail, in Rumoca and in OpenModelica.
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Rotational.Examples.ElasticBearing \
      --target housing.J --keep-parameter-chains
```
...
simulate(Modelica.Mechanics.Rotational.Examples.ElasticBearing, simflags="-override housing.J=0");
EOF
```
