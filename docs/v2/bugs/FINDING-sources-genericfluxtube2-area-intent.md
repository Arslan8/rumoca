...
| **Signature** | `physical:physical-intent-question:var:106:7b488e31` |
...
```
predicate     genericFluxTube2.area > 0
premise       unknown
...
| `required` | genericFluxTube2.area &gt; 0 |
| `declared_min` | None |
...
| `question` | `genericFluxTube2.area` carries no bound that would exclude it, which is unusual for an area — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Magnetic.FluxTubes.Sources \
      --target genericFluxTube2.area --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "genericFluxTube2.area"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param genericFluxTube2.area=0
```
