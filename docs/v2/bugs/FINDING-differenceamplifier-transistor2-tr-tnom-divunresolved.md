# FINDING-00299: `Transistor2.Tr.Tnom` in `DifferenceAmplifier`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Electrical.Analog.Examples.DifferenceAmplifier` |
| **Reached as** | `Transistor2.Tr.Tnom` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-zero-unresolved` |
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/NPN.mo:74` |
| **Signature** | `divisor:divisor-zero-unresolved:exp:1503:92b3866d` |
...
```
denominator            Transistor2.Tr.Tnom
witness                Transistor2.Tr.Tnom = 0
  found by             set to zero
denominator at witness 0.0
at declared values     300.15
path condition         (Transistor2.Tr.useTemperatureDependency)
constraints consulted  Transistor2.Tr.Tnom: min=0
```
...
| `denominator_at_declared_values` | 300.15 |
| `denominator_range` | [0, inf] |
| `path_condition` | (Transistor2.Tr.useTemperatureDependency) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: Transistor2.Tr.useTemperatureDependency |
| `constraints` | Transistor2.Tr.Tnom: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.DifferenceAmplifier \
      --target Transistor2.Tr.Tnom --sanitizer divisor
```
...
```console
$ sed -n '74p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/NPN.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "Transistor2.Tr.Tnom"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/NPN.mo:74` |' docs/v2/bugs/
```
