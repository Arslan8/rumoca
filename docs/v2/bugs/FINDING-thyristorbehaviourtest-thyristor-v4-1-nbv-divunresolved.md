...
| **Signature** | `divisor:divisor-zero-unresolved:exp:427:01087622` |
...
```
denominator            (thyristor_v4_1.Nbv * thyristor_v4_1.Vt)
witness                thyristor_v4_1.Nbv = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.0296
path condition         ((thyristor_v4_1.vAK < -thyristor_v4_1.VRRM))
constraints consulted  thyristor_v4_1.Nbv: unbounded
```
...
| `path_condition` | ((thyristor_v4_1.vAK &lt; -thyristor_v4_1.VRRM)) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: (thyristor_v4_1.vAK &lt; -thyristor_v4_1.VRRM) |
| `constraints` | thyristor_v4_1.Nbv: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.ThyristorBehaviourTest \
      --target thyristor_v4_1.Nbv --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "thyristor_v4_1.Nbv"
```
