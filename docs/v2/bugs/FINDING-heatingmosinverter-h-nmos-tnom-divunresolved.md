...
| **Signature** | `divisor:divisor-zero-unresolved:exp:670:6a74c375` |
...
```
denominator            H_NMOS.Tnom
witness                H_NMOS.Tnom = 0
  found by             set to zero
denominator at witness 0.0
at declared values     300.15
path condition         (H_NMOS.useTemperatureDependency)
constraints consulted  H_NMOS.Tnom: min=0
```
...
| `path_condition` | (H_NMOS.useTemperatureDependency) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: H_NMOS.useTemperatureDependency |
| `constraints` | H_NMOS.Tnom: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.HeatingMOSInverter \
      --target H_NMOS.Tnom --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "H_NMOS.Tnom"
```
