# DECL-0902: `clearance` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Fluid/Machines.mo:10` |
| **Parameter** | `clearance` |
| **Type** | `SI.Volume` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '10p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Machines.mo"
```

It should declare `clearance` with type `SI.Volume` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Machines.mo" | head
```
...
```modelica
// Fluid/Machines.mo:10
parameter SI.Volume clearance(min=0) = ...;
```
