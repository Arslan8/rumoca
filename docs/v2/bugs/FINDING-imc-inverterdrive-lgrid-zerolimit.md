...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/InductionMachines/IMC_InverterDrive.mo:12` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:4:d1df16ad` |
...
```
contract    Modelica.Electrical.Analog.Basic.Inductor.L: algebraic_limit (declared, from class_catalog) — inherited from inductor.L, which it binds: L*der(i) = v; at L = 0 the element is an ideal short, v = 0, which the component documentation states explicitly
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive.LGrid'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive \
      --target LGrid --sanitizer physical
```
...
```console
$ sed -n '12p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/InductionMachines/IMC_InverterDrive.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "LGrid"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param LGrid=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/InductionMachines/IMC_InverterDrive.mo:12` |' docs/v2/bugs/
```
