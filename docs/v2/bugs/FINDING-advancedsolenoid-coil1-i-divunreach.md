# FINDING-04296: `coil1.i` in `AdvancedSolenoid`
...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:953:071ae022` |
...
path condition         ((abs(coil1.i) > coil1.eps))
...
| `denominator_range` |  |
| `path_condition` | ((abs(coil1.i) &gt; coil1.eps)) |
| `verdict` | UNSAT |
| `proof` | the denominator is zero as declared, and ((abs(coil1.i) &gt; coil1.eps)) |
| `constraints` |  |
| `guard_that_excludes_it` | ((abs(coil1.i) &gt; coil1.eps)) |
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.SolenoidActuator.Components.AdvancedSolenoid \
      --target coil1.i --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "coil1.i"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param coil1.i=0
```
