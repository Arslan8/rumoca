...
| **Reached as** | `Transistor2.Tr.Phie` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/NPN.mo:82` |
| **Signature** | `divisor:divisor-reachable-zero:exp:1602:24115f0e` |
...
```
denominator            Transistor2.Tr.Phie
witness                Transistor2.Tr.Phie = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.8
path condition         always evaluated
constraints consulted  Transistor2.Tr.Phie: unbounded
```
...
| `constraints` | Transistor2.Tr.Phie: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.DifferenceAmplifier \
      --target Transistor2.Tr.Phie --sanitizer divisor
```
...
```console
$ sed -n '82p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/NPN.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "Transistor2.Tr.Phie"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/NPN.mo:82` |' docs/v2/bugs/
```
