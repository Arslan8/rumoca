# FINDING-04234: `cActuator.armature.d` in `ArmatureStroke`
...
| **Reached as** | `cActuator.armature.d` |
...
| **Signature** | `physical:physical-intent-question:var:269:fd22fe35` |
...
```
predicate     cActuator.armature.d >= 0
premise       unknown
...
> `cActuator.armature.d` carries no bound that would exclude it, which is unusual for a translational damping constant — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
| `required` | cActuator.armature.d &gt;= 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.ArmatureStroke \
      --target cActuator.armature.d --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "cActuator.armature.d"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param cActuator.armature.d=0
```
