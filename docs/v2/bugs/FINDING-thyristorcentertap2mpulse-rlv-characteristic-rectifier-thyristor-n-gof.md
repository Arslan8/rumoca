# FINDING-03800: `rectifier.thyristor_n.Goff` in `ThyristorCenterTap2mPulse_RLV_Characteristic`
...
| **Signature** | `physical:physical-rule-does-not-apply:var:255:01de05b7` |
...
```
predicate     rectifier.thyristor_n.Goff > 0
premise       refuted
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTap2mPulse.ThyristorCenterTap2mPulse_RLV_Characteristic \
      --target rectifier.thyristor_n.Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "rectifier.thyristor_n.Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param rectifier.thyristor_n.Goff=0
```
