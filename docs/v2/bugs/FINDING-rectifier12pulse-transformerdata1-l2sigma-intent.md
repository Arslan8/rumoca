# FINDING-03143: `transformerData1.L2sigma` in `Rectifier12pulse`
...
| **Reached as** | `transformerData1.L2sigma` |
...
| **Signature** | `physical:physical-intent-question:var:341:4aecbb2a` |
...
```
predicate     transformerData1.L2sigma > 0
premise       unknown
...
> `transformerData1.L2sigma` carries no bound that would exclude it, which is unusual for an inductance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
| `required` | transformerData1.L2sigma &gt; 0 |
| `declared_min` | None |
...
| `question` | `transformerData1.L2sigma` carries no bound that would exclude it, which is unusual for an inductance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.Transformers.Rectifier12pulse \
      --target transformerData1.L2sigma --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "transformerData1.L2sigma"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param transformerData1.L2sigma=0
```
