# FINDING-04025: `LLoad` in `ChopperStepDown_RL`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/Examples/DCDC/ChopperStepDown/ChopperStepDown_RL.mo:5` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:170:e6d90daa` |
...
```
contract    Modelica.Electrical.Analog.Basic.Inductor.L: algebraic_limit (proven, from equation) — inherited from loadInductor.L, which it binds: the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.PowerConverters.Examples.DCDC.ChopperStepDown.ChopperStepDown_RL.LLoad'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCDC.ChopperStepDown.ChopperStepDown_RL \
      --target LLoad --sanitizer physical
```
...
```console
$ sed -n '5p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/Examples/DCDC/ChopperStepDown/ChopperStepDown_RL.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "LLoad"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param LLoad=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/Examples/DCDC/ChopperStepDown/ChopperStepDown_RL.mo:5` |' docs/v2/bugs/
```
