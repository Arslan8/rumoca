# FINDING-03493: `pulse2.twomPulse.filter[2].transferFunction[1].a_end` in `ThyristorBridge2mPulse_DC_Drive`
...
| **Reached as** | `pulse2.twomPulse.filter[2].transferFunction[1].a_end` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Blocks/Continuous.mo:1166` |
| **Signature** | `divisor:divisor-reachable-zero:exp:4200:3114ead4` |
...
```
denominator            pulse2.twomPulse.filter[2].transferFunction[1].a_end
witness                pulse2.twomPulse.filter[2].transferFunction[1].a_end = 0
  found by             set to zero
denominator at witness 0.0
at declared values     None
path condition         always evaluated
constraints consulted  pulse2.twomPulse.filter[2].transferFunction[1].a_end: unbounded
```
...
| `constraints` | pulse2.twomPulse.filter[2].transferFunction[1].a_end: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.ThyristorBridge2mPulse_DC_Drive \
      --target pulse2.twomPulse.filter[2].transferFunction[1].a_end --sanitizer divisor
```
...
```console
$ sed -n '1166p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Blocks/Continuous.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pulse2.twomPulse.filter[2].transferFunction[1].a_end"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param pulse2.twomPulse.filter[2].transferFunction[1].a_end=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Blocks/Continuous.mo:1166` |' docs/v2/bugs/
```
