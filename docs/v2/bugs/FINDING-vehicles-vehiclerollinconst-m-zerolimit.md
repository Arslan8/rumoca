...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:466:cfc7f1a6` |
...
```
predicate     vehicleRollInConst.m > 0
premise       refuted
...
| `required` | vehicleRollInConst.m &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Translational.Vehicles \
      --target vehicleRollInConst.m --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "vehicleRollInConst.m"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param vehicleRollInConst.m=0
```
