# FINDING-05340: `genericFluxTube1.material.B_myMax` in `Sources`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `ModelicaTest.Magnetic.FluxTubes.Sources` |
| **Reached as** | `genericFluxTube1.material.B_myMax` |
...
| **Signature** | `divisor:divisor-reachable-zero:exp:293:3cd296db` |
...
```
denominator            genericFluxTube1.material.B_myMax
witness                genericFluxTube1.material.B_myMax = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  genericFluxTube1.material.B_myMax: unbounded
```
...
| `constraints` | genericFluxTube1.material.B_myMax: unbounded |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Magnetic.FluxTubes.Sources \
      --target genericFluxTube1.material.B_myMax --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "genericFluxTube1.material.B_myMax"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param genericFluxTube1.material.B_myMax=0
```
