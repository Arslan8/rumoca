# FINDING-04593: `inertia2.rotorWith3DEffects.nJ` in `BevelGear1D`
...
| **Reached as** | `inertia2.rotorWith3DEffects.nJ` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/Rotor1D.mo:147` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:345:2ded39de` |
...
```
predicate     inertia2.rotorWith3DEffects.nJ > 0
premise       refuted
...
```
contract    Modelica.Mechanics.MultiBody.Parts.Rotor1D.RotorWith3DEffects.nJ: algebraic_limit (proven, from equation) — the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
...
```
{'quantity': 'MomentOfInertia', 'unit': 'kg.m2', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Mechanics.MultiBody.Parts.Rotor1D.RotorWith3DEffects.nJ'}
```
...
| `required` | inertia2.rotorWith3DEffects.nJ &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Rotational3DEffects.BevelGear1D \
      --target inertia2.rotorWith3DEffects.nJ --sanitizer physical
```
...
```console
$ sed -n '147p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/Rotor1D.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inertia2.rotorWith3DEffects.nJ"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inertia2.rotorWith3DEffects.nJ=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/Rotor1D.mo:147` |' docs/v2/bugs/
```
