# DECL-0157: `Lq` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Machines/Utilities/DQCurrentController.mo:13` |
| **Parameter** | `Lq` |
| **Type** | `SI.Inductance` |
| **Declared modifiers** | `—` |
...
It should declare `Lq` with type `SI.Inductance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/DQCurrentController.mo" | head
```
...
```modelica
// Electrical/Machines/Utilities/DQCurrentController.mo:13
parameter SI.Inductance Lq(min=0) = ...;
```
