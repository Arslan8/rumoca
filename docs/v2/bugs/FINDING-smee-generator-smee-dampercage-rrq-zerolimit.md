# FINDING-02459: `smee.damperCage.Rrq` in `SMEE_Generator`
...
| **Reached as** | `smee.damperCage.Rrq` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/Components/DamperCage.mo:9` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:443:2dffddf7` |
...
```
predicate     smee.damperCage.Rrq >= 0
premise       refuted
...
```
contract    Modelica.Electrical.Machines.BasicMachines.Components.DamperCage.Rrq: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'semantic_role': 'component.machine.winding_resistance', 'binding_source': 'component_type', 'confidence': 'QUANTITY_AND_UNIT', 'declaring_class': 'Modelica.Electrical.Machines.BasicMachines.Components.DamperCage', 'member': 'Rrq', 'premise_state': 'established', 'authority': 'component_contract', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.Components.DamperCage.Rrq'}
```
...
| `required` | smee.damperCage.Rrq &gt;= 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMEE_Generator \
      --target smee.damperCage.Rrq --sanitizer physical
```
...
```console
$ sed -n '9p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/Components/DamperCage.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smee.damperCage.Rrq"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param smee.damperCage.Rrq=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/Components/DamperCage.mo:9` |' docs/v2/bugs/
```
