# FINDING-03920: `rectifier.thyristor.Goff` in `ThyristorCenterTapmPulse_RLV_Characteristic`
...
| **Reached as** | `rectifier.thyristor.Goff` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:158:21159caa` |
...
```
predicate     rectifier.thyristor.Goff > 0
premise       refuted
...
| `required` | rectifier.thyristor.Goff &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTapmPulse.ThyristorCenterTapmPulse_RLV_Characteristic \
      --target rectifier.thyristor.Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "rectifier.thyristor.Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param rectifier.thyristor.Goff=0
```
