# FINDING-04305: `g_mFeYokeBot.l` in `AdvancedSolenoid`
...
| **Signature** | `divisor:divisor-introduced-by-translation:exp:1285:4d98aa91` |
...
```
contract    Modelica.Magnetic.FluxTubes.Shapes.FixedShape.HollowCylinderRadialFlux.l: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
behavior    
confidence  proven
source      
```
...
```
denominator            g_mFeYokeBot.A
witness                g_mFeYokeBot.l = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.00021056524760685588
path condition         always evaluated
constraints consulted  g_mFeYokeBot.l: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.SolenoidActuator.Components.AdvancedSolenoid \
      --target g_mFeYokeBot.l --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "g_mFeYokeBot.l"
```
