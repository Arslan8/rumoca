# FINDING-00768: `Nand.TP1.L` in `NandGate`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Electrical.Analog.Examples.NandGate` |
| **Reached as** | `Nand.TP1.L` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-guarded-by-assertion` |
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/PMOS.mo:43` |
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:654:aa35581b` |
...
```
denominator            (Nand.TP1.L + Nand.TP1.dL)
witness                none needed
  found by             the denominator is zero as declared
denominator at witness 0.0
at declared values     1.0000000000000002e-06
...
| `denominator_at_declared_values` | 1.0000000000000002e-06 |
| `denominator_range` |  |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.NandGate \
      --target Nand.TP1.L --sanitizer divisor
```
...
```console
$ sed -n '43p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/PMOS.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "Nand.TP1.L"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param Nand.TP1.L=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/PMOS.mo:43` |' docs/v2/bugs/
```
