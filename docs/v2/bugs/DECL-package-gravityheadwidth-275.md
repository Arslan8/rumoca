# DECL-0573: `gravityHeadWidth` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/package.mo:275` |
| **Parameter** | `gravityHeadWidth` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '275p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/package.mo"
```

It should declare `gravityHeadWidth` with type `SI.Length` and no `min`.
...
```modelica
// Mechanics/MultiBody/package.mo:275
parameter SI.Length gravityHeadWidth(min=0) = ...;
```
