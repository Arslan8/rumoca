...
| **Severity** | high |
| **Declaration** | `examples/modelsan/Tank.mo:8` |
| **Signature** | `divisor:divisor-reachable-zero:exp:17:3da2334f` |
...
constraints consulted  area: min=0
```
...
```console
$ sed -n '8p' "examples/modelsan/Tank.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "area"
```
...
```console
$ grep -rl '| `examples/modelsan/Tank.mo:8` |' docs/v2/bugs/
```
