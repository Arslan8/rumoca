...
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:6749:aed64983` |
...
```
denominator            (1 + (oLine3.G[7].alpha * (oLine3.G[7].T_heatPort - oLine3.G[7].T_ref)))
witness                none needed
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.CompareLineTrunks \
      --target oLine3.G[7].alpha --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "oLine3.G[7].alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param oLine3.G[7].alpha=0
```
