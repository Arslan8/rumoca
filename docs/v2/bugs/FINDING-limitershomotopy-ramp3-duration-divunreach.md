...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:606:ae0ac16a` |
...
```
denominator            ramp3.duration
witness                ramp3.duration = 0
  found by             set to zero
denominator at witness 0.0
at declared values     100.0
path condition         not ((time < ramp3.startTime)) and ((time < (ramp3.startTime + ramp3.duration)))
constraints consulted  ramp3.duration: min=0
```
...
| `path_condition` | not ((time &lt; ramp3.startTime)) and ((time &lt; (ramp3.startTime + ramp3.duration))) |
| `verdict` | UNSAT |
| `proof` | the path conditions cannot hold together: time must be both &gt; 1 and &lt; 1 |
| `constraints` | ramp3.duration: min=0 |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Blocks.LimitersHomotopy \
      --target ramp3.duration --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "ramp3.duration"
```
