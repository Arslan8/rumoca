# DECL-0006: `A_cross_nom` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Fluid/Dissipation.mo:12631` |
| **Parameter** | `A_cross_nom` |
| **Type** | `SI.Area` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '12631p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Dissipation.mo"
```

It should declare `A_cross_nom` with type `SI.Area` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Dissipation.mo" | head
```
...
```modelica
// Fluid/Dissipation.mo:12631
parameter SI.Area A_cross_nom(min=0) = ...;
```
