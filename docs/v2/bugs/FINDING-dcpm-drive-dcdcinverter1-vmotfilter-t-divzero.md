...
| **Signature** | `divisor:divisor-reachable-zero:exp:2848:c3eb6624` |
...
```
denominator            dcdcInverter1.vMotFilter.T
witness                dcdcInverter1.vMotFilter.T = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.002
path condition         always evaluated
constraints consulted  dcdcInverter1.vMotFilter.T: unbounded
```
...
| `constraints` | dcdcInverter1.vMotFilter.T: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Drive \
      --target dcdcInverter1.vMotFilter.T --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcdcInverter1.vMotFilter.T"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param dcdcInverter1.vMotFilter.T=0
```
