# DECL-0453: `t_poleBot` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/FluxTubes/Examples/SolenoidActuator/Components/AdvancedSolenoid.mo:22` |
| **Parameter** | `t_poleBot` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '22p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/SolenoidActuator/Components/AdvancedSolenoid.mo"
```

It should declare `t_poleBot` with type `SI.Length` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/SolenoidActuator/Components/AdvancedSolenoid.mo" | head
```
...
```modelica
// Magnetic/FluxTubes/Examples/SolenoidActuator/Components/AdvancedSolenoid.mo:22
parameter SI.Length t_poleBot(min=0) = ...;
```
