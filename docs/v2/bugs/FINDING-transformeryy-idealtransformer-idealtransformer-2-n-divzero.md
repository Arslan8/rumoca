# FINDING-03394: `idealTransformer.idealTransformer[2].n` in `TransformerYY`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Electrical.Polyphase.Examples.TransformerYY` |
| **Reached as** | `idealTransformer.idealTransformer[2].n` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Ideal/IdealTransformer.mo:14` |
| **Signature** | `divisor:divisor-reachable-zero:exp:939:292388f8` |
...
```
denominator            idealTransformer.idealTransformer[2].n
witness                idealTransformer.idealTransformer[2].n = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  idealTransformer.idealTransformer[2].n: unbounded
```
...
| `constraints` | idealTransformer.idealTransformer[2].n: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Polyphase.Examples.TransformerYY \
      --target idealTransformer.idealTransformer[2].n --sanitizer divisor
```
...
```console
$ sed -n '14p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Ideal/IdealTransformer.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "idealTransformer.idealTransformer[2].n"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param idealTransformer.idealTransformer[2].n=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Ideal/IdealTransformer.mo:14` |' docs/v2/bugs/
```
