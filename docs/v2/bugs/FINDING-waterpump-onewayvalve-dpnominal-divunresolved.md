# FINDING-05127: `oneWayValve.dpNominal` in `WaterPump`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Thermal.FluidHeatFlow.Examples.WaterPump` |
| **Reached as** | `oneWayValve.dpNominal` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-zero-unresolved` |
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/Components/OneWayValve.mo:22` |
| **Signature** | `divisor:divisor-zero-unresolved:exp:982:f1ed11ea` |
...
```
denominator            oneWayValve.dpNominal
witness                oneWayValve.dpNominal = 0
  found by             set to zero
denominator at witness 0.0
at declared values     500000.0
path condition         (oneWayValve.backward)
constraints consulted  oneWayValve.dpNominal: unbounded
```
...
| `denominator_at_declared_values` | 500000.0 |
| `denominator_range` | [-inf, inf] |
| `path_condition` | (oneWayValve.backward) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: oneWayValve.backward |
| `constraints` | oneWayValve.dpNominal: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.WaterPump \
      --target oneWayValve.dpNominal --sanitizer divisor
```
...
```console
$ sed -n '22p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/Components/OneWayValve.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "oneWayValve.dpNominal"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/Components/OneWayValve.mo:22` |' docs/v2/bugs/
```
