# DECL-0594: `R_mm` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/FundamentalWave/Examples/Components/PolyphaseInductance.mo:12` |
| **Parameter** | `R_mm` |
| **Type** | `SI.Reluctance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '12p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FundamentalWave/Examples/Components/PolyphaseInductance.mo"
```

It should declare `R_mm` with type `SI.Reluctance` and no `min`.
...
```modelica
// Magnetic/FundamentalWave/Examples/Components/PolyphaseInductance.mo:12
parameter SI.Reluctance R_mm(min=0) = ...;
```
