# DECL-0013: `crossArea_2` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Fluid/Examples/HeatExchanger.mo:119` |
| **Parameter** | `crossArea_2` |
| **Type** | `SI.Area` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '119p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Examples/HeatExchanger.mo"
```

It should declare `crossArea_2` with type `SI.Area` and no `min`.
...
```modelica
// Fluid/Examples/HeatExchanger.mo:119
parameter SI.Area crossArea_2(min=0) = ...;
```
