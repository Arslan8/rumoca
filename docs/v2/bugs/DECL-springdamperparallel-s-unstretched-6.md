# DECL-0543: `s_unstretched` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/Forces/SpringDamperParallel.mo:6` |
| **Parameter** | `s_unstretched` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '6p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Forces/SpringDamperParallel.mo"
```

It should declare `s_unstretched` with type `SI.Length` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Forces/SpringDamperParallel.mo" | head
```
...
```modelica
// Mechanics/MultiBody/Forces/SpringDamperParallel.mo:6
parameter SI.Length s_unstretched(min=0) = ...;
```
