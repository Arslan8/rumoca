# DECL-0280: `Lm` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/QuasiStatic/FundamentalWave/BasicMachines/InductionMachines/IM_SquirrelCage.mo:22` |
| **Parameter** | `Lm` |
| **Type** | `SI.Inductance` |
| **Declared modifiers** | `(start=3*ZsRef*sqrt(1 - 0.0667)` |
...
```console
$ sed -n '22p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/QuasiStatic/FundamentalWave/BasicMachines/InductionMachines/IM_SquirrelCage.mo"
```

It should declare `Lm` with type `SI.Inductance` and no `min`.
...
```modelica
// Magnetic/QuasiStatic/FundamentalWave/BasicMachines/InductionMachines/IM_SquirrelCage.mo:22
parameter SI.Inductance Lm(min=0) = ...;
```
