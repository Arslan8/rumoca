# FINDING-01937: `pwm.svPWM.f` in `IMC_InverterDrive`
...
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCAC/Control/SVPWM.mo:4` |
| **Signature** | `divisor:divisor-reachable-zero:exp:3554:0956cfb4` |
...
```
denominator            pwm.svPWM.f
witness                pwm.svPWM.f = 0
  found by             set to zero
denominator at witness 0.0
at declared values     2000.0
path condition         always evaluated
constraints consulted  pwm.svPWM.f: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive \
      --target pwm.svPWM.f --sanitizer divisor
```
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCAC/Control/SVPWM.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pwm.svPWM.f"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCAC/Control/SVPWM.mo:4` |' docs/v2/bugs/
```
