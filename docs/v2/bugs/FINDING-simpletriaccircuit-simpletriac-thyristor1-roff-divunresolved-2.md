...
| **Signature** | `divisor:divisor-zero-unresolved:exp:790:0c12b88e` |
...
```
denominator            simpleTriac.thyristor1.Roff
witness                simpleTriac.thyristor1.Roff = 0
  found by             set to zero
denominator at witness 0.0
at declared values     15686274.509803923
path condition         not ((simpleTriac.thyristor1.vAK < -simpleTriac.thyristor1.VRRM)) and ((simpleTriac.thyristor1.vControl < simpleTriac.thyristor1.Voff))
constraints consulted  simpleTriac.thyristor1.Roff: unbounded
```
...
| `path_condition` | not ((simpleTriac.thyristor1.vAK &lt; -simpleTriac.thyristor1.VRRM)) and ((simpleTriac.thyristor1.vControl &lt; simpleTriac.thyristor1.Voff)) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: (simpleTriac.thyristor1.vAK &lt; -simpleTriac.thyristor1.VRRM) |
| `constraints` | simpleTriac.thyristor1.Roff: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.SimpleTriacCircuit \
      --target simpleTriac.thyristor1.Roff --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "simpleTriac.thyristor1.Roff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param simpleTriac.thyristor1.Roff=0
```
