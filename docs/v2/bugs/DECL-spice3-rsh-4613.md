# DECL-0815: `RSH` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Spice3.mo:4613` |
| **Parameter** | `RSH` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '4613p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Spice3.mo"
```

It should declare `RSH` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/Spice3.mo:4613
parameter SI.Resistance RSH(min=0) = ...;
```
