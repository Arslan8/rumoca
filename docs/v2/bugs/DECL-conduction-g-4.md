# DECL-0895: `G` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Thermal/HeatTransfer/Examples/Utilities/Conduction.mo:4` |
| **Parameter** | `G` |
...
It should declare `G` with type `SI.ThermalConductance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/HeatTransfer/Examples/Utilities/Conduction.mo" | head
```
...
```modelica
// Thermal/HeatTransfer/Examples/Utilities/Conduction.mo:4
parameter SI.ThermalConductance G(min=0) = ...;
```
