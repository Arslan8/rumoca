# FINDING-00775: `Nand.TN1.Tnom` in `NandGate`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Electrical.Analog.Examples.NandGate` |
| **Reached as** | `Nand.TN1.Tnom` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-zero-unresolved` |
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/NMOS.mo:52` |
| **Signature** | `divisor:divisor-zero-unresolved:exp:1142:980daeb2` |
...
```
denominator            Nand.TN1.Tnom
witness                Nand.TN1.Tnom = 0
  found by             set to zero
denominator at witness 0.0
at declared values     300.15
path condition         (Nand.TN1.useTemperatureDependency)
constraints consulted  Nand.TN1.Tnom: min=0
```
...
| `path_condition` | (Nand.TN1.useTemperatureDependency) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: Nand.TN1.useTemperatureDependency |
| `constraints` | Nand.TN1.Tnom: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.NandGate \
      --target Nand.TN1.Tnom --sanitizer divisor
```
...
```console
$ sed -n '52p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/NMOS.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "Nand.TN1.Tnom"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/NMOS.mo:52` |' docs/v2/bugs/
```
