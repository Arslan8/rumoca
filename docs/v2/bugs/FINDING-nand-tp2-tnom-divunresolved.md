...
| **Signature** | `divisor:divisor-zero-unresolved:exp:584:c9b8bb14` |
...
```
denominator            TP2.Tnom
witness                TP2.Tnom = 0
  found by             set to zero
denominator at witness 0.0
at declared values     300.15
path condition         (TP2.useTemperatureDependency)
constraints consulted  TP2.Tnom: min=0
```
...
| `path_condition` | (TP2.useTemperatureDependency) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: TP2.useTemperatureDependency |
| `constraints` | TP2.Tnom: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Utilities.Nand \
      --target TP2.Tnom --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "TP2.Tnom"
```
