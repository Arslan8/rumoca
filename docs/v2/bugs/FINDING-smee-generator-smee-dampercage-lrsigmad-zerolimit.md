# FINDING-02456: `smee.damperCage.Lrsigmad` in `SMEE_Generator`
...
| **Reached as** | `smee.damperCage.Lrsigmad` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/Components/DamperCage.mo:3` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:440:8431a91c` |
...
```
predicate     smee.damperCage.Lrsigmad > 0
premise       refuted
...
```
contract    Modelica.Electrical.Machines.BasicMachines.Components.DamperCage.Lrsigmad: algebraic_limit (proven, from equation) — the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.Components.DamperCage.Lrsigmad'}
```
...
| `required` | smee.damperCage.Lrsigmad &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMEE_Generator \
      --target smee.damperCage.Lrsigmad --sanitizer physical
```
...
```console
$ sed -n '3p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/Components/DamperCage.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smee.damperCage.Lrsigmad"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param smee.damperCage.Lrsigmad=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/Components/DamperCage.mo:3` |' docs/v2/bugs/
```
