# DECL-0818: `RB` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Spice3.mo:5027` |
| **Parameter** | `RB` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '5027p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Spice3.mo"
```

It should declare `RB` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/Spice3.mo:5027
parameter SI.Resistance RB(min=0) = ...;
```
