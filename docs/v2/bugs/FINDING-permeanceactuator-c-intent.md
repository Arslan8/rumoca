# FINDING-04265: `c` in `PermeanceActuator`
...
| **Signature** | `physical:physical-intent-question:var:16:e8381e2a` |
...
| `question` | `c` carries no bound that would exclude it, which is unusual for a translational spring constant — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "c"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param c=0
```
