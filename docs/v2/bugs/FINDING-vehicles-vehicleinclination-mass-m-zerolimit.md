...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:796:abf9784d` |
...
```
predicate     vehicleInclination.mass.m > 0
premise       refuted
...
| `required` | vehicleInclination.mass.m &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Translational.Vehicles \
      --target vehicleInclination.mass.m --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "vehicleInclination.mass.m"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param vehicleInclination.mass.m=0
```
