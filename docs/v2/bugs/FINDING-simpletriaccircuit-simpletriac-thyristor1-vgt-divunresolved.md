...
| **Signature** | `divisor:divisor-zero-unresolved:exp:702:3fb4e1d8` |
...
```
denominator            simpleTriac.thyristor1.VGT
witness                simpleTriac.thyristor1.VGT = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.7
path condition         not ((simpleTriac.thyristor1.vGK < simpleTriac.thyristor1.vRef))
constraints consulted  simpleTriac.thyristor1.VGT: unbounded
```
...
| `denominator_at_declared_values` | 0.7 |
...
| `constraints` | simpleTriac.thyristor1.VGT: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.SimpleTriacCircuit \
      --target simpleTriac.thyristor1.VGT --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "simpleTriac.thyristor1.VGT"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param simpleTriac.thyristor1.VGT=0
```
