# FINDING-01292: `dcseData.coreParameters.GcRef` in `DC_CompareCharacteristics`
...
| **Reached as** | `dcseData.coreParameters.GcRef` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:734:b8829acd` |
...
| `observed` | 0.0 |
| `required` | dcseData.coreParameters.GcRef &gt; 0 |
| `where` | declaration |
| `contract` | Modelica.Electrical.Machines.Losses.CoreParameters.GcRef: feature_disabled (declared, from bound) — the declaration&#x27;s own binding evaluates to zero at the declared configuration, so zero is the value the library chose rather than one it failed to exclude |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DC_CompareCharacteristics \
      --target dcseData.coreParameters.GcRef --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcseData.coreParameters.GcRef"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param dcseData.coreParameters.GcRef=0
```
