# FINDING-04595: `bodyBox.mo` in `BevelGear1D`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/BodyBox.mo:101` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:627:e88b39f1` |
...
```
contract    Modelica.Mechanics.MultiBody.Parts.BodyBox.mo: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Mass', 'unit': 'kg', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Mechanics.MultiBody.Parts.BodyBox.mo'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Rotational3DEffects.BevelGear1D \
      --target bodyBox.mo --sanitizer physical
```
...
```console
$ sed -n '101p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/BodyBox.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "bodyBox.mo"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param bodyBox.mo=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/BodyBox.mo:101` |' docs/v2/bugs/
```
