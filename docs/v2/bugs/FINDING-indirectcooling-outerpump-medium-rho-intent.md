...
| **Signature** | `physical:physical-intent-question:var:42:dc406ba8` |
...
```
predicate     outerPump.medium.rho > 0
premise       unknown
...
| `required` | outerPump.medium.rho &gt; 0 |
| `declared_min` | 0.0 |
...
| `question` | `outerPump.medium.rho` is bounded only by min=0, which is unusual for a density — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.IndirectCooling \
      --target outerPump.medium.rho --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "outerPump.medium.rho"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param outerPump.medium.rho=0
```
