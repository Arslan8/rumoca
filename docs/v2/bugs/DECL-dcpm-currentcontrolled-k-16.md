# DECL-0697: `k` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Machines/Examples/DCMachines/DCPM_CurrentControlled.mo:16` |
| **Parameter** | `k` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
It should declare `k` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/Machines/Examples/DCMachines/DCPM_CurrentControlled.mo:16
parameter SI.Resistance k(min=0) = ...;
```
