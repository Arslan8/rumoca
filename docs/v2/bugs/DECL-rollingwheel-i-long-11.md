# DECL-0361: `I_long` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/Parts/RollingWheel.mo:11` |
| **Parameter** | `I_long` |
| **Type** | `SI.Inertia` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '11p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/RollingWheel.mo"
```

It should declare `I_long` with type `SI.Inertia` and no `min`.
...
```modelica
// Mechanics/MultiBody/Parts/RollingWheel.mo:11
parameter SI.Inertia I_long(min=0) = ...;
```
