# DECL-0004: `AS` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Spice3.mo:4664` |
| **Parameter** | `AS` |
| **Type** | `SI.Area` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '4664p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Spice3.mo"
```

It should declare `AS` with type `SI.Area` and no `min`.
...
```modelica
// Electrical/Spice3.mo:4664
parameter SI.Area AS(min=0) = ...;
```
