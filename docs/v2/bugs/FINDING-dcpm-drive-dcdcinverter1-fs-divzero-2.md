# FINDING-01523: `dcdcInverter1.fS` in `DCPM_Drive`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Drive` |
| **Reached as** | `dcdcInverter1.fS` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/ControlledDCDrives/Utilities/DcdcInverter.mo:5` |
| **Signature** | `divisor:divisor-reachable-zero:exp:961:2d50e9e5` |
...
```
denominator            dcdcInverter1.fS
witness                dcdcInverter1.fS = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1000.0
path condition         always evaluated
constraints consulted  dcdcInverter1.fS: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Drive \
      --target dcdcInverter1.fS --sanitizer divisor
```
...
```console
$ sed -n '5p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/ControlledDCDrives/Utilities/DcdcInverter.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcdcInverter1.fS"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/ControlledDCDrives/Utilities/DcdcInverter.mo:5` |' docs/v2/bugs/
```
