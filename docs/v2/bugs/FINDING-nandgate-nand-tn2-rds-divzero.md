...
| **Signature** | `divisor:divisor-reachable-zero:exp:1228:f8aa8feb` |
...
```
denominator            Nand.TN2.RDS
witness                Nand.TN2.RDS = 0
  found by             set to zero
denominator at witness 0.0
at declared values     10000000.0
path condition         not (((Nand.TN2.RDS < 1e-20) and (Nand.TN2.RDS > -1e-20)))
constraints consulted  Nand.TN2.RDS: unbounded
```
...
| `path_condition` | not (((Nand.TN2.RDS &lt; 1e-20) and (Nand.TN2.RDS &gt; -1e-20))) |
| `verdict` | SAT |
| `proof` | every guard still holds under the witness |
| `constraints` | Nand.TN2.RDS: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.NandGate \
      --target Nand.TN2.RDS --sanitizer divisor
```
