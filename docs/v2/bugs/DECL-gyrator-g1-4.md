# DECL-0044: `G1` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Basic/Gyrator.mo:4` |
| **Parameter** | `G1` |
| **Type** | `SI.Conductance` |
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/Gyrator.mo"
```

It should declare `G1` with type `SI.Conductance` and no `min`.
...
```modelica
// Electrical/Analog/Basic/Gyrator.mo:4
parameter SI.Conductance G1(min=0) = ...;
```
