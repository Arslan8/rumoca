# DECL-0387: `PS` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Spice3.mo:4460` |
| **Parameter** | `PS` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '4460p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Spice3.mo"
```

It should declare `PS` with type `SI.Length` and no `min`.
...
```modelica
// Electrical/Spice3.mo:4460
parameter SI.Length PS(min=0) = ...;
```
