...
| **Reached as** | `dcpmData2.frictionParameters.wLinear` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-zero-unresolved` |
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Losses/Friction.mo:15` |
| **Signature** | `divisor:divisor-zero-unresolved:exp:2381:fc736a0d` |
...
```
denominator            dcpmData2.frictionParameters.wLinear
witness                dcpmData2.frictionParameters.wLinear = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.14844025288212
path condition         not (noEvent((abs(dcpm2.friction.w) > dcpmData2.frictionParameters.wLinear)))
constraints consulted  dcpmData2.frictionParameters.wLinear: unbounded
```
...
| `denominator_at_declared_values` | 0.14844025288212 |
| `denominator_range` | [-inf, inf] |
| `path_condition` | not (noEvent((abs(dcpm2.friction.w) &gt; dcpmData2.frictionParameters.wLinear))) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: noEvent((abs(dcpm2.friction.w) &gt; dcpmData2.frictionParameters.wLinear)) |
| `constraints` | dcpmData2.frictionParameters.wLinear: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_withLosses \
      --target dcpmData2.frictionParameters.wLinear --sanitizer divisor
```
...
```console
$ sed -n '15p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Losses/Friction.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcpmData2.frictionParameters.wLinear"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param dcpmData2.frictionParameters.wLinear=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Losses/Friction.mo:15` |' docs/v2/bugs/
```
