# DECL-0197: `LDC` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Polyphase/Examples/Utilities/PolyphaseRectifierData.mo:22` |
| **Parameter** | `LDC` |
| **Type** | `SI.Inductance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '22p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Examples/Utilities/PolyphaseRectifierData.mo"
```

It should declare `LDC` with type `SI.Inductance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Examples/Utilities/PolyphaseRectifierData.mo" | head
```
...
```modelica
// Electrical/Polyphase/Examples/Utilities/PolyphaseRectifierData.mo:22
parameter SI.Inductance LDC(min=0) = ...;
```
