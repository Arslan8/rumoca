# FINDING-01169: `simpleTriac.thyristor1.IGT` in `SimpleTriacCircuit`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Electrical.Analog.Examples.SimpleTriacCircuit` |
| **Reached as** | `simpleTriac.thyristor1.IGT` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-zero-unresolved` |
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/Thyristor.mo:52` |
| **Signature** | `divisor:divisor-zero-unresolved:exp:709:8da27fdb` |
...
```
denominator            simpleTriac.thyristor1.IGT
witness                simpleTriac.thyristor1.IGT = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.005
path condition         not ((simpleTriac.thyristor1.vGK < simpleTriac.thyristor1.vRef))
constraints consulted  simpleTriac.thyristor1.IGT: unbounded
```
...
| `denominator_at_declared_values` | 0.005 |
| `denominator_range` | [-inf, inf] |
| `path_condition` | not ((simpleTriac.thyristor1.vGK &lt; simpleTriac.thyristor1.vRef)) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: (simpleTriac.thyristor1.vGK &lt; simpleTriac.thyristor1.vRef) |
| `constraints` | simpleTriac.thyristor1.IGT: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.SimpleTriacCircuit \
      --target simpleTriac.thyristor1.IGT --sanitizer divisor
```
...
```console
$ sed -n '52p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/Thyristor.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "simpleTriac.thyristor1.IGT"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param simpleTriac.thyristor1.IGT=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/Thyristor.mo:52` |' docs/v2/bugs/
```
