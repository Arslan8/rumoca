...
| **Signature** | `divisor:divisor-reachable-zero:exp:1171:a5321d57` |
...
```
denominator            g_mFeArm.l
witness                g_mFeArm.l = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.022375000000000003
path condition         always evaluated
constraints consulted  g_mFeArm.l: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.SolenoidActuator.Components.AdvancedSolenoid \
      --target g_mFeArm.l --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "g_mFeArm.l"
```
