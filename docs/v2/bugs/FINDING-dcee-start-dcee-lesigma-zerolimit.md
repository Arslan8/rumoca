# FINDING-01358: `dcee.Lesigma` in `DCEE_Start`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:327:e9bf0f39` |
...
```
contract    Modelica.Electrical.Machines.BasicMachines.DCMachines.DC_ElectricalExcited.Lesigma: feature_disabled (declared, from bound) — the declaration's own binding evaluates to zero at the declared configuration, so zero is the value the library chose rather than one it failed to exclude
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.DCMachines.DC_ElectricalExcited.Lesigma'}
```
