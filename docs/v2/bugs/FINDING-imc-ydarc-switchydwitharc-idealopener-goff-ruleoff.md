...
| **Reached as** | `switchYDwithArc.idealOpener.Goff` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Ideal/OpenerWithArc.mo:6` |
| **Signature** | `physical:physical-rule-does-not-apply:var:802:cb5733b6` |
...
```
predicate     switchYDwithArc.idealOpener.Goff > 0
premise       refuted
...
```
contract    Modelica.Electrical.Polyphase.Ideal.OpenerWithArc.Goff: allowed (declared, from class_catalog) — an ideal switch family member; zero off-conductance is the documented ideal [matched by prefix, lower confidence]
behavior    allowed
confidence  declared
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Polyphase.Ideal.OpenerWithArc.Goff'}
```
...
| `required` | switchYDwithArc.idealOpener.Goff &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_YDarc \
      --target switchYDwithArc.idealOpener.Goff --sanitizer physical
```
...
```console
$ sed -n '6p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Ideal/OpenerWithArc.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switchYDwithArc.idealOpener.Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switchYDwithArc.idealOpener.Goff=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Ideal/OpenerWithArc.mo:6` |' docs/v2/bugs/
```
