# FINDING-03386: `idealTransformer.idealTransformer[1].Lm1` in `TransformerYY`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Electrical.Polyphase.Examples.TransformerYY` |
| **Reached as** | `idealTransformer.idealTransformer[1].Lm1` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Ideal/IdealTransformer.mo:7` |
| **Signature** | `physical:physical-intent-question:var:146:ad4f6ae7` |
...
```
predicate     idealTransformer.idealTransformer[1].Lm1 > 0
premise       unknown
authority     quantity_or_unit
role          —
declaration   Modelica.Electrical.Analog.Ideal.IdealTransformer.Lm1
```
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Ideal.IdealTransformer.Lm1'}
```
...
| `required` | idealTransformer.idealTransformer[1].Lm1 &gt; 0 |
| `declared_min` | None |
| `variable_role` | parameter |
| `observation` | permissive-declaration |
| `premise_state` | unknown |
| `authority` | quantity_or_unit |
| `canonical_declaration` | Modelica.Electrical.Analog.Ideal.IdealTransformer.Lm1 |
| `question` | `idealTransformer.idealTransformer[1].Lm1` carries no bound that would exclude it, which is unusual for an inductance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Polyphase.Examples.TransformerYY \
      --target idealTransformer.idealTransformer[1].Lm1 --sanitizer physical
```
...
```console
$ sed -n '7p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Ideal/IdealTransformer.mo"
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Polyphase.Examples.TransformerYY --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "idealTransformer.idealTransformer[1].Lm1"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param idealTransformer.idealTransformer[1].Lm1=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Ideal/IdealTransformer.mo:7` |' docs/v2/bugs/
```
