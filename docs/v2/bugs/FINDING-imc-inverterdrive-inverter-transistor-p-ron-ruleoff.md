# FINDING-01869: `inverter.transistor_p.Ron` in `IMC_InverterDrive`
...
| **Reached as** | `inverter.transistor_p.Ron` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Ideal/IdealGTOThyristor.mo:4` |
| **Signature** | `physical:physical-rule-does-not-apply:var:546:2c9c646c` |
...
```
predicate     inverter.transistor_p.Ron > 0
premise       refuted
...
```
contract    Modelica.Electrical.Polyphase.Ideal.IdealGTOThyristor.Ron: allowed (declared, from class_catalog) — an ideal switch family member; zero on-resistance is the documented ideal [matched by prefix, lower confidence]
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Polyphase.Ideal.IdealGTOThyristor.Ron'}
```
...
| `required` | inverter.transistor_p.Ron &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive \
      --target inverter.transistor_p.Ron --sanitizer physical
```
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Ideal/IdealGTOThyristor.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inverter.transistor_p.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inverter.transistor_p.Ron=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Ideal/IdealGTOThyristor.mo:4` |' docs/v2/bugs/
```
