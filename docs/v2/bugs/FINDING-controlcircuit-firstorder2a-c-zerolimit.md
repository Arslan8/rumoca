...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:411:f07f79a8` |
...
```
contract    Modelica.Electrical.Analog.Basic.Capacitor.C: algebraic_limit (proven, from equation) — inherited from firstOrder2A.c.C, which it binds: the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.ControlCircuit \
      --target firstOrder2A.C --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "firstOrder2A.C"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param firstOrder2A.C=0
```
