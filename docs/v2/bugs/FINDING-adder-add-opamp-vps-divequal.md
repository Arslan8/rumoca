...
| **Signature** | `divisor:divisor-zero-when-parameters-equal:exp:214:4df8ced4` |
...
```
denominator            (add.opAmp.vps - add.opAmp.vns)
witness                add.opAmp.Vps = -15
  found by             set equal to add.opAmp.Vns
denominator at witness 0.0
at declared values     30.0
path condition         always evaluated
constraints consulted  add.opAmp.Vps: unbounded
```
...
| `denominator` | (add.opAmp.vps - add.opAmp.vns) |
| `witness` | add.opAmp.Vps = -15 |
| `witness_rationale` | set equal to add.opAmp.Vns |
...
| `constraints` | add.opAmp.Vps: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.Adder \
      --target add.opAmp.Vps --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "add.opAmp.Vps"
```
