# FINDING-03219: `transformerData2.f` in `Rectifier12pulse`
...
| **Signature** | `divisor:divisor-reachable-zero:exp:2935:b64790fc` |
...
```
denominator            ((2 * (2 * asin(1.0))) * transformerData2.f)
witness                transformerData2.f = 0
  found by             set to zero
denominator at witness 0.0
at declared values     314.1592653589793
path condition         always evaluated
constraints consulted  transformerData2.f: unbounded
```
...
| `constraints` | transformerData2.f: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.Transformers.Rectifier12pulse \
      --target transformerData2.f --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "transformerData2.f"
```
