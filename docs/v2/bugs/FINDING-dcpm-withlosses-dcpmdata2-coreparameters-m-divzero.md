...
| **Signature** | `divisor:divisor-reachable-zero:exp:987:edea7b05` |
...
```
denominator            dcpmData2.coreParameters.m
witness                dcpmData2.coreParameters.m = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         not ((dcpmData2.coreParameters.PRef <= 0))
constraints consulted  dcpmData2.coreParameters.m: unbounded
```
...
| `constraints` | dcpmData2.coreParameters.m: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_withLosses \
      --target dcpmData2.coreParameters.m --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcpmData2.coreParameters.m"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param dcpmData2.coreParameters.m=0
```
