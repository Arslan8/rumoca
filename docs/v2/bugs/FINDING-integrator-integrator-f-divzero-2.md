...
| **Signature** | `divisor:divisor-reachable-zero:exp:87:59c8406c` |
...
```
denominator            (((2 * (2 * asin(1.0))) * integrator.f) * integrator.R)
witness                integrator.f = 0
  found by             set to zero
denominator at witness 0.0
at declared values     62831.853071795864
path condition         always evaluated
constraints consulted  integrator.f: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.Integrator \
      --target integrator.f --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "integrator.f"
```
