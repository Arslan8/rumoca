# FINDING-03122: `diode1.idealDiode[1].Ron` in `Rectifier12pulse`
...
| **Reached as** | `diode1.idealDiode[1].Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:148:f9212c46` |
...
```
predicate     diode1.idealDiode[1].Ron > 0
premise       refuted
...
| `required` | diode1.idealDiode[1].Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.Transformers.Rectifier12pulse \
      --target diode1.idealDiode[1].Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "diode1.idealDiode[1].Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param diode1.idealDiode[1].Ron=0
```
