# FINDING-05276: `smpmData.statorCoreParameters.m` in `SMPM_VoltageSourceWithLosses`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `ModelicaTest.Electrical.Machines.SMPM_VoltageSourceWithLosses` |
| **Reached as** | `smpmData.statorCoreParameters.m` |
...
| **Signature** | `divisor:divisor-reachable-zero:exp:1938:8d3f1777` |
...
```
denominator            smpmData.statorCoreParameters.m
witness                smpmData.statorCoreParameters.m = 0
  found by             set to zero
denominator at witness 0.0
at declared values     3.0
path condition         not ((smpmData.statorCoreParameters.PRef <= 0))
constraints consulted  smpmData.statorCoreParameters.m: unbounded
```
...
| `denominator_at_declared_values` | 3.0 |
...
| `constraints` | smpmData.statorCoreParameters.m: unbounded |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Electrical.Machines.SMPM_VoltageSourceWithLosses \
      --target smpmData.statorCoreParameters.m --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smpmData.statorCoreParameters.m"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param smpmData.statorCoreParameters.m=0
```
