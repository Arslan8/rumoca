...
| **Declaration** | `Magnetic/FundamentalWave/Examples/BasicMachines/SynchronousMachines/ComparisonPolyphase/SMPM_Inverter_Polyphase.mo:17` |
| **Parameter** | `J_Load` |
| **Type** | `SI.Inertia` |
| **Declared modifiers** | `—` |
...
`SI.Inertia` declares no `min` in `Units.mo`, and this declaration adds none of its own. It therefore accepts a negative value, and `J*a = tau` determines angular acceleration only for `J > 0`.
...
```console
$ sed -n '17p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FundamentalWave/Examples/BasicMachines/SynchronousMachines/ComparisonPolyphase/SMPM_Inverter_Polyphase.mo"
```

It should declare `J_Load` with type `SI.Inertia` and no `min`.

**2. Confirm the type supplies no bound either** — this is the whole claim, and
a bound inherited from the type would refute it:

```console
$ grep -n "type Inertia " "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Units.mo"
```
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FundamentalWave/Examples/BasicMachines/SynchronousMachines/ComparisonPolyphase/SMPM_Inverter_Polyphase.mo" | head
```
...
- `SI.Inertia` turns out to carry a `min` after all, or this declaration carries one that the catalog missed.
...
```modelica
// Magnetic/FundamentalWave/Examples/BasicMachines/SynchronousMachines/ComparisonPolyphase/SMPM_Inverter_Polyphase.mo:17
parameter SI.Inertia J_Load(min=0) = ...;
```
