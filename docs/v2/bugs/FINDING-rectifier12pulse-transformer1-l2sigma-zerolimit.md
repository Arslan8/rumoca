# FINDING-03147: `transformer1.L2sigma` in `Rectifier12pulse`
...
| **Reached as** | `transformer1.L2sigma` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:352:5dc55d6c` |
...
```
predicate     transformer1.L2sigma > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Basic.Inductor.L: algebraic_limit (declared, from class_catalog) — inherited from transformer1.l2sigma.L, which it binds: L*der(i) = v; at L = 0 the element is an ideal short, v = 0, which the component documentation states explicitly
...
| `required` | transformer1.L2sigma &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.Transformers.Rectifier12pulse \
      --target transformer1.L2sigma --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "transformer1.L2sigma"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param transformer1.L2sigma=0
```
