# FINDING-04519: `spring.lineForce.m` in `SpringWithMass`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:269:77faec64` |
...
```
contract    Modelica.Mechanics.MultiBody.Forces.LineForceWithMass.m: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
