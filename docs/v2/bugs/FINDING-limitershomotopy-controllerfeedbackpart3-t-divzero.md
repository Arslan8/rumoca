...
| **Reached as** | `controllerFeedbackPart3.T` |
...
| **Signature** | `divisor:divisor-reachable-zero:exp:545:a349478f` |
...
```
denominator            controllerFeedbackPart3.T
witness                controllerFeedbackPart3.T = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  controllerFeedbackPart3.T: unbounded
```
...
| `constraints` | controllerFeedbackPart3.T: unbounded |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Blocks.LimitersHomotopy \
      --target controllerFeedbackPart3.T --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "controllerFeedbackPart3.T"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param controllerFeedbackPart3.T=0
```
