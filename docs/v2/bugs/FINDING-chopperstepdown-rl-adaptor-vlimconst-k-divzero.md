...
| **Reached as** | `adaptor.vLimConst.k` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCDC/Control/Voltage2DutyCycle.mo:39` |
| **Signature** | `divisor:divisor-reachable-zero:exp:696:62d17550` |
...
```
denominator            adaptor.vLimInt
witness                adaptor.vLimConst.k = 0
  found by             set to zero
denominator at witness 0.0
at declared values     60.0
path condition         always evaluated
constraints consulted  adaptor.vLimConst.k: unbounded
```
...
| `constraints` | adaptor.vLimConst.k: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCDC.ChopperStepDown.ChopperStepDown_RL \
      --target adaptor.vLimConst.k --sanitizer divisor
```
...
```console
$ sed -n '39p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCDC/Control/Voltage2DutyCycle.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "adaptor.vLimConst.k"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCDC/Control/Voltage2DutyCycle.mo:39` |' docs/v2/bugs/
```
