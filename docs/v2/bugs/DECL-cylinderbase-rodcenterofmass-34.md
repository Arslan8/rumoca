# DECL-0536: `rodCenterOfMass` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/Examples/Loops/Utilities/CylinderBase.mo:34` |
| **Parameter** | `rodCenterOfMass` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '34p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Examples/Loops/Utilities/CylinderBase.mo"
```

It should declare `rodCenterOfMass` with type `SI.Length` and no `min`.
...
```modelica
// Mechanics/MultiBody/Examples/Loops/Utilities/CylinderBase.mo:34
parameter SI.Length rodCenterOfMass(min=0) = ...;
```
