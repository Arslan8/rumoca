# FINDING-01236: `TP1.Tnom` in `Nand`
...
| **Signature** | `divisor:divisor-zero-unresolved:exp:388:91603945` |
...
```
denominator            TP1.Tnom
witness                TP1.Tnom = 0
  found by             set to zero
denominator at witness 0.0
at declared values     300.15
path condition         (TP1.useTemperatureDependency)
constraints consulted  TP1.Tnom: min=0
```
...
| `path_condition` | (TP1.useTemperatureDependency) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: TP1.useTemperatureDependency |
| `constraints` | TP1.Tnom: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Utilities.Nand \
      --target TP1.Tnom --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "TP1.Tnom"
```
