# DECL-0042: `ATank` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Thermal/FluidHeatFlow/Components/OpenTank.mo:5` |
| **Parameter** | `ATank` |
| **Type** | `SI.Area` |
...
It should declare `ATank` with type `SI.Area` and no `min`.
...
```modelica
// Thermal/FluidHeatFlow/Components/OpenTank.mo:5
parameter SI.Area ATank(min=0) = ...;
```
