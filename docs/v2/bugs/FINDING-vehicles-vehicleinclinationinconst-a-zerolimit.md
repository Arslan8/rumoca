# FINDING-05401: `vehicleInclinationInConst.A` in `Vehicles`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `ModelicaTest.Translational.Vehicles` |
| **Reached as** | `vehicleInclinationInConst.A` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Components/Vehicle.mo:7` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:825:6812f506` |
...
```
contract    Modelica.Mechanics.Translational.Components.Vehicle.A: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Area', 'unit': 'm2', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Mechanics.Translational.Components.Vehicle.A'}
```
...
| `observed` | 0.0 |
| `required` | vehicleInclinationInConst.A &gt; 0 |
| `where` | declaration |
| `contract` | Modelica.Mechanics.Translational.Components.Vehicle.A: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Translational.Vehicles \
      --target vehicleInclinationInConst.A --sanitizer physical
```
...
```console
$ sed -n '7p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Components/Vehicle.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "vehicleInclinationInConst.A"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param vehicleInclinationInConst.A=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Components/Vehicle.mo:7` |' docs/v2/bugs/
```
