# DECL-0598: `R_m` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/QuasiStatic/FundamentalWave/Examples/Components/PolyphaseInductance.mo:11` |
| **Parameter** | `R_m` |
| **Type** | `SI.Reluctance` |
| **Declared modifiers** | `—` |
...
`SI.Reluctance` declares no `min` in `Units.mo`, and this declaration adds none of its own. It therefore accepts a negative value, and the magnetic analogue of resistance.
...
```console
$ sed -n '11p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/QuasiStatic/FundamentalWave/Examples/Components/PolyphaseInductance.mo"
```

It should declare `R_m` with type `SI.Reluctance` and no `min`.

**2. Confirm the type supplies no bound either** — this is the whole claim, and
a bound inherited from the type would refute it:

```console
$ grep -n "type Reluctance " "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Units.mo"
```
...
- `SI.Reluctance` turns out to carry a `min` after all, or this declaration carries one that the catalog missed.
...
```modelica
// Magnetic/QuasiStatic/FundamentalWave/Examples/Components/PolyphaseInductance.mo:11
