# FINDING-03188: `transformerData2.R2` in `Rectifier12pulse`
...
| **Reached as** | `transformerData2.R2` |
...
| **Signature** | `physical:physical-intent-question:var:1389:43c54ef3` |
...
```
predicate     transformerData2.R2 > 0
premise       unknown
...
> `transformerData2.R2` carries no bound that would exclude it, which is unusual for a resistance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
| `required` | transformerData2.R2 &gt; 0 |
| `declared_min` | None |
...
| `question` | `transformerData2.R2` carries no bound that would exclude it, which is unusual for a resistance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.Transformers.Rectifier12pulse \
      --target transformerData2.R2 --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "transformerData2.R2"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param transformerData2.R2=0
```
