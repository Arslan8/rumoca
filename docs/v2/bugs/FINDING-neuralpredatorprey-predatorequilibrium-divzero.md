# FINDING-05452: `predatorEquilibrium` in `NeuralPredatorPrey`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `NeuralPredatorPrey` |
| **Reached as** | `predatorEquilibrium` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `examples/models/NeuralPredatorPrey.mo:54` |
| **Signature** | `divisor:divisor-reachable-zero:exp:2116:1225c0f2` |
...
```
denominator            predatorEquilibrium
witness                predatorEquilibrium = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  predatorEquilibrium: unbounded
```
...
| `constraints` | predatorEquilibrium: unbounded |
...
```console
$ python3 tools/sweep/check_one.py NeuralPredatorPrey \
      --target predatorEquilibrium --sanitizer divisor
```
...
```console
$ sed -n '54p' "examples/models/NeuralPredatorPrey.mo"
```
...
```console
$ python3 tools/sweep/check_one.py NeuralPredatorPrey --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "predatorEquilibrium"
```
...
```console
$ grep -rl '| `examples/models/NeuralPredatorPrey.mo:54` |' docs/v2/bugs/
```
