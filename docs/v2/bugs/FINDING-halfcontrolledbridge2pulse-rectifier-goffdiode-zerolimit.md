# FINDING-03634: `rectifier.GoffDiode` in `HalfControlledBridge2Pulse`
...
| **Reached as** | `rectifier.GoffDiode` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/HalfControlledBridge2Pulse.mo:8` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:63:4916a166` |
...
```
predicate     rectifier.GoffDiode > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealDiode.Goff: feature_disabled (proven, from equation) — inherited from rectifier.diode_n1.Goff, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.PowerConverters.ACDC.HalfControlledBridge2Pulse.GoffDiode'}
```
...
| `required` | rectifier.GoffDiode &gt; 0 |
| `declared_min` | 0.0 |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2Pulse.HalfControlledBridge2Pulse \
      --target rectifier.GoffDiode --sanitizer physical
```
...
```console
$ sed -n '8p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/HalfControlledBridge2Pulse.mo"
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2Pulse.HalfControlledBridge2Pulse --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "rectifier.GoffDiode"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param rectifier.GoffDiode=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/HalfControlledBridge2Pulse.mo:8` |' docs/v2/bugs/
```
