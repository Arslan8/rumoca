# FINDING-02986: `smrData.fsNominal` in `SMR_Inverter`
...
| **Signature** | `divisor:divisor-reachable-zero:exp:1513:6c3d8a00` |
...
```
denominator            ((2 * (2 * asin(1.0))) * smrData.fsNominal)
witness                smrData.fsNominal = 0
  found by             set to zero
denominator at witness 0.0
at declared values     314.1592653589793
path condition         always evaluated
constraints consulted  smrData.fsNominal: unbounded
```
...
| `constraints` | smrData.fsNominal: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMR_Inverter \
      --target smrData.fsNominal --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smrData.fsNominal"
```
