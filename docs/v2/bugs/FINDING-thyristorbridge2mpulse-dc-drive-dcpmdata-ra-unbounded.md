...
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/ParameterRecords/DcPermanentMagnetData.mo:19` |
| **Signature** | `physical:physical-domain-unenforced:var:969:9b89eb7c` |
...
```
predicate     dcpmData.Ra >= 0
premise       established
authority     component_contract
role          component.machine.winding_resistance
declaration   Modelica.Electrical.Machines.Utilities.ParameterRecords.DcPermanentMagnetData.Ra
```
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'semantic_role': 'component.machine.winding_resistance', 'binding_source': 'component_type', 'confidence': 'QUANTITY_AND_UNIT', 'declaring_class': 'Modelica.Electrical.Machines.Utilities.ParameterRecords.DcPermanentMagnetData', 'member': 'Ra', 'premise_state': 'established', 'authority': 'component_contract', 'canonical_declaration': 'Modelica.Electrical.Machines.Utilities.ParameterRecords.DcPermanentMagnetData.Ra'}
```
...
| `required` | dcpmData.Ra &gt;= 0 |
| `declared_min` | None |
| `variable_role` | parameter |
| `premise_state` | established |
| `authority` | component_contract |
| `semantic_role` | component.machine.winding_resistance |
| `canonical_declaration` | Modelica.Electrical.Machines.Utilities.ParameterRecords.DcPermanentMagnetData.Ra |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.ThyristorBridge2mPulse_DC_Drive \
      --target dcpmData.Ra --sanitizer physical
```
...
```console
$ sed -n '19p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/ParameterRecords/DcPermanentMagnetData.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcpmData.Ra"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param dcpmData.Ra=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/ParameterRecords/DcPermanentMagnetData.mo:19` |' docs/v2/bugs/
```
