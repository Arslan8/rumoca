# FINDING-03522: `pulse2.twomPulse.filter[2].fa` in `ThyristorBridge2mPulse_DC_Drive`
...
| **Reached as** | `pulse2.twomPulse.filter[2].fa` |
...
| **Signature** | `divisor:divisor-reachable-zero:exp:1367:38c93d6a` |
...
```
denominator            ((2 * (2 * asin(1.0))) * pulse2.twomPulse.filter[2].fa)
witness                pulse2.twomPulse.filter[2].fa = 0
  found by             set to zero
denominator at witness 0.0
at declared values     36.57876381465106
path condition         always evaluated
constraints consulted  pulse2.twomPulse.filter[2].fa: unbounded
```
...
| `constraints` | pulse2.twomPulse.filter[2].fa: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.ThyristorBridge2mPulse_DC_Drive \
      --target pulse2.twomPulse.filter[2].fa --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pulse2.twomPulse.filter[2].fa"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param pulse2.twomPulse.filter[2].fa=0
```
