# FINDING-05119: `idealPump.wNominal` in `WaterPump`
...
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/Sources/IdealPump.mo:24` |
| **Signature** | `divisor:divisor-reachable-zero:exp:751:37aa8cbd` |
...
```
denominator            idealPump.wNominal
witness                idealPump.wNominal = 0
  found by             set to zero
denominator at witness 0.0
at declared values     104.71975511966
path condition         always evaluated
constraints consulted  idealPump.wNominal: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.WaterPump \
      --target idealPump.wNominal --sanitizer divisor
```
...
```console
$ sed -n '24p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/Sources/IdealPump.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "idealPump.wNominal"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/Sources/IdealPump.mo:24` |' docs/v2/bugs/
```
