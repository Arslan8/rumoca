...
| **Signature** | `physical:physical-rule-does-not-apply:var:455:f3e7b7df` |
...
```
predicate     R1.IdealCommutingSwitch1.Goff > 0
premise       refuted
...
| `required` | R1.IdealCommutingSwitch1.Goff &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.CauerLowPassSC \
      --target R1.IdealCommutingSwitch1.Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "R1.IdealCommutingSwitch1.Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param R1.IdealCommutingSwitch1.Goff=0
```
