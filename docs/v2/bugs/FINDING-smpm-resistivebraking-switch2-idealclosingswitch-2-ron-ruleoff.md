# FINDING-02786: `switch2.idealClosingSwitch[2].Ron` in `SMPM_ResistiveBraking`
...
| **Reached as** | `switch2.idealClosingSwitch[2].Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:886:9b0fe941` |
...
```
predicate     switch2.idealClosingSwitch[2].Ron > 0
premise       refuted
...
| `required` | switch2.idealClosingSwitch[2].Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_ResistiveBraking \
      --target switch2.idealClosingSwitch[2].Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switch2.idealClosingSwitch[2].Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switch2.idealClosingSwitch[2].Ron=0
```
