...
| **Reached as** | `simpleTriac.thyristor.Nbv` |
...
| **Signature** | `divisor:divisor-zero-unresolved:exp:572:eb504ae3` |
...
```
denominator            (simpleTriac.thyristor.Nbv * simpleTriac.thyristor.Vt)
witness                simpleTriac.thyristor.Nbv = 0
...
constraints consulted  simpleTriac.thyristor.Nbv: unbounded
```
...
| `constraints` | simpleTriac.thyristor.Nbv: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.SimpleTriacCircuit \
      --target simpleTriac.thyristor.Nbv --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "simpleTriac.thyristor.Nbv"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param simpleTriac.thyristor.Nbv=0
```
