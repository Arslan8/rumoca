...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:61:d0306883` |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.DemoPowerSupplyWithBuffer \
      --target bufferCapacitor.C --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.DemoPowerSupplyWithBuffer --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "bufferCapacitor.C"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param bufferCapacitor.C=0
```
