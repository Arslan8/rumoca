...
| **Signature** | `divisor:divisor-reachable-zero:exp:2857:833ca31e` |
...
```
denominator            dcdcInverter1.iDCFilter.T
witness                dcdcInverter1.iDCFilter.T = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.002
path condition         always evaluated
constraints consulted  dcdcInverter1.iDCFilter.T: unbounded
```
...
| `constraints` | dcdcInverter1.iDCFilter.T: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Drive \
      --target dcdcInverter1.iDCFilter.T --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcdcInverter1.iDCFilter.T"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param dcdcInverter1.iDCFilter.T=0
```
