...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:308:2b436de1` |
...
```
predicate     transformerL.inductor[3].L > 0
premise       refuted
...
| `required` | transformerL.inductor[3].L &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Polyphase.Examples.TransformerYY \
      --target transformerL.inductor[3].L --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "transformerL.inductor[3].L"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param transformerL.inductor[3].L=0
```
