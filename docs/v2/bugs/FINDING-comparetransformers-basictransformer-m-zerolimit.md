...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/Transformer.mo:6` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:178:8d8de0c9` |
...
```
predicate     basicTransformer.M > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Basic.Transformer.M: algebraic_limit (proven, from equation) — the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Basic.Transformer.M'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.CompareTransformers \
      --target basicTransformer.M --sanitizer physical
```
...
```console
$ sed -n '6p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/Transformer.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "basicTransformer.M"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param basicTransformer.M=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/Transformer.mo:6` |' docs/v2/bugs/
```
