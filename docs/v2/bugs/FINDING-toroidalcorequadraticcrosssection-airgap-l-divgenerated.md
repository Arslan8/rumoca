# FINDING-04211: `airGap.l` in `ToroidalCoreQuadraticCrossSection`
...
| **Signature** | `divisor:divisor-introduced-by-translation:exp:400:52bd6319` |
...
```
contract    Modelica.Magnetic.FluxTubes.Shapes.FixedShape.HollowCylinderCircumferentialFlux.l: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
behavior    
confidence  proven
source      
```
...
```
denominator            airGap.A
witness                airGap.l = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.00010000000000000002
path condition         always evaluated
constraints consulted  airGap.l: unbounded
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.BasicExamples.ToroidalCoreQuadraticCrossSection \
      --target airGap.l --sanitizer divisor
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.BasicExamples.ToroidalCoreQuadraticCrossSection --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "airGap.l"
```
