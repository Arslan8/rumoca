# FINDING-01441: `Ra` in `DCPM_CurrentControlled`
...
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/DCMachines/DCPM_CurrentControlled.mo:8` |
| **Signature** | `physical:physical-domain-unenforced:var:3:97aa1b72` |
...
role          —
declaration   Modelica.Electrical.Machines.Examples.DCMachines.DCPM_CurrentControlled.Ra
```
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.Examples.DCMachines.DCPM_CurrentControlled.Ra'}
```
...
| `authority` | source_arithmetic |
| `canonical_declaration` | Modelica.Electrical.Machines.Examples.DCMachines.DCPM_CurrentControlled.Ra |
...
```console
$ sed -n '8p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/DCMachines/DCPM_CurrentControlled.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "Ra"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param Ra=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/DCMachines/DCPM_CurrentControlled.mo:8` |' docs/v2/bugs/
```
