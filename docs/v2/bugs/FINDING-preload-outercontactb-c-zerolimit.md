...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:111:cddfe8f8` |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Translational.Examples.PreLoad \
      --target outerContactB.c --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Translational.Examples.PreLoad --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "outerContactB.c"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param outerContactB.c=0
```
