# DECL-0599: `transResistance` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Basic/CCV.mo:5` |
| **Parameter** | `transResistance` |
| **Type** | `SI.Resistance` |
...
```console
$ sed -n '5p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/CCV.mo"
```

It should declare `transResistance` with type `SI.Resistance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/CCV.mo" | head
```
...
```modelica
// Electrical/Analog/Basic/CCV.mo:5
parameter SI.Resistance transResistance(min=0) = ...;
```
