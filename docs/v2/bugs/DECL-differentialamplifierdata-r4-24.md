# DECL-0636: `R4` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Examples/OpAmps/OpAmpCircuits/DifferentialAmplifierData.mo:24` |
| **Parameter** | `R4` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
It should declare `R4` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/Analog/Examples/OpAmps/OpAmpCircuits/DifferentialAmplifierData.mo:24
parameter SI.Resistance R4(min=0) = ...;
```
