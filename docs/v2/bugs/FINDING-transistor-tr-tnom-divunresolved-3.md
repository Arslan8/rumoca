# FINDING-01258: `Tr.Tnom` in `Transistor`
...
| **Signature** | `divisor:divisor-zero-unresolved:exp:371:7e8d3721` |
...
path condition         (Tr.useTemperatureDependency)
constraints consulted  Tr.Tnom: min=0
```
...
| `path_condition` | (Tr.useTemperatureDependency) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: Tr.useTemperatureDependency |
| `constraints` | Tr.Tnom: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Utilities.Transistor \
      --target Tr.Tnom --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "Tr.Tnom"
```
