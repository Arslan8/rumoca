# DECL-0009: `side_pipeArea` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Fluid/Examples/AST_BatchPlant.mo:606` |
| **Parameter** | `side_pipeArea` |
| **Type** | `SI.Area` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '606p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Examples/AST_BatchPlant.mo"
```

It should declare `side_pipeArea` with type `SI.Area` and no `min`.
...
```modelica
// Fluid/Examples/AST_BatchPlant.mo:606
parameter SI.Area side_pipeArea(min=0) = ...;
```
