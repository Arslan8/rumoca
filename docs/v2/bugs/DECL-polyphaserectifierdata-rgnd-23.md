# DECL-0745: `RGnd` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Polyphase/Examples/Utilities/PolyphaseRectifierData.mo:23` |
| **Parameter** | `RGnd` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '23p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Examples/Utilities/PolyphaseRectifierData.mo"
```

It should declare `RGnd` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/Polyphase/Examples/Utilities/PolyphaseRectifierData.mo:23
parameter SI.Resistance RGnd(min=0) = ...;
```
