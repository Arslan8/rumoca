...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:52:dd4e711d` |
...
```
denominator            len.duration
witness                len.duration = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         not ((time < len.startTime)) and ((time < (len.startTime + len.duration)))
constraints consulted  len.duration: min=0
```
...
| `path_condition` | not ((time &lt; len.startTime)) and ((time &lt; (len.startTime + len.duration))) |
| `verdict` | UNSAT |
| `proof` | the path conditions cannot hold together: time must be both &gt; 0 and &lt; 0 |
| `constraints` | len.duration: min=0 |
...
```console
$ python3 tools/sweep/check_one.py Modelica.ComplexBlocks.Examples.TestConversionBlock \
      --target len.duration --sanitizer divisor
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.ComplexBlocks.Examples.TestConversionBlock --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "len.duration"
```
