...
| **Reached as** | `integrator.opAmp.Vps` |
...
| **Signature** | `divisor:divisor-zero-when-parameters-equal:exp:246:c935f27e` |
...
```
denominator            (integrator.opAmp.vps - integrator.opAmp.vns)
witness                integrator.opAmp.Vps = -15
  found by             set equal to integrator.opAmp.Vns
denominator at witness 0.0
at declared values     30.0
path condition         always evaluated
constraints consulted  integrator.opAmp.Vps: unbounded
```
...
| `denominator` | (integrator.opAmp.vps - integrator.opAmp.vns) |
| `witness` | integrator.opAmp.Vps = -15 |
| `witness_rationale` | set equal to integrator.opAmp.Vns |
...
| `constraints` | integrator.opAmp.Vps: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.Integrator \
      --target integrator.opAmp.Vps --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "integrator.opAmp.Vps"
```
