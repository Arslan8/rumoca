# FINDING-01866: `inverter.GoffTransistor` in `IMC_InverterDrive`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCAC/Polyphase2Level.mo:7` |
| **Signature** | `physical:physical-rule-does-not-apply:var:521:fd807431` |
...
```
contract    Modelica.Electrical.Polyphase.Ideal.IdealGTOThyristor.Goff: allowed (declared, from class_catalog) — inherited from inverter.transistor_p.Goff, which it binds: an ideal switch family member; zero off-conductance is the documented ideal
behavior    allowed
confidence  declared
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.PowerConverters.DCAC.Polyphase2Level.GoffTransistor'}
```
...
```console
$ sed -n '7p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCAC/Polyphase2Level.mo"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCAC/Polyphase2Level.mo:7` |' docs/v2/bugs/
```
