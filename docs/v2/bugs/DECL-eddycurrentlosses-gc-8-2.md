# DECL-0084: `Gc` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/QuasiStatic/FundamentalWave/Examples/Components/EddyCurrentLosses.mo:8` |
| **Parameter** | `Gc` |
| **Type** | `SI.Conductance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '8p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/QuasiStatic/FundamentalWave/Examples/Components/EddyCurrentLosses.mo"
```

It should declare `Gc` with type `SI.Conductance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/QuasiStatic/FundamentalWave/Examples/Components/EddyCurrentLosses.mo" | head
```
...
```modelica
// Magnetic/QuasiStatic/FundamentalWave/Examples/Components/EddyCurrentLosses.mo:8
parameter SI.Conductance Gc(min=0) = ...;
```
