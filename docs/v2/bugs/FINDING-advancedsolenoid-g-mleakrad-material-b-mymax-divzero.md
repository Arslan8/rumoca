# FINDING-04318: `G_mLeakRad.material.B_myMax` in `AdvancedSolenoid`
...
| **Reached as** | `G_mLeakRad.material.B_myMax` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/BaseClasses/FixedShape.mo:31` |
| **Signature** | `divisor:divisor-reachable-zero:exp:1934:c7260ecc` |
...
```
denominator            G_mLeakRad.material.B_myMax
witness                G_mLeakRad.material.B_myMax = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  G_mLeakRad.material.B_myMax: unbounded
```
...
| `constraints` | G_mLeakRad.material.B_myMax: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.SolenoidActuator.Components.AdvancedSolenoid \
      --target G_mLeakRad.material.B_myMax --sanitizer divisor
```
...
```console
$ sed -n '31p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/BaseClasses/FixedShape.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "G_mLeakRad.material.B_myMax"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param G_mLeakRad.material.B_myMax=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/BaseClasses/FixedShape.mo:31` |' docs/v2/bugs/
```
