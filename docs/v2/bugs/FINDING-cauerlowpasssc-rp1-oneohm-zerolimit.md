...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/Utilities/SwitchedCapacitor.mo:30` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:706:0991ff59` |
...
```
contract    Modelica.Electrical.Analog.Examples.Utilities.SwitchedCapacitor.oneOhm: forbidden (proven, from bound) — the declaration excludes zero (declared constant)
behavior    forbidden
confidence  proven
source      bound
```
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Examples.Utilities.SwitchedCapacitor.oneOhm'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.CauerLowPassSC \
      --target Rp1.oneOhm --sanitizer physical
```
...
```console
$ sed -n '30p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/Utilities/SwitchedCapacitor.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "Rp1.oneOhm"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param Rp1.oneOhm=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/Utilities/SwitchedCapacitor.mo:30` |' docs/v2/bugs/
```
