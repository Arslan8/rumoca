# FINDING-05087: `ramp2.duration` in `DoubleRamp`
...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:95:c92bdf03` |
...
```
denominator            ramp2.duration
witness                ramp2.duration = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.2
path condition         not ((time < ramp2.startTime)) and ((time < (ramp2.startTime + ramp2.duration)))
constraints consulted  ramp2.duration: min=0
```
...
| `path_condition` | not ((time &lt; ramp2.startTime)) and ((time &lt; (ramp2.startTime + ramp2.duration))) |
| `verdict` | UNSAT |
| `proof` | the path conditions cannot hold together: time must be both &gt; 0.6 and &lt; 0.6 |
| `constraints` | ramp2.duration: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.Utilities.DoubleRamp \
      --target ramp2.duration --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "ramp2.duration"
```
