# FINDING-04664: `spring.c` in `FirstGrounded`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Rotational/Components/Spring.mo:4` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:45:d771bd60` |
...
```
contract    Modelica.Mechanics.Rotational.Components.Spring.c: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'RotationalSpringConstant', 'unit': 'N.m/rad', 'confidence': 'QUANTITY', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Mechanics.Rotational.Components.Spring.c'}
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Rotational.Examples.FirstGrounded \
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Rotational/Components/Spring.mo"
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Rotational.Examples.FirstGrounded --keep-artifact /tmp/m.rbc \
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Rotational/Components/Spring.mo:4` |' docs/v2/bugs/
```
