# FINDING-03919: `rectifier.thyristor.Ron` in `ThyristorCenterTapmPulse_RLV_Characteristic`
...
| **Reached as** | `rectifier.thyristor.Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:157:30d44737` |
...
```
predicate     rectifier.thyristor.Ron > 0
premise       refuted
...
| `required` | rectifier.thyristor.Ron &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTapmPulse.ThyristorCenterTapmPulse_RLV_Characteristic \
      --target rectifier.thyristor.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "rectifier.thyristor.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param rectifier.thyristor.Ron=0
```
