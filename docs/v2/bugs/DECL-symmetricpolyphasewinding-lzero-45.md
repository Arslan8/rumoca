# DECL-0251: `Lzero` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/FundamentalWave/BasicMachines/Components/SymmetricPolyphaseWinding.mo:45` |
| **Parameter** | `Lzero` |
| **Type** | `SI.Inductance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '45p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FundamentalWave/BasicMachines/Components/SymmetricPolyphaseWinding.mo"
```

It should declare `Lzero` with type `SI.Inductance` and no `min`.
...
```modelica
// Magnetic/FundamentalWave/BasicMachines/Components/SymmetricPolyphaseWinding.mo:45
parameter SI.Inductance Lzero(min=0) = ...;
```
