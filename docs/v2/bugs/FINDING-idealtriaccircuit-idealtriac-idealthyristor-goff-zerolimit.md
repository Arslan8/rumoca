...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:46:76a4eb86` |
...
```
predicate     idealTriac.idealThyristor.Goff > 0
premise       refuted
...
| `required` | idealTriac.idealThyristor.Goff &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.IdealTriacCircuit \
      --target idealTriac.idealThyristor.Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "idealTriac.idealThyristor.Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param idealTriac.idealThyristor.Goff=0
```
