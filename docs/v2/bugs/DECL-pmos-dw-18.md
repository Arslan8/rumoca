# DECL-0381: `dW` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Semiconductors/PMOS.mo:18` |
| **Parameter** | `dW` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '18p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/PMOS.mo"
```

It should declare `dW` with type `SI.Length` and no `min`.
...
```modelica
// Electrical/Analog/Semiconductors/PMOS.mo:18
parameter SI.Length dW(min=0) = ...;
```
