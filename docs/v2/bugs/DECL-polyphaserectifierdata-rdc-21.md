# DECL-0744: `RDC` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Polyphase/Examples/Utilities/PolyphaseRectifierData.mo:21` |
| **Parameter** | `RDC` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '21p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Polyphase/Examples/Utilities/PolyphaseRectifierData.mo"
```

It should declare `RDC` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/Polyphase/Examples/Utilities/PolyphaseRectifierData.mo:21
parameter SI.Resistance RDC(min=0) = ...;
```
