# DECL-0254: `Lrzero` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/FundamentalWave/BasicMachines/InductionMachines/IM_SlipRing.mo:45` |
| **Parameter** | `Lrzero` |
| **Type** | `SI.Inductance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '45p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FundamentalWave/BasicMachines/InductionMachines/IM_SlipRing.mo"
```

It should declare `Lrzero` with type `SI.Inductance` and no `min`.
...
```modelica
// Magnetic/FundamentalWave/BasicMachines/InductionMachines/IM_SlipRing.mo:45
parameter SI.Inductance Lrzero(min=0) = ...;
```
