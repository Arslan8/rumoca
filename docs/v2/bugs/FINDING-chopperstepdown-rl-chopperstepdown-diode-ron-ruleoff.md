# FINDING-04021: `chopperStepDown.diode.Ron` in `ChopperStepDown_RL`
...
| **Reached as** | `chopperStepDown.diode.Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:73:f6f7a177` |
...
```
predicate     chopperStepDown.diode.Ron > 0
premise       refuted
...
| `required` | chopperStepDown.diode.Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCDC.ChopperStepDown.ChopperStepDown_RL \
      --target chopperStepDown.diode.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "chopperStepDown.diode.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param chopperStepDown.diode.Ron=0
```
