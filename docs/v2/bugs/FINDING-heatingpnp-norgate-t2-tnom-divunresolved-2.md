# FINDING-00397: `T2.Tnom` in `HeatingPNP_NORGate`
...
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/PNP.mo:75` |
| **Signature** | `divisor:divisor-zero-unresolved:exp:1294:bb3a778d` |
...
path condition         (T2.useTemperatureDependency)
constraints consulted  T2.Tnom: min=0
```
...
| `path_condition` | (T2.useTemperatureDependency) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: T2.useTemperatureDependency |
| `constraints` | T2.Tnom: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.HeatingPNP_NORGate \
      --target T2.Tnom --sanitizer divisor
```
...
```console
$ sed -n '75p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/PNP.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "T2.Tnom"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/PNP.mo:75` |' docs/v2/bugs/
```
