# FINDING-04221: `pmActuator.armature.c` in `ArmatureStroke`
...
| **Reached as** | `pmActuator.armature.c` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/Utilities/TranslatoryArmatureAndStopper.mo:9` |
| **Signature** | `physical:physical-intent-question:var:46:52d110b8` |
...
```
predicate     pmActuator.armature.c > 0
premise       unknown
authority     quantity_or_unit
role          —
declaration   Modelica.Magnetic.FluxTubes.Examples.Utilities.TranslatoryArmatureAndStopper.c
```
...
> `pmActuator.armature.c` carries no bound that would exclude it, which is unusual for a translational spring constant — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
```
{'quantity': 'TranslationalSpringConstant', 'unit': 'N/m', 'confidence': 'QUANTITY', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Magnetic.FluxTubes.Examples.Utilities.TranslatoryArmatureAndStopper.c'}
```
...
| `required` | pmActuator.armature.c &gt; 0 |
| `declared_min` | None |
| `variable_role` | parameter |
| `observation` | permissive-declaration |
| `premise_state` | unknown |
| `authority` | quantity_or_unit |
| `canonical_declaration` | Modelica.Magnetic.FluxTubes.Examples.Utilities.TranslatoryArmatureAndStopper.c |
| `question` | `pmActuator.armature.c` carries no bound that would exclude it, which is unusual for a translational spring constant — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.ArmatureStroke \
      --target pmActuator.armature.c --sanitizer physical
```
...
```console
$ sed -n '9p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/Utilities/TranslatoryArmatureAndStopper.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pmActuator.armature.c"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param pmActuator.armature.c=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/Utilities/TranslatoryArmatureAndStopper.mo:9` |' docs/v2/bugs/
```
