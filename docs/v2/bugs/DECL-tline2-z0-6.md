# DECL-0663: `Z0` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Lines/TLine2.mo:6` |
| **Parameter** | `Z0` |
| **Type** | `SI.Resistance` |
...
```console
$ sed -n '6p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Lines/TLine2.mo"
```

It should declare `Z0` with type `SI.Resistance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Lines/TLine2.mo" | head
```
...
```modelica
// Electrical/Analog/Lines/TLine2.mo:6
parameter SI.Resistance Z0(min=0) = ...;
```
