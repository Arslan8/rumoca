# FINDING-02046: `aimc.Rr` in `IMC_Transformer`
...
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/InductionMachines/IM_SquirrelCage.mo:33` |
| **Signature** | `physical:physical-domain-unenforced:var:390:19e8337f` |
...
declaration   Modelica.Electrical.Machines.BasicMachines.InductionMachines.IM_SquirrelCage.Rr
```
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'semantic_role': 'component.machine.winding_resistance', 'binding_source': 'component_type', 'confidence': 'QUANTITY_AND_UNIT', 'declaring_class': 'Modelica.Electrical.Machines.BasicMachines.InductionMachines.IM_SquirrelCage', 'member': 'Rr', 'premise_state': 'established', 'authority': 'component_contract', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.InductionMachines.IM_SquirrelCage.Rr'}
```
...
| `canonical_declaration` | Modelica.Electrical.Machines.BasicMachines.InductionMachines.IM_SquirrelCage.Rr |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_Transformer \
      --target aimc.Rr --sanitizer physical
```
...
```console
$ sed -n '33p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/InductionMachines/IM_SquirrelCage.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "aimc.Rr"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param aimc.Rr=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/InductionMachines/IM_SquirrelCage.mo:33` |' docs/v2/bugs/
```
