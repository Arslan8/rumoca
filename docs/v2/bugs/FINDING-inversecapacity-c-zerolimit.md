# FINDING-05155: `C` in `InverseCapacity`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:0:137c057c` |
...
```
contract    Modelica.Thermal.HeatTransfer.Components.HeatCapacitor.C: algebraic_limit (proven, from equation) — inherited from mass.C, which it binds: the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
...
```console
$ python3 tools/sweep/check_one.py Modelica.Thermal.HeatTransfer.Examples.Utilities.InverseCapacity \
...
```console
$ python3 tools/sweep/check_one.py Modelica.Thermal.HeatTransfer.Examples.Utilities.InverseCapacity --keep-artifact /tmp/m.rbc \
