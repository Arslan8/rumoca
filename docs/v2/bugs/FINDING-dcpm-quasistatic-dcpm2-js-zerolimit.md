# FINDING-01554: `dcpm2.Js` in `DCPM_QuasiStatic`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:523:65438551` |
...
```
contract    Modelica.Mechanics.Rotational.Components.Inertia.J: algebraic_limit (proven, from equation) — inherited from dcpm2.inertiaStator.J, which it binds: the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_QuasiStatic \
      --target dcpm2.Js --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcpm2.Js"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param dcpm2.Js=0
```
