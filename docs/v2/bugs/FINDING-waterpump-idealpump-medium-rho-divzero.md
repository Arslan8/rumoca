# FINDING-05114: `idealPump.medium.rho` in `WaterPump`
...
| **Signature** | `divisor:divisor-reachable-zero:exp:672:deb0d9a4` |
...
```
denominator            idealPump.medium.rho
witness                idealPump.medium.rho = 0
  found by             set to zero
denominator at witness 0.0
at declared values     995.6
path condition         always evaluated
constraints consulted  idealPump.medium.rho: min=0
```
...
| `constraints` | idealPump.medium.rho: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.WaterPump \
      --target idealPump.medium.rho --sanitizer divisor
```
