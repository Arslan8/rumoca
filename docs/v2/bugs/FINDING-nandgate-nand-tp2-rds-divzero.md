# FINDING-00770: `Nand.TP2.RDS` in `NandGate`
...
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/PMOS.mo:42` |
| **Signature** | `divisor:divisor-reachable-zero:exp:838:6e4b0917` |
...
```
denominator            Nand.TP2.RDS
witness                Nand.TP2.RDS = 0
  found by             set to zero
denominator at witness 0.0
at declared values     10000000.0
path condition         not (((Nand.TP2.RDS < 1e-20) and (Nand.TP2.RDS > -1e-20)))
constraints consulted  Nand.TP2.RDS: unbounded
```
...
| `path_condition` | not (((Nand.TP2.RDS &lt; 1e-20) and (Nand.TP2.RDS &gt; -1e-20))) |
| `verdict` | SAT |
| `proof` | every guard still holds under the witness |
| `constraints` | Nand.TP2.RDS: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.NandGate \
      --target Nand.TP2.RDS --sanitizer divisor
```
...
```console
$ sed -n '42p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/PMOS.mo"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/PMOS.mo:42` |' docs/v2/bugs/
```
