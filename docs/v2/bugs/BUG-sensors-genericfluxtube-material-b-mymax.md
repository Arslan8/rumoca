...
| **Trigger** | `genericFluxTube.material.B_myMax = 0` |
...
`ModelicaTest.Magnetic.FluxTubes.Sensors` runs cleanly at its declared values. Setting `genericFluxTube.material.B_myMax = 0` — a value the declaration permits — makes it fail, in Rumoca and in OpenModelica.
...
$ python3 tools/sweep/check_one.py ModelicaTest.Magnetic.FluxTubes.Sensors \
      --target genericFluxTube.material.B_myMax --keep-parameter-chains
```
...
simulate(ModelicaTest.Magnetic.FluxTubes.Sensors, simflags="-override genericFluxTube.material.B_myMax=0");
EOF
```
