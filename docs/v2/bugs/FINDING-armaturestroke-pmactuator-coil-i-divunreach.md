...
| **Reached as** | `pmActuator.coil.i` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-unreachable-under-witness` |
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Basic/ElectroMagneticConverter.mo:41` |
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:986:2b3307eb` |
...
| `denominator_range` |  |
| `path_condition` | ((abs(pmActuator.coil.i) &gt; pmActuator.coil.eps)) |
| `verdict` | UNSAT |
| `proof` | the denominator is zero as declared, and ((abs(pmActuator.coil.i) &gt; pmActuator.coil.eps)) |
| `constraints` |  |
| `guard_that_excludes_it` | ((abs(pmActuator.coil.i) &gt; pmActuator.coil.eps)) |
| `note` | the branch containing the division is not taken where the denominator vanishes, so it is never evaluated there |
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.ArmatureStroke \
      --target pmActuator.coil.i --sanitizer divisor
```
...
```console
$ sed -n '41p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Basic/ElectroMagneticConverter.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pmActuator.coil.i"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param pmActuator.coil.i=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Basic/ElectroMagneticConverter.mo:41` |' docs/v2/bugs/
```
