# FINDING-02798: `smpm.spacePhasorS.turnsRatio` in `SMPM_ResistiveBraking`
...
| **Reached as** | `smpm.spacePhasorS.turnsRatio` |
...
| **Signature** | `divisor:divisor-reachable-zero:exp:4866:6a659eec` |
...
```
denominator            smpm.spacePhasorS.turnsRatio
witness                smpm.spacePhasorS.turnsRatio = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  smpm.spacePhasorS.turnsRatio: unbounded
```
...
| `constraints` | smpm.spacePhasorS.turnsRatio: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_ResistiveBraking \
      --target smpm.spacePhasorS.turnsRatio --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smpm.spacePhasorS.turnsRatio"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param smpm.spacePhasorS.turnsRatio=0
```
