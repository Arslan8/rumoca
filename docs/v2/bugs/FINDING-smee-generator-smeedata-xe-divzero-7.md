# FINDING-02520: `smeeData.xe` in `SMEE_Generator`
...
| **Reached as** | `smeeData.xe` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo:134` |
| **Signature** | `divisor:divisor-reachable-zero:exp:2075:808e78b1` |
...
```
denominator            smeeData.xe
witness                smeeData.xe = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.5384615384615383
path condition         always evaluated
constraints consulted  smeeData.xe: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMEE_Generator \
      --target smeeData.xe --sanitizer divisor
```
...
```console
$ sed -n '134p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smeeData.xe"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo:134` |' docs/v2/bugs/
```
