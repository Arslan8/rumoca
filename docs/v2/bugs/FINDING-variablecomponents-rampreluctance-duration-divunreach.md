# FINDING-05357: `rampReluctance.duration` in `VariableComponents`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `ModelicaTest.Magnetic.FluxTubes.VariableComponents` |
| **Reached as** | `rampReluctance.duration` |
...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:132:4ad653eb` |
...
```
denominator            rampReluctance.duration
witness                rampReluctance.duration = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.3
path condition         not ((time < rampReluctance.startTime)) and ((time < (rampReluctance.startTime + rampReluctance.duration)))
constraints consulted  rampReluctance.duration: min=0
```
...
| `denominator_at_declared_values` | 0.3 |
| `denominator_range` | [0, inf] |
| `path_condition` | not ((time &lt; rampReluctance.startTime)) and ((time &lt; (rampReluctance.startTime + rampReluctance.duration))) |
| `verdict` | UNSAT |
| `proof` | the path conditions cannot hold together: time must be both &gt; 0.1 and &lt; 0.1 |
| `constraints` | rampReluctance.duration: min=0 |
...
```console
$ python3 tools/sweep/check_one.py ModelicaTest.Magnetic.FluxTubes.VariableComponents \
      --target rampReluctance.duration --sanitizer divisor
```
...
```console
$ python3 tools/sweep/check_one.py ModelicaTest.Magnetic.FluxTubes.VariableComponents --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "rampReluctance.duration"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param rampReluctance.duration=0
```
