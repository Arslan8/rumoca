# FINDING-02488: `smeeData.xdTransient` in `SMEE_Generator`
...
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo:76` |
| **Signature** | `divisor:divisor-zero-when-parameters-equal:exp:1892:fca7aeea` |
...
```
denominator            (smeeData.xdTransient - smeeData.xdSubtransient)
witness                smeeData.xdTransient = 0.121429
  found by             set equal to smeeData.xdSubtransient
denominator at witness 0.0
at declared values     0.016071429000000012
path condition         always evaluated
constraints consulted  smeeData.xdTransient: unbounded
```
...
| `denominator` | (smeeData.xdTransient - smeeData.xdSubtransient) |
| `witness` | smeeData.xdTransient = 0.121429 |
| `witness_rationale` | set equal to smeeData.xdSubtransient |
| `denominator_at_witness` | 0.0 |
| `denominator_at_declared_values` | 0.016071429000000012 |
| `denominator_range` | [-inf, inf] |
| `path_condition` | always evaluated |
| `verdict` | SAT |
| `proof` | the division is not inside any branch |
| `constraints` | smeeData.xdTransient: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMEE_Generator \
      --target smeeData.xdTransient --sanitizer divisor
```
...
```console
$ sed -n '76p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smeeData.xdTransient"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param smeeData.xdTransient=0.121429
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo:76` |' docs/v2/bugs/
```
