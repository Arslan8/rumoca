# FINDING-03504: `pulse2.twomPulse.filter[3].transferFunction[2].a_end` in `ThyristorBridge2mPulse_DC_Drive`
...
| **Reached as** | `pulse2.twomPulse.filter[3].transferFunction[2].a_end` |
...
| **Signature** | `divisor:divisor-reachable-zero:exp:4362:b2f2dea6` |
...
```
denominator            pulse2.twomPulse.filter[3].transferFunction[2].a_end
witness                pulse2.twomPulse.filter[3].transferFunction[2].a_end = 0
  found by             set to zero
denominator at witness 0.0
at declared values     None
path condition         always evaluated
constraints consulted  pulse2.twomPulse.filter[3].transferFunction[2].a_end: unbounded
```
...
| `constraints` | pulse2.twomPulse.filter[3].transferFunction[2].a_end: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.ThyristorBridge2mPulse_DC_Drive \
      --target pulse2.twomPulse.filter[3].transferFunction[2].a_end --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pulse2.twomPulse.filter[3].transferFunction[2].a_end"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param pulse2.twomPulse.filter[3].transferFunction[2].a_end=0
```
