# FINDING-04141: `hbridge.inverter_p.GoffTransistor` in `HBridge_TrianglePWM_RL`
...
| **Reached as** | `hbridge.inverter_p.GoffTransistor` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:82:9c71c020` |
...
```
predicate     hbridge.inverter_p.GoffTransistor > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealGTOThyristor.Goff: feature_disabled (proven, from equation) — inherited from hbridge.inverter_p.transistor_p.Goff, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
| `required` | hbridge.inverter_p.GoffTransistor &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCDC.HBridge.HBridge_TrianglePWM_RL \
      --target hbridge.inverter_p.GoffTransistor --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "hbridge.inverter_p.GoffTransistor"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param hbridge.inverter_p.GoffTransistor=0
```
