...
| **Signature** | `divisor:divisor-reachable-zero:exp:481:22be0b67` |
...
```
denominator            motor[3].tau_down
witness                motor[3].tau_down = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.025
path condition         always evaluated
constraints consulted  motor[3].tau_down: unbounded
```
...
| `constraints` | motor[3].tau_down: unbounded |
...
$ python3 tools/sweep/check_one.py RigidBody.Examples.QuadrotorSIL \
      --target motor[3].tau_down --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "motor[3].tau_down"
```
