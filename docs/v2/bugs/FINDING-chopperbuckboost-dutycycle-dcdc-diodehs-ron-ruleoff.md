# FINDING-04003: `dcdc.diodeHS.Ron` in `ChopperBuckBoost_DutyCycle`
...
| **Signature** | `physical:physical-rule-does-not-apply:var:107:00b282b6` |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCDC.ChopperBuckBoost.ChopperBuckBoost_DutyCycle \
      --target dcdc.diodeHS.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcdc.diodeHS.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param dcdc.diodeHS.Ron=0
```
