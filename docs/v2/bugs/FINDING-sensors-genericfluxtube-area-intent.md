# FINDING-05351: `genericFluxTube.area` in `Sensors`
...
| **Signature** | `physical:physical-intent-question:var:28:c3044b80` |
...
```
predicate     genericFluxTube.area > 0
premise       unknown
...
| `question` | `genericFluxTube.area` carries no bound that would exclude it, which is unusual for an area — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
```console
$ python3 tools/sweep/check_one.py ModelicaTest.Magnetic.FluxTubes.Sensors \
      --target genericFluxTube.area --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py ModelicaTest.Magnetic.FluxTubes.Sensors --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "genericFluxTube.area"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param genericFluxTube.area=0
```
