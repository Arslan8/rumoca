# FINDING-01422: `volumeFlow.medium.rho` in `DCPM_Cooling`
...
| **Signature** | `divisor:divisor-reachable-zero:exp:1625:5cdd2be5` |
...
```
denominator            volumeFlow.medium.rho
witness                volumeFlow.medium.rho = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  volumeFlow.medium.rho: min=0
```
...
| `constraints` | volumeFlow.medium.rho: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Cooling \
      --target volumeFlow.medium.rho --sanitizer divisor
```
