# FINDING-00266: `switch2.Goff` in `ControlledSwitchWithArc`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:80:592a7040` |
...
```
contract    Modelica.Electrical.Analog.Ideal.ControlledCloserWithArc.Goff: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Ideal.ControlledCloserWithArc.Goff'}
```
