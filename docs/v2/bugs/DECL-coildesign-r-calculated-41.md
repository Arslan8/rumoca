# DECL-0831: `R_calculated` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/FluxTubes/Examples/Utilities/CoilDesign.mo:41` |
| **Parameter** | `R_calculated` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '41p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/Utilities/CoilDesign.mo"
```

It should declare `R_calculated` with type `SI.Resistance` and no `min`.
...
```modelica
// Magnetic/FluxTubes/Examples/Utilities/CoilDesign.mo:41
parameter SI.Resistance R_calculated(min=0) = ...;
```
