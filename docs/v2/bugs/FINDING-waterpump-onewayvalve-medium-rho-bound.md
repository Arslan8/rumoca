# FINDING-05100: `oneWayValve.medium.rho` in `WaterPump`
...
| **Signature** | `physical:physical-bound-permits-zero:var:162:e2c38657` |
...
```
predicate     oneWayValve.medium.rho > 0
premise       established
...
| `required` | oneWayValve.medium.rho &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.WaterPump \
      --target oneWayValve.medium.rho --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "oneWayValve.medium.rho"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param oneWayValve.medium.rho=0
```
