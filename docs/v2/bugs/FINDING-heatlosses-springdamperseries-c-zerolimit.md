...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Forces/SpringDamperSeries.mo:6` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:744:6923eace` |
...
```
predicate     springDamperSeries.c > 0
premise       refuted
...
```
contract    Modelica.Mechanics.MultiBody.Forces.SpringDamperSeries.c: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'TranslationalSpringConstant', 'unit': 'N/m', 'confidence': 'QUANTITY', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Mechanics.MultiBody.Forces.SpringDamperSeries.c'}
```
...
| `required` | springDamperSeries.c &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Elementary.HeatLosses \
      --target springDamperSeries.c --sanitizer physical
```
...
```console
$ sed -n '6p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Forces/SpringDamperSeries.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "springDamperSeries.c"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param springDamperSeries.c=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Forces/SpringDamperSeries.mo:6` |' docs/v2/bugs/
```
