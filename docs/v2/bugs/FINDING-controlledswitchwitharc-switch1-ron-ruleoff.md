...
| **Signature** | `physical:physical-rule-does-not-apply:var:30:891d80c9` |
...
```
contract    Modelica.Electrical.Analog.Ideal.ControlledIdealClosingSwitch.Ron: allowed (declared, from class_catalog) — an ideal switch family member; zero on-resistance is the documented ideal [matched by prefix, lower confidence]
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Ideal.ControlledIdealClosingSwitch.Ron'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.ControlledSwitchWithArc \
      --target switch1.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switch1.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switch1.Ron=0
```
