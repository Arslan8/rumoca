# DECL-0566: `headLength` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/package.mo:173` |
...
```console
$ sed -n '173p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/package.mo"
```

It should declare `headLength` with type `SI.Length` and no `min`.
...
```modelica
// Mechanics/MultiBody/package.mo:173
parameter SI.Length headLength(min=0) = ...;
```
