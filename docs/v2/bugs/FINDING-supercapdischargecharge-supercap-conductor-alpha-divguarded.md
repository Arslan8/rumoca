# FINDING-01265: `superCap.conductor.alpha` in `SuperCapDischargeCharge`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Electrical.Batteries.Examples.SuperCapDischargeCharge` |
| **Reached as** | `superCap.conductor.alpha` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-guarded-by-assertion` |
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/Conductor.mo:16` |
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:567:13c796c7` |
...
The denominator can be zeroed arithmetically, but the model asserts it is bounded away from zero, so the assignment is one the model already rejects.

It does **not** claim that anything is wrong — this is reported so a reader can see the guard was found, and can challenge it if it is insufficient.
...
```
denominator            (1 + (superCap.conductor.alpha * (superCap.conductor.T_heatPort - superCap.conductor.T_ref)))
witness                none needed
  found by             the denominator is zero as declared
denominator at witness 0.0
at declared values     None
...
| `denominator_at_declared_values` | None |
| `denominator_range` |  |
| `path_condition` | always evaluated |
| `verdict` | UNSAT |
| `proof` | the model asserts this denominator is bounded away from zero, so zero is outside its declared domain |
| `constraints` |  |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Batteries.Examples.SuperCapDischargeCharge \
      --target superCap.conductor.alpha --sanitizer divisor
```
...
```console
$ sed -n '16p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/Conductor.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "superCap.conductor.alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param superCap.conductor.alpha=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/Conductor.mo:16` |' docs/v2/bugs/
```
