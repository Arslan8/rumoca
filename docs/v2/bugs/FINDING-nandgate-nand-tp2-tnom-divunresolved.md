# FINDING-00772: `Nand.TP2.Tnom` in `NandGate`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Electrical.Analog.Examples.NandGate` |
| **Reached as** | `Nand.TP2.Tnom` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-zero-unresolved` |
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/PMOS.mo:52` |
| **Signature** | `divisor:divisor-zero-unresolved:exp:948:a58da514` |
...
```
denominator            Nand.TP2.Tnom
witness                Nand.TP2.Tnom = 0
  found by             set to zero
denominator at witness 0.0
at declared values     300.15
path condition         (Nand.TP2.useTemperatureDependency)
constraints consulted  Nand.TP2.Tnom: min=0
```
...
| `path_condition` | (Nand.TP2.useTemperatureDependency) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: Nand.TP2.useTemperatureDependency |
| `constraints` | Nand.TP2.Tnom: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.NandGate \
      --target Nand.TP2.Tnom --sanitizer divisor
```
...
```console
$ sed -n '52p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/PMOS.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "Nand.TP2.Tnom"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/PMOS.mo:52` |' docs/v2/bugs/
```
