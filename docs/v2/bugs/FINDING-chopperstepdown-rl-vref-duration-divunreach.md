# FINDING-04028: `vRef.duration` in `ChopperStepDown_RL`
...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:715:423c8ab2` |
...
```
denominator            vRef.duration
witness                vRef.duration = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.05
path condition         not ((time < vRef.startTime)) and ((time < (vRef.startTime + vRef.duration)))
constraints consulted  vRef.duration: min=0
```
...
| `denominator_at_declared_values` | 0.05 |
| `denominator_range` | [0, inf] |
| `path_condition` | not ((time &lt; vRef.startTime)) and ((time &lt; (vRef.startTime + vRef.duration))) |
| `verdict` | UNSAT |
| `proof` | the path conditions cannot hold together: time must be both &gt; 0.025 and &lt; 0.025 |
| `constraints` | vRef.duration: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCDC.ChopperStepDown.ChopperStepDown_RL \
      --target vRef.duration --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "vRef.duration"
```
