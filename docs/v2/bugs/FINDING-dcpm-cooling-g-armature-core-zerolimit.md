...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/DCMachines/DCPM_Cooling.mo:24` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:12:5a32d086` |
...
```
contract    Modelica.Thermal.HeatTransfer.Components.ThermalConductor.G: feature_disabled (proven, from equation) — inherited from armatureCore.G, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'ThermalConductance', 'unit': 'W/K', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Cooling.G_armature_core'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Cooling \
      --target G_armature_core --sanitizer physical
```
...
```console
$ sed -n '24p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/DCMachines/DCPM_Cooling.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "G_armature_core"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param G_armature_core=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/DCMachines/DCPM_Cooling.mo:24` |' docs/v2/bugs/
```
