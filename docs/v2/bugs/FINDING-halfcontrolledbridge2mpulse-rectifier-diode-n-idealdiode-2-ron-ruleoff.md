# FINDING-03440: `rectifier.diode_n.idealDiode[2].Ron` in `HalfControlledBridge2mPulse`
...
| **Reached as** | `rectifier.diode_n.idealDiode[2].Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:271:f7006a69` |
...
```
predicate     rectifier.diode_n.idealDiode[2].Ron > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealDiode.Ron: allowed (declared, from class_catalog) — an ideal switch family member; zero on-resistance is the documented ideal [matched by prefix, lower confidence]
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Ideal.IdealDiode.Ron'}
```
...
| `required` | rectifier.diode_n.idealDiode[2].Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.HalfControlledBridge2mPulse \
      --target rectifier.diode_n.idealDiode[2].Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "rectifier.diode_n.idealDiode[2].Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param rectifier.diode_n.idealDiode[2].Ron=0
```
