# FINDING-01562: `dcpm1.wNominal` in `DCPM_QuasiStatic`
...
| **Signature** | `divisor:divisor-reachable-zero:exp:225:4701d350` |
...
```
denominator            (dcpm1.wNominal * dcpm1.psi_eNominal)
witness                dcpm1.wNominal = 0
  found by             set to zero
denominator at witness 0.0
at declared values     149.22565104551515
path condition         always evaluated
constraints consulted  dcpm1.wNominal: unbounded
```
...
| `constraints` | dcpm1.wNominal: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_QuasiStatic \
      --target dcpm1.wNominal --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcpm1.wNominal"
```
