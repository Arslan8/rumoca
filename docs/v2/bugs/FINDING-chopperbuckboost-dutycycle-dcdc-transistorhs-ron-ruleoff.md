# FINDING-04001: `dcdc.transistorHS.Ron` in `ChopperBuckBoost_DutyCycle`
...
| **Reached as** | `dcdc.transistorHS.Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:89:61bb216e` |
...
```
predicate     dcdc.transistorHS.Ron > 0
premise       refuted
...
| `required` | dcdc.transistorHS.Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCDC.ChopperBuckBoost.ChopperBuckBoost_DutyCycle \
      --target dcdc.transistorHS.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcdc.transistorHS.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param dcdc.transistorHS.Ron=0
```
