# FINDING-03679: `pulse2.twoPulse.filter[1].transferFunction[2].a_end` in `ThyristorBridge2Pulse_DC_Drive`
...
| **Reached as** | `pulse2.twoPulse.filter[1].transferFunction[2].a_end` |
...
| **Signature** | `divisor:divisor-reachable-zero:exp:2259:867b5063` |
...
```
denominator            pulse2.twoPulse.filter[1].transferFunction[2].a_end
witness                pulse2.twoPulse.filter[1].transferFunction[2].a_end = 0
  found by             set to zero
denominator at witness 0.0
at declared values     None
path condition         always evaluated
constraints consulted  pulse2.twoPulse.filter[1].transferFunction[2].a_end: unbounded
```
...
| `constraints` | pulse2.twoPulse.filter[1].transferFunction[2].a_end: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2Pulse.ThyristorBridge2Pulse_DC_Drive \
      --target pulse2.twoPulse.filter[1].transferFunction[2].a_end --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pulse2.twoPulse.filter[1].transferFunction[2].a_end"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param pulse2.twoPulse.filter[1].transferFunction[2].a_end=0
```
