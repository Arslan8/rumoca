# DECL-0601: `Rcm` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Basic/OpAmpDetailed.mo:7` |
| **Parameter** | `Rcm` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '7p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/OpAmpDetailed.mo"
```

It should declare `Rcm` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/Analog/Basic/OpAmpDetailed.mo:7
parameter SI.Resistance Rcm(min=0) = ...;
```
