...
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:5686:3b22f8ac` |
...
```
denominator            (1 + (oLine2.G[1].alpha * (oLine2.G[1].T_heatPort - oLine2.G[1].T_ref)))
witness                none needed
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.CompareLineTrunks \
      --target oLine2.G[1].alpha --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "oLine2.G[1].alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param oLine2.G[1].alpha=0
```
