...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/HeatTransfer/Components/ThermalConductor.mo:5` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:102:7ea938cb` |
...
```
predicate     conductor.thermalConductor.G > 0
premise       refuted
...
```
contract    Modelica.Thermal.HeatTransfer.Components.ThermalConductor.G: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'ThermalConductance', 'unit': 'W/K', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Thermal.HeatTransfer.Components.ThermalConductor.G'}
```
...
| `required` | conductor.thermalConductor.G &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.HeatTransfer.Examples.GenerationOfFMUs \
      --target conductor.thermalConductor.G --sanitizer physical
```
...
```console
$ sed -n '5p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/HeatTransfer/Components/ThermalConductor.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "conductor.thermalConductor.G"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param conductor.thermalConductor.G=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/HeatTransfer/Components/ThermalConductor.mo:5` |' docs/v2/bugs/
```
