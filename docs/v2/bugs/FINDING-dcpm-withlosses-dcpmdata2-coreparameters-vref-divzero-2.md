# FINDING-01652: `dcpmData2.coreParameters.VRef` in `DCPM_withLosses`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Electrical.Machines.Examples.DCMachines.DCPM_withLosses` |
| **Reached as** | `dcpmData2.coreParameters.VRef` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Losses/CoreParameters.mo:19` |
| **Signature** | `divisor:divisor-reachable-zero:exp:997:49681b20` |
...
```
denominator            (dcpmData2.coreParameters.VRef ^ 2)
witness                dcpmData2.coreParameters.VRef = 2.22507e-308
  found by             at its declared minimum
denominator at witness 0.0
at declared values     None
path condition         not ((dcpmData2.coreParameters.PRef <= 0))
constraints consulted  dcpmData2.coreParameters.VRef: min=2.22507e-308
```
...
| `denominator` | (dcpmData2.coreParameters.VRef ^ 2) |
| `witness` | dcpmData2.coreParameters.VRef = 2.22507e-308 |
| `witness_rationale` | at its declared minimum |
| `denominator_at_witness` | 0.0 |
| `denominator_at_declared_values` | None |
| `denominator_range` | [0, inf] |
| `path_condition` | not ((dcpmData2.coreParameters.PRef &lt;= 0)) |
| `verdict` | SAT |
| `proof` | every guard still holds under the witness |
| `constraints` | dcpmData2.coreParameters.VRef: min=2.22507e-308 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_withLosses \
      --target dcpmData2.coreParameters.VRef --sanitizer divisor
```
...
```console
$ sed -n '19p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Losses/CoreParameters.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcpmData2.coreParameters.VRef"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param dcpmData2.coreParameters.VRef=2.22507e-308
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Losses/CoreParameters.mo:19` |' docs/v2/bugs/
```
