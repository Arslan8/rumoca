...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:1058:c12bef28` |
...
```
denominator            speedRamp.duration
witness                speedRamp.duration = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.1
path condition         not ((time < speedRamp.startTime)) and ((time < (speedRamp.startTime + speedRamp.duration)))
constraints consulted  speedRamp.duration: min=0
```
...
| `path_condition` | not ((time &lt; speedRamp.startTime)) and ((time &lt; (speedRamp.startTime + speedRamp.duration))) |
| `verdict` | UNSAT |
| `proof` | the path conditions cannot hold together: time must be both &gt; 0.4 and &lt; 0.4 |
| `constraints` | speedRamp.duration: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.PumpAndValve \
      --target speedRamp.duration --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "speedRamp.duration"
```
