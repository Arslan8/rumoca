# DECL-0888: `c_wall` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Fluid/Examples/HeatExchanger.mo:146` |
| **Parameter** | `c_wall` |
...
```console
$ sed -n '146p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Examples/HeatExchanger.mo"
```

It should declare `c_wall` with type `SI.SpecificHeatCapacity` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Examples/HeatExchanger.mo" | head
```
...
```modelica
// Fluid/Examples/HeatExchanger.mo:146
parameter SI.SpecificHeatCapacity c_wall(min=0) = ...;
```
