# BUG-005: `inductor2.L` in `ParallelResonance`
...
`Modelica.Electrical.Analog.Examples.ParallelResonance` runs cleanly at its declared values. Setting `inductor2.L = 0` — a value the declaration permits — makes it fail, in Rumoca and in OpenModelica.
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.ParallelResonance \
      --target inductor2.L --keep-parameter-chains
```
...
simulate(Modelica.Electrical.Analog.Examples.ParallelResonance, simflags="-override inductor2.L=0");
EOF
```
