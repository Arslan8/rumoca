...
| **Reached as** | `SaturatingInductance1.i` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-zero-at-declared-values` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/SaturatingInductor.mo:27` |
| **Signature** | `divisor:divisor-zero-at-declared-values:exp:177:62e1e7ce` |
...
It does **not** claim that any parameter can be blamed: nothing was changed, so this is a broken baseline rather than a latent hazard.
...
```
denominator            (SaturatingInductance1.i / SaturatingInductance1.Ipar)
witness                none needed
  found by             the denominator is zero as declared
denominator at witness 0.0
at declared values     0.0
path condition         not (noEvent(((abs(SaturatingInductance1.i) / SaturatingInductance1.Ipar) < 2.2250738585072014e-308)))
...
| `denominator` | (SaturatingInductance1.i / SaturatingInductance1.Ipar) |
| `witness` | none needed |
| `witness_rationale` | the denominator is zero as declared |
| `denominator_at_witness` | 0.0 |
| `denominator_at_declared_values` | 0.0 |
| `denominator_range` |  |
| `path_condition` | not (noEvent(((abs(SaturatingInductance1.i) / SaturatingInductance1.Ipar) &lt; 2.2250738585072014e-308))) |
| `verdict` | SAT |
| `proof` | no assignment is needed: the denominator is zero at the model&#x27;s own declared values |
| `constraints` |  |
| `note` | the division fails without anything being changed, so no parameter can be blamed and none needs to be |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.ShowSaturatingInductor \
      --target SaturatingInductance1.i --sanitizer divisor
```
...
```console
$ sed -n '27p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/SaturatingInductor.mo"
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.ShowSaturatingInductor --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "SaturatingInductance1.i"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param SaturatingInductance1.i=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/SaturatingInductor.mo:27` |' docs/v2/bugs/
```
