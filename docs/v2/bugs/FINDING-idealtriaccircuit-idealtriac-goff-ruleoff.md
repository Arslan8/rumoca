...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Ideal/IdealTriac.mo:5` |
| **Signature** | `physical:physical-rule-does-not-apply:var:41:cb6d16d6` |
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealTriac.Goff: allowed (declared, from class_catalog) — an ideal switch family member; zero off-conductance is the documented ideal [matched by prefix, lower confidence]
behavior    allowed
confidence  declared
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Ideal.IdealTriac.Goff'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.IdealTriacCircuit \
      --target idealTriac.Goff --sanitizer physical
```
...
```console
$ sed -n '5p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Ideal/IdealTriac.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "idealTriac.Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param idealTriac.Goff=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Ideal/IdealTriac.mo:5` |' docs/v2/bugs/
```
