# DECL-0529: `cylinderTopPosition` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/Examples/Loops/Utilities/CylinderBase.mo:4` |
| **Parameter** | `cylinderTopPosition` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Examples/Loops/Utilities/CylinderBase.mo"
```

It should declare `cylinderTopPosition` with type `SI.Length` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Examples/Loops/Utilities/CylinderBase.mo" | head
```
...
```modelica
// Mechanics/MultiBody/Examples/Loops/Utilities/CylinderBase.mo:4
parameter SI.Length cylinderTopPosition(min=0) = ...;
```
