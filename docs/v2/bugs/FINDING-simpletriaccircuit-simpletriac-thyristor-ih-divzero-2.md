...
| **Signature** | `divisor:divisor-reachable-zero:exp:157:716d652f` |
...
```
denominator            simpleTriac.thyristor.IH
witness                simpleTriac.thyristor.IH = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.006
path condition         always evaluated
constraints consulted  simpleTriac.thyristor.IH: unbounded
```
...
| `constraints` | simpleTriac.thyristor.IH: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.SimpleTriacCircuit \
      --target simpleTriac.thyristor.IH --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "simpleTriac.thyristor.IH"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param simpleTriac.thyristor.IH=0
```
