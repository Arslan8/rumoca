...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:0:8345c33b` |
...
```
contract    Modelica.Mechanics.Translational.Components.Spring.c: feature_disabled (proven, from equation) — inherited from spring.c, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Translational.Examples.Utilities.Spring \
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Translational.Examples.Utilities.Spring --keep-artifact /tmp/m.rbc \
