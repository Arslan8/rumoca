# FINDING-04225: `pmActuator.armature.stopper_xMin.c` in `ArmatureStroke`
...
| **Reached as** | `pmActuator.armature.stopper_xMin.c` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:104:c2d12441` |
...
```
predicate     pmActuator.armature.stopper_xMin.c > 0
premise       refuted
...
```
contract    Modelica.Mechanics.Translational.Components.ElastoGap.c: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
| `required` | pmActuator.armature.stopper_xMin.c &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.ArmatureStroke \
      --target pmActuator.armature.stopper_xMin.c --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pmActuator.armature.stopper_xMin.c"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param pmActuator.armature.stopper_xMin.c=0
```
