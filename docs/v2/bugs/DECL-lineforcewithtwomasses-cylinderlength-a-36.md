# DECL-0540: `cylinderLength_a` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/Forces/LineForceWithTwoMasses.mo:36` |
| **Parameter** | `cylinderLength_a` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '36p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Forces/LineForceWithTwoMasses.mo"
```

It should declare `cylinderLength_a` with type `SI.Length` and no `min`.
...
```modelica
// Mechanics/MultiBody/Forces/LineForceWithTwoMasses.mo:36
parameter SI.Length cylinderLength_a(min=0) = ...;
```
