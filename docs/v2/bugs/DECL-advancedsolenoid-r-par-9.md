# DECL-0829: `R_par` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/FluxTubes/Examples/SolenoidActuator/Components/AdvancedSolenoid.mo:9` |
| **Parameter** | `R_par` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
It should declare `R_par` with type `SI.Resistance` and no `min`.
...
```modelica
// Magnetic/FluxTubes/Examples/SolenoidActuator/Components/AdvancedSolenoid.mo:9
parameter SI.Resistance R_par(min=0) = ...;
```
