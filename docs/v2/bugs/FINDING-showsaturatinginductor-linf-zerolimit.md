# FINDING-01134: `Linf` in `ShowSaturatingInductor`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/ShowSaturatingInductor.mo:9` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:3:9040ccfe` |
...
```
contract    Modelica.Electrical.Analog.Basic.SaturatingInductor.Linf: feature_disabled (proven, from equation) — inherited from SaturatingInductance1.Linf, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Examples.ShowSaturatingInductor.Linf'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.ShowSaturatingInductor \
      --target Linf --sanitizer physical
```
...
```console
$ sed -n '9p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/ShowSaturatingInductor.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "Linf"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param Linf=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/ShowSaturatingInductor.mo:9` |' docs/v2/bugs/
```
