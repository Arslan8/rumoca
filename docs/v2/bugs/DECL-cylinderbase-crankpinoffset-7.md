# DECL-0531: `crankPinOffset` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/Examples/Loops/Utilities/CylinderBase.mo:7` |
| **Parameter** | `crankPinOffset` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '7p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Examples/Loops/Utilities/CylinderBase.mo"
```

It should declare `crankPinOffset` with type `SI.Length` and no `min`.
...
```modelica
// Mechanics/MultiBody/Examples/Loops/Utilities/CylinderBase.mo:7
parameter SI.Length crankPinOffset(min=0) = ...;
```
