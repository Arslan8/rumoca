...
| **Signature** | `divisor:divisor-zero-when-parameters-equal:exp:864:81a71109` |
...
```
denominator            (firstOrder1A.opAmp.vps - firstOrder1A.opAmp.vns)
witness                firstOrder1A.opAmp.Vps = -15
  found by             set equal to firstOrder1A.opAmp.Vns
denominator at witness 0.0
at declared values     30.0
path condition         always evaluated
constraints consulted  firstOrder1A.opAmp.Vps: unbounded
```
...
| `denominator` | (firstOrder1A.opAmp.vps - firstOrder1A.opAmp.vns) |
| `witness` | firstOrder1A.opAmp.Vps = -15 |
| `witness_rationale` | set equal to firstOrder1A.opAmp.Vns |
...
| `constraints` | firstOrder1A.opAmp.Vps: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.ControlCircuit \
      --target firstOrder1A.opAmp.Vps --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "firstOrder1A.opAmp.Vps"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param firstOrder1A.opAmp.Vps=-15
```
