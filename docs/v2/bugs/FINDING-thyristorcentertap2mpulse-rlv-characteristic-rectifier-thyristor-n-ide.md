# FINDING-03801: `rectifier.thyristor_n.idealThyristor[1].Ron` in `ThyristorCenterTap2mPulse_RLV_Characteristic`
...
| **Reached as** | `rectifier.thyristor_n.idealThyristor[1].Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:259:491491df` |
...
```
predicate     rectifier.thyristor_n.idealThyristor[1].Ron > 0
premise       refuted
...
| `required` | rectifier.thyristor_n.idealThyristor[1].Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTap2mPulse.ThyristorCenterTap2mPulse_RLV_Characteristic \
      --target rectifier.thyristor_n.idealThyristor[1].Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "rectifier.thyristor_n.idealThyristor[1].Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param rectifier.thyristor_n.idealThyristor[1].Ron=0
```
