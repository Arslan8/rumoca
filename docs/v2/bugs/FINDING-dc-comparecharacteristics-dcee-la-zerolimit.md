# FINDING-01323: `dcee.La` in `DC_CompareCharacteristics`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:469:1121f13a` |
...
```
contract    Modelica.Electrical.Machines.BasicMachines.Components.InductorDC.L: algebraic_limit (proven, from equation) — inherited from dcee.la.L, which it binds: the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.QuasiStaticDCMachines.DC_ElectricalExcited.La'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DC_CompareCharacteristics \
      --target dcee.La --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcee.La"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param dcee.La=0
```
