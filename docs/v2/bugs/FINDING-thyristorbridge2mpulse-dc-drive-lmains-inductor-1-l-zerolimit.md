# FINDING-03485: `lMains.inductor[1].L` in `ThyristorBridge2mPulse_DC_Drive`
...
| **Reached as** | `lMains.inductor[1].L` |
...
```
predicate     lMains.inductor[1].L > 0
premise       refuted
...
| `required` | lMains.inductor[1].L &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.ThyristorBridge2mPulse_DC_Drive \
      --target lMains.inductor[1].L --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "lMains.inductor[1].L"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param lMains.inductor[1].L=0
```
