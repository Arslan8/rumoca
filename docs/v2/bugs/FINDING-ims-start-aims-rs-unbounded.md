...
| **Signature** | `physical:physical-domain-unenforced:var:13:b61e26a0` |
...
role          —
declaration   Modelica.Electrical.Machines.BasicMachines.InductionMachines.IM_SlipRing.Rs
```
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'semantic_role': 'component.machine.winding_resistance', 'binding_source': 'component_type', 'confidence': 'QUANTITY_AND_UNIT', 'declaring_class': 'Modelica.Electrical.Machines.BasicMachines.InductionMachines.IM_SlipRing', 'member': 'Rs', 'premise_state': 'established', 'authority': 'component_contract', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.InductionMachines.IM_SlipRing.Rs'}
```
...
| `authority` | source_arithmetic |
| `canonical_declaration` | Modelica.Electrical.Machines.BasicMachines.InductionMachines.IM_SlipRing.Rs |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMS_Start \
      --target aims.Rs --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "aims.Rs"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param aims.Rs=0
```
