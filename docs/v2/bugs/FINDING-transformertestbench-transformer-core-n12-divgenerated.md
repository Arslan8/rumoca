# FINDING-03301: `transformer.core.n12` in `TransformerTestbench`
...
| **Reached as** | `transformer.core.n12` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-introduced-by-translation` |
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/Components/PartialCore.mo:13` |
| **Signature** | `divisor:divisor-introduced-by-translation:exp:2568:57b7881b` |
...
```
contract    Modelica.Electrical.Machines.BasicMachines.Components.IdealCore.n12: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
behavior    
confidence  proven
source      
```
...
```
denominator            transformer.core.n12
witness                transformer.core.n12 = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  transformer.core.n12: unbounded
```
...
| `constraints` | transformer.core.n12: unbounded |
| `contract` | Modelica.Electrical.Machines.BasicMachines.Components.IdealCore.n12: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.Transformers.TransformerTestbench \
      --target transformer.core.n12 --sanitizer divisor
```
...
```console
$ sed -n '13p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/Components/PartialCore.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "transformer.core.n12"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/Components/PartialCore.mo:13` |' docs/v2/bugs/
```
