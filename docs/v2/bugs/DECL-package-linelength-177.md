# DECL-0568: `lineLength` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/package.mo:177` |
...
```console
$ sed -n '177p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/package.mo"
```

It should declare `lineLength` with type `SI.Length` and no `min`.
...
```modelica
// Mechanics/MultiBody/package.mo:177
parameter SI.Length lineLength(min=0) = ...;
```
