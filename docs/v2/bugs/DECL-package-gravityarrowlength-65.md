# DECL-0555: `gravityArrowLength` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/package.mo:65` |
| **Parameter** | `gravityArrowLength` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '65p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/package.mo"
```

It should declare `gravityArrowLength` with type `SI.Length` and no `min`.
...
```modelica
// Mechanics/MultiBody/package.mo:65
parameter SI.Length gravityArrowLength(min=0) = ...;
```
