# DECL-0552: `length` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/Visualizers/Advanced/PipeWithScalarField.mo:11` |
| **Parameter** | `length` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '11p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Visualizers/Advanced/PipeWithScalarField.mo"
```

It should declare `length` with type `SI.Length` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Visualizers/Advanced/PipeWithScalarField.mo" | head
```
...
```modelica
// Mechanics/MultiBody/Visualizers/Advanced/PipeWithScalarField.mo:11
parameter SI.Length length(min=0) = ...;
```
