# FINDING-00228: `G.alpha` in `ChuaCircuit`
...
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:173:a3cb3520` |
...
```
denominator            (1 + (G.alpha * (G.T_heatPort - G.T_ref)))
witness                none needed
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.ChuaCircuit \
      --target G.alpha --sanitizer divisor
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.ChuaCircuit --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "G.alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param G.alpha=0
```
