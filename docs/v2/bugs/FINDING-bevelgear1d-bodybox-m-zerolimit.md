...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:629:d7ad59dc` |
...
```
contract    Modelica.Mechanics.MultiBody.Parts.Body.m: feature_disabled (proven, from equation) — inherited from bodyBox.body.m, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Rotational3DEffects.BevelGear1D \
      --target bodyBox.m --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "bodyBox.m"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param bodyBox.m=0
```
