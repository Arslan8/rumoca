...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:359:2d615366` |
...
```
contract    Modelica.Mechanics.MultiBody.Forces.Spring.m: feature_disabled (declared, from bound) — the declaration's own binding evaluates to zero at the declared configuration, so zero is the value the library chose rather than one it failed to exclude
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Elementary.ThreeSprings \
      --target spring1.m --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "spring1.m"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param spring1.m=0
```
