# DECL-0008: `top_pipeArea` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Fluid/Examples/AST_BatchPlant.mo:605` |
| **Parameter** | `top_pipeArea` |
| **Type** | `SI.Area` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '605p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Examples/AST_BatchPlant.mo"
```

It should declare `top_pipeArea` with type `SI.Area` and no `min`.
...
```modelica
// Fluid/Examples/AST_BatchPlant.mo:605
parameter SI.Area top_pipeArea(min=0) = ...;
```
