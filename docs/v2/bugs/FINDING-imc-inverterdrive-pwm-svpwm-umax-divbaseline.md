...
| **Reached as** | `pwm.svPWM.uMax` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-zero-at-declared-values` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCAC/Control/SVPWM.mo:34` |
| **Signature** | `divisor:divisor-zero-at-declared-values:exp:8225:901a064e` |
...
```
denominator            ((2 / 3) * pwm.svPWM.uMax)
witness                none needed
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive \
      --target pwm.svPWM.uMax --sanitizer divisor
```
...
```console
$ sed -n '34p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCAC/Control/SVPWM.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pwm.svPWM.uMax"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param pwm.svPWM.uMax=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCAC/Control/SVPWM.mo:34` |' docs/v2/bugs/
```
