# FINDING-02075: `idealCommutingSwitch.Ron` in `IMC_Transformer`
...
| **Reached as** | `idealCommutingSwitch.Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:1095:484c15b7` |
...
```
predicate     idealCommutingSwitch.Ron > 0
premise       refuted
...
| `required` | idealCommutingSwitch.Ron &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_Transformer \
      --target idealCommutingSwitch.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "idealCommutingSwitch.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param idealCommutingSwitch.Ron=0
```
