# FINDING-02951: `smr.Lmq` in `SMR_Inverter`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/SynchronousMachines/SM_ReluctanceRotor.mo:45` |
| **Signature** | `physical:physical-intent-question:var:395:b0189821` |
...
role          —
declaration   Modelica.Electrical.Machines.BasicMachines.SynchronousMachines.SM_ReluctanceRotor.Lmq
```
...
> `smr.Lmq` carries no bound that would exclude it, which is unusual for an inductance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.SynchronousMachines.SM_ReluctanceRotor.Lmq'}
```
...
| `authority` | quantity_or_unit |
| `canonical_declaration` | Modelica.Electrical.Machines.BasicMachines.SynchronousMachines.SM_ReluctanceRotor.Lmq |
| `question` | `smr.Lmq` carries no bound that would exclude it, which is unusual for an inductance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMR_Inverter \
      --target smr.Lmq --sanitizer physical
```
...
```console
$ sed -n '45p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/SynchronousMachines/SM_ReluctanceRotor.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smr.Lmq"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param smr.Lmq=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/SynchronousMachines/SM_ReluctanceRotor.mo:45` |' docs/v2/bugs/
```
