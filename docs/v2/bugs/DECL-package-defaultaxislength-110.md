# DECL-0559: `defaultAxisLength` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/package.mo:110` |
| **Parameter** | `defaultAxisLength` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '110p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/package.mo"
```

It should declare `defaultAxisLength` with type `SI.Length` and no `min`.
...
```modelica
// Mechanics/MultiBody/package.mo:110
parameter SI.Length defaultAxisLength(min=0) = ...;
```
