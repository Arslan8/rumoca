# FINDING-01656: `dcse.Lesigma` in `DCSE_SinglePhase`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:326:453d97b1` |
...
```
contract    Modelica.Electrical.Machines.BasicMachines.DCMachines.DC_SeriesExcited.Lesigma: feature_disabled (declared, from bound) — the declaration's own binding evaluates to zero at the declared configuration, so zero is the value the library chose rather than one it failed to exclude
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.DCMachines.DC_SeriesExcited.Lesigma'}
```
