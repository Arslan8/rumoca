...
| **Signature** | `divisor:divisor-reachable-zero:exp:227:86095fd9` |
...
```
denominator            ((pipe1.V_flowNominal - pipe1.V_flowLaminar) ^ 2)
witness                pipe1.V_flowNominal = 0.1
  found by             at its declared minimum
denominator at witness 0.0
at declared values     0.81
path condition         always evaluated
constraints consulted  pipe1.V_flowNominal: min=0.1, assert pipe1.V_flowNominal greater 0.1
```
...
| `constraints` | pipe1.V_flowNominal: min=0.1, assert pipe1.V_flowNominal greater 0.1 |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.ParallelPumpDropOut \
      --target pipe1.V_flowNominal --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pipe1.V_flowNominal"
```
