# FINDING-03153: `transformer1.l2sigma.inductor[1].L` in `Rectifier12pulse`
...
| **Reached as** | `transformer1.l2sigma.inductor[1].L` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:600:55681e7b` |
...
```
predicate     transformer1.l2sigma.inductor[1].L > 0
premise       refuted
...
| `required` | transformer1.l2sigma.inductor[1].L &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.Transformers.Rectifier12pulse \
      --target transformer1.l2sigma.inductor[1].L --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "transformer1.l2sigma.inductor[1].L"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param transformer1.l2sigma.inductor[1].L=0
```
