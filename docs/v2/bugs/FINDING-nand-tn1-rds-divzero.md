...
| **Signature** | `divisor:divisor-reachable-zero:exp:670:96a840ef` |
...
path condition         not (((TN1.RDS < 1e-20) and (TN1.RDS > -1e-20)))
constraints consulted  TN1.RDS: unbounded
```
...
| `path_condition` | not (((TN1.RDS &lt; 1e-20) and (TN1.RDS &gt; -1e-20))) |
| `verdict` | SAT |
| `proof` | every guard still holds under the witness |
| `constraints` | TN1.RDS: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Utilities.Nand \
      --target TN1.RDS --sanitizer divisor
```
