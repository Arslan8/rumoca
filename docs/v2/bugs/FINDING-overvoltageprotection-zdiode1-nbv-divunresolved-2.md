# FINDING-01089: `zDiode1.Nbv` in `OvervoltageProtection`
...
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/ZDiode.mo:18` |
| **Signature** | `divisor:divisor-zero-unresolved:exp:484:67cc2e2c` |
...
```
denominator            (zDiode1.Nbv * zDiode1.Vt)
witness                zDiode1.Nbv = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.0296
path condition         not ((zDiode1.v > (zDiode1.Maxexp * zDiode1.Vt))) and not (((zDiode1.v + zDiode1.Bv) < -(zDiode1.Maxexp * (zDiode1.Nbv * zDiode1.Vt))))
constraints consulted  zDiode1.Nbv: unbounded
```
...
| `denominator_at_declared_values` | 0.0296 |
| `denominator_range` | [-inf, inf] |
| `path_condition` | not ((zDiode1.v &gt; (zDiode1.Maxexp * zDiode1.Vt))) and not (((zDiode1.v + zDiode1.Bv) &lt; -(zDiode1.Maxexp * (zDiode1.Nbv * zDiode1.Vt)))) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: ((zDiode1.v + zDiode1.Bv) &lt; -(zDiode1.Maxexp * (zDiode1.Nbv * zDiode1.Vt))); (zDiode1.v &gt; (zDiode1.Maxexp * zDiode1.Vt)) |
| `constraints` | zDiode1.Nbv: unbounded |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OvervoltageProtection \
      --target zDiode1.Nbv --sanitizer divisor
```
...
```console
$ sed -n '18p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/ZDiode.mo"
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OvervoltageProtection --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "zDiode1.Nbv"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/ZDiode.mo:18` |' docs/v2/bugs/
```
