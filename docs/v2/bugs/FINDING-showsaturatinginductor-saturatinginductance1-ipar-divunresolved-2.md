...
| **Signature** | `divisor:divisor-zero-unresolved:exp:176:4201be3d` |
...
```
denominator            SaturatingInductance1.Ipar
witness                SaturatingInductance1.Ipar = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.1
path condition         not (noEvent(((abs(SaturatingInductance1.i) / SaturatingInductance1.Ipar) < 2.2250738585072014e-308)))
constraints consulted  SaturatingInductance1.Ipar: unbounded
```
...
| `denominator_at_declared_values` | 0.1 |
...
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: noEvent(((abs(SaturatingInductance1.i) / SaturatingInductance1.Ipar) &lt; 2.2250738585072014e |
| `constraints` | SaturatingInductance1.Ipar: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.ShowSaturatingInductor \
      --target SaturatingInductance1.Ipar --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "SaturatingInductance1.Ipar"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param SaturatingInductance1.Ipar=0
```
