...
| **Reached as** | `controllerFeedbackPart6.T` |
...
| **Signature** | `divisor:divisor-reachable-zero:exp:803:ac33083b` |
...
```
denominator            controllerFeedbackPart6.T
witness                controllerFeedbackPart6.T = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  controllerFeedbackPart6.T: unbounded
```
...
| `constraints` | controllerFeedbackPart6.T: unbounded |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Blocks.LimitersHomotopy \
      --target controllerFeedbackPart6.T --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "controllerFeedbackPart6.T"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param controllerFeedbackPart6.T=0
```
