# FINDING-04018: `chopperStepDown.GoffDiode` in `ChopperStepDown_RL`
...
| **Reached as** | `chopperStepDown.GoffDiode` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCDC/ChopperStepDown.mo:13` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:53:65d0f9a9` |
...
```
predicate     chopperStepDown.GoffDiode > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealDiode.Goff: feature_disabled (proven, from equation) — inherited from chopperStepDown.diode.Goff, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.PowerConverters.DCDC.ChopperStepDown.GoffDiode'}
```
...
| `required` | chopperStepDown.GoffDiode &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCDC.ChopperStepDown.ChopperStepDown_RL \
      --target chopperStepDown.GoffDiode --sanitizer physical
```
...
```console
$ sed -n '13p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCDC/ChopperStepDown.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "chopperStepDown.GoffDiode"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param chopperStepDown.GoffDiode=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCDC/ChopperStepDown.mo:13` |' docs/v2/bugs/
```
