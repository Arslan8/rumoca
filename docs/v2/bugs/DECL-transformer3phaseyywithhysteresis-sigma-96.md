# DECL-0088: `sigma` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/FluxTubes/Examples/Hysteresis/Components/Transformer3PhaseYyWithHysteresis.mo:96` |
| **Parameter** | `sigma` |
| **Type** | `SI.Conductivity` |
| **Declared modifiers** | `—` |
...
`SI.Conductivity` declares no `min` in `Units.mo`, and this declaration adds none of its own. It therefore accepts a negative value, and a material property that is non-negative by definition.
...
```console
$ sed -n '96p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/Hysteresis/Components/Transformer3PhaseYyWithHysteresis.mo"
```

It should declare `sigma` with type `SI.Conductivity` and no `min`.

**2. Confirm the type supplies no bound either** — this is the whole claim, and
a bound inherited from the type would refute it:

```console
$ grep -n "type Conductivity " "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Units.mo"
```
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/Hysteresis/Components/Transformer3PhaseYyWithHysteresis.mo" | head
```
...
- `SI.Conductivity` turns out to carry a `min` after all, or this declaration carries one that the catalog missed.
...
```modelica
// Magnetic/FluxTubes/Examples/Hysteresis/Components/Transformer3PhaseYyWithHysteresis.mo:96
parameter SI.Conductivity sigma(min=0) = ...;
```
