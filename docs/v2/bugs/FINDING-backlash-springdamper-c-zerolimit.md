# FINDING-04642: `springDamper.c` in `Backlash`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Rotational/Components/SpringDamper.mo:3` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:14:fb8fedd8` |
...
```
contract    Modelica.Mechanics.Rotational.Components.SpringDamper.c: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'RotationalSpringConstant', 'unit': 'N.m/rad', 'confidence': 'QUANTITY', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Mechanics.Rotational.Components.SpringDamper.c'}
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Rotational.Examples.Backlash \
...
```console
$ sed -n '3p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Rotational/Components/SpringDamper.mo"
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Rotational.Examples.Backlash --keep-artifact /tmp/m.rbc \
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Rotational/Components/SpringDamper.mo:3` |' docs/v2/bugs/
```
