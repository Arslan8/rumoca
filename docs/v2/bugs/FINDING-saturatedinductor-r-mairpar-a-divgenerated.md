# FINDING-04193: `r_mAirPar.a` in `SaturatedInductor`
...
| **Signature** | `divisor:divisor-introduced-by-translation:exp:358:be8ef495` |
...
```
contract    Modelica.Magnetic.FluxTubes.Shapes.FixedShape.Cuboid.a: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
behavior    
confidence  proven
source      
```
...
```
denominator            r_mAirPar.A
witness                r_mAirPar.a = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.0006250000000000001
path condition         always evaluated
constraints consulted  r_mAirPar.a: unbounded
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.BasicExamples.SaturatedInductor \
      --target r_mAirPar.a --sanitizer divisor
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.BasicExamples.SaturatedInductor --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "r_mAirPar.a"
```
