...
| **Signature** | `divisor:divisor-zero-unresolved:exp:752:e2ad3754` |
...
```
denominator            Nand.TP1.Tnom
witness                Nand.TP1.Tnom = 0
  found by             set to zero
denominator at witness 0.0
at declared values     300.15
path condition         (Nand.TP1.useTemperatureDependency)
constraints consulted  Nand.TP1.Tnom: min=0
```
...
| `path_condition` | (Nand.TP1.useTemperatureDependency) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: Nand.TP1.useTemperatureDependency |
| `constraints` | Nand.TP1.Tnom: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.NandGate \
      --target Nand.TP1.Tnom --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "Nand.TP1.Tnom"
```
