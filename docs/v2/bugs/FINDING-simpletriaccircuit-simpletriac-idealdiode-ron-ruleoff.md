...
```
predicate     simpleTriac.idealDiode.Ron > 0
premise       refuted
...
| `required` | simpleTriac.idealDiode.Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.SimpleTriacCircuit \
      --target simpleTriac.idealDiode.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "simpleTriac.idealDiode.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param simpleTriac.idealDiode.Ron=0
```
