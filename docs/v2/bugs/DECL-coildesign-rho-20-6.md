# DECL-0885: `rho_20` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/FluxTubes/Examples/Utilities/CoilDesign.mo:6` |
| **Parameter** | `rho_20` |
| **Type** | `SI.Resistivity` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '6p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/Utilities/CoilDesign.mo"
```

It should declare `rho_20` with type `SI.Resistivity` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/Utilities/CoilDesign.mo" | head
```
...
```modelica
// Magnetic/FluxTubes/Examples/Utilities/CoilDesign.mo:6
parameter SI.Resistivity rho_20(min=0) = ...;
```
