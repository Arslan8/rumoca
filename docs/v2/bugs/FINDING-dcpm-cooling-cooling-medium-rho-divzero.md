# FINDING-01425: `cooling.medium.rho` in `DCPM_Cooling`
...
| **Signature** | `divisor:divisor-reachable-zero:exp:1703:454002da` |
...
```
denominator            cooling.medium.rho
witness                cooling.medium.rho = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  cooling.medium.rho: min=0
```
...
| `constraints` | cooling.medium.rho: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Cooling \
      --target cooling.medium.rho --sanitizer divisor
```
