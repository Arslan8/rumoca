# FINDING-03487: `lMains.inductor[3].L` in `ThyristorBridge2mPulse_DC_Drive`
...
| **Reached as** | `lMains.inductor[3].L` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:1110:cd3f90ad` |
...
```
predicate     lMains.inductor[3].L > 0
premise       refuted
...
| `required` | lMains.inductor[3].L &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.ThyristorBridge2mPulse_DC_Drive \
      --target lMains.inductor[3].L --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "lMains.inductor[3].L"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param lMains.inductor[3].L=0
```
