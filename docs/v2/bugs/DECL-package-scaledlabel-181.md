...
| **Declaration** | `Mechanics/MultiBody/package.mo:181` |
| **Parameter** | `scaledLabel` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '181p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/package.mo"
```

It should declare `scaledLabel` with type `SI.Length` and no `min`.
...
```modelica
// Mechanics/MultiBody/package.mo:181
parameter SI.Length scaledLabel(min=0) = ...;
```
