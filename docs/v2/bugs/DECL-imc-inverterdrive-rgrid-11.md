# DECL-0698: `RGrid` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Machines/Examples/InductionMachines/IMC_InverterDrive.mo:11` |
| **Parameter** | `RGrid` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
It should declare `RGrid` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/Machines/Examples/InductionMachines/IMC_InverterDrive.mo:11
parameter SI.Resistance RGrid(min=0) = ...;
```
