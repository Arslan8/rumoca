# DECL-0287: `JL` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Machines/Examples/ControlledDCDrives/Utilities/DriveDataDCPM.mo:42` |
| **Parameter** | `JL` |
| **Type** | `SI.Inertia` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '42p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/ControlledDCDrives/Utilities/DriveDataDCPM.mo"
```

It should declare `JL` with type `SI.Inertia` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/ControlledDCDrives/Utilities/DriveDataDCPM.mo" | head
```
...
```modelica
// Electrical/Machines/Examples/ControlledDCDrives/Utilities/DriveDataDCPM.mo:42
parameter SI.Inertia JL(min=0) = ...;
```
