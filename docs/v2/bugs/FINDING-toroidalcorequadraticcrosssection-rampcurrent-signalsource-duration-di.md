# FINDING-04213: `rampCurrent.signalSource.duration` in `ToroidalCoreQuadraticCrossSection`
...
| **Reached as** | `rampCurrent.signalSource.duration` |
...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:525:9ffcf99c` |
...
```
denominator            rampCurrent.signalSource.duration
witness                rampCurrent.signalSource.duration = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.015
path condition         not ((time < rampCurrent.signalSource.startTime)) and ((time < (rampCurrent.signalSource.startTime + rampCurrent.signalSource.duration)))
constraints consulted  rampCurrent.signalSource.duration: min=0
```
...
| `denominator_at_declared_values` | 0.015 |
| `denominator_range` | [0, inf] |
| `path_condition` | not ((time &lt; rampCurrent.signalSource.startTime)) and ((time &lt; (rampCurrent.signalSource.startTime + rampCurrent.signalSource.duration))) |
| `verdict` | UNSAT |
| `proof` | the path conditions cannot hold together: time must be both &gt; 0.01 and &lt; 0.01 |
| `constraints` | rampCurrent.signalSource.duration: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.BasicExamples.ToroidalCoreQuadraticCrossSection \
      --target rampCurrent.signalSource.duration --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "rampCurrent.signalSource.duration"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param rampCurrent.signalSource.duration=0
```
