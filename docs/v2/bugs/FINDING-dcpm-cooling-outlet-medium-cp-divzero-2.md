...
| **Signature** | `divisor:divisor-reachable-zero:exp:1853:64edef87` |
...
```
denominator            outlet.medium.cp
witness                outlet.medium.cp = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  outlet.medium.cp: unbounded
```
...
| `constraints` | outlet.medium.cp: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Cooling \
      --target outlet.medium.cp --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "outlet.medium.cp"
```
