# FINDING-03974: `inverter.RonDiode` in `SinglePhaseTwoLevel_R`
...
| **Signature** | `physical:physical-rule-does-not-apply:var:47:068b1f89` |
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealDiode.Ron: allowed (declared, from class_catalog) — inherited from inverter.diode_p.Ron, which it binds: an ideal switch family member; zero on-resistance is the documented ideal
behavior    allowed
confidence  declared
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCAC.SinglePhaseTwoLevel.SinglePhaseTwoLevel_R \
      --target inverter.RonDiode --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inverter.RonDiode"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inverter.RonDiode=0
```
