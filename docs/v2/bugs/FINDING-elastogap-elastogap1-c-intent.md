...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Components/ElastoGap.mo:4` |
| **Signature** | `physical:physical-intent-question:var:64:6a6be58b` |
...
```
predicate     elastoGap1.c > 0
premise       unknown
authority     quantity_or_unit
role          —
declaration   Modelica.Mechanics.Translational.Components.ElastoGap.c
```
...
> `elastoGap1.c` is bounded only by min=0, which is unusual for a translational spring constant — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
```
{'quantity': 'TranslationalSpringConstant', 'unit': 'N/m', 'confidence': 'QUANTITY', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Mechanics.Translational.Components.ElastoGap.c'}
```
...
| `required` | elastoGap1.c &gt; 0 |
| `declared_min` | 0.0 |
| `variable_role` | parameter |
| `observation` | permissive-declaration |
| `premise_state` | unknown |
| `authority` | quantity_or_unit |
| `canonical_declaration` | Modelica.Mechanics.Translational.Components.ElastoGap.c |
| `question` | `elastoGap1.c` is bounded only by min=0, which is unusual for a translational spring constant — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Translational.Examples.ElastoGap \
      --target elastoGap1.c --sanitizer physical
```
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Components/ElastoGap.mo"
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Translational.Examples.ElastoGap --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "elastoGap1.c"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param elastoGap1.c=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Components/ElastoGap.mo:4` |' docs/v2/bugs/
```
