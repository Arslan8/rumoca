# FINDING-03922: `rectifier.thyristor.idealThyristor[1].Goff` in `ThyristorCenterTapmPulse_RLV_Characteristic`
...
| **Reached as** | `rectifier.thyristor.idealThyristor[1].Goff` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:163:8fa4e15d` |
...
```
predicate     rectifier.thyristor.idealThyristor[1].Goff > 0
premise       refuted
...
| `required` | rectifier.thyristor.idealThyristor[1].Goff &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTapmPulse.ThyristorCenterTapmPulse_RLV_Characteristic \
      --target rectifier.thyristor.idealThyristor[1].Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "rectifier.thyristor.idealThyristor[1].Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param rectifier.thyristor.idealThyristor[1].Goff=0
```
