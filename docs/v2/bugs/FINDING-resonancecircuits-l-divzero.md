# FINDING-01125: `L` in `ResonanceCircuits`
...
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/ResonanceCircuits.mo:8` |
| **Signature** | `divisor:divisor-reachable-zero:exp:17:245490ef` |
...
```
denominator            ((2 * (2 * asin(1.0))) * sqrt((L * C)))
witness                L = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.06283185307179587
path condition         always evaluated
constraints consulted  L: unbounded
```
...
```console
$ sed -n '8p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/ResonanceCircuits.mo"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/ResonanceCircuits.mo:8` |' docs/v2/bugs/
```
