...
| **Signature** | `divisor:divisor-reachable-zero:exp:442:1304b0ba` |
...
```
denominator            (((airGap.r_o + airGap.r_i) / 2) * airGap.alpha)
witness                airGap.alpha = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.0009999999999999788
path condition         always evaluated
constraints consulted  airGap.alpha: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.BasicExamples.ToroidalCoreQuadraticCrossSection \
      --target airGap.alpha --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "airGap.alpha"
```
