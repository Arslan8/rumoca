# FINDING-03624: `rectifier.diode_p1.Ron` in `DiodeBridge2Pulse`
...
| **Reached as** | `rectifier.diode_p1.Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:44:a14b521c` |
...
```
predicate     rectifier.diode_p1.Ron > 0
premise       refuted
...
| `required` | rectifier.diode_p1.Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2Pulse.DiodeBridge2Pulse \
      --target rectifier.diode_p1.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "rectifier.diode_p1.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param rectifier.diode_p1.Ron=0
```
