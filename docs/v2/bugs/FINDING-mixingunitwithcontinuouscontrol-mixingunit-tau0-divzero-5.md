# FINDING-00071: `mixingUnit.tau0` in `MixingUnitWithContinuousControl`
...
| **Reached as** | `mixingUnit.tau0` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Clocked/Examples/Systems/Utilities/ComponentsMixingUnit/MixingUnit.mo:28` |
| **Signature** | `divisor:divisor-reachable-zero:exp:320:cb9e308a` |
...
```
denominator            mixingUnit.tau0
witness                mixingUnit.tau0 = 0
  found by             set to zero
denominator at witness 0.0
at declared values     60.0
path condition         always evaluated
constraints consulted  mixingUnit.tau0: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Clocked.Examples.Systems.Utilities.ComponentsMixingUnit.MixingUnitWithContinuousControl \
      --target mixingUnit.tau0 --sanitizer divisor
```
...
```console
$ sed -n '28p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Clocked/Examples/Systems/Utilities/ComponentsMixingUnit/MixingUnit.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "mixingUnit.tau0"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Clocked/Examples/Systems/Utilities/ComponentsMixingUnit/MixingUnit.mo:28` |' docs/v2/bugs/
```
