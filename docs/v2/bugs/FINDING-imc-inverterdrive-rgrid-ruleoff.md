# FINDING-01838: `RGrid` in `IMC_InverterDrive`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/InductionMachines/IMC_InverterDrive.mo:11` |
| **Signature** | `physical:physical-rule-does-not-apply:var:3:ccd55693` |
...
```
contract    Modelica.Electrical.Analog.Basic.Resistor.R: allowed (declared, from class_catalog) — inherited from resistor.R, which it binds: the component explicitly supports signed resistance, so neither a positivity rule nor a nonzero rule follows from the quantity alone
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive.RGrid'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive \
      --target RGrid --sanitizer physical
```
...
```console
$ sed -n '11p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/InductionMachines/IMC_InverterDrive.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "RGrid"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param RGrid=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/InductionMachines/IMC_InverterDrive.mo:11` |' docs/v2/bugs/
```
