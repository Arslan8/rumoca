# FINDING-03917: `rectifier.RonThyristor` in `ThyristorCenterTapmPulse_RLV_Characteristic`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/ThyristorCenterTapmPulse.mo:7` |
| **Signature** | `physical:physical-rule-does-not-apply:var:133:de798711` |
...
```
contract    Modelica.Electrical.Polyphase.Ideal.IdealThyristor.Ron: allowed (declared, from class_catalog) — inherited from rectifier.thyristor.Ron, which it binds: an ideal switch family member; zero on-resistance is the documented ideal
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.PowerConverters.ACDC.ThyristorCenterTapmPulse.RonThyristor'}
```
...
```console
$ sed -n '7p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/ThyristorCenterTapmPulse.mo"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/ThyristorCenterTapmPulse.mo:7` |' docs/v2/bugs/
```
