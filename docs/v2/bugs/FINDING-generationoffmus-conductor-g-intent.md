# FINDING-05143: `conductor.G` in `GenerationOfFMUs`
...
| **Found by** | **PhysicalSan** — `packages/modelsan/modelsan/sanitizers/physical.py`, rule `thermal.conductance.positive` |
| **Reported as** | `physical-intent-question` |
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/HeatTransfer/Examples/Utilities/Conduction.mo:4` |
| **Signature** | `physical:physical-intent-question:var:73:6dc721ba` |
...
```
predicate     conductor.G > 0
premise       unknown
authority     quantity_or_unit
role          —
declaration   Modelica.Thermal.HeatTransfer.Examples.Utilities.Conduction.G
```
...
> `conductor.G` carries no bound that would exclude it, which is unusual for a thermal conductance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.

The rule it applied is `thermal.conductance.positive`:

> heat flows from hot to cold; G <= 0 reverses the second law
...
```
{'quantity': 'ThermalConductance', 'unit': 'W/K', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Thermal.HeatTransfer.Examples.Utilities.Conduction.G'}
```
...
| `authority` | quantity_or_unit |
| `canonical_declaration` | Modelica.Thermal.HeatTransfer.Examples.Utilities.Conduction.G |
| `question` | `conductor.G` carries no bound that would exclude it, which is unusual for a thermal conductance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.HeatTransfer.Examples.GenerationOfFMUs \
      --target conductor.G --sanitizer physical
```
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/HeatTransfer/Examples/Utilities/Conduction.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "conductor.G"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param conductor.G=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/HeatTransfer/Examples/Utilities/Conduction.mo:4` |' docs/v2/bugs/
```
