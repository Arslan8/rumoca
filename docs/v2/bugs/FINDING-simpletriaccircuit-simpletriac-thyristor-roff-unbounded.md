...
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/Thyristor.mo:40` |
| **Signature** | `physical:physical-domain-unenforced:var:95:053d118e` |
...
```
predicate     simpleTriac.thyristor.Roff > 0
premise       established
authority     source_arithmetic
role          —
declaration   Modelica.Electrical.Analog.Semiconductors.Thyristor.Roff
```
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Semiconductors.Thyristor.Roff'}
```
...
| `required` | simpleTriac.thyristor.Roff &gt; 0 |
| `declared_min` | None |
| `variable_role` | parameter |
| `premise_state` | established |
| `authority` | source_arithmetic |
| `canonical_declaration` | Modelica.Electrical.Analog.Semiconductors.Thyristor.Roff |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.SimpleTriacCircuit \
      --target simpleTriac.thyristor.Roff --sanitizer physical
```
...
```console
$ sed -n '40p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/Thyristor.mo"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param simpleTriac.thyristor.Roff=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/Thyristor.mo:40` |' docs/v2/bugs/
```
