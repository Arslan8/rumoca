# FINDING-05277: `smpmData.statorCoreParameters.VRef` in `SMPM_VoltageSourceWithLosses`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `ModelicaTest.Electrical.Machines.SMPM_VoltageSourceWithLosses` |
| **Reached as** | `smpmData.statorCoreParameters.VRef` |
...
| **Signature** | `divisor:divisor-reachable-zero:exp:1936:d1e8ef84` |
...
```
denominator            (smpmData.statorCoreParameters.VRef ^ 2)
witness                smpmData.statorCoreParameters.VRef = 2.22507e-308
  found by             at its declared minimum
denominator at witness 0.0
at declared values     10000.0
path condition         not ((smpmData.statorCoreParameters.PRef <= 0))
constraints consulted  smpmData.statorCoreParameters.VRef: min=2.22507e-308
```
...
| `denominator_at_declared_values` | 10000.0 |
| `denominator_range` | [0, inf] |
| `path_condition` | not ((smpmData.statorCoreParameters.PRef &lt;= 0)) |
| `verdict` | SAT |
| `proof` | every guard still holds under the witness |
| `constraints` | smpmData.statorCoreParameters.VRef: min=2.22507e-308 |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Electrical.Machines.SMPM_VoltageSourceWithLosses \
      --target smpmData.statorCoreParameters.VRef --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smpmData.statorCoreParameters.VRef"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param smpmData.statorCoreParameters.VRef=2.22507e-308
```
