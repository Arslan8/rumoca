# FINDING-02755: `smpmData.Rrd` in `SMPM_ResistiveBraking`
...
| **Signature** | `physical:physical-domain-unenforced:var:13:da7df2fa` |
...
```
predicate     smpmData.Rrd >= 0
premise       established
authority     component_contract
role          component.machine.winding_resistance
declaration   Modelica.Electrical.Machines.Utilities.ParameterRecords.SM_PermanentMagnetData.Rrd
```
...
| `required` | smpmData.Rrd &gt;= 0 |
| `declared_min` | None |
| `variable_role` | parameter |
| `premise_state` | established |
| `authority` | component_contract |
| `semantic_role` | component.machine.winding_resistance |
| `canonical_declaration` | Modelica.Electrical.Machines.Utilities.ParameterRecords.SM_PermanentMagnetData.Rrd |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_ResistiveBraking \
      --target smpmData.Rrd --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smpmData.Rrd"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param smpmData.Rrd=0
```
