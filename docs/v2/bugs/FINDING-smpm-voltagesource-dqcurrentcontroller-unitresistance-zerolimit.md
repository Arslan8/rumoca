# FINDING-02840: `dqCurrentController.unitResistance` in `SMPM_VoltageSource`
...
| **Reached as** | `dqCurrentController.unitResistance` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/DQCurrentController.mo:75` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:685:9d0d97d2` |
...
```
predicate     dqCurrentController.unitResistance > 0
premise       refuted
...
```
contract    Modelica.Electrical.Machines.Utilities.DQCurrentController.unitResistance: forbidden (proven, from bound) — the declaration excludes zero (declared constant)
behavior    forbidden
confidence  proven
source      bound
```
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.Utilities.DQCurrentController.unitResistance'}
```
...
| `required` | dqCurrentController.unitResistance &gt; 0 |
| `declared_min` | None |
| `variable_role` | constant |
| `premise_state` | refuted |
| `authority` | component_contract |
| `contract` | Modelica.Electrical.Machines.Utilities.DQCurrentController.unitResistance: forbidden (proven, from bound) — the declaration excludes zero (declared constant) |
| `contract_behavior` | forbidden |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_VoltageSource \
      --target dqCurrentController.unitResistance --sanitizer physical
```
...
```console
$ sed -n '75p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/DQCurrentController.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dqCurrentController.unitResistance"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param dqCurrentController.unitResistance=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/DQCurrentController.mo:75` |' docs/v2/bugs/
```
