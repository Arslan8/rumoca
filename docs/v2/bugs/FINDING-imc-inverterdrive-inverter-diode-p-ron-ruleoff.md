# FINDING-01877: `inverter.diode_p.Ron` in `IMC_InverterDrive`
...
| **Reached as** | `inverter.diode_p.Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:624:6125ac4a` |
...
```
predicate     inverter.diode_p.Ron > 0
premise       refuted
...
| `required` | inverter.diode_p.Ron &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive \
      --target inverter.diode_p.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inverter.diode_p.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inverter.diode_p.Ron=0
```
