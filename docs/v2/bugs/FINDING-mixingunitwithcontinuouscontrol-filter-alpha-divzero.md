# FINDING-00063: `filter.alpha` in `MixingUnitWithContinuousControl`
...
| **Reached as** | `filter.alpha` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Blocks/Continuous.mo:1523` |
| **Signature** | `divisor:divisor-reachable-zero:exp:258:a42da547` |
...
| `constraints` | filter.alpha: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Clocked.Examples.Systems.Utilities.ComponentsMixingUnit.MixingUnitWithContinuousControl \
      --target filter.alpha --sanitizer divisor
```
...
```console
$ sed -n '1523p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Blocks/Continuous.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "filter.alpha"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Blocks/Continuous.mo:1523` |' docs/v2/bugs/
```
