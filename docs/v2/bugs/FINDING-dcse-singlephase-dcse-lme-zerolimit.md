# FINDING-01669: `dcse.Lme` in `DCSE_SinglePhase`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:325:42cf56f4` |
...
```
contract    Modelica.Electrical.Machines.BasicMachines.DCMachines.DC_SeriesExcited.Lme: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.DCMachines.DC_SeriesExcited.Lme'}
```
