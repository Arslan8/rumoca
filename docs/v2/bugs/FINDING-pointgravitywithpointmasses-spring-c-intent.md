# FINDING-04509: `spring.c` in `PointGravityWithPointMasses`
...
| **Signature** | `physical:physical-intent-question:var:342:13194905` |
...
> `spring.c` is bounded only by min=0, which is unusual for a translational spring constant — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
| `question` | `spring.c` is bounded only by min=0, which is unusual for a translational spring constant — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Elementary.PointGravityWithPointMasses \
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Elementary.PointGravityWithPointMasses --keep-artifact /tmp/m.rbc \
