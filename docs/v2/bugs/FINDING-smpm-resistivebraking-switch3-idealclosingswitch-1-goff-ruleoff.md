# FINDING-02793: `switch3.idealClosingSwitch[1].Goff` in `SMPM_ResistiveBraking`
...
| **Reached as** | `switch3.idealClosingSwitch[1].Goff` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:944:d7b920dd` |
...
```
predicate     switch3.idealClosingSwitch[1].Goff > 0
premise       refuted
...
| `required` | switch3.idealClosingSwitch[1].Goff &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_ResistiveBraking \
      --target switch3.idealClosingSwitch[1].Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switch3.idealClosingSwitch[1].Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switch3.idealClosingSwitch[1].Goff=0
```
