# DECL-0091: `Twidth` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Clocked/BooleanSignals/TimeBasedSources/Pulse.mo:14` |
| **Parameter** | `Twidth` |
| **Type** | `SI.Duration` |
| **Declared modifiers** | `—` |
...
`SI.Duration` declares no `min` in `Units.mo`, and this declaration adds none of its own. It therefore accepts a negative value, and a negative duration runs time backwards.
...
```console
$ sed -n '14p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Clocked/BooleanSignals/TimeBasedSources/Pulse.mo"
```

It should declare `Twidth` with type `SI.Duration` and no `min`.

**2. Confirm the type supplies no bound either** — this is the whole claim, and
a bound inherited from the type would refute it:

```console
$ grep -n "type Duration " "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Units.mo"
```
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Clocked/BooleanSignals/TimeBasedSources/Pulse.mo" | head
```
...
- `SI.Duration` turns out to carry a `min` after all, or this declaration carries one that the catalog missed.
...
```modelica
// Clocked/BooleanSignals/TimeBasedSources/Pulse.mo:14
parameter SI.Duration Twidth(min=0) = ...;
```
