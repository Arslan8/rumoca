# FINDING-01528: `dcdcInverter2.fS` in `DCPM_Drive`
...
| **Signature** | `divisor:divisor-reachable-zero:exp:1167:cdeec9cd` |
...
```
denominator            dcdcInverter2.fS
witness                dcdcInverter2.fS = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1000.0
path condition         always evaluated
constraints consulted  dcdcInverter2.fS: unbounded
```
...
| `constraints` | dcdcInverter2.fS: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Drive \
      --target dcdcInverter2.fS --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcdcInverter2.fS"
```
