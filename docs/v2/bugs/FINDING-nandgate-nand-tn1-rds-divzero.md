# FINDING-00773: `Nand.TN1.RDS` in `NandGate`
...
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/NMOS.mo:42` |
| **Signature** | `divisor:divisor-reachable-zero:exp:1034:55245197` |
...
```
denominator            Nand.TN1.RDS
witness                Nand.TN1.RDS = 0
  found by             set to zero
denominator at witness 0.0
at declared values     10000000.0
path condition         not (((Nand.TN1.RDS < 1e-20) and (Nand.TN1.RDS > -1e-20)))
constraints consulted  Nand.TN1.RDS: unbounded
```
...
| `denominator_at_declared_values` | 10000000.0 |
| `denominator_range` | [-inf, inf] |
| `path_condition` | not (((Nand.TN1.RDS &lt; 1e-20) and (Nand.TN1.RDS &gt; -1e-20))) |
| `verdict` | SAT |
| `proof` | every guard still holds under the witness |
| `constraints` | Nand.TN1.RDS: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.NandGate \
      --target Nand.TN1.RDS --sanitizer divisor
```
...
```console
$ sed -n '42p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/NMOS.mo"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/NMOS.mo:42` |' docs/v2/bugs/
```
