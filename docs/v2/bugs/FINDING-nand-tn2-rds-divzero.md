...
| **Signature** | `divisor:divisor-reachable-zero:exp:864:d324a15b` |
...
path condition         not (((TN2.RDS < 1e-20) and (TN2.RDS > -1e-20)))
constraints consulted  TN2.RDS: unbounded
```
...
| `path_condition` | not (((TN2.RDS &lt; 1e-20) and (TN2.RDS &gt; -1e-20))) |
| `verdict` | SAT |
| `proof` | every guard still holds under the witness |
| `constraints` | TN2.RDS: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Utilities.Nand \
      --target TN2.RDS --sanitizer divisor
```
