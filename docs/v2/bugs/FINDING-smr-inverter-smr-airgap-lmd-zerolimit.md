# FINDING-02947: `smr.airGap.Lmd` in `SMR_Inverter`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:389:a9542029` |
...
```
contract    Modelica.Electrical.Machines.BasicMachines.Components.AirGapR.L: feature_disabled (proven, from equation) — inherited from smr.airGap.L, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMR_Inverter \
      --target smr.airGap.Lmd --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smr.airGap.Lmd"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param smr.airGap.Lmd=0
```
