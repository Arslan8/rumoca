# DECL-0059: `GoffT` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Machines/Examples/ControlledDCDrives/Utilities/SwitchingDcDc.mo:7` |
| **Parameter** | `GoffT` |
| **Type** | `SI.Conductance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '7p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/ControlledDCDrives/Utilities/SwitchingDcDc.mo"
```

It should declare `GoffT` with type `SI.Conductance` and no `min`.
...
```modelica
// Electrical/Machines/Examples/ControlledDCDrives/Utilities/SwitchingDcDc.mo:7
parameter SI.Conductance GoffT(min=0) = ...;
```
