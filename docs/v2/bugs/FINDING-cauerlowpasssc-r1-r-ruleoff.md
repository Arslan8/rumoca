# FINDING-00166: `R1.R` in `CauerLowPassSC`
...
| **Signature** | `physical:physical-rule-does-not-apply:var:436:19b0da72` |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.CauerLowPassSC \
      --target R1.R --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "R1.R"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param R1.R=0
```
