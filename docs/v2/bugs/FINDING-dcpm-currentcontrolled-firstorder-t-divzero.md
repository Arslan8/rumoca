# FINDING-01457: `firstOrder.T` in `DCPM_CurrentControlled`
...
| **Signature** | `divisor:divisor-reachable-zero:exp:1479:d8b09e6f` |
...
```
denominator            firstOrder.T
witness                firstOrder.T = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.001
path condition         always evaluated
constraints consulted  firstOrder.T: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_CurrentControlled \
      --target firstOrder.T --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "firstOrder.T"
```
