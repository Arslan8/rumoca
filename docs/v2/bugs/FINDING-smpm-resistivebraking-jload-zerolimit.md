# FINDING-02750: `JLoad` in `SMPM_ResistiveBraking`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/SynchronousMachines/SMPM_ResistiveBraking.mo:7` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:1:1d3116a0` |
...
```
contract    Modelica.Mechanics.Rotational.Components.Inertia.J: algebraic_limit (proven, from equation) — inherited from inertiaLoad.J, which it binds: the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
...
```
{'quantity': 'MomentOfInertia', 'unit': 'kg.m2', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_ResistiveBraking.JLoad'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_ResistiveBraking \
      --target JLoad --sanitizer physical
```
...
```console
$ sed -n '7p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/SynchronousMachines/SMPM_ResistiveBraking.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "JLoad"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param JLoad=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/SynchronousMachines/SMPM_ResistiveBraking.mo:7` |' docs/v2/bugs/
```
