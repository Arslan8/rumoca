# FINDING-04809: `innerPipe.V_flowNominal` in `IndirectCooling`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Thermal.FluidHeatFlow.Examples.IndirectCooling` |
| **Reached as** | `innerPipe.V_flowNominal` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/BaseClasses/SimpleFriction.mo:29` |
| **Signature** | `divisor:divisor-reachable-zero:exp:637:171e5712` |
...
```
denominator            ((innerPipe.V_flowNominal - innerPipe.V_flowLaminar) ^ 2)
witness                innerPipe.V_flowNominal = 0.1
  found by             at its declared minimum
denominator at witness 0.0
at declared values     0.81
path condition         always evaluated
constraints consulted  innerPipe.V_flowNominal: min=0.1, assert innerPipe.V_flowNominal greater 0.1
```
...
| `denominator_at_declared_values` | 0.81 |
| `denominator_range` | [0, inf] |
| `path_condition` | always evaluated |
| `verdict` | SAT |
| `proof` | the division is not inside any branch |
| `constraints` | innerPipe.V_flowNominal: min=0.1, assert innerPipe.V_flowNominal greater 0.1 |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.IndirectCooling \
      --target innerPipe.V_flowNominal --sanitizer divisor
```
...
```console
$ sed -n '29p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/BaseClasses/SimpleFriction.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "innerPipe.V_flowNominal"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param innerPipe.V_flowNominal=0.1
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/BaseClasses/SimpleFriction.mo:29` |' docs/v2/bugs/
```
