# FINDING-03527: `pulse2.twomPulse.filter[3].fCut` in `ThyristorBridge2mPulse_DC_Drive`
...
| **Reached as** | `pulse2.twomPulse.filter[3].fCut` |
...
| **Signature** | `divisor:divisor-reachable-zero:exp:1659:c654e6e6` |
...
```
denominator            ((2 * (2 * asin(1.0))) * pulse2.twomPulse.filter[3].fCut)
witness                pulse2.twomPulse.filter[3].fCut = 0
  found by             set to zero
denominator at witness 0.0
at declared values     628.3185307179587
path condition         always evaluated
constraints consulted  pulse2.twomPulse.filter[3].fCut: unbounded
```
...
| `constraints` | pulse2.twomPulse.filter[3].fCut: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.ThyristorBridge2mPulse_DC_Drive \
      --target pulse2.twomPulse.filter[3].fCut --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pulse2.twomPulse.filter[3].fCut"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param pulse2.twomPulse.filter[3].fCut=0
```
