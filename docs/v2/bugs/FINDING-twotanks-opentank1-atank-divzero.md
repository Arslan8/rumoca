# FINDING-05078: `openTank1.ATank` in `TwoTanks`
...
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/Components/OpenTank.mo:47` |
| **Signature** | `divisor:divisor-reachable-zero:exp:270:e109ff16` |
...
```
denominator            openTank1.ATank
witness                openTank1.ATank = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  openTank1.ATank: unbounded
```
...
| `constraints` | openTank1.ATank: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.TwoTanks \
      --target openTank1.ATank --sanitizer divisor
```
...
```console
$ sed -n '47p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/Components/OpenTank.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "openTank1.ATank"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/Components/OpenTank.mo:47` |' docs/v2/bugs/
```
