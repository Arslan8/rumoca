# FINDING-01928: `vfController.fNominal` in `IMC_InverterDrive`
...
| **Reached as** | `vfController.fNominal` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-zero-unresolved` |
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/VfController.mo:21` |
| **Signature** | `divisor:divisor-zero-unresolved:exp:5777:dcdf2ae9` |
...
```
denominator            vfController.fNominal
witness                vfController.fNominal = 0
  found by             set to zero
denominator at witness 0.0
at declared values     50.0
path condition         ((abs(vfController.u) < vfController.fNominal))
constraints consulted  vfController.fNominal: unbounded
```
...
| `denominator_at_declared_values` | 50.0 |
| `denominator_range` | [-inf, inf] |
| `path_condition` | ((abs(vfController.u) &lt; vfController.fNominal)) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: (abs(vfController.u) &lt; vfController.fNominal) |
| `constraints` | vfController.fNominal: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive \
      --target vfController.fNominal --sanitizer divisor
```
...
```console
$ sed -n '21p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/VfController.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "vfController.fNominal"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/VfController.mo:21` |' docs/v2/bugs/
```
