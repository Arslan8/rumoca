# FINDING-02563: `diodeBridge2mPulse.diode_p.idealDiode[1].Goff` in `SMPM_Braking`
...
| **Reached as** | `diodeBridge2mPulse.diode_p.idealDiode[1].Goff` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:650:514e1a19` |
...
```
predicate     diodeBridge2mPulse.diode_p.idealDiode[1].Goff > 0
premise       refuted
...
| `required` | diodeBridge2mPulse.diode_p.idealDiode[1].Goff &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_Braking \
      --target diodeBridge2mPulse.diode_p.idealDiode[1].Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "diodeBridge2mPulse.diode_p.idealDiode[1].Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param diodeBridge2mPulse.diode_p.idealDiode[1].Goff=0
```
