...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:74:dfd83ffc` |
...
```
contract    Modelica.Electrical.Analog.Basic.Resistor.R: feature_disabled (proven, from equation) — inherited from feedback.r3.R, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.Subtracter \
      --target feedback.R3 --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "feedback.R3"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param feedback.R3=0
```
