...
| **Signature** | `divisor:divisor-reachable-zero:exp:642:047d1a42` |
...
```
denominator            Nand.TP1.RDS
witness                Nand.TP1.RDS = 0
  found by             set to zero
denominator at witness 0.0
at declared values     10000000.0
path condition         not (((Nand.TP1.RDS < 1e-20) and (Nand.TP1.RDS > -1e-20)))
constraints consulted  Nand.TP1.RDS: unbounded
```
...
| `path_condition` | not (((Nand.TP1.RDS &lt; 1e-20) and (Nand.TP1.RDS &gt; -1e-20))) |
| `verdict` | SAT |
| `proof` | every guard still holds under the witness |
| `constraints` | Nand.TP1.RDS: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.NandGate \
      --target Nand.TP1.RDS --sanitizer divisor
```
