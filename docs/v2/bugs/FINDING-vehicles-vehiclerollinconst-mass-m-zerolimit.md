...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:559:6aecca73` |
...
```
predicate     vehicleRollInConst.mass.m > 0
premise       refuted
...
| `required` | vehicleRollInConst.mass.m &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Translational.Vehicles \
      --target vehicleRollInConst.mass.m --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "vehicleRollInConst.mass.m"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param vehicleRollInConst.mass.m=0
```
