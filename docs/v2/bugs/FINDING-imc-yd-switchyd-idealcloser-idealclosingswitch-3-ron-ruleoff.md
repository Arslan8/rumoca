# FINDING-02233: `switchYD.idealCloser.idealClosingSwitch[3].Ron` in `IMC_YD`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Electrical.Machines.Examples.InductionMachines.IMC_YD` |
| **Reached as** | `switchYD.idealCloser.idealClosingSwitch[3].Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:749:001ba6a8` |
...
```
predicate     switchYD.idealCloser.idealClosingSwitch[3].Ron > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealClosingSwitch.Ron: allowed (declared, from class_catalog) — an ideal switch family member; zero on-resistance is the documented ideal [matched by prefix, lower confidence]
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Ideal.IdealClosingSwitch.Ron'}
```
...
| `required` | switchYD.idealCloser.idealClosingSwitch[3].Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_YD \
      --target switchYD.idealCloser.idealClosingSwitch[3].Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switchYD.idealCloser.idealClosingSwitch[3].Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switchYD.idealCloser.idealClosingSwitch[3].Ron=0
```
