...
| **Signature** | `divisor:divisor-reachable-zero:exp:870:c8d14c38` |
...
```
denominator            idealTransformer.n
witness                idealTransformer.n = 0
  found by             set to zero
denominator at witness 0.0
at declared values     2.0
path condition         always evaluated
constraints consulted  idealTransformer.n: unbounded
```
...
| `constraints` | idealTransformer.n: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.CompareTransformers \
      --target idealTransformer.n --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "idealTransformer.n"
```
