# FINDING-05120: `volumeFlowSensor.medium.rho` in `WaterPump`
...
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/BaseClasses/TwoPort.mo:34` |
| **Signature** | `divisor:divisor-reachable-zero:exp:805:c51340fb` |
...
```
denominator            volumeFlowSensor.medium.rho
witness                volumeFlowSensor.medium.rho = 0
  found by             set to zero
denominator at witness 0.0
at declared values     995.6
path condition         always evaluated
constraints consulted  volumeFlowSensor.medium.rho: min=0
```
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.WaterPump \
      --target volumeFlowSensor.medium.rho --sanitizer divisor
```
...
```console
$ sed -n '34p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/BaseClasses/TwoPort.mo"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param volumeFlowSensor.medium.rho=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/BaseClasses/TwoPort.mo:34` |' docs/v2/bugs/
```
