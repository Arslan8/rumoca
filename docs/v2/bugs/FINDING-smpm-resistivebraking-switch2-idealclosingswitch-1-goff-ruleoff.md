# FINDING-02785: `switch2.idealClosingSwitch[1].Goff` in `SMPM_ResistiveBraking`
...
| **Reached as** | `switch2.idealClosingSwitch[1].Goff` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:870:220b909a` |
...
```
predicate     switch2.idealClosingSwitch[1].Goff > 0
premise       refuted
...
| `required` | switch2.idealClosingSwitch[1].Goff &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_ResistiveBraking \
      --target switch2.idealClosingSwitch[1].Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switch2.idealClosingSwitch[1].Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switch2.idealClosingSwitch[1].Goff=0
```
