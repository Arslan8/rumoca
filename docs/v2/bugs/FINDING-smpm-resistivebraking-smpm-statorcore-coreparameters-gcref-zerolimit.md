# FINDING-02749: `smpm.statorCore.coreParameters.GcRef` in `SMPM_ResistiveBraking`
...
| **Reached as** | `smpm.statorCore.coreParameters.GcRef` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:210:5662c533` |
...
| `observed` | 0.0 |
| `required` | smpm.statorCore.coreParameters.GcRef &gt; 0 |
| `where` | declaration |
| `contract` | Modelica.Electrical.Machines.Losses.CoreParameters.GcRef: feature_disabled (declared, from bound) — the declaration&#x27;s own binding evaluates to zero at the declared configuration, so zero is the value the library chose rather than one it failed to exclude |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_ResistiveBraking \
      --target smpm.statorCore.coreParameters.GcRef --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smpm.statorCore.coreParameters.GcRef"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param smpm.statorCore.coreParameters.GcRef=0
```
