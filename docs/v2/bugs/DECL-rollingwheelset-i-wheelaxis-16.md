# DECL-0362: `I_wheelAxis` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/Parts/RollingWheelSet.mo:16` |
| **Parameter** | `I_wheelAxis` |
| **Type** | `SI.Inertia` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '16p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/RollingWheelSet.mo"
```

It should declare `I_wheelAxis` with type `SI.Inertia` and no `min`.
...
```modelica
// Mechanics/MultiBody/Parts/RollingWheelSet.mo:16
parameter SI.Inertia I_wheelAxis(min=0) = ...;
```
