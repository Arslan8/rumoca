...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Ideal/ControlledIdealTwoWaySwitch.mo:4` |
| **Signature** | `physical:physical-rule-does-not-apply:var:5:8f8f4789` |
...
```
contract    Modelica.Electrical.Analog.Ideal.ControlledIdealTwoWaySwitch.Ron: allowed (declared, from class_catalog) — an ideal switch family member; zero on-resistance is the documented ideal [matched by prefix, lower confidence]
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Ideal.ControlledIdealTwoWaySwitch.Ron'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Utilities.RealSwitch \
      --target S.Ron --sanitizer physical
```
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Ideal/ControlledIdealTwoWaySwitch.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "S.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param S.Ron=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Ideal/ControlledIdealTwoWaySwitch.mo:4` |' docs/v2/bugs/
```
