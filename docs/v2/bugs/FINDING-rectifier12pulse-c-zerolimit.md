# FINDING-03119: `C` in `Rectifier12pulse`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/Transformers/Rectifier6pulse.mo:9` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:4:46edca39` |
...
```
contract    Modelica.Electrical.Machines.Examples.Transformers.Rectifier12pulse.C: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Capacitance', 'unit': 'F', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.Examples.Transformers.Rectifier12pulse.C'}
```
...
```console
$ sed -n '9p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/Transformers/Rectifier6pulse.mo"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/Transformers/Rectifier6pulse.mo:9` |' docs/v2/bugs/
```
