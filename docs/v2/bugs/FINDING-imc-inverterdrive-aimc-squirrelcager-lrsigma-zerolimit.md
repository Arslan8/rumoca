# FINDING-01916: `aimc.squirrelCageR.Lrsigma` in `IMC_InverterDrive`
...
| **Reached as** | `aimc.squirrelCageR.Lrsigma` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/Components/SquirrelCage.mo:3` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:1343:1f781a06` |
...
```
predicate     aimc.squirrelCageR.Lrsigma > 0
premise       refuted
...
```
contract    Modelica.Electrical.Machines.BasicMachines.Components.SquirrelCage.Lrsigma: algebraic_limit (proven, from equation) — the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.Components.SquirrelCage.Lrsigma'}
```
...
| `required` | aimc.squirrelCageR.Lrsigma &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive \
      --target aimc.squirrelCageR.Lrsigma --sanitizer physical
```
...
```console
$ sed -n '3p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/Components/SquirrelCage.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "aimc.squirrelCageR.Lrsigma"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param aimc.squirrelCageR.Lrsigma=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/BasicMachines/Components/SquirrelCage.mo:3` |' docs/v2/bugs/
```
