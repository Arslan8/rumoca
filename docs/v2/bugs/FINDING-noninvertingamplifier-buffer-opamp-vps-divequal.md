...
| **Signature** | `divisor:divisor-zero-when-parameters-equal:exp:233:96acf51b` |
...
```
denominator            (buffer.opAmp.vps - buffer.opAmp.vns)
witness                buffer.opAmp.Vps = -15
  found by             set equal to buffer.opAmp.Vns
denominator at witness 0.0
at declared values     30.0
path condition         always evaluated
constraints consulted  buffer.opAmp.Vps: unbounded
```
...
| `denominator` | (buffer.opAmp.vps - buffer.opAmp.vns) |
| `witness` | buffer.opAmp.Vps = -15 |
| `witness_rationale` | set equal to buffer.opAmp.Vns |
...
| `constraints` | buffer.opAmp.Vps: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.NonInvertingAmplifier \
      --target buffer.opAmp.Vps --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "buffer.opAmp.Vps"
```
