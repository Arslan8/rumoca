# FINDING-01340: `dcse.Ra` in `DC_CompareCharacteristics`
...
| **Signature** | `physical:physical-domain-unenforced:var:838:6708c00a` |
...
declaration   Modelica.Electrical.Machines.BasicMachines.QuasiStaticDCMachines.DC_SeriesExcited.Ra
```
...
| `canonical_declaration` | Modelica.Electrical.Machines.BasicMachines.QuasiStaticDCMachines.DC_SeriesExcited.Ra |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DC_CompareCharacteristics \
      --target dcse.Ra --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcse.Ra"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param dcse.Ra=0
```
