# FINDING-05252: `smpmData.frictionParameters.wLinear` in `SMPM_VoltageSourceWithLosses`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `ModelicaTest.Electrical.Machines.SMPM_VoltageSourceWithLosses` |
| **Reached as** | `smpmData.frictionParameters.wLinear` |
...
| **Signature** | `divisor:divisor-zero-unresolved:exp:3370:efaa2fc8` |
...
```
denominator            smpmData.frictionParameters.wLinear
witness                smpmData.frictionParameters.wLinear = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.15707963267948966
path condition         not (noEvent((abs(smpm.friction.w) > smpmData.frictionParameters.wLinear)))
constraints consulted  smpmData.frictionParameters.wLinear: unbounded
```
...
| `denominator_at_declared_values` | 0.15707963267948966 |
| `denominator_range` | [-inf, inf] |
| `path_condition` | not (noEvent((abs(smpm.friction.w) &gt; smpmData.frictionParameters.wLinear))) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: noEvent((abs(smpm.friction.w) &gt; smpmData.frictionParameters.wLinear)) |
| `constraints` | smpmData.frictionParameters.wLinear: unbounded |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Electrical.Machines.SMPM_VoltageSourceWithLosses \
      --target smpmData.frictionParameters.wLinear --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smpmData.frictionParameters.wLinear"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param smpmData.frictionParameters.wLinear=0
```
