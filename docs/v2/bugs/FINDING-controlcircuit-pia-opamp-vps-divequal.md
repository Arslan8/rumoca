...
| **Signature** | `divisor:divisor-zero-when-parameters-equal:exp:829:f15aae62` |
...
```
denominator            (PIA.opAmp.vps - PIA.opAmp.vns)
witness                PIA.opAmp.Vps = -15
  found by             set equal to PIA.opAmp.Vns
denominator at witness 0.0
at declared values     30.0
path condition         always evaluated
constraints consulted  PIA.opAmp.Vps: unbounded
```
...
| `denominator` | (PIA.opAmp.vps - PIA.opAmp.vns) |
| `witness` | PIA.opAmp.Vps = -15 |
| `witness_rationale` | set equal to PIA.opAmp.Vns |
...
| `constraints` | PIA.opAmp.Vps: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.ControlCircuit \
      --target PIA.opAmp.Vps --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "PIA.opAmp.Vps"
```
