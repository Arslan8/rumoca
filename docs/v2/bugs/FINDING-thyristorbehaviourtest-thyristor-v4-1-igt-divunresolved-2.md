...
| **Signature** | `divisor:divisor-zero-unresolved:exp:356:3dafea1d` |
...
```
denominator            thyristor_v4_1.IGT
witness                thyristor_v4_1.IGT = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.005
path condition         not ((thyristor_v4_1.vGK < thyristor_v4_1.vRef))
constraints consulted  thyristor_v4_1.IGT: unbounded
```
...
| `path_condition` | not ((thyristor_v4_1.vGK &lt; thyristor_v4_1.vRef)) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: (thyristor_v4_1.vGK &lt; thyristor_v4_1.vRef) |
| `constraints` | thyristor_v4_1.IGT: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.ThyristorBehaviourTest \
      --target thyristor_v4_1.IGT --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "thyristor_v4_1.IGT"
```
