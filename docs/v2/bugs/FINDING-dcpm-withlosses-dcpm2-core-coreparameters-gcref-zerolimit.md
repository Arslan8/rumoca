# FINDING-01625: `dcpm2.core.coreParameters.GcRef` in `DCPM_withLosses`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:400:343e70c5` |
...
```
predicate     dcpm2.core.coreParameters.GcRef > 0
premise       refuted
...
```
contract    Modelica.Electrical.Machines.Losses.CoreParameters.GcRef: feature_disabled (declared, from class_catalog) — GcRef = if PRef <= 0 then 0 else PRef/VRef^2/m, and Losses.Core selects Gc = 0 on the same condition: zero is how the library disables core losses
