# FINDING-05345: `genericFluxTube2.material.B_myMax` in `Sources`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `ModelicaTest.Magnetic.FluxTubes.Sources` |
| **Reached as** | `genericFluxTube2.material.B_myMax` |
...
| **Signature** | `divisor:divisor-reachable-zero:exp:419:248abb9d` |
...
```
denominator            genericFluxTube2.material.B_myMax
witness                genericFluxTube2.material.B_myMax = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  genericFluxTube2.material.B_myMax: unbounded
```
...
| `constraints` | genericFluxTube2.material.B_myMax: unbounded |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Magnetic.FluxTubes.Sources \
      --target genericFluxTube2.material.B_myMax --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "genericFluxTube2.material.B_myMax"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param genericFluxTube2.material.B_myMax=0
```
