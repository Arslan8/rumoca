# FINDING-01316: `dceeData.Re` in `DC_CompareCharacteristics`
...
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/ParameterRecords/DcElectricalExcitedData.mo:6` |
| **Signature** | `physical:physical-domain-unenforced:var:372:060d65ab` |
...
```
predicate     dceeData.Re >= 0
premise       established
authority     component_contract
role          component.machine.winding_resistance
declaration   Modelica.Electrical.Machines.Utilities.ParameterRecords.DcElectricalExcitedData.Re
```
...
| `required` | dceeData.Re &gt;= 0 |
| `declared_min` | None |
| `variable_role` | parameter |
| `premise_state` | established |
| `authority` | component_contract |
| `semantic_role` | component.machine.winding_resistance |
| `canonical_declaration` | Modelica.Electrical.Machines.Utilities.ParameterRecords.DcElectricalExcitedData.Re |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DC_CompareCharacteristics \
      --target dceeData.Re --sanitizer physical
```
...
```console
$ sed -n '6p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/ParameterRecords/DcElectricalExcitedData.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dceeData.Re"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param dceeData.Re=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/ParameterRecords/DcElectricalExcitedData.mo:6` |' docs/v2/bugs/
```
