# FINDING-02570: `diodeBridge2mPulse.diode_n.idealDiode[1].Ron` in `SMPM_Braking`
...
| **Reached as** | `diodeBridge2mPulse.diode_n.idealDiode[1].Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:723:8e40757b` |
...
```
predicate     diodeBridge2mPulse.diode_n.idealDiode[1].Ron > 0
premise       refuted
...
| `required` | diodeBridge2mPulse.diode_n.idealDiode[1].Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_Braking \
      --target diodeBridge2mPulse.diode_n.idealDiode[1].Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "diodeBridge2mPulse.diode_n.idealDiode[1].Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param diodeBridge2mPulse.diode_n.idealDiode[1].Ron=0
```
