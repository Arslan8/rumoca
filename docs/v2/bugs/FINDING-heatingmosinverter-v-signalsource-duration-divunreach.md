...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:758:51361eb3` |
...
```
denominator            V.signalSource.duration
witness                V.signalSource.duration = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.01
path condition         not ((time < V.signalSource.startTime)) and ((time < (V.signalSource.startTime + V.signalSource.duration)))
constraints consulted  V.signalSource.duration: min=0
```
...
| `path_condition` | not ((time &lt; V.signalSource.startTime)) and ((time &lt; (V.signalSource.startTime + V.signalSource.duration))) |
| `verdict` | UNSAT |
| `proof` | the path conditions cannot hold together: time must be both &gt; 0 and &lt; 0 |
| `constraints` | V.signalSource.duration: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.HeatingMOSInverter \
      --target V.signalSource.duration --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "V.signalSource.duration"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param V.signalSource.duration=0
```
