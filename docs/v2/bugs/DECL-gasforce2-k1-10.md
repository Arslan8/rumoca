# DECL-0908: `k1` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/Examples/Loops/Utilities/GasForce2.mo:10` |
| **Parameter** | `k1` |
| **Type** | `SI.Volume` |
| **Declared modifiers** | `—` |
...
`SI.Volume` declares no `min` in `Units.mo`, and this declaration adds none of its own. It therefore accepts a negative value, and a negative volume is not a geometry.
...
```console
$ sed -n '10p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Examples/Loops/Utilities/GasForce2.mo"
```

It should declare `k1` with type `SI.Volume` and no `min`.

**2. Confirm the type supplies no bound either** — this is the whole claim, and
a bound inherited from the type would refute it:

```console
$ grep -n "type Volume " "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Units.mo"
```
...
- `SI.Volume` turns out to carry a `min` after all, or this declaration carries one that the catalog missed.
...
```modelica
// Mechanics/MultiBody/Examples/Loops/Utilities/GasForce2.mo:10
parameter SI.Volume k1(min=0) = ...;
```
