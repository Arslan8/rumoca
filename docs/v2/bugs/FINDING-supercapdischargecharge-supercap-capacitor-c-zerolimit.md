...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:75:86eb53e9` |
...
```
predicate     superCap.capacitor.C > 0
premise       refuted
...
| `required` | superCap.capacitor.C &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Batteries.Examples.SuperCapDischargeCharge \
      --target superCap.capacitor.C --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "superCap.capacitor.C"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param superCap.capacitor.C=0
```
