# FINDING-01890: `inverter.transistor_n.idealGTOThyristor[2].Goff` in `IMC_InverterDrive`
...
| **Reached as** | `inverter.transistor_n.idealGTOThyristor[2].Goff` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:731:29a2f152` |
...
```
predicate     inverter.transistor_n.idealGTOThyristor[2].Goff > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealGTOThyristor.Goff: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Ideal.IdealGTOThyristor.Goff'}
```
...
| `required` | inverter.transistor_n.idealGTOThyristor[2].Goff &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive \
      --target inverter.transistor_n.idealGTOThyristor[2].Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inverter.transistor_n.idealGTOThyristor[2].Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inverter.transistor_n.idealGTOThyristor[2].Goff=0
```
