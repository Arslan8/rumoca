# FINDING-04323: `g_mFeYokeSide2.l` in `AdvancedSolenoid`
...
| **Reached as** | `g_mFeYokeSide2.l` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/FixedShape/HollowCylinderAxialFlux.mo:20` |
| **Signature** | `divisor:divisor-reachable-zero:exp:2106:16b1f6e6` |
...
```
denominator            g_mFeYokeSide2.l
witness                g_mFeYokeSide2.l = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.01575
path condition         always evaluated
constraints consulted  g_mFeYokeSide2.l: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.SolenoidActuator.Components.AdvancedSolenoid \
      --target g_mFeYokeSide2.l --sanitizer divisor
```
...
```console
$ sed -n '20p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/FixedShape/HollowCylinderAxialFlux.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "g_mFeYokeSide2.l"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/FixedShape/HollowCylinderAxialFlux.mo:20` |' docs/v2/bugs/
```
