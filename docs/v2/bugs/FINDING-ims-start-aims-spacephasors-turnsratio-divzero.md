# FINDING-02315: `aims.spacePhasorS.turnsRatio` in `IMS_Start`
...
| **Reached as** | `aims.spacePhasorS.turnsRatio` |
...
| **Signature** | `divisor:divisor-reachable-zero:exp:6024:433eb968` |
...
```
denominator            aims.spacePhasorS.turnsRatio
witness                aims.spacePhasorS.turnsRatio = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  aims.spacePhasorS.turnsRatio: unbounded
```
...
| `constraints` | aims.spacePhasorS.turnsRatio: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMS_Start \
      --target aims.spacePhasorS.turnsRatio --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "aims.spacePhasorS.turnsRatio"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param aims.spacePhasorS.turnsRatio=0
```
