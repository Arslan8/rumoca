# FINDING-04586: `bodyBox.body.I` in `BevelGear1D`
...
| **Signature** | `physical:physical-inertia-tensor-undecided:par:666:f8fe80af` |
...
This one is an **aggregate** rule: its subject is the `symmetric_inertia_tensor` that `bodyBox.body` assembles from several declarations, not any one of them. The fields it covers are excluded from the scalar rule, so this finding is the only thing said about them.

```
matrix       [[I_11, I_21, I_31], [I_21, I_22, I_32], [I_31, I_32, I_33]] (not numerically known)
eigenvalues  not computed
verdict      unknown
reason       `bodyBox.body.I_11` is bound to `bodyBox.I[1, 1]`, which this analysis cannot evaluate, so the tensor cannot be assembled
```
...
| `component` | bodyBox.body |
| `declaring_class` | Modelica.Mechanics.MultiBody.Parts.Body |
| `fields` | bodyBox.body.I_11, bodyBox.body.I_22, bodyBox.body.I_33, bodyBox.body.I_21, bodyBox.body.I_31, bodyBox.body.I_32 |
| `matrix` | [[I_11, I_21, I_31], [I_21, I_22, I_32], [I_31, I_32, I_33]] (not numerically known) |
| `required` | bodyBox.body.I is positive semidefinite |
| `verdict` | unknown |
| `reason` | `bodyBox.body.I_11` is bound to `bodyBox.I[1, 1]`, which this analysis cannot evaluate, so the tensor cannot be assembled |
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Rotational3DEffects.BevelGear1D \
      --target bodyBox.body.I --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "bodyBox.body.I"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param bodyBox.body.I=0
```
