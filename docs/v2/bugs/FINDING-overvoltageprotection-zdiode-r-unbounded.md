# FINDING-01075: `zDiode.R` in `OvervoltageProtection`
...
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/ZDiode.mo:8` |
| **Signature** | `physical:physical-domain-unenforced:var:68:14f47082` |
...
```
predicate     zDiode.R > 0
premise       established
authority     source_arithmetic
role          —
declaration   Modelica.Electrical.Analog.Semiconductors.ZDiode.R
```
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Semiconductors.ZDiode.R'}
```
...
| `authority` | source_arithmetic |
| `canonical_declaration` | Modelica.Electrical.Analog.Semiconductors.ZDiode.R |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OvervoltageProtection \
      --target zDiode.R --sanitizer physical
```
...
```console
$ sed -n '8p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/ZDiode.mo"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param zDiode.R=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/ZDiode.mo:8` |' docs/v2/bugs/
```
