...
| **Signature** | `divisor:divisor-reachable-zero:exp:677:73a56784` |
...
```
denominator            idealPump.medium.cp
witness                idealPump.medium.cp = 0
  found by             set to zero
denominator at witness 0.0
at declared values     4177.0
path condition         always evaluated
constraints consulted  idealPump.medium.cp: unbounded
```
...
| `constraints` | idealPump.medium.cp: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.WaterPump \
      --target idealPump.medium.cp --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "idealPump.medium.cp"
```
