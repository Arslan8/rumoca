# FINDING-01144: `SaturatingInductance1.Inom` in `ShowSaturatingInductor`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Electrical.Analog.Examples.ShowSaturatingInductor` |
| **Reached as** | `SaturatingInductance1.Inom` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/SaturatingInductor.mo:21` |
| **Signature** | `divisor:divisor-reachable-zero:exp:255:52f037f2` |
...
```
denominator            SaturatingInductance1.Inom
witness                SaturatingInductance1.Inom = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  SaturatingInductance1.Inom: unbounded
```
...
| `constraints` | SaturatingInductance1.Inom: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.ShowSaturatingInductor \
      --target SaturatingInductance1.Inom --sanitizer divisor
```
...
```console
$ sed -n '21p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/SaturatingInductor.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "SaturatingInductance1.Inom"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param SaturatingInductance1.Inom=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/SaturatingInductor.mo:21` |' docs/v2/bugs/
```
