# DECL-0693: `Ra` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Machines/Examples/ControlledDCDrives/Utilities/DriveDataDCPM.mo:11` |
| **Parameter** | `Ra` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '11p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/ControlledDCDrives/Utilities/DriveDataDCPM.mo"
```

It should declare `Ra` with type `SI.Resistance` and no `min`.
...
```modelica
// Electrical/Machines/Examples/ControlledDCDrives/Utilities/DriveDataDCPM.mo:11
parameter SI.Resistance Ra(min=0) = ...;
```
