...
| **Reach** | 6 models set this declaration |
...
`Modelica.Mechanics.Translational.Examples.Damper` runs cleanly at its declared values. Setting `mass1.m = 0` — a value the declaration permits — makes it fail, in Rumoca and in OpenModelica.
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Translational.Examples.Damper \
      --target mass1.m --keep-parameter-chains
```
...
simulate(Modelica.Mechanics.Translational.Examples.Damper, simflags="-override mass1.m=0");
EOF
```
