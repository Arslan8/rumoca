...
| **Signature** | `divisor:divisor-zero-unresolved:exp:479:314516bb` |
...
```
denominator            H_PMOS.Tnom
witness                H_PMOS.Tnom = 0
  found by             set to zero
denominator at witness 0.0
at declared values     300.15
path condition         (H_PMOS.useTemperatureDependency)
constraints consulted  H_PMOS.Tnom: min=0
```
...
| `path_condition` | (H_PMOS.useTemperatureDependency) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: H_PMOS.useTemperatureDependency |
| `constraints` | H_PMOS.Tnom: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.HeatingMOSInverter \
      --target H_PMOS.Tnom --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "H_PMOS.Tnom"
```
