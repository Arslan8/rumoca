# FINDING-04591: `inertia2.J` in `BevelGear1D`
...
| **Signature** | `physical:physical-intent-question:var:301:981ffd77` |
...
> `inertia2.J` is bounded only by min=0, which is unusual for a moment of inertia — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
| `question` | `inertia2.J` is bounded only by min=0, which is unusual for a moment of inertia — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Rotational3DEffects.BevelGear1D \
      --target inertia2.J --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inertia2.J"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inertia2.J=0
```
