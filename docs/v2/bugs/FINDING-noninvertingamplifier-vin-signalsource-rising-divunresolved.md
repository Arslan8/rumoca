...
| **Signature** | `divisor:divisor-zero-unresolved:exp:271:8cd7d0d8` |
...
```
denominator            vIn.signalSource.rising
witness                vIn.signalSource.rising = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.02
path condition         not ((((time < vIn.signalSource.startTime) or (vIn.signalSource.nperiod == 0)) or ((vIn.signalSource.nperiod > 0) and (vIn.signalSource.count >= vIn.signalSource.nperiod)))) and ((time < (vIn.signalSource.T_start + vIn.signalSource.T_rising)))
constraints consulted  vIn.signalSource.rising: min=0
```
...
| `denominator_at_declared_values` | 0.02 |
| `denominator_range` | [0, inf] |
| `path_condition` | not ((((time &lt; vIn.signalSource.startTime) or (vIn.signalSource.nperiod == 0)) or ((vIn.signalSource.nperiod &gt; 0) and (vIn.signalSource.count &gt;= vIn.signalSource.nperiod)))) and ((time &lt; (vIn.signalSource.T_start + vIn.signalSource.T_rising))) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: (time &lt; (vIn.signalSource.T_start + vIn.signalSource.T_rising)); (time &lt; vIn.signalSource.startTime) |
| `constraints` | vIn.signalSource.rising: min=0 |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.NonInvertingAmplifier \
      --target vIn.signalSource.rising --sanitizer divisor
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.NonInvertingAmplifier --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "vIn.signalSource.rising"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param vIn.signalSource.rising=0
```
