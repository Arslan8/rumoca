# DECL-0010: `bottom_pipeArea` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Fluid/Examples/AST_BatchPlant.mo:607` |
| **Parameter** | `bottom_pipeArea` |
| **Type** | `SI.Area` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '607p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Examples/AST_BatchPlant.mo"
```

It should declare `bottom_pipeArea` with type `SI.Area` and no `min`.
...
```modelica
// Fluid/Examples/AST_BatchPlant.mo:607
parameter SI.Area bottom_pipeArea(min=0) = ...;
```
