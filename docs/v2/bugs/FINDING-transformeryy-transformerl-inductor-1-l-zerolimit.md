...
| **Reached as** | `transformerL.inductor[1].L` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:294:dc61b9e2` |
...
```
predicate     transformerL.inductor[1].L > 0
premise       refuted
...
| `required` | transformerL.inductor[1].L &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Polyphase.Examples.TransformerYY \
      --target transformerL.inductor[1].L --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "transformerL.inductor[1].L"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param transformerL.inductor[1].L=0
```
