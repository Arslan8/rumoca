...
| **Signature** | `divisor:divisor-reachable-zero:exp:891:3f4ef6e0` |
...
```
denominator            oneWayValve.medium.cp
witness                oneWayValve.medium.cp = 0
  found by             set to zero
denominator at witness 0.0
at declared values     4177.0
path condition         always evaluated
constraints consulted  oneWayValve.medium.cp: unbounded
```
...
| `constraints` | oneWayValve.medium.cp: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.WaterPump \
      --target oneWayValve.medium.cp --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "oneWayValve.medium.cp"
```
