# FINDING-02974: `smr.spacePhasorS.turnsRatio` in `SMR_Inverter`
...
| **Reached as** | `smr.spacePhasorS.turnsRatio` |
...
| **Signature** | `divisor:divisor-reachable-zero:exp:3838:da36b1a1` |
...
```
denominator            smr.spacePhasorS.turnsRatio
witness                smr.spacePhasorS.turnsRatio = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  smr.spacePhasorS.turnsRatio: unbounded
```
...
| `constraints` | smr.spacePhasorS.turnsRatio: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMR_Inverter \
      --target smr.spacePhasorS.turnsRatio --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smr.spacePhasorS.turnsRatio"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param smr.spacePhasorS.turnsRatio=0
```
