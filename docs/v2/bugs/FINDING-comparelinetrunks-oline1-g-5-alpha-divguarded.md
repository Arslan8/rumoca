...
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:4993:b44a4e62` |
...
```
denominator            (1 + (oLine1.G[5].alpha * (oLine1.G[5].T_heatPort - oLine1.G[5].T_ref)))
witness                none needed
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.CompareLineTrunks \
      --target oLine1.G[5].alpha --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "oLine1.G[5].alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param oLine1.G[5].alpha=0
```
