# DECL-0560: `defaultJointLength` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/package.mo:113` |
| **Parameter** | `defaultJointLength` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '113p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/package.mo"
```

It should declare `defaultJointLength` with type `SI.Length` and no `min`.
...
```modelica
// Mechanics/MultiBody/package.mo:113
parameter SI.Length defaultJointLength(min=0) = ...;
```
