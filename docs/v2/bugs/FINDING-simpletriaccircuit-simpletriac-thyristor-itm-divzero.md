...
| **Signature** | `divisor:divisor-reachable-zero:exp:138:7d8d9c5e` |
...
```
denominator            simpleTriac.thyristor.ITM
witness                simpleTriac.thyristor.ITM = 0
  found by             set to zero
denominator at witness 0.0
at declared values     25.0
path condition         always evaluated
constraints consulted  simpleTriac.thyristor.ITM: unbounded
```
...
| `constraints` | simpleTriac.thyristor.ITM: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.SimpleTriacCircuit \
      --target simpleTriac.thyristor.ITM --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "simpleTriac.thyristor.ITM"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param simpleTriac.thyristor.ITM=0
```
