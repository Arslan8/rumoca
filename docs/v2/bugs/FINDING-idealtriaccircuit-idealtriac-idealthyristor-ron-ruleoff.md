...
| **Signature** | `physical:physical-rule-does-not-apply:var:45:8eae74fe` |
...
```
predicate     idealTriac.idealThyristor.Ron > 0
premise       refuted
...
| `required` | idealTriac.idealThyristor.Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.IdealTriacCircuit \
      --target idealTriac.idealThyristor.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "idealTriac.idealThyristor.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param idealTriac.idealThyristor.Ron=0
```
