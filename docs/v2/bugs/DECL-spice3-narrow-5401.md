# DECL-0398: `NARROW` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Spice3.mo:5401` |
| **Parameter** | `NARROW` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '5401p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Spice3.mo"
```

It should declare `NARROW` with type `SI.Length` and no `min`.
...
```modelica
// Electrical/Spice3.mo:5401
parameter SI.Length NARROW(min=0) = ...;
```
