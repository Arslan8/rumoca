...
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:578:5817f7d9` |
...
```
denominator            (1 + (conductor1.alpha * (conductor1.T_heatPort - conductor1.T_ref)))
witness                none needed
...
$ python3 tools/sweep/check_one.py ModelicaTest.Magnetic.FluxTubes.BasicComponents \
      --target conductor1.alpha --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "conductor1.alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param conductor1.alpha=0
```
