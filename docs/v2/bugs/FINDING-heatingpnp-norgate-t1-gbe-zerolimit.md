# FINDING-00378: `T1.Gbe` in `HeatingPNP_NORGate`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/PNP.mo:17` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:189:0340091e` |
...
```
contract    Modelica.Electrical.Analog.Semiconductors.PNP.Gbe: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Semiconductors.PNP.Gbe'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.HeatingPNP_NORGate \
      --target T1.Gbe --sanitizer physical
```
...
```console
$ sed -n '17p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/PNP.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "T1.Gbe"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param T1.Gbe=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/PNP.mo:17` |' docs/v2/bugs/
```
