# FINDING-02225: `switchYD.Ron` in `IMC_YD`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SwitchYD.mo:4` |
| **Signature** | `physical:physical-rule-does-not-apply:var:643:a6ccedb6` |
...
```
contract    Modelica.Electrical.Polyphase.Ideal.IdealClosingSwitch.Ron: allowed (declared, from class_catalog) — inherited from switchYD.idealCloser.Ron, which it binds: an ideal switch family member; zero on-resistance is the documented ideal
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.Utilities.SwitchYD.Ron'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_YD \
      --target switchYD.Ron --sanitizer physical
```
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SwitchYD.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switchYD.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switchYD.Ron=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SwitchYD.mo:4` |' docs/v2/bugs/
```
