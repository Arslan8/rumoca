...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/SaturatingInductor.mo:10` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:34:32d0a0cd` |
...
```
predicate     SaturatingInductance1.Lnom > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Basic.SaturatingInductor.Lnom: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Basic.SaturatingInductor.Lnom'}
```
...
| `required` | SaturatingInductance1.Lnom &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.ShowSaturatingInductor \
      --target SaturatingInductance1.Lnom --sanitizer physical
```
...
```console
$ sed -n '10p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/SaturatingInductor.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "SaturatingInductance1.Lnom"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param SaturatingInductance1.Lnom=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Basic/SaturatingInductor.mo:10` |' docs/v2/bugs/
```
