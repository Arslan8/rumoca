# FINDING-00408: `HeatingDiode1.TNOM` in `HeatingRectifier`
...
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/Diode.mo:34` |
| **Signature** | `divisor:divisor-reachable-zero:exp:213:0230bdff` |
...
```
denominator            HeatingDiode1.TNOM
witness                HeatingDiode1.TNOM = 0
  found by             set to zero
denominator at witness 0.0
at declared values     300.15
path condition         always evaluated
constraints consulted  HeatingDiode1.TNOM: min=0
```
...
| `constraints` | HeatingDiode1.TNOM: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.HeatingRectifier \
      --target HeatingDiode1.TNOM --sanitizer divisor
```
...
```console
$ sed -n '34p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/Diode.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "HeatingDiode1.TNOM"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/Diode.mo:34` |' docs/v2/bugs/
```
