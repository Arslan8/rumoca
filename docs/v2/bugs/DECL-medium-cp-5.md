...
| **Declaration** | `Thermal/FluidHeatFlow/Media/Medium.mo:5` |
| **Parameter** | `cp` |
| **Type** | `SI.SpecificHeatCapacity` |
| **Declared modifiers** | `—` |
...
`SI.SpecificHeatCapacity` declares no `min` in `Units.mo`, and this declaration adds none of its own. It therefore accepts a negative value, and a negative heat capacity makes heating cool the body.
...
```console
$ sed -n '5p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/Media/Medium.mo"
```

It should declare `cp` with type `SI.SpecificHeatCapacity` and no `min`.

**2. Confirm the type supplies no bound either** — this is the whole claim, and
a bound inherited from the type would refute it:

```console
$ grep -n "type SpecificHeatCapacity " "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Units.mo"
```
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/Media/Medium.mo" | head
```
...
- `SI.SpecificHeatCapacity` turns out to carry a `min` after all, or this declaration carries one that the catalog missed.
...
```modelica
// Thermal/FluidHeatFlow/Media/Medium.mo:5
parameter SI.SpecificHeatCapacity cp(min=0) = ...;
```
