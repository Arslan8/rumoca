# FINDING-03208: `transformerData1.f` in `Rectifier12pulse`
...
| **Signature** | `divisor:divisor-reachable-zero:exp:830:d93aba7c` |
...
```
denominator            ((2 * (2 * asin(1.0))) * transformerData1.f)
witness                transformerData1.f = 0
  found by             set to zero
denominator at witness 0.0
at declared values     314.1592653589793
path condition         always evaluated
constraints consulted  transformerData1.f: unbounded
```
...
| `constraints` | transformerData1.f: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.Transformers.Rectifier12pulse \
      --target transformerData1.f --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "transformerData1.f"
```
