...
| **Reached as** | `rectifier.diode.idealDiode[2].Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:156:ce5205db` |
...
```
predicate     rectifier.diode.idealDiode[2].Ron > 0
premise       refuted
...
| `required` | rectifier.diode.idealDiode[2].Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTapmPulse.DiodeCenterTapmPulse \
      --target rectifier.diode.idealDiode[2].Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "rectifier.diode.idealDiode[2].Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param rectifier.diode.idealDiode[2].Ron=0
```
