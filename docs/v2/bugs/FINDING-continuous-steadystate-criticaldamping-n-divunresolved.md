# FINDING-05173: `criticalDamping.n` in `Continuous_SteadyState`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `ModelicaTest.Blocks.Continuous_SteadyState` |
| **Reached as** | `criticalDamping.n` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-zero-unresolved` |
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Blocks/Continuous.mo:1521` |
| **Signature** | `divisor:divisor-zero-unresolved:exp:606:0ad86a2c` |
...
```
denominator            criticalDamping.n
witness                criticalDamping.n = 0
  found by             set to zero
denominator at witness 0.0
at declared values     2.0
path condition         (criticalDamping.normalized)
constraints consulted  criticalDamping.n: unbounded
```
...
| `denominator_at_declared_values` | 2.0 |
| `denominator_range` | [-inf, inf] |
| `path_condition` | (criticalDamping.normalized) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: criticalDamping.normalized |
| `constraints` | criticalDamping.n: unbounded |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Blocks.Continuous_SteadyState \
      --target criticalDamping.n --sanitizer divisor
```
...
```console
$ sed -n '1521p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Blocks/Continuous.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "criticalDamping.n"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Blocks/Continuous.mo:1521` |' docs/v2/bugs/
```
