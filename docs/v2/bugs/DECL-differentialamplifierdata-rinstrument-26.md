# DECL-0637: `RInstrument` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Examples/OpAmps/OpAmpCircuits/DifferentialAmplifierData.mo:26` |
| **Parameter** | `RInstrument` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
It should declare `RInstrument` with type `SI.Resistance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/OpAmps/OpAmpCircuits/DifferentialAmplifierData.mo" | head
```
...
```modelica
// Electrical/Analog/Examples/OpAmps/OpAmpCircuits/DifferentialAmplifierData.mo:26
parameter SI.Resistance RInstrument(min=0) = ...;
```
