# FINDING-02497: `smeeData.xdSubtransient` in `SMEE_Generator`
...
| **Signature** | `divisor:divisor-reachable-zero:exp:1945:1911bfd5` |
...
```
denominator            smeeData.xdSubtransient
witness                smeeData.xdSubtransient = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.121428571
path condition         always evaluated
constraints consulted  smeeData.xdSubtransient: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMEE_Generator \
      --target smeeData.xdSubtransient --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smeeData.xdSubtransient"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param smeeData.xdSubtransient=0
```
