# FINDING-02157: `switchYDwithArc.idealCloser.closerWithArc[1].Ron` in `IMC_YDarc`
...
| **Reached as** | `switchYDwithArc.idealCloser.closerWithArc[1].Ron` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Interfaces/IdealSwitchWithArc.mo:4` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:723:6462395b` |
...
```
predicate     switchYDwithArc.idealCloser.closerWithArc[1].Ron > 0
premise       refuted
...
```
contract    Modelica.Electrical.Analog.Ideal.CloserWithArc.Ron: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Ideal.CloserWithArc.Ron'}
```
...
| `required` | switchYDwithArc.idealCloser.closerWithArc[1].Ron &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_YDarc \
      --target switchYDwithArc.idealCloser.closerWithArc[1].Ron --sanitizer physical
```
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Interfaces/IdealSwitchWithArc.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "switchYDwithArc.idealCloser.closerWithArc[1].Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param switchYDwithArc.idealCloser.closerWithArc[1].Ron=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Interfaces/IdealSwitchWithArc.mo:4` |' docs/v2/bugs/
```
