# FINDING-04180: `upperYoke.l` in `QuadraticCoreAirgap`
...
| **Reached as** | `upperYoke.l` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/FixedShape/Cuboid.mo:18` |
| **Signature** | `divisor:divisor-reachable-zero:exp:484:770ebc3c` |
...
```
denominator            upperYoke.l
witness                upperYoke.l = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.09000000000000001
path condition         always evaluated
constraints consulted  upperYoke.l: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.BasicExamples.QuadraticCoreAirgap \
      --target upperYoke.l --sanitizer divisor
```
...
```console
$ sed -n '18p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/FixedShape/Cuboid.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "upperYoke.l"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Shapes/FixedShape/Cuboid.mo:18` |' docs/v2/bugs/
```
