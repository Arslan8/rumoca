...
```
predicate     simpleTriac.idealDiode.Goff > 0
premise       refuted
...
| `required` | simpleTriac.idealDiode.Goff &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.SimpleTriacCircuit \
      --target simpleTriac.idealDiode.Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "simpleTriac.idealDiode.Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param simpleTriac.idealDiode.Goff=0
```
