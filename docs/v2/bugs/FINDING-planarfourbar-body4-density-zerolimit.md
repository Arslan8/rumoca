...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:580:85a9343a` |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Loops.PlanarFourbar \
      --target body4.density --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Loops.PlanarFourbar --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "body4.density"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param body4.density=0
```
