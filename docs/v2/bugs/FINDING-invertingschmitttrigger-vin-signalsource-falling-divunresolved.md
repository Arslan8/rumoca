# FINDING-00911: `vIn.signalSource.falling` in `InvertingSchmittTrigger`
...
| **Reached as** | `vIn.signalSource.falling` |
...
| **Signature** | `divisor:divisor-zero-unresolved:exp:354:25547a7d` |
...
```
denominator            vIn.signalSource.falling
witness                vIn.signalSource.falling = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.02
path condition         not ((((time < vIn.signalSource.startTime) or (vIn.signalSource.nperiod == 0)) or ((vIn.signalSource.nperiod > 0) and (vIn.signalSource.count >= vIn.signalSource.nperiod)))) and not ((time < (vIn.signalSource.T_start + vIn.signalSource.T_rising))) and not ((time < (vIn.signalSource.T_start + vIn.signalSource.T_width))) and ((time < (vIn.signalSource.T_start + vIn.signalSource.T_falling)))
constraints consulted  vIn.signalSource.falling: min=0
```
...
| `path_condition` | not ((((time &lt; vIn.signalSource.startTime) or (vIn.signalSource.nperiod == 0)) or ((vIn.signalSource.nperiod &gt; 0) and (vIn.signalSource.count &gt;= vIn.signalSource.nperiod)))) and not ((time &lt; (vIn.signalSource.T_start + vIn.signalSource.T_rising))) and not ((time &lt; (vIn.signalSource.T_start + vIn.signalSource.T_width))) and ((time &lt; (vIn.signalSource.T_start + vIn.signalSource.T_falling))) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: (time &lt; (vIn.signalSource.T_start + vIn.signalSource.T_falling)); (time &lt; (vIn.signalSource.T_start + vIn.signalSource.T_rising)); (time &lt; (vIn.signalSource.T_start + vIn.signalSource.T_width)) |
| `constraints` | vIn.signalSource.falling: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.InvertingSchmittTrigger \
      --target vIn.signalSource.falling --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "vIn.signalSource.falling"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param vIn.signalSource.falling=0
```
