# FINDING-00122: `R4.R` in `CauerLowPassSC`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/Utilities/SwitchedCapacitor.mo:5` |
| **Signature** | `physical:physical-rule-does-not-apply:var:164:00ceb5a5` |
...
```
contract    Modelica.Electrical.Analog.Examples.Utilities.SwitchedCapacitor.R: allowed (declared, from class_catalog) — the component is documented as a "switched capacitor which can represent a positive or negative resistance", and CauerLowPassSC instantiates four of them at R = -1; a positivity rule contradicts the component's stated purpose
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Examples.Utilities.SwitchedCapacitor.R'}
```
...
| `note` | the component documents this quantity as taking the value it holds, so the rule derived from the quantity alone does not bind it |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.CauerLowPassSC \
      --target R4.R --sanitizer physical
```
...
```console
$ sed -n '5p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/Utilities/SwitchedCapacitor.mo"
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.CauerLowPassSC --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "R4.R"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param R4.R=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/Utilities/SwitchedCapacitor.mo:5` |' docs/v2/bugs/
```
