...
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:8019:1f311a1c` |
...
```
denominator            (1 + (oLine5.G[1].alpha * (oLine5.G[1].T_heatPort - oLine5.G[1].T_ref)))
witness                none needed
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.SmoothStep \
      --target oLine5.G[1].alpha --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "oLine5.G[1].alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param oLine5.G[1].alpha=0
```
