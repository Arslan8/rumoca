# FINDING-03995: `dcdc.RonDiode` in `ChopperBuckBoost_DutyCycle`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCDC/ChopperBuckBoost.mo:12` |
| **Signature** | `physical:physical-rule-does-not-apply:var:51:3b5fff5e` |
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealDiode.Ron: allowed (declared, from class_catalog) — inherited from dcdc.diodeLS.Ron, which it binds: an ideal switch family member; zero on-resistance is the documented ideal
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.PowerConverters.DCDC.ChopperBuckBoost.RonDiode'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCDC.ChopperBuckBoost.ChopperBuckBoost_DutyCycle \
      --target dcdc.RonDiode --sanitizer physical
```
...
```console
$ sed -n '12p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCDC/ChopperBuckBoost.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcdc.RonDiode"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param dcdc.RonDiode=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/DCDC/ChopperBuckBoost.mo:12` |' docs/v2/bugs/
```
