# FINDING-03295: `transformer.l1sigma.inductor[2].L` in `TransformerTestbench`
...
| **Reached as** | `transformer.l1sigma.inductor[2].L` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:684:f88d9323` |
...
```
predicate     transformer.l1sigma.inductor[2].L > 0
premise       refuted
...
| `required` | transformer.l1sigma.inductor[2].L &gt; 0 |
| `declared_min` | None |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.Transformers.TransformerTestbench \
      --target transformer.l1sigma.inductor[2].L --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "transformer.l1sigma.inductor[2].L"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param transformer.l1sigma.inductor[2].L=0
```
