# FINDING-01262: `superCap.Qnom` in `SuperCapDischargeCharge`
...
| **Found by** | **PhysicalSan** — `packages/modelsan/modelsan/sanitizers/physical.py`, rule `battery.capacity.positive` |
| **Reported as** | `physical-intent-question` |
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Batteries/BatteryStacks/SuperCap.mo:8` |
| **Signature** | `physical:physical-intent-question:var:32:1cbb9271` |
...
```
predicate     superCap.Qnom > 0
premise       unknown
authority     quantity_or_unit
role          —
declaration   Modelica.Electrical.Batteries.BatteryStacks.SuperCap.Qnom
```
...
> `superCap.Qnom` carries no bound that would exclude it, which is unusual for an electric charge — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.

The rule it applied is `battery.capacity.positive`:

> capacity is the divisor that normalises charge into SOC
...
```
{'quantity': 'ElectricCharge', 'unit': 'C', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Batteries.BatteryStacks.SuperCap.Qnom'}
```
...
| `authority` | quantity_or_unit |
| `canonical_declaration` | Modelica.Electrical.Batteries.BatteryStacks.SuperCap.Qnom |
| `question` | `superCap.Qnom` carries no bound that would exclude it, which is unusual for an electric charge — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Batteries.Examples.SuperCapDischargeCharge \
      --target superCap.Qnom --sanitizer physical
```
...
```console
$ sed -n '8p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Batteries/BatteryStacks/SuperCap.mo"
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Batteries.Examples.SuperCapDischargeCharge --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "superCap.Qnom"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param superCap.Qnom=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Batteries/BatteryStacks/SuperCap.mo:8` |' docs/v2/bugs/
```
