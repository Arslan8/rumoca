# DECL-0832: `R_actual` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/FluxTubes/Examples/Utilities/CoilDesign.mo:58` |
| **Parameter** | `R_actual` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '58p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/Utilities/CoilDesign.mo"
```

It should declare `R_actual` with type `SI.Resistance` and no `min`.
...
```modelica
// Magnetic/FluxTubes/Examples/Utilities/CoilDesign.mo:58
parameter SI.Resistance R_actual(min=0) = ...;
```
