...
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:7553:fd1a9507` |
...
```
denominator            (1 + (oLine4.G[6].alpha * (oLine4.G[6].T_heatPort - oLine4.G[6].T_ref)))
witness                none needed
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.CompareLineTrunks \
      --target oLine4.G[6].alpha --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "oLine4.G[6].alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param oLine4.G[6].alpha=0
```
