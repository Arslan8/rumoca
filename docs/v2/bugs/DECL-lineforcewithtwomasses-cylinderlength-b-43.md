# DECL-0541: `cylinderLength_b` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/Forces/LineForceWithTwoMasses.mo:43` |
| **Parameter** | `cylinderLength_b` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '43p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Forces/LineForceWithTwoMasses.mo"
```

It should declare `cylinderLength_b` with type `SI.Length` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Forces/LineForceWithTwoMasses.mo" | head
```
...
```modelica
// Mechanics/MultiBody/Forces/LineForceWithTwoMasses.mo:43
parameter SI.Length cylinderLength_b(min=0) = ...;
```
