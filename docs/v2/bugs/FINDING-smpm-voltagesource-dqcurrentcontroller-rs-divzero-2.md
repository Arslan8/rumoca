# FINDING-02862: `dqCurrentController.Rs` in `SMPM_VoltageSource`
...
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/DQCurrentController.mo:61` |
| **Signature** | `divisor:divisor-reachable-zero:exp:1327:68bbd06a` |
...
```
denominator            dqCurrentController.Rs
witness                dqCurrentController.Rs = 0
  found by             set to zero
denominator at witness 0.0
at declared values     None
path condition         always evaluated
constraints consulted  dqCurrentController.Rs: unbounded
```
...
| `constraints` | dqCurrentController.Rs: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_VoltageSource \
      --target dqCurrentController.Rs --sanitizer divisor
```
...
```console
$ sed -n '61p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/DQCurrentController.mo"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param dqCurrentController.Rs=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/DQCurrentController.mo:61` |' docs/v2/bugs/
```
