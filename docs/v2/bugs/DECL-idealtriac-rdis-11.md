# DECL-0658: `Rdis` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Ideal/IdealTriac.mo:11` |
| **Parameter** | `Rdis` |
| **Type** | `SI.Resistance` |
| **Declared modifiers** | `—` |
...
It should declare `Rdis` with type `SI.Resistance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Ideal/IdealTriac.mo" | head
```
...
```modelica
// Electrical/Analog/Ideal/IdealTriac.mo:11
parameter SI.Resistance Rdis(min=0) = ...;
```
