# DECL-0030: `A_wireCalculated` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/FluxTubes/Examples/Utilities/CoilDesign.mo:38` |
| **Parameter** | `A_wireCalculated` |
| **Type** | `SI.Area` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '38p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/Utilities/CoilDesign.mo"
```

It should declare `A_wireCalculated` with type `SI.Area` and no `min`.
...
```modelica
// Magnetic/FluxTubes/Examples/Utilities/CoilDesign.mo:38
parameter SI.Area A_wireCalculated(min=0) = ...;
```
