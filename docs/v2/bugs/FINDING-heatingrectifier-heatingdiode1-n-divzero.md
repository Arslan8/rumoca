# FINDING-00406: `HeatingDiode1.N` in `HeatingRectifier`
...
| **Signature** | `divisor:divisor-reachable-zero:exp:200:1f6509de` |
...
```
denominator            HeatingDiode1.N
witness                HeatingDiode1.N = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  HeatingDiode1.N: unbounded
```
...
| `constraints` | HeatingDiode1.N: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.HeatingRectifier \
      --target HeatingDiode1.N --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "HeatingDiode1.N"
```
