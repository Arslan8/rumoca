# FINDING-02804: `smpmData.fsNominal` in `SMPM_ResistiveBraking`
...
| **Signature** | `divisor:divisor-reachable-zero:exp:2321:cd798206` |
...
```
denominator            ((2 * (2 * asin(1.0))) * smpmData.fsNominal)
witness                smpmData.fsNominal = 0
  found by             set to zero
denominator at witness 0.0
at declared values     314.1592653589793
path condition         always evaluated
constraints consulted  smpmData.fsNominal: unbounded
```
...
| `constraints` | smpmData.fsNominal: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_ResistiveBraking \
      --target smpmData.fsNominal --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smpmData.fsNominal"
```
