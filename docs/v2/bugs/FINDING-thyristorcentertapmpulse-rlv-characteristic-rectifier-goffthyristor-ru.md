# FINDING-03918: `rectifier.GoffThyristor` in `ThyristorCenterTapmPulse_RLV_Characteristic`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/ThyristorCenterTapmPulse.mo:9` |
| **Signature** | `physical:physical-rule-does-not-apply:var:134:66879b95` |
...
```
contract    Modelica.Electrical.Polyphase.Ideal.IdealThyristor.Goff: allowed (declared, from class_catalog) — inherited from rectifier.thyristor.Goff, which it binds: an ideal switch family member; zero off-conductance is the documented ideal
behavior    allowed
confidence  declared
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.PowerConverters.ACDC.ThyristorCenterTapmPulse.GoffThyristor'}
```
...
```console
$ sed -n '9p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/ThyristorCenterTapmPulse.mo"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/ThyristorCenterTapmPulse.mo:9` |' docs/v2/bugs/
```
