# FINDING-05464: `wheel_radius` in `RoverPlant`
...
| **Severity** | high |
| **Declaration** | `target/cmm/CMM-a642c381/RigidBody/Examples/RoverPlant.mo:67` |
| **Signature** | `divisor:divisor-reachable-zero:exp:529:f0a639cd` |
...
```
denominator            wheel_radius
witness                wheel_radius = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.15
path condition         always evaluated
constraints consulted  wheel_radius: unbounded
```
...
```console
$ python3 tools/sweep/check_one.py RigidBody.Examples.RoverPlant \
      --target wheel_radius --sanitizer divisor
```
...
```console
$ sed -n '67p' "target/cmm/CMM-a642c381/RigidBody/Examples/RoverPlant.mo"
```
...
```console
$ python3 tools/sweep/check_one.py RigidBody.Examples.RoverPlant --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "wheel_radius"
```
...
```console
$ grep -rl '| `target/cmm/CMM-a642c381/RigidBody/Examples/RoverPlant.mo:67` |' docs/v2/bugs/
```
