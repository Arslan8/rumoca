# FINDING-00031: `tau0` in `MixingUnit`
...
| **Signature** | `divisor:divisor-reachable-zero:exp:37:19c05a2e` |
...
constraints consulted  tau0: unbounded
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Clocked.Examples.Systems.Utilities.ComponentsMixingUnit.MixingUnit \
      --target tau0 --sanitizer divisor
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Clocked.Examples.Systems.Utilities.ComponentsMixingUnit.MixingUnit --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "tau0"
```
