# DECL-0373: `len` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Examples/Lines/LightningSegmentedTransmissionLine.mo:12` |
| **Parameter** | `len` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '12p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/Lines/LightningSegmentedTransmissionLine.mo"
```

It should declare `len` with type `SI.Length` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/Lines/LightningSegmentedTransmissionLine.mo" | head
```
...
```modelica
// Electrical/Analog/Examples/Lines/LightningSegmentedTransmissionLine.mo:12
parameter SI.Length len(min=0) = ...;
```
