# DECL-0021: `vesselArea` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Fluid/Vessels.mo:276` |
...
```console
$ sed -n '276p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Vessels.mo"
```

It should declare `vesselArea` with type `SI.Area` and no `min`.
...
```modelica
// Fluid/Vessels.mo:276
parameter SI.Area vesselArea(min=0) = ...;
```
