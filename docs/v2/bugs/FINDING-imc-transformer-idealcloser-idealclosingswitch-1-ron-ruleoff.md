# FINDING-02051: `idealCloser.idealClosingSwitch[1].Ron` in `IMC_Transformer`
...
| **Reached as** | `idealCloser.idealClosingSwitch[1].Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:591:aaa7c88c` |
...
```
predicate     idealCloser.idealClosingSwitch[1].Ron > 0
premise       refuted
...
| `required` | idealCloser.idealClosingSwitch[1].Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMC_Transformer \
      --target idealCloser.idealClosingSwitch[1].Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "idealCloser.idealClosingSwitch[1].Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param idealCloser.idealClosingSwitch[1].Ron=0
```
