# DECL-0363: `I_wheelLong` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/Parts/RollingWheelSet.mo:17` |
| **Parameter** | `I_wheelLong` |
| **Type** | `SI.Inertia` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '17p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/RollingWheelSet.mo"
```

It should declare `I_wheelLong` with type `SI.Inertia` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/RollingWheelSet.mo" | head
```
...
```modelica
// Mechanics/MultiBody/Parts/RollingWheelSet.mo:17
parameter SI.Inertia I_wheelLong(min=0) = ...;
```
