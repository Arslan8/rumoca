# FINDING-01371: `dcee.Lme` in `DCEE_Start`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:326:48b00673` |
...
```
contract    Modelica.Electrical.Machines.BasicMachines.DCMachines.DC_ElectricalExcited.Lme: forbidden (proven, from bound) — the declaration excludes zero (declared final, so no modifier can override it)
behavior    forbidden
confidence  proven
source      bound
```
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.DCMachines.DC_ElectricalExcited.Lme'}
```
