# FINDING-03623: `rectifier.GoffDiode` in `DiodeBridge2Pulse`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/DiodeBridge2Pulse.mo:7` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:42:a0cbbe32` |
...
```
contract    Modelica.Electrical.Analog.Ideal.IdealDiode.Goff: feature_disabled (proven, from equation) — inherited from rectifier.diode_p1.Goff, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Conductance', 'unit': 'S', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.PowerConverters.ACDC.DiodeBridge2Pulse.GoffDiode'}
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2Pulse.DiodeBridge2Pulse \
...
```console
$ sed -n '7p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/DiodeBridge2Pulse.mo"
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2Pulse.DiodeBridge2Pulse --keep-artifact /tmp/m.rbc \
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/DiodeBridge2Pulse.mo:7` |' docs/v2/bugs/
```
