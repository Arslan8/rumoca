...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:35:b9259693` |
...
```
predicate     With_Ron_Goff_Vknee.Goff > 0
premise       refuted
...
| `required` | With_Ron_Goff_Vknee.Goff &gt; 0 |
| `declared_min` | 0.0 |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.CharacteristicIdealDiodes \
      --target With_Ron_Goff_Vknee.Goff --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.CharacteristicIdealDiodes --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "With_Ron_Goff_Vknee.Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param With_Ron_Goff_Vknee.Goff=0
```
