# FINDING-02504: `smeeData.omega` in `SMEE_Generator`
...
| **Reached as** | `smeeData.omega` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo:82` |
| **Signature** | `divisor:divisor-reachable-zero:exp:1989:e374a2e8` |
...
```
denominator            (smeeData.omega * smeeData.Td0Subtransient)
witness                smeeData.omega = 0
  found by             set to zero
denominator at witness 0.0
at declared values     2.1875000753132685
path condition         always evaluated
constraints consulted  smeeData.omega: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.SynchronousMachines.SMEE_Generator \
      --target smeeData.omega --sanitizer divisor
```
...
```console
$ sed -n '82p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "smeeData.omega"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/SynchronousMachineData.mo:82` |' docs/v2/bugs/
```
