...
| **Declaration** | `Thermal/HeatTransfer/Components/HeatCapacitor.mo:3` |
| **Parameter** | `C` |
...
It should declare `C` with type `SI.HeatCapacity` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/HeatTransfer/Components/HeatCapacitor.mo" | head
```
...
```modelica
// Thermal/HeatTransfer/Components/HeatCapacitor.mo:3
parameter SI.HeatCapacity C(min=0) = ...;
```
