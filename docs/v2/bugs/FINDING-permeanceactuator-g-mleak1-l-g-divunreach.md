...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:936:b160081e` |
...
```
denominator            g_mLeak1.l_g
witness                g_mLeak1.l_g = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.006499999999999999
path condition         not ((g_mLeak1.t <= g_mLeak1.r_0))
constraints consulted  g_mLeak1.l_g: min=0
```
...
| `path_condition` | not ((g_mLeak1.t &lt;= g_mLeak1.r_0)) |
| `verdict` | UNSAT |
| `proof` | the guard not ((g_mLeak1.t &lt;= g_mLeak1.r_0)) is false under the witness |
| `constraints` | g_mLeak1.l_g: min=0 |
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.Components.PermeanceActuator \
      --target g_mLeak1.l_g --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "g_mLeak1.l_g"
```
