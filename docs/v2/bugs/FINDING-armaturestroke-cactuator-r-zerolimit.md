# FINDING-04227: `cActuator.R` in `ArmatureStroke`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/MovingCoilActuator/Components/ConstantActuator.mo:6` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:244:a33d6655` |
...
```
contract    Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.Components.ConstantActuator.R: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.Components.ConstantActuator.R'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.ArmatureStroke \
      --target cActuator.R --sanitizer physical
```
...
```console
$ sed -n '6p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/MovingCoilActuator/Components/ConstantActuator.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "cActuator.R"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param cActuator.R=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/MovingCoilActuator/Components/ConstantActuator.mo:6` |' docs/v2/bugs/
```
