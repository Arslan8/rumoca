...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:859:dc6fd164` |
...
```
denominator            ramp6.duration
witness                ramp6.duration = 0
  found by             set to zero
denominator at witness 0.0
at declared values     100.0
path condition         not ((time < ramp6.startTime)) and ((time < (ramp6.startTime + ramp6.duration)))
constraints consulted  ramp6.duration: min=0
```
...
| `path_condition` | not ((time &lt; ramp6.startTime)) and ((time &lt; (ramp6.startTime + ramp6.duration))) |
| `verdict` | UNSAT |
| `proof` | the path conditions cannot hold together: time must be both &gt; 1 and &lt; 1 |
| `constraints` | ramp6.duration: min=0 |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Blocks.LimitersHomotopy \
      --target ramp6.duration --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "ramp6.duration"
```
