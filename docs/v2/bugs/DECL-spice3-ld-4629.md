# DECL-0389: `LD` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Spice3.mo:4629` |
| **Parameter** | `LD` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '4629p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Spice3.mo"
```

It should declare `LD` with type `SI.Length` and no `min`.
...
```modelica
// Electrical/Spice3.mo:4629
parameter SI.Length LD(min=0) = ...;
```
