# FINDING-04594: `bodyBox.density` in `BevelGear1D`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/BodyBox.mo:37` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:611:377befad` |
...
```
contract    Modelica.Mechanics.MultiBody.Parts.BodyBox.density: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Density', 'unit': 'kg/m3', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Mechanics.MultiBody.Parts.BodyBox.density'}
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Rotational3DEffects.BevelGear1D \
      --target bodyBox.density --sanitizer physical
```
...
```console
$ sed -n '37p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/BodyBox.mo"
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Rotational3DEffects.BevelGear1D --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "bodyBox.density"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param bodyBox.density=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/BodyBox.mo:37` |' docs/v2/bugs/
```
