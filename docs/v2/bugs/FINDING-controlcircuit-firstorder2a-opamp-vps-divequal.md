...
| **Signature** | `divisor:divisor-zero-when-parameters-equal:exp:945:ce3d186e` |
...
```
denominator            (firstOrder2A.opAmp.vps - firstOrder2A.opAmp.vns)
witness                firstOrder2A.opAmp.Vps = -15
  found by             set equal to firstOrder2A.opAmp.Vns
denominator at witness 0.0
at declared values     30.0
path condition         always evaluated
constraints consulted  firstOrder2A.opAmp.Vps: unbounded
```
...
| `denominator` | (firstOrder2A.opAmp.vps - firstOrder2A.opAmp.vns) |
| `witness` | firstOrder2A.opAmp.Vps = -15 |
| `witness_rationale` | set equal to firstOrder2A.opAmp.Vns |
...
| `constraints` | firstOrder2A.opAmp.Vps: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.OpAmps.ControlCircuit \
      --target firstOrder2A.opAmp.Vps --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "firstOrder2A.opAmp.Vps"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param firstOrder2A.opAmp.Vps=-15
```
