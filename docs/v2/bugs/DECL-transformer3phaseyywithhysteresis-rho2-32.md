# DECL-0884: `rho2` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/FluxTubes/Examples/Hysteresis/Components/Transformer3PhaseYyWithHysteresis.mo:32` |
| **Parameter** | `rho2` |
| **Type** | `SI.Resistivity` |
| **Declared modifiers** | `—` |
...
`SI.Resistivity` declares no `min` in `Units.mo`, and this declaration adds none of its own. It therefore accepts a negative value, and a material property that is non-negative by definition.
...
```console
$ sed -n '32p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/Hysteresis/Components/Transformer3PhaseYyWithHysteresis.mo"
```

It should declare `rho2` with type `SI.Resistivity` and no `min`.

**2. Confirm the type supplies no bound either** — this is the whole claim, and
a bound inherited from the type would refute it:

```console
$ grep -n "type Resistivity " "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Units.mo"
```
...
- `SI.Resistivity` turns out to carry a `min` after all, or this declaration carries one that the catalog missed.
...
```modelica
// Magnetic/FluxTubes/Examples/Hysteresis/Components/Transformer3PhaseYyWithHysteresis.mo:32
parameter SI.Resistivity rho2(min=0) = ...;
```
