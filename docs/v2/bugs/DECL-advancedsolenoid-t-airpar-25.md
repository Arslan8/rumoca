# DECL-0454: `t_airPar` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/FluxTubes/Examples/SolenoidActuator/Components/AdvancedSolenoid.mo:25` |
| **Parameter** | `t_airPar` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '25p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/SolenoidActuator/Components/AdvancedSolenoid.mo"
```

It should declare `t_airPar` with type `SI.Length` and no `min`.
...
```modelica
// Magnetic/FluxTubes/Examples/SolenoidActuator/Components/AdvancedSolenoid.mo:25
parameter SI.Length t_airPar(min=0) = ...;
```
