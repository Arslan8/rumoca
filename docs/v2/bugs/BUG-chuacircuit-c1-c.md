...
`Modelica.Electrical.Analog.Examples.ChuaCircuit` runs cleanly at its declared values. Setting `C1.C = 0` — a value the declaration permits — makes it fail, in Rumoca and in OpenModelica.
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.ChuaCircuit \
      --target C1.C --keep-parameter-chains
```
...
simulate(Modelica.Electrical.Analog.Examples.ChuaCircuit, simflags="-override C1.C=0");
EOF
```
