# FINDING-05456: `C` in `SwitchedRLC_MSL`
...
| **Severity** | low |
| **Declaration** | `examples/models/SwitchedRLC_MSL.mo:8` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:3:f709fce5` |
...
```
{'quantity': 'Capacitance', 'unit': 'F', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'SwitchedRLC_MSL.C'}
```
...
```console
$ sed -n '8p' "examples/models/SwitchedRLC_MSL.mo"
```
...
```console
$ grep -rl '| `examples/models/SwitchedRLC_MSL.mo:8` |' docs/v2/bugs/
```
