# FINDING-05141: `inverseCapacity.C` in `GenerationOfFMUs`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Thermal.HeatTransfer.Examples.GenerationOfFMUs` |
| **Reached as** | `inverseCapacity.C` |
| **Found by** | **PhysicalSan** — `packages/modelsan/modelsan/sanitizers/physical.py`, rule `thermal.heat_capacity.positive` |
| **Reported as** | `physical-intent-question` |
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/HeatTransfer/Examples/Utilities/InverseCapacity.mo:5` |
| **Signature** | `physical:physical-intent-question:var:43:9c6ba070` |
...
```
predicate     inverseCapacity.C > 0
premise       unknown
authority     quantity_or_unit
role          —
declaration   Modelica.Thermal.HeatTransfer.Examples.Utilities.InverseCapacity.C
```
...
> `inverseCapacity.C` is bounded only by min=0, which is unusual for a heat capacity — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.

The rule it applied is `thermal.heat_capacity.positive`:

> C*dT = dQ; a non-positive heat capacity makes a body cool when heated
...
```
{'quantity': 'HeatCapacity', 'unit': 'J/K', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Thermal.HeatTransfer.Examples.Utilities.InverseCapacity.C'}
```
...
| `required` | inverseCapacity.C &gt; 0 |
| `declared_min` | 0.0 |
| `variable_role` | parameter |
| `observation` | permissive-declaration |
| `premise_state` | unknown |
| `authority` | quantity_or_unit |
| `canonical_declaration` | Modelica.Thermal.HeatTransfer.Examples.Utilities.InverseCapacity.C |
| `question` | `inverseCapacity.C` is bounded only by min=0, which is unusual for a heat capacity — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Thermal.HeatTransfer.Examples.GenerationOfFMUs \
      --target inverseCapacity.C --sanitizer physical
```
...
```console
$ sed -n '5p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/HeatTransfer/Examples/Utilities/InverseCapacity.mo"
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Thermal.HeatTransfer.Examples.GenerationOfFMUs --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inverseCapacity.C"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inverseCapacity.C=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/HeatTransfer/Examples/Utilities/InverseCapacity.mo:5` |' docs/v2/bugs/
```
