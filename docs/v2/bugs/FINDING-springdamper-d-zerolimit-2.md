# FINDING-04747: `d` in `SpringDamper`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:1:05ccf144` |
...
```
contract    Modelica.Mechanics.Translational.Examples.Utilities.SpringDamperNoRelativeStates.d: algebraic_limit (proven, from equation) — inherited from springDamper.d, which it binds: the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
