# DECL-0554: `ri` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Mechanics/MultiBody/Visualizers/VoluminousWheel.mo:24` |
| **Parameter** | `ri` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '24p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Visualizers/VoluminousWheel.mo"
```

It should declare `ri` with type `SI.Length` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Visualizers/VoluminousWheel.mo" | head
```
...
```modelica
// Mechanics/MultiBody/Visualizers/VoluminousWheel.mo:24
parameter SI.Length ri(min=0) = ...;
```
