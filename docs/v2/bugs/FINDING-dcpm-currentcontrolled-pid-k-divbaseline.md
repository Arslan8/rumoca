...
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Blocks/Continuous.mo:857` |
| **Signature** | `divisor:divisor-zero-at-declared-values:exp:630:b1ee44f3` |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DCPM_CurrentControlled \
      --target PID.k --sanitizer divisor
```
...
```console
$ sed -n '857p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Blocks/Continuous.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "PID.k"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param PID.k=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Blocks/Continuous.mo:857` |' docs/v2/bugs/
```
