...
| **Reached as** | `simpleTriac.thyristor.IGT` |
...
| **Signature** | `divisor:divisor-zero-unresolved:exp:501:3e41d2a6` |
...
```
denominator            simpleTriac.thyristor.IGT
witness                simpleTriac.thyristor.IGT = 0
...
constraints consulted  simpleTriac.thyristor.IGT: unbounded
```
...
| `constraints` | simpleTriac.thyristor.IGT: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.SimpleTriacCircuit \
      --target simpleTriac.thyristor.IGT --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "simpleTriac.thyristor.IGT"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param simpleTriac.thyristor.IGT=0
```
