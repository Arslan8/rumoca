# FINDING-04709: `springDamper.c` in `GenerationOfFMUs`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Examples/Utilities/SpringDamper.mo:4` |
| **Signature** | `physical:physical-intent-question:var:86:e1a57956` |
...
```
predicate     springDamper.c > 0
premise       unknown
authority     quantity_or_unit
role          —
declaration   Modelica.Mechanics.Translational.Examples.Utilities.SpringDamper.c
```
...
```
{'quantity': 'TranslationalSpringConstant', 'unit': 'N/m', 'confidence': 'QUANTITY', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Mechanics.Translational.Examples.Utilities.SpringDamper.c'}
```
...
| `authority` | quantity_or_unit |
| `canonical_declaration` | Modelica.Mechanics.Translational.Examples.Utilities.SpringDamper.c |
| `question` | `springDamper.c` carries no bound that would exclude it, which is unusual for a translational spring constant — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Translational.Examples.GenerationOfFMUs \
      --target springDamper.c --sanitizer physical
```
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Examples/Utilities/SpringDamper.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "springDamper.c"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param springDamper.c=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Examples/Utilities/SpringDamper.mo:4` |' docs/v2/bugs/
```
