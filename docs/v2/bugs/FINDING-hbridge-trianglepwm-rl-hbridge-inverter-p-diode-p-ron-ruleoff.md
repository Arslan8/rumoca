# FINDING-04146: `hbridge.inverter_p.diode_p.Ron` in `HBridge_TrianglePWM_RL`
...
| **Reached as** | `hbridge.inverter_p.diode_p.Ron` |
...
| **Signature** | `physical:physical-rule-does-not-apply:var:105:34da4894` |
...
```
predicate     hbridge.inverter_p.diode_p.Ron > 0
premise       refuted
...
| `required` | hbridge.inverter_p.diode_p.Ron &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.DCDC.HBridge.HBridge_TrianglePWM_RL \
      --target hbridge.inverter_p.diode_p.Ron --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "hbridge.inverter_p.diode_p.Ron"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param hbridge.inverter_p.diode_p.Ron=0
```
