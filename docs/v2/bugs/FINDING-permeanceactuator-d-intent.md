# FINDING-04266: `d` in `PermeanceActuator`
...
| **Signature** | `physical:physical-intent-question:var:17:9b7cf167` |
...
| `question` | `d` carries no bound that would exclude it, which is unusual for a translational damping constant — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "d"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param d=0
```
