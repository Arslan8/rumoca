# FINDING-04223: `pmActuator.armature.mass.m` in `ArmatureStroke`
...
| **Reached as** | `pmActuator.armature.mass.m` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Components/Mass.mo:3` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:60:7865a320` |
...
```
predicate     pmActuator.armature.mass.m > 0
premise       refuted
...
```
contract    Modelica.Mechanics.Translational.Components.Mass.m: algebraic_limit (proven, from equation) — the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
...
```
{'quantity': 'Mass', 'unit': 'kg', 'semantic_role': 'component.translational.mass', 'binding_source': 'component_type', 'confidence': 'QUANTITY_AND_UNIT', 'declaring_class': 'Modelica.Mechanics.Translational.Components.Mass', 'member': 'm', 'premise_state': 'established', 'authority': 'component_contract', 'canonical_declaration': 'Modelica.Mechanics.Translational.Components.Mass.m'}
```
...
| `required` | pmActuator.armature.mass.m &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.ArmatureStroke \
      --target pmActuator.armature.mass.m --sanitizer physical
```
...
```console
$ sed -n '3p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Components/Mass.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pmActuator.armature.mass.m"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param pmActuator.armature.mass.m=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Components/Mass.mo:3` |' docs/v2/bugs/
```
