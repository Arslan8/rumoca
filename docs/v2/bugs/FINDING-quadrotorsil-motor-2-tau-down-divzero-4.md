# FINDING-05487: `motor[2].tau_down` in `QuadrotorSIL`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `RigidBody.Examples.QuadrotorSIL` |
| **Reached as** | `motor[2].tau_down` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/cmm/CMM-a642c381/RigidBody/Examples/QuadrotorSIL.mo:85` |
| **Signature** | `divisor:divisor-reachable-zero:exp:440:1755255b` |
...
```
denominator            motor[2].tau_down
witness                motor[2].tau_down = 0
  found by             set to zero
denominator at witness 0.0
at declared values     0.025
path condition         always evaluated
constraints consulted  motor[2].tau_down: unbounded
```
...
$ python3 tools/sweep/check_one.py RigidBody.Examples.QuadrotorSIL \
      --target motor[2].tau_down --sanitizer divisor
```
...
```console
$ sed -n '85p' "target/cmm/CMM-a642c381/RigidBody/Examples/QuadrotorSIL.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "motor[2].tau_down"
```
...
```console
$ grep -rl '| `target/cmm/CMM-a642c381/RigidBody/Examples/QuadrotorSIL.mo:85` |' docs/v2/bugs/
```
