...
| **Signature** | `divisor:divisor-reachable-zero:exp:641:02af6df5` |
...
```
denominator            criticalDamping.alpha
witness                criticalDamping.alpha = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  criticalDamping.alpha: unbounded
```
...
| `constraints` | criticalDamping.alpha: unbounded |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Blocks.Continuous_InitialState \
      --target criticalDamping.alpha --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "criticalDamping.alpha"
```
