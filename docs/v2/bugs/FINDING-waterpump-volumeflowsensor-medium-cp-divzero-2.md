# FINDING-05122: `volumeFlowSensor.medium.cp` in `WaterPump`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `Modelica.Thermal.FluidHeatFlow.Examples.WaterPump` |
| **Reached as** | `volumeFlowSensor.medium.cp` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/BaseClasses/TwoPort.mo:36` |
| **Signature** | `divisor:divisor-reachable-zero:exp:815:a43a87a3` |
...
```
denominator            volumeFlowSensor.medium.cp
witness                volumeFlowSensor.medium.cp = 0
  found by             set to zero
denominator at witness 0.0
at declared values     4177.0
path condition         always evaluated
constraints consulted  volumeFlowSensor.medium.cp: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.WaterPump \
      --target volumeFlowSensor.medium.cp --sanitizer divisor
```
...
```console
$ sed -n '36p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/BaseClasses/TwoPort.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "volumeFlowSensor.medium.cp"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check \
      --param volumeFlowSensor.medium.cp=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/BaseClasses/TwoPort.mo:36` |' docs/v2/bugs/
```
