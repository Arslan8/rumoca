# BUG-009: `capacitor2.C` in `ParallelResonance`
...
`Modelica.Electrical.Analog.Examples.ParallelResonance` runs cleanly at its declared values. Setting `capacitor2.C = 0` — a value the declaration permits — makes it fail, in Rumoca and in OpenModelica.
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.ParallelResonance \
      --target capacitor2.C --keep-parameter-chains
```
...
simulate(Modelica.Electrical.Analog.Examples.ParallelResonance, simflags="-override capacitor2.C=0");
EOF
```
