# FINDING-05429: `vehicleRollInConst.inertia.J` in `Vehicles`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `ModelicaTest.Translational.Vehicles` |
| **Reached as** | `vehicleRollInConst.inertia.J` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:528:84f92dfd` |
...
```
predicate     vehicleRollInConst.inertia.J > 0
premise       refuted
...
| `required` | vehicleRollInConst.inertia.J &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Translational.Vehicles \
      --target vehicleRollInConst.inertia.J --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "vehicleRollInConst.inertia.J"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param vehicleRollInConst.inertia.J=0
```
