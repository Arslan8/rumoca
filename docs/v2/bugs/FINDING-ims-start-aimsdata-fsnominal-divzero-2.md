# FINDING-02325: `aimsData.fsNominal` in `IMS_Start`
...
| **Signature** | `divisor:divisor-reachable-zero:exp:2371:663b0a8a` |
...
```
denominator            ((2 * (2 * asin(1.0))) * aimsData.fsNominal)
witness                aimsData.fsNominal = 0
  found by             set to zero
denominator at witness 0.0
at declared values     314.1592653589793
path condition         always evaluated
constraints consulted  aimsData.fsNominal: unbounded
```
...
| `constraints` | aimsData.fsNominal: unbounded |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMS_Start \
      --target aimsData.fsNominal --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "aimsData.fsNominal"
```
