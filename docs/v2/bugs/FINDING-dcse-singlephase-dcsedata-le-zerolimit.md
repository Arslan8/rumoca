# FINDING-01676: `dcseData.Le` in `DCSE_SinglePhase`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:419:c980cc79` |
...
```
contract    Modelica.Electrical.Machines.BasicMachines.DCMachines.DC_SeriesExcited.Le: feature_disabled (proven, from equation) — inherited from dcse.Le, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
