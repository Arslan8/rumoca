# DECL-0583: `h_g` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Thermal/FluidHeatFlow/Components/Pipe.mo:8` |
| **Parameter** | `h_g` |
| **Type** | `SI.Length` |
...
```console
$ sed -n '8p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/Components/Pipe.mo"
```

It should declare `h_g` with type `SI.Length` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Thermal/FluidHeatFlow/Components/Pipe.mo" | head
```
...
```modelica
// Thermal/FluidHeatFlow/Components/Pipe.mo:8
parameter SI.Length h_g(min=0) = ...;
```
