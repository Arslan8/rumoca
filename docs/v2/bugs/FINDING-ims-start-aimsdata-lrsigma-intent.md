# FINDING-02312: `aimsData.Lrsigma` in `IMS_Start`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/ParameterRecords/IM_SlipRingData.mo:9` |
| **Signature** | `physical:physical-intent-question:var:1103:7913513a` |
...
```
predicate     aimsData.Lrsigma > 0
premise       unknown
authority     quantity_or_unit
role          —
declaration   Modelica.Electrical.Machines.Utilities.ParameterRecords.IM_SlipRingData.Lrsigma
```
...
> `aimsData.Lrsigma` carries no bound that would exclude it, which is unusual for an inductance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration.
...
```
{'quantity': 'Inductance', 'unit': 'H', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.Utilities.ParameterRecords.IM_SlipRingData.Lrsigma'}
```
...
| `required` | aimsData.Lrsigma &gt; 0 |
| `declared_min` | None |
| `variable_role` | parameter |
| `observation` | permissive-declaration |
| `premise_state` | unknown |
| `authority` | quantity_or_unit |
| `canonical_declaration` | Modelica.Electrical.Machines.Utilities.ParameterRecords.IM_SlipRingData.Lrsigma |
| `question` | `aimsData.Lrsigma` carries no bound that would exclude it, which is unusual for an inductance — but nothing declares what this component is, so the analyzer cannot tell whether it is intended. Is it? If so, declare it: add a component contract, or a `[[contracts]]` entry naming the intended `sign_domain`. If not, bound the declaration. |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.InductionMachines.IMS_Start \
      --target aimsData.Lrsigma --sanitizer physical
```
...
```console
$ sed -n '9p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/ParameterRecords/IM_SlipRingData.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "aimsData.Lrsigma"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param aimsData.Lrsigma=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Utilities/ParameterRecords/IM_SlipRingData.mo:9` |' docs/v2/bugs/
```
