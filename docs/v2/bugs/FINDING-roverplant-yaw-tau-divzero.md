# FINDING-05470: `yaw_tau` in `RoverPlant`
...
| **Severity** | high |
| **Declaration** | `target/cmm/CMM-a642c381/RigidBody/Examples/RoverPlant.mo:63` |
| **Signature** | `divisor:divisor-reachable-zero:exp:696:b31e7fec` |
...
at declared values     0.25
path condition         always evaluated
constraints consulted  yaw_tau: unbounded
```
...
$ python3 tools/sweep/check_one.py RigidBody.Examples.RoverPlant \
      --target yaw_tau --sanitizer divisor
```
...
```console
$ sed -n '63p' "target/cmm/CMM-a642c381/RigidBody/Examples/RoverPlant.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "yaw_tau"
```
...
```console
$ grep -rl '| `target/cmm/CMM-a642c381/RigidBody/Examples/RoverPlant.mo:63` |' docs/v2/bugs/
```
