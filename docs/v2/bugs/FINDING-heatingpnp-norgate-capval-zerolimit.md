# FINDING-00367: `CapVal` in `HeatingPNP_NORGate`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/HeatingPNP_NORGate.mo:4` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:0:8b852949` |
...
```
contract    Modelica.Electrical.Analog.Examples.HeatingPNP_NORGate.CapVal: feature_disabled (declared, from bound) — the declaration's own binding evaluates to zero at the declared configuration, so zero is the value the library chose rather than one it failed to exclude
...
```
{'quantity': 'Capacitance', 'unit': 'F', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Analog.Examples.HeatingPNP_NORGate.CapVal'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.HeatingPNP_NORGate \
      --target CapVal --sanitizer physical
```
...
```console
$ sed -n '4p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/HeatingPNP_NORGate.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "CapVal"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param CapVal=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Examples/HeatingPNP_NORGate.mo:4` |' docs/v2/bugs/
```
