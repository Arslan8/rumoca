# FINDING-03159: `diode3.idealDiode[1].Goff` in `Rectifier12pulse`
...
| **Reached as** | `diode3.idealDiode[1].Goff` |
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:803:bcbb42b2` |
...
```
predicate     diode3.idealDiode[1].Goff > 0
premise       refuted
...
| `required` | diode3.idealDiode[1].Goff &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.Transformers.Rectifier12pulse \
      --target diode3.idealDiode[1].Goff --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "diode3.idealDiode[1].Goff"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param diode3.idealDiode[1].Goff=0
```
