...
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:12887:0a942c96` |
...
```
denominator            (1 + (oLine50.G[26].alpha * (oLine50.G[26].T_heatPort - oLine50.G[26].T_ref)))
witness                none needed
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.SmoothStep \
      --target oLine50.G[26].alpha --sanitizer divisor
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.SmoothStep --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "oLine50.G[26].alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param oLine50.G[26].alpha=0
```
