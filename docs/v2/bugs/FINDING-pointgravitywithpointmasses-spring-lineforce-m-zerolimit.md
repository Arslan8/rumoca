# FINDING-04511: `spring.lineForce.m` in `PointGravityWithPointMasses`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Forces/LineForceWithMass.mo:23` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:386:67441895` |
...
```
predicate     spring.lineForce.m > 0
premise       refuted
...
```
contract    Modelica.Mechanics.MultiBody.Forces.LineForceWithMass.m: feature_disabled (declared, from bound) — the declaration's own binding evaluates to zero at the declared configuration, so zero is the value the library chose rather than one it failed to exclude
...
```
{'quantity': 'Mass', 'unit': 'kg', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Mechanics.MultiBody.Forces.LineForceWithMass.m'}
```
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Elementary.PointGravityWithPointMasses \
      --target spring.lineForce.m --sanitizer physical
```
...
```console
$ sed -n '23p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Forces/LineForceWithMass.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "spring.lineForce.m"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param spring.lineForce.m=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Forces/LineForceWithMass.mo:23` |' docs/v2/bugs/
```
