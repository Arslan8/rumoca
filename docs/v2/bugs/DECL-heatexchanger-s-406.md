# DECL-0406: `s` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Fluid/Examples/HeatExchanger.mo:406` |
| **Parameter** | `s` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '406p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Examples/HeatExchanger.mo"
```

It should declare `s` with type `SI.Length` and no `min`.
...
```modelica
// Fluid/Examples/HeatExchanger.mo:406
parameter SI.Length s(min=0) = ...;
```
