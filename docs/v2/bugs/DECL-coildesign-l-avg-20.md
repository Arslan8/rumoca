# DECL-0464: `l_avg` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Magnetic/FluxTubes/Examples/Utilities/CoilDesign.mo:20` |
| **Parameter** | `l_avg` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '20p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Examples/Utilities/CoilDesign.mo"
```

It should declare `l_avg` with type `SI.Length` and no `min`.
...
```modelica
// Magnetic/FluxTubes/Examples/Utilities/CoilDesign.mo:20
parameter SI.Length l_avg(min=0) = ...;
```
