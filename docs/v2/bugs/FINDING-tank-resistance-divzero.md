# FINDING-05459: `resistance` in `Tank`
...
| **Severity** | high |
| **Declaration** | `examples/modelsan/Tank.mo:7` |
| **Signature** | `divisor:divisor-reachable-zero:exp:12:3e7e4030` |
...
| `constraints` | resistance: unbounded |
...
```console
$ python3 tools/sweep/check_one.py Tank \
      --target resistance --sanitizer divisor
```
...
```console
$ sed -n '7p' "examples/modelsan/Tank.mo"
```
...
```console
$ python3 tools/sweep/check_one.py Tank --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "resistance"
```
...
```console
$ grep -rl '| `examples/modelsan/Tank.mo:7` |' docs/v2/bugs/
```
