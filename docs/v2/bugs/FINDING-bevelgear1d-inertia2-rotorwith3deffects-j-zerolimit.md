...
| **Reached as** | `inertia2.rotorWith3DEffects.J` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/Rotor1D.mo:78` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:323:21608404` |
...
```
predicate     inertia2.rotorWith3DEffects.J > 0
premise       refuted
...
```
contract    Modelica.Mechanics.MultiBody.Parts.Rotor1D.RotorWith3DEffects.J: algebraic_limit (proven, from equation) — the parameter multiplies a derivative, so at zero the differential relation becomes an algebraic constraint rather than an undefined quotient
...
```
{'quantity': 'MomentOfInertia', 'unit': 'kg.m2', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Mechanics.MultiBody.Parts.Rotor1D.RotorWith3DEffects.J'}
```
...
| `required` | inertia2.rotorWith3DEffects.J &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Rotational3DEffects.BevelGear1D \
      --target inertia2.rotorWith3DEffects.J --sanitizer physical
```
...
```console
$ sed -n '78p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/Rotor1D.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inertia2.rotorWith3DEffects.J"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inertia2.rotorWith3DEffects.J=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/MultiBody/Parts/Rotor1D.mo:78` |' docs/v2/bugs/
```
