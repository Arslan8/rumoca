...
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:1420:520c305c` |
...
```
denominator            (1 + (conductor4.conductor.alpha * (conductor4.conductor.T_heatPort - conductor4.conductor.T_ref)))
witness                none needed
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.GenerationOfFMUs \
      --target conductor4.conductor.alpha --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "conductor4.conductor.alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param conductor4.conductor.alpha=0
```
