# FINDING-05451: `preyEquilibrium` in `NeuralPredatorPrey`

|  |  |
|---|---|
| **Tier** | **Candidate** — static only, not execution-confirmed |
| **Model** | `NeuralPredatorPrey` |
| **Reached as** | `preyEquilibrium` |
| **Found by** | **DivisorSan** — `packages/modelsan/modelsan/sanitizers/divisor.py` |
| **Reported as** | `divisor-reachable-zero` |
| **Severity** | high |
| **Declaration** | `examples/models/NeuralPredatorPrey.mo:53` |
| **Signature** | `divisor:divisor-reachable-zero:exp:2105:fa2d32f4` |
...
```
denominator            preyEquilibrium
witness                preyEquilibrium = 0
  found by             set to zero
denominator at witness 0.0
at declared values     1.0
path condition         always evaluated
constraints consulted  preyEquilibrium: unbounded
```
...
| `constraints` | preyEquilibrium: unbounded |
...
$ python3 tools/sweep/check_one.py NeuralPredatorPrey \
      --target preyEquilibrium --sanitizer divisor
```
...
```console
$ sed -n '53p' "examples/models/NeuralPredatorPrey.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "preyEquilibrium"
```
...
```console
$ grep -rl '| `examples/models/NeuralPredatorPrey.mo:53` |' docs/v2/bugs/
```
