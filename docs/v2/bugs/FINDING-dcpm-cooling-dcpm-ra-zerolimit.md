# FINDING-01390: `dcpm.Ra` in `DCPM_Cooling`
...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:20:51f00663` |
...
```
contract    Modelica.Electrical.Machines.BasicMachines.DCMachines.DC_PermanentMagnet.Ra: feature_disabled (proven, from equation) — the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'semantic_role': 'component.machine.winding_resistance', 'binding_source': 'component_type', 'confidence': 'QUANTITY_AND_UNIT', 'declaring_class': 'Modelica.Electrical.Machines.BasicMachines.DCMachines.DC_PermanentMagnet', 'member': 'Ra', 'premise_state': 'established', 'authority': 'component_contract', 'canonical_declaration': 'Modelica.Electrical.Machines.BasicMachines.DCMachines.DC_PermanentMagnet.Ra'}
```
