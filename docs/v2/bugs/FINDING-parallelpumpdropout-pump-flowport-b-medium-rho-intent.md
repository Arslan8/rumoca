...
| **Signature** | `physical:physical-intent-question:var:66:ad4f3609` |
...
```
predicate     pump.flowPort_b.medium.rho > 0
premise       unknown
...
| `required` | pump.flowPort_b.medium.rho &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Thermal.FluidHeatFlow.Examples.ParallelPumpDropOut \
      --target pump.flowPort_b.medium.rho --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "pump.flowPort_b.medium.rho"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param pump.flowPort_b.medium.rho=0
```
