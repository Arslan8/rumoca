# DECL-0392: `PD` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Spice3.mo:4665` |
| **Parameter** | `PD` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '4665p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Spice3.mo"
```

It should declare `PD` with type `SI.Length` and no `min`.
...
```modelica
// Electrical/Spice3.mo:4665
parameter SI.Length PD(min=0) = ...;
```
