...
| **Signature** | `divisor:divisor-guarded-by-assertion:exp:5797:ea8c6361` |
...
```
denominator            (1 + (oLine2.G[4].alpha * (oLine2.G[4].T_heatPort - oLine2.G[4].T_ref)))
witness                none needed
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.CompareLineTrunks \
      --target oLine2.G[4].alpha --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "oLine2.G[4].alpha"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param oLine2.G[4].alpha=0
```
