# FINDING-02525: `R` in `SMPM_Braking`
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/SynchronousMachines/SMPM_Braking.mo:8` |
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:2:9494e90f` |
...
```
contract    Modelica.Blocks.Math.Gain.k: feature_disabled (proven, from equation) — inherited from gain.k, which it binds: the parameter appears only as a multiplier, so at zero the term it scales drops out and the rest of the model is unaffected
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_Braking.R'}
```
...
```console
$ sed -n '8p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/SynchronousMachines/SMPM_Braking.mo"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Machines/Examples/SynchronousMachines/SMPM_Braking.mo:8` |' docs/v2/bugs/
```
