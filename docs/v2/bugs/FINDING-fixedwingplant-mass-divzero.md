# FINDING-05461: `mass` in `FixedWingPlant`
...
| **Severity** | high |
| **Declaration** | `target/cmm/CMM-a642c381/RigidBody/package.mo:56` |
| **Signature** | `divisor:divisor-reachable-zero:exp:711:1be072c9` |
...
at declared values     5.5
path condition         always evaluated
constraints consulted  mass: unbounded
```
...
```console
$ python3 tools/sweep/check_one.py RigidBody.Examples.FixedWingPlant \
      --target mass --sanitizer divisor
```
...
```console
$ sed -n '56p' "target/cmm/CMM-a642c381/RigidBody/package.mo"
```
...
```console
$ python3 tools/sweep/check_one.py RigidBody.Examples.FixedWingPlant --keep-artifact /tmp/m.rbc \
      --sanitizer divisor >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "mass"
```
...
```console
$ grep -rl '| `target/cmm/CMM-a642c381/RigidBody/package.mo:56` |' docs/v2/bugs/
```
