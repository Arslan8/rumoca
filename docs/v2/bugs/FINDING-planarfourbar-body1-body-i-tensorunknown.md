...
| **Signature** | `physical:physical-inertia-tensor-undecided:par:269:672d9e06` |
...
This one is an **aggregate** rule: its subject is the `symmetric_inertia_tensor` that `body1.body` assembles from several declarations, not any one of them. The fields it covers are excluded from the scalar rule, so this finding is the only thing said about them.

```
matrix       [[I_11, I_21, I_31], [I_21, I_22, I_32], [I_31, I_32, I_33]] (not numerically known)
eigenvalues  not computed
verdict      unknown
reason       `body1.body.I_11` is bound to `body1.I[1, 1]`, which this analysis cannot evaluate, so the tensor cannot be assembled
```
...
| `component` | body1.body |
| `declaring_class` | Modelica.Mechanics.MultiBody.Parts.Body |
| `fields` | body1.body.I_11, body1.body.I_22, body1.body.I_33, body1.body.I_21, body1.body.I_31, body1.body.I_32 |
| `matrix` | [[I_11, I_21, I_31], [I_21, I_22, I_32], [I_31, I_32, I_33]] (not numerically known) |
| `required` | body1.body.I is positive semidefinite |
| `verdict` | unknown |
| `reason` | `body1.body.I_11` is bound to `body1.I[1, 1]`, which this analysis cannot evaluate, so the tensor cannot be assembled |
...
$ python3 tools/sweep/check_one.py Modelica.Mechanics.MultiBody.Examples.Loops.PlanarFourbar \
      --target body1.body.I --sanitizer physical
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "body1.body.I"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param body1.body.I=0
```
