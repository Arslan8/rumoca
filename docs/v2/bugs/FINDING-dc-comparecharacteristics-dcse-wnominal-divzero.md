# FINDING-01353: `dcse.wNominal` in `DC_CompareCharacteristics`
...
| **Signature** | `divisor:divisor-reachable-zero:exp:1693:d7dbf30a` |
...
```
denominator            (dcse.wNominal * dcse.psi_eNominal)
witness                dcse.wNominal = 0
  found by             set to zero
denominator at witness 0.0
at declared values     7.382742735936015
path condition         always evaluated
constraints consulted  dcse.wNominal: unbounded
```
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Machines.Examples.DCMachines.DC_CompareCharacteristics \
      --target dcse.wNominal --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "dcse.wNominal"
```
