# FINDING-03789: `rectifier.RonThyristor` in `ThyristorCenterTap2mPulse_RLV_Characteristic`
...
| **Reached as** | `rectifier.RonThyristor` |
...
| **Severity** | low |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/ThyristorCenterTap2mPulse.mo:7` |
| **Signature** | `physical:physical-rule-does-not-apply:var:150:c3487318` |
...
```
predicate     rectifier.RonThyristor > 0
premise       refuted
...
```
contract    Modelica.Electrical.Polyphase.Ideal.IdealThyristor.Ron: allowed (declared, from class_catalog) — inherited from rectifier.thyristor_p.Ron, which it binds: an ideal switch family member; zero on-resistance is the documented ideal
behavior    allowed
confidence  declared
...
```
{'quantity': 'Resistance', 'unit': 'Ohm', 'confidence': 'QUANTITY_AND_UNIT', 'premise_state': 'unknown', 'authority': 'quantity_or_unit', 'canonical_declaration': 'Modelica.Electrical.PowerConverters.ACDC.ThyristorCenterTap2mPulse.RonThyristor'}
```
...
| `required` | rectifier.RonThyristor &gt; 0 |
| `declared_min` | 0.0 |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTap2mPulse.ThyristorCenterTap2mPulse_RLV_Characteristic \
      --target rectifier.RonThyristor --sanitizer physical
```
...
```console
$ sed -n '7p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/ThyristorCenterTap2mPulse.mo"
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "rectifier.RonThyristor"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param rectifier.RonThyristor=0
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/PowerConverters/ACDC/ThyristorCenterTap2mPulse.mo:7` |' docs/v2/bugs/
```
