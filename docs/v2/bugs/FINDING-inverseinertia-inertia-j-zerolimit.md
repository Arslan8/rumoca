...
| **Signature** | `physical:physical-zero-is-a-supported-limit:var:5:bc60be80` |
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Rotational.Examples.Utilities.InverseInertia \
      --target inertia.J --sanitizer physical
```
...
```console
$ python3 tools/sweep/check_one.py Modelica.Mechanics.Rotational.Examples.Utilities.InverseInertia --keep-artifact /tmp/m.rbc \
      --sanitizer physical >/dev/null
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "inertia.J"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param inertia.J=0
```
