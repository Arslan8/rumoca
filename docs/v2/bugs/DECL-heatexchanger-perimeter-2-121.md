# DECL-0405: `perimeter_2` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Fluid/Examples/HeatExchanger.mo:121` |
| **Parameter** | `perimeter_2` |
| **Type** | `SI.Length` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '121p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Fluid/Examples/HeatExchanger.mo"
```

It should declare `perimeter_2` with type `SI.Length` and no `min`.
...
```modelica
// Fluid/Examples/HeatExchanger.mo:121
parameter SI.Length perimeter_2(min=0) = ...;
```
