# DECL-0052: `Gp` declared without a lower bound

|  |  |
|---|---|
| **Tier** | **Latent** — a permitted value, not an observed failure |
| **Declaration** | `Electrical/Analog/Semiconductors/Diode2.mo:13` |
| **Parameter** | `Gp` |
| **Type** | `SI.Conductance` |
| **Declared modifiers** | `—` |
...
```console
$ sed -n '13p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/Diode2.mo"
```

It should declare `Gp` with type `SI.Conductance` and no `min`.
...
```console
$ grep -n -- "-1e40\|-1E40" "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Electrical/Analog/Semiconductors/Diode2.mo" | head
```
...
```modelica
// Electrical/Analog/Semiconductors/Diode2.mo:13
parameter SI.Conductance Gp(min=0) = ...;
```
