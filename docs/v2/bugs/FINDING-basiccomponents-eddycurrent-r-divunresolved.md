...
| **Severity** | medium |
| **Declaration** | `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Basic/EddyCurrent.mo:29` |
| **Signature** | `divisor:divisor-zero-unresolved:exp:460:74e73763` |
...
```
denominator            eddyCurrent.R
witness                eddyCurrent.R = 0
  found by             set to zero
denominator at witness 0.0
at declared values     9.8e-08
path condition         not (eddyCurrent.useConductance)
constraints consulted  eddyCurrent.R: unbounded
```
...
| `denominator_at_declared_values` | 9.8e-08 |
| `denominator_range` | [-inf, inf] |
| `path_condition` | not (eddyCurrent.useConductance) |
| `verdict` | UNKNOWN |
| `proof` | the path depends on quantities the artifact does not determine: eddyCurrent.useConductance |
| `constraints` | eddyCurrent.R: unbounded |
...
$ python3 tools/sweep/check_one.py ModelicaTest.Magnetic.FluxTubes.BasicComponents \
      --target eddyCurrent.R --sanitizer divisor
```
...
```console
$ sed -n '29p' "target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Basic/EddyCurrent.mo"
```
...
```console
$ grep -rl '| `target/msl/ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Magnetic/FluxTubes/Basic/EddyCurrent.mo:29` |' docs/v2/bugs/
```
