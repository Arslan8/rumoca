# FINDING-00543: `tLine1.F` in `CompareLineTrunks`
...
| **Signature** | `divisor:divisor-unreachable-under-witness:exp:3712:a1ec6a1e` |
...
| `denominator_range` |  |
| `path_condition` | ((tLine1.F &gt; 0)) |
| `verdict` | UNSAT |
| `proof` | the denominator is zero as declared, and ((tLine1.F &gt; 0)) |
| `constraints` |  |
| `guard_that_excludes_it` | ((tLine1.F &gt; 0)) |
...
$ python3 tools/sweep/check_one.py Modelica.Electrical.Analog.Examples.Lines.CompareLineTrunks \
      --target tLine1.F --sanitizer divisor
```
...
$ ./target/debug/rumoca bitcode disasm /tmp/m.rbc | grep -n "tLine1.F"
```
...
$ ./target/debug/rumoca compile-bitcode /tmp/m.rbc --simulate --check --param tLine1.F=0
```
