...
| | |
|---|---|
| **Declaration** | `Modelica 4.1.0/Clocked/BooleanSignals/TimeBasedSources/Pulse.mo:14` |
| **Parameter** | `Twidth` |
| **Quantity** | `SI.Duration` |
| **Declared modifiers** | none |
| **Status** | **latent — a permitted value, not an observed failure** |

## What is wrong

`SI.Duration` declares no `min` in `Units.mo`, and this
declaration adds none of its own. It therefore accepts a negative value, and
a negative duration runs time backwards.
