...
| | |
|---|---|
| **Declaration** | `Modelica 4.1.0/Electrical/Machines/Examples/DCMachines/DCPM_Cooling.mo:24` |
| **Parameter** | `G_armature_core` |
| **Quantity** | `SI.Thermalconductance` |
| **Declared modifiers** | none |
| **Status** | **latent — a permitted value, not an observed failure** |

## What is wrong

`SI.Thermalconductance` declares no `min` in `Units.mo`, and this
declaration adds none of its own. It therefore accepts a negative value, and
negative thermal conductance moves heat up a gradient.
