...
| | |
|---|---|
| **Declaration** | `Modelica 4.1.0/Magnetic/FluxTubes/Examples/Hysteresis/Components/Transformer3PhaseYyWithHysteresis.mo:96` |
| **Parameter** | `sigma` |
| **Quantity** | `SI.Conductivity` |
| **Declared modifiers** | none |
| **Status** | **latent — a permitted value, not an observed failure** |

## What is wrong

`SI.Conductivity` declares no `min` in `Units.mo`, and this
declaration adds none of its own. It therefore accepts a negative value, and
a material property that is non-negative by definition.
