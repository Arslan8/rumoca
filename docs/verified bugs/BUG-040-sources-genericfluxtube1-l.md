...
`genericFluxTube1` is an instance of `Modelica.Magnetic.FluxTubes.Shapes.FixedShape.GenericFluxTube`.

## What happens

The model simulates cleanly at its declared values in both tools. Overriding
`genericFluxTube1.l` to `0` — a value nothing in the declaration excludes — makes it fail in
both:

```console
$ rumoca compile-bitcode sources.rbc --simulate --check --param genericFluxTube1.l=0
```
