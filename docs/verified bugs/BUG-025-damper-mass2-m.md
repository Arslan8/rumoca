# BUG-025: `Damper.mass2.m` fails at `0`, a value its declaration permits

| | |
|---|---|
| **Model** | `Modelica.Mechanics.Translational.Examples.Damper` |
| **File** | `ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Examples/Damper.mo:15` |
| **Instance** | `mass2` |
| **Parameter** | `m` |
| **Trigger** | `mass2.m = 0` |
| **Reach** | this trigger reproduces in 5 models |
...
```modelica
// ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Examples/Damper.mo:15
Translational.Components.Mass mass2
```

`mass2` is an instance of `Modelica.Mechanics.Translational.Components.Mass`.

## What happens

The model simulates cleanly at its declared values in both tools. Overriding
`mass2.m` to `0` — a value nothing in the declaration excludes — makes it fail in
both:

```console
$ rumoca compile-bitcode damper.rbc --simulate --check --param mass2.m=0
```
