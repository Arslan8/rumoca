# BUG-041: `Sources.genericFluxTube1.material.B_myMax` fails at `0`, a value its declaration permits

| | |
|---|---|
| **Model** | `ModelicaTest.Magnetic.FluxTubes.Sources` |
| **File** | `ModelicaStandardLibrary-4.1.0/ModelicaTest/Magnetic/FluxTubes.mo:559` |
| **Instance** | `genericFluxTube1.material` |
| **Parameter** | `B_myMax` |
...
```modelica
// ModelicaStandardLibrary-4.1.0/ModelicaTest/Magnetic/FluxTubes.mo:559
Modelica.Magnetic.FluxTubes.Shapes.FixedShape.GenericFluxTube genericFluxTube1
```

`genericFluxTube1.material` is an instance of `Modelica.Magnetic.FluxTubes.Material.SoftMagnetic.BaseData`.

## What happens

The model simulates cleanly at its declared values in both tools. Overriding
`genericFluxTube1.material.B_myMax` to `0` — a value nothing in the declaration excludes — makes it fail in
both:

```console
$ rumoca compile-bitcode sources.rbc --simulate --check --param genericFluxTube1.material.B_myMax=0
```
