# BUG-045: `Sources.genericFluxTube3.material.B_myMax` fails at `0`, a value its declaration permits

| | |
|---|---|
| **Model** | `ModelicaTest.Magnetic.FluxTubes.Sources` |
| **File** | `ModelicaStandardLibrary-4.1.0/ModelicaTest/Magnetic/FluxTubes.mo:591` |
| **Instance** | `genericFluxTube3.material` |
| **Parameter** | `B_myMax` |
...
`genericFluxTube3.material` is an instance of `Modelica.Magnetic.FluxTubes.Material.SoftMagnetic.BaseData`.

## What happens

The model simulates cleanly at its declared values in both tools. Overriding
`genericFluxTube3.material.B_myMax` to `0` — a value nothing in the declaration excludes — makes it fail in
both:

```console
$ rumoca compile-bitcode sources.rbc --simulate --check --param genericFluxTube3.material.B_myMax=0
```
