# BUG-021: `Damper.mass3.m` fails at `0`, a value its declaration permits

| | |
|---|---|
| **Model** | `Modelica.Mechanics.Translational.Examples.Damper` |
| **File** | `ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Examples/Damper.mo:24` |
| **Instance** | `mass3` |
| **Parameter** | `m` |
| **Trigger** | `mass3.m = 0` |
| **Reach** | this trigger reproduces in 18 models |
...
```modelica
// ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Translational/Examples/Damper.mo:24
Translational.Components.Mass mass3
```

`mass3` is an instance of `Modelica.Mechanics.Translational.Components.Mass`.

## What happens

The model simulates cleanly at its declared values in both tools. Overriding
`mass3.m` to `0` — a value nothing in the declaration excludes — makes it fail in
both:

```console
$ rumoca compile-bitcode damper.rbc --simulate --check --param mass3.m=0
```
