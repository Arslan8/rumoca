# BUG-036: `ElasticBearing.housing.J` fails at `0`, a value its declaration permits

| | |
|---|---|
| **Model** | `Modelica.Mechanics.Rotational.Examples.ElasticBearing` |
| **File** | `ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Rotational/Examples/ElasticBearing.mo:29` |
| **Instance** | `housing` |
| **Parameter** | `J` |
| **Trigger** | `housing.J = 0` |
...
```modelica
// ModelicaStandardLibrary-4.1.0/Modelica 4.1.0/Mechanics/Rotational/Examples/ElasticBearing.mo:29
Rotational.Components.Inertia housing
```

`housing` is an instance of `Modelica.Mechanics.Rotational.Components.Inertia`.

## What happens

The model simulates cleanly at its declared values in both tools. Overriding
`housing.J` to `0` — a value nothing in the declaration excludes — makes it fail in
both:

```console
$ rumoca compile-bitcode elasticbearing.rbc --simulate --check --param housing.J=0
```
